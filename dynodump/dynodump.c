 ...
 */
static int elferr(const char *str)
{
...
int dynodump(const char *file)
{
	...
	for (ndx = 1, scn = 0;
		...
		if ((_icache->c_shdr = shdr =
		     elf_getshdr(_icache->c_scn)) == NULL)
			return (elferr("elf_getshdr"));

		if ((_icache->c_data =
		     elf_getdata(_icache->c_scn, NULL)) == NULL)
			return (elferr("elf_getdata"));
	...
	*_ostrs++ = '\0';
	...
	for (ndx = 1; ndx < oehdr->e_shnum; ndx++, _ocache++, _icache++) {
		...
		if (_ocache->c_shdr->sh_type == M_REL_SHT_TYPE)
			update_reloc(ocache, _ocache, icache, _icache,
				     oehdr->e_shnum);
	}
