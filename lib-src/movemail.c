...
along with this program.  If not, see <http://www.gnu.org/licenses/>.
 ...
 Please mail bugs and suggestions to the SXEmacs maintainer.
*/
 ...
 *
 * Added POP (Post Office Protocol) service.  When compiled
 * -DMAIL_USE_POP movemail will accept input filename arguments of the
...
#else
# error         "movemail cannot be built without getopt, preferably getopt_long"
#endif
		if (opt == EOF)
			break;
...
static void usage(int lock_method)
{
	printf ("Usage: movemail [-rvxkh] [-l lines ] [-m method ] [-i] "
...
static char *unparse_lock_method(int lock_method)
{
	switch (lock_method) {
	case DOTLOCKING:
		return "dot";
...
static int parse_lock_method(char *method_name)
{
...
static char *dot_filename(char *filename)
{
...
#ifndef DISABLE_DIRECT_ACCESS
static void lock_dot(char *filename)
{
	struct stat st;
	long now;
	int tem;
...
static void unlock_dot(char *filename)
{
	unlink(dot_filename(filename));
}

static void maybe_unlock_dot(void)
{
	if (dotlock_filename)
...
static int popmail(char *user, char *outfile, char *password)
{
	...
	for (idx = 0; idx < nmsgs; idx++) {
		...
		if (!regexp_pattern
		    ||
		    pop_search_top(server, i, match_lines,
				   regexp_pattern) == POP_RETRIEVED) {
			VERBOSE(("retrieving message %d     \n", i));
			mbx_delimit_begin(mbf);
			if (pop_retr(server, i, mbx_write, mbf) !=
			    POP_RETRIEVED) {
				error("%s", Errmsg, NULL);
				close(mbfi);
				return (1);
...
static int
pop_retr(popserver server, int msgno, int (*action) (char *, FILE *),
	 FILE * arg)
{
	char *line;
	int ret;
...
static int
pop_search_top(popserver server, int msgno, int lines,
	       struct re_pattern_buffer *regexp)
{
	char *line;
	int ret;
	...
	while (!(ret = pop_top_next(server, &line))) {
		...
		if (match != POP_RETRIEVED) {
			if ((ret =
			     re_match(regexp, line, strlen(line), 0,
				      0)) == -2) {
...
static int mbx_write(char *line, FILE * mbf)
{
...
static struct re_pattern_buffer *compile_regex(char *pattern)
{
	const char *err;
	...
	err = re_compile_pattern(pattern, strlen(pattern), patbuf);
	if (err != NULL) {
