...
static int nextcharin(FILE * infile, int PortableNewlines)
{
	int c;
...
#ifdef NEWLINE_CHAR
static void almostputc(int c, FILE * outfile, int PortableNewlines)
{
	if (CRpending) {
		if (c == 10) {
       ...
       char **boundaries, int *boundaryct, int PortableNewlines)
{
	...
	while ((c1 = getc(infile)) != EOF) {
		...
		if (c3 == '=') {
			...
			c3 = char64(c3);
			almostputc((((c2 & 0XF) << 4) | ((c3 & 0x3C) >> 2)),
				   outfile, PortableNewlines);
			if (c4 == '=') {
				DataDone = 1;
			} else {
				c4 = char64(c4);
	...
	if (CRpending)
		putc(13, outfile);	/* Don't drop a lone trailing char 13 */
}
	...
	for (i = 1; i < argc; ++i) {
		if (argv[i][0] == '-') {
			...
			case 'o':
				if (++i >= argc) {
					fprintf(stderr,
						"mimencode: -o requires a file name.\n");
					exit(-1);
				}
				fpo = fopen(argv[i], "w");
				if (!fpo) {
