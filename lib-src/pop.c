...
SXEEmacs is distributed in the hope that it will be useful, but
 ...
 */
popserver pop_open(char *host, char *username, char *password, int flags)
{
	int sock;
	popserver server;
	...
	if ((!host) && (!(flags & POP_NO_HESIOD))) {
		...
		if (office && office->po_type
		    && (!strcmp(office->po_type, "POP"))
...
#define DONT_NEED_PASSWORD 0
	...
	if (strlen(username) > ERROR_MAX - 6) {
		pop_close(server);
		strcpy(pop_error, "Username too long; recompile pop.c with "
		       "larger ERROR_MAX");
		return (0);
	}
	...
	if (strlen(password) > ERROR_MAX - 6) {
		pop_close(server);
		strcpy(pop_error,
		       "Password too long; recompile pop.c with larger "
		       "ERROR_MAX");
		return (0);
	}
 ...
 */
char *pop_retrieve(popserver server, int message, int markfrom)
{
...
int pop_retrieve_first(popserver server, int message, char **response)
{
	sprintf(pop_error, "RETR %d", message);
	return (pop_multi_first(server, pop_error, response));
}

int pop_retrieve_next(popserver server, char **line)
{
	return (pop_multi_next(server, line));
}

int pop_retrieve_flush(popserver server)
{
	return (pop_multi_flush(server));
}

int pop_top_first(popserver server, int message, int lines, char **response)
{
	sprintf(pop_error, "TOP %d %d", message, lines);
	return (pop_multi_first(server, pop_error, response));
}

int pop_top_next(popserver server, char **line)
{
	return (pop_multi_next(server, line));
}

int pop_top_flush(popserver server)
{
	return (pop_multi_flush(server));
}

int pop_multi_first(popserver server, char *command, char **response)
{
	if (server->in_multi) {
		strcpy(pop_error,
...
int pop_multi_next(popserver server, char **line)
{
	char *fromserver;
...
int pop_multi_flush(popserver server)
{
	char *line;
 ...
 * 	into pop_error.
 */
static int socket_connection(char *host, int flags)
{
...
#define SOCKET_ERROR "Could not create socket for POP connection: "
	...
	while (*hostent->h_addr_list) {
		memcpy(&addr.sin_addr, *hostent->h_addr_list,
		       hostent->h_length);
...
#define KRB_ERROR "Kerberos error connecting to POP server: "
	if (!(flags & POP_NO_KERBEROS)) {
		...
		rem = krb5_sendauth((krb5_pointer) & sock, "KPOPV1.0",
				    client, server,
				    AP_OPTS_MUTUAL_REQUIRED, 0,	/* no checksum */
				    0,	/* no creds, use ccache instead */
				    ccdef, 0,	/* don't need seq # */
				    0,	/* don't need subsession key */
				    &err_ret, 0);	/* don't need reply */
 ...
 * Side effects: Closes the connection on error.
 */
static char *pop_getline(popserver server)
{
 ...
 * Side effects: Closes the connection on error.
 */
static int sendline(popserver server, char *line)
{
 ...
 */
static int fullwrite(int fd, char *buf, int nbytes)
{
	char *cp;
	int ret;
 ...
 */
static int getok(popserver server)
{
	char *fromline;
 ...
 * Side effects: Closes the connection on error.
 */
static int gettermination(popserver server)
{
	char *fromserver;
 ...
 */
static void pop_trash(popserver server)
{
	if (server->file >= 0) {
...
static char *find_crlf(char *in_string)
{
	while (1) {
		if (!*in_string)
			return (0);
