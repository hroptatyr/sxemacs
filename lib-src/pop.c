...
#if defined WITH_KERBEROS && defined HAVE_KERBEROS
# if defined HAVE_KRB5_KRB5_H
#  include <krb/krb5.h>
# elif defined HAVE_KRB5_H
#  include <krb5.h>
# elif defined HAVE_KRB_KRB_H
#  include <krb/krb.h>
# elif defined HAVE_KRB_H
#  include <krb.h>
# elif defined HAVE_KERBEROS_KRB_H
#  include <kerberos/krb.h>
# elif defined HAVE_KERBEROSIV_KRB_H
#  include <kerberosIV/krb.h>
# else
#  error "What are you doing here?"
# endif
# if !defined HAVE_KERBEROS5 && defined HAVE_KRB_COM_ERR_H
#  include <krb/com_err.h>
# elif !defined HAVE_KERBEROS5 && defined HAVE_KERBEROSIV_KRB_ERR_H
#  include <kerberosIV/krb_err.h>
# elif !defined HAVE_KERBEROS5 && defined COM_ERR_H
#  include <com_err.h>
# endif
#endif /* KERBEROS */

#if defined WITH_KERBEROS && defined HAVE_KERBEROS
#if !defined HAVE_KERBEROS5
...
#endif /* WITH_KERBEROS && HAVE_KERBEROS */
     ...
     int *size;
{
	...
	if (sendline (server, "STAT") ||
	    (pop_getline (server, &fromserver) < 0))
		return (-1);
	...
	while ((ret = pop_retrieve_next (server, &fromserver)) >= 0) {
		...
		if (markfrom && fromserver[0] == 'F' && fromserver[1] == 'r' &&
			...
			if (++fromcount == 5) {
				...
				if (! ptr) {
					strcpy (pop_error, "Out of memory "
						"in pop_retrieve");
					pop_retrieve_flush (server);
					return (-1);
				}
     ...
     char **response;
{
	...
	if (sendline (server, command) ||
	    (pop_getline (server, response) < 0)) {
		return (-1);
	}
...
# endif /* HAVE_KERBEROS5 */
	...
	if (! found_port) {
		...
		if (servent) {
			...
			/** "kpop" service is  never used:
			 * look for 20060515 to see why **/
#if defined WITH_KERBEROS && defined HAVE_KERBEROS
...
#define POP_SOCKET_ERROR "Could not create socket for POP connection: "
	...
	do {
		...
		if ((! hostent) &&
			...
			strcpy (pop_error, "Could not determine "
				"POP server's address");
			return (-1);
		}
	} while (! hostent);
...
#define GETLINE_ERROR "Error reading from server: "
	...
	if (server->data) {
		...
		if (cp) {
			...
			*cp = '\0';
			...
			if (pop_debug)
				/* Embedded nulls will truncate this output
				   prematurely, but that's OK because it's just
			...
			*line = server->buffer + found;
			...
			/* Record the fact that we've searched the data already
			   in the buffer for a CRLF, so that when we search
			   below, we don't have to search the same data twice.
			   There's a "- 1" here to account for the fact that the
			   last character of the data we have may be the CR of a
			   CRLF pair, of which we haven't read the second half
			   yet, so we may have to search it again when we read
			   more data. */
	...
	while (1) {
		...
		if (ret < 0) {
			...
			strcpy (pop_error, "Unexpected EOF from "
				"server in pop_getline");
			pop_trash (server);
			return (-1);
		} else {
			char *cp;
