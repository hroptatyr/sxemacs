...
#ifndef PATH_MAX
#define PATH_MAX 4096 /* The most common limit... */
#endif
...
int main(int argc, char *argv[])
{
	...
	if (verbose)
		printf
		    ("ellcc driver version %s for EMODULES version %s (%ld)\n",
 ...
 */
static void add_to_argv(const char *str)
{
	int sm = 0;
...
{
	int i;
	char ts[PATH_MAX];		/* Plenty big enough */
...
static void do_link_mode(void)
{
	...
	int once=0;
	char *t, ts[PATH_MAX];
        char builddir[PATH_MAX+1];	/* Plenty big enough */
	...
	ts[0]=builddir[0]='\0';
        ...
        if ( (t=getenv("abs_builddir")) ) {
                strcpy(builddir,t);
        } else 
#if defined(HAVE_DIRNAME) && defined(HAVE_REALPATH)
        {
		strncpy(ts, mod_output, countof(ts));
		t = dirname(ts);
		memmove(ts, t, countof(ts));
                realpath(ts,builddir);
        }
#else
        if ( ! strrchr(mod_output, '/') ) {
	...
	/* 
	 * Checking for inter-module linkage and adding the
	 * appropriate linker flags.  
	 *
         * In Darwin/MacOS/X the linker only has the option of
         * mapping each individual shared object to it's expected
         * install location. Therefore we need to add the dylib_file
         * option for *each* .ell we are linking against.
	 *
	 * Most other environments, specially using the GNU toolchain
	 * expect the rpath option. In this case, it only needs to be
	 * emitted once.
	 */
	for (i = 1; i < exec_argc; i++) {
		t = prog_argv[exec_args[i]];
		if ( ! expect_out && ! once && strstr( t, ".ell" ) ) {
			char *dyn_file  = NULL;
			int   maxlen    = strlen(ELLCC_MODDIR)+1;
			char *rlink_fmt = NULL;
#if defined(__DARWIN_UNIX03)
                        if ( builddir[0] ) {
                                rlink_fmt="-dylib_file %s/%s:%s/%s";
                        } else {
                                rlink_fmt="-dylib_file %s/%s:%s";
                        }
			maxlen += strlen(rlink_fmt)+3*strlen(t)+strlen(builddir);
			dyn_file=malloc(maxlen+1);
			snprintf(dyn_file, maxlen, rlink_fmt, 
				 ELLCC_MODDIR, t, builddir, t );

			/* MacOS linker does not want the .ell as
			 * object to link. We shall remove it from
			 * prog_argv by not copying it...
			 * All we need to do is not set once to 1 
			 */

#else
			if (! once ) {
                                if ( builddir[0] ) {
                                        rlink_fmt="-Wl,-rpath,%s -Wl,-rpath,%s";
                                } else {
                                        rlink_fmt="%s -Wl,-rpath,%s";
                                }
				maxlen += strlen(rlink_fmt) + strlen(builddir);
				dyn_file=malloc(maxlen+1);
				snprintf(dyn_file, maxlen, rlink_fmt, builddir, ELLCC_MODDIR );
				once = 1;
			}
#endif
			add_to_argv(dyn_file);
			free(dyn_file);
			if (! once)
				continue;
		} else {
			expect_out = strcmp(t, "-o" ) == 0;
		}
		exec_argv[real_argc++] = strdup(prog_argv[exec_args[i]]);
	}
