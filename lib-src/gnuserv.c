...
*/
void ipc_handle_signal(int sig)
{
	ipc_exit(0);
}				/* ipc_handle_signal */
...
*/
void ipc_init(struct msgbuf **msgpp)
{
...
*/
void handle_ipc_request(struct msgbuf *msgp)
{
...
*/
static void echo_request(int s)
{
	char buf[GSERV_BUFSZ];
	int len;
...
*/
static int permitted(unsigned long host_addr, int fd)
{
	int key;
	struct entry *entry;
	...
	if (fd > 0) {
		...
		if (timed_read(fd, auth_protocol, AUTH_NAMESZ, AUTH_TIMEOUT, 1)
		    <= 0)
			return FALSE;
		...
		if (strcmp(auth_protocol, DEFAUTH_NAME) &&
			...
			printf
			    ("authentication protocol (%s) from client is invalid...\n",
			     auth_protocol);
			printf
			    ("... Was the client an old version of gnuclient/gnudoit?\004\n");
		...
		if (!strcmp(auth_protocol, MCOOKIE_NAME)) {
			...
			printf
			    ("Xauth authentication failed, trying GNU_SECURE auth...\004\n");
...
/*
  add_host -- add the given host to the list of permitted hosts, provided it isn't
              already there.
*/
static void add_host(unsigned long host_addr)
{
	int key;
	...
	if (!permitted(host_addr, -1)) {
		if ((new_entry =
		     (struct entry *)malloc(sizeof(struct entry))) == NULL) {
			fprintf(stderr,
...
static int internet_init(void)
{
	...
	if (bind(ls, (struct sockaddr *)&server, sizeof(struct sockaddr_in)) ==
	    -1) {
		perror(progname);
...
/*
  handle_internet_request -- accept a request from a client and send the information
                             to stdout (the gnu process).
*/
static void handle_internet_request(int ls)
{
	int s;
...
/*
  handle_unix_request -- accept a request from a client and send the information
                         to stdout (the gnu process).
*/
static void handle_unix_request(int ls)
{
	int s;
