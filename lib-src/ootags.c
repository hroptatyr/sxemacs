...
This file is not considered part of SXEmacs.
...
#else
# error "ootags cannot be built without getopt, preferably getopt_long"
#endif				/* LONG_OPTIONS */
...
/* If you add to this enum, you must add a corresponding entry to the
   preceding array. */
enum oo_browser_constructs { C_NULL, C_CLASS, C_METHOD, C_CONSTANT,
	    C_ENUMERATION,
...
void pfnote
PP((char *name, bool is_func, char *linestart, int linelen, int lno, long cno));
void new_pfnote
PP((char *name, int namelen, bool is_func, char *linestart, int linelen,
    int lno, long cno));
...
char *Lisp_suffixes[] = { "cl", "clisp", "el", "l", "lisp", "lsp", "ml", NULL };
...
char *plain_C_suffixes[] = { "pc",	/* Pro*C file */
	"m",			/* Objective C file */
	"lm",			/* Objective lex file */
	NULL
};

char *Postscript_suffixes[] = { "ps", NULL };
...
char *Yacc_suffixes[] = { "y", "ym", NULL };	/* .ym is Objective yacc file */
 ...
 */
language *get_language_from_interpreter(interpreter)
char *interpreter;
{
	language *lang;
	char **iname;
 ...
 */
language *get_language_from_suffix(suffix)
char *suffix;
{
	language *lang;
	char **ext;
...

/* Record a tag. */
void pfnote(name, is_func, linestart, linelen, lno, cno)
...
void new_pfnote(name, namelen, is_func, linestart, linelen, lno, cno)
char *name;			/* tag name, or NULL if unnamed */
	...
	if (!CTAGS) {
		...
		if (*cp == '\0') {	/* rule #1 */
			...
			if (notinname(linestart[linelen - 1]))
				cp -= 1;	/* rule #4 */
#ifdef OO_BROWSER
			if (!oo_browser_format && cp >= linestart	/* rule #2 */
#else
	...
	if (!CTAGS) {
		...
		if (oo_browser_format) {
			...
			if (np->construct != C_CLASS
			    ...
			    && np->construct != C_OBJC) {
				oo_browser_class =
				    oo_browser_default_classes[np->construct];
				switch (np->construct) {
				case C_CONSTANT:
				case C_ENUMERATION:
				case C_ENUM_LABEL:
				...
				if (np->name != NULL)
					fprintf(tagf, "%s@%c %s@%s\n",
						oo_browser_class,
						oo_browser_prefix, np->name,
						np->pat);
				else
					fprintf(tagf, "%s@%c ???@%s\n",
						oo_browser_class,
						oo_browser_prefix, np->pat);
			}
		} else {
#endif
			if (np->name != NULL)
...
static struct C_stab_entry *in_word_set(char *str, unsigned int len)
{
...
enum sym_type C_symtype(str, len, c_ext)
char *str;
int len;
int c_ext;
{
...
bool consider_token(str, len, c, c_ext, cblev, parlev, is_func_or_var)
...
bool *is_func_or_var;		/* OUT: function or variable found */
{
	...
	case st_C_extern:
		...
		if (typdef == ttypedseen
		    || (typedefs_and_cplusplus && cblev == 0
			&& structdef == snone)) {
			structdef = skeyseen;
			structtype = toktype;
		}
...
#else
		if (structtype == st_C_struct
		    || structtype == st_C_union || structtype == st_C_class)
#endif
	...
	case st_C_ignore:
		...
		return FALSE;
	case st_none:
		if (constantypedefs && structdef == sinbody
		    && structtype == st_C_enum)
#ifdef OO_BROWSER
		{
...
bool isfun;
{
	...
	if (tok.valid) {
		if (traditional_tag_style) {
			...
			pfnote(name, isfun,
			       tok.buffer, tok.linelen, tok.lineno,
			       tok.linepos);
		} else
	...
	*lp = 0;
	...
	while (!feof(inf)) {
		...
		if ((definedef != dnone
		    ...
		    && typdef != tignore
		    && definedef != dignorerest && fvdef != finlist) {
			if (midtoken) {
				if (endtoken(c)) {
					if (c == ':' && cplpl && *lp == ':'
					    && begtoken(*(lp + 1))) {
...
/*	      if (constantypedefs && structdef == snone)*/
				{
					tok.named = TRUE;
					switch ((unsigned int)structtype) {
					case st_C_enum:
						set_construct(C_ENUMERATION);
						break;
					case st_C_class:
...
/* Read environment and prepend it to the default string.
   Build token table. */
struct TEX_tabent *TEX_decode_env(evarname, defenv)
char *evarname;
char *defenv;
{
	register char *env, *p;
...
char *s;
{
	...
	if (strneq(s, "-define", 7) || strneq(s, "-record", 7)) {
		...
		if (s[pos++] == '(') {
			...
			if (len != 0)
				pfnote((CTAGS) ? savenstr(&s[pos], len) : NULL,
				       TRUE, s, pos + len, lineno, linecharno);
		}
	}
	return;
}
...
void add_regex(regexp_pattern, lang)
char *regexp_pattern;
language *lang;
{
	char *name;
 ...
 * file).
 */
long readline_internal(lbp, stream)
linebuffer *lbp;
...
char *relative_filename(file, dir)
char *file, *dir;
{
	char *fp, *dp, *afn, *res;
	int i;
...
char *absolute_filename(file, dir)
char *file, *dir;
{
	char *slashp, *cp, *res;
...
char *absolute_dirname(file, dir)
char *file, *dir;
{
	char *slashp, *res;
	char save;
...
void grow_linebuffer(lbp, toksize)
linebuffer *lbp;
int toksize;
{
