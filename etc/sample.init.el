...
;;         Updated for SXEmacs by Steve Youngs <steve@sxemacs.org>
...
;; -- #### update documentation in (lispref)Starting Up, in
...
;;; -- the SXEmacs User's Manual (Access using the online Info browser:
;;;       Use `Help->Info (Online Docs)->SXEmacs User's Manual'
;;;       (if there is such an entry); or get to the Info contents
;;;       page using `Help->Info Contents' or `C-h i', and then
;;;       *middle-click* the SXEmacs link or move the cursor into
;;;       the link and hit ENTER.  This manual contains a great
;;;       deal of documentation on customization: Scroll down to
;;;       the Customization link and select it in the same fashion
;;;       as for the SXEmacs link just mentioned.)

;;; -- the SXEmacs FAQ (`C-h F' for the local version; get either the
;;;       local version or the very latest version off the net using
;;;       the Help menu)

;;; -- the SXEmacs Lisp Reference Manual, containing detailed
;;;       documentation on elisp. (Access using Info, just like for the
;;;       SXEmacs User's Manual.)

;;; -- the documentation strings for specific commands, functions,
;;;       key sequences, and variables.  NOTE: This is *not* the same
;;;       information as in the SXEmacs User's Manual or SXEmacs Lisp
...
;;; -- Getting Started with SXEmacs [aka the "New User's Guide"], a
;;;       more introductory manual than the SXEmacs User's Manual.
;;;       (Access using Info, just like for the SXEmacs User's Manual.
;;;       There are some sections on customization here.)

;;; -- the SXEmacs tutorial, a very simple introduction to SXEmacs for
;;;       total beginners. (`C-h t' for English; get the version in
;;;       various languages from the Help menu)

;;; -- the SXEmacs web site, www.sxemacs.org.  And the XEmacs web
;;;    site, www.xemacs.org.
...
;;; -- the SXEmacs mailing lists (sxemacs-devel@sxemacs.org and
;;;       sxemacs-patches@sxemacs.org).  These lists are set to
...
;;; -- the SXEmacs internals manual, for those interested in working on
;;;       the SXEmacs C code. (Available through Info.)

;;; -- the freenode IRC channel, #sxemacs.  Most of the core SXEmacs
;;;       developers can be found in this channel.

;;; -- `Help->About SXEmacs' to find out who the maintainers are.
...
;;; SXEmacs allows you to make persistent changes to editor behavior by
;;; saving code in files which are by default loaded at startup.

;; These files are just Lisp libraries with names built in to SXEmacs.
;; There are files for the use of the user (the init file and the
;; custom file), for the site administrator (default.el and
;; site-start.el), and for the SXEmacs maintainers (auto-autoloads
;; files).  See the Lispref for user and site files (node Starting Up,
;; currently inaccurate (it doesn't describe the custom
...
;; SXEmacs itself via the Custom subsystem and manual customizations,
;; respectively.  Originally (in XEmacs),both were placed in the same
;; file, usually ~/.emacs, but occasionally XEmacs would trash user
;; settings when automatically changing options, and more frequently
;; users would trash the automatically generated code.  So these
;; functions have been reallocated to separate files, usually named
;; custom-${USER}.el (where `${USER}' is your user login name), and
;; init.el, respectively.
...
;; Normal practice for Custom at initialization is to (1) reset
;; all customized faces before applying customizations and (2)
;; force all variables to the values specified in custom.el.  For
;; this reason, and because it is generally the case that the init
;; file can usefully depend on customized variables, but Custom
;; pays no attention to behavior of the init file, it is probably
;; a good idea to force custom.el to be loaded before the init
;; file.  Making this the default has been discussed in the XEmacs
;; camp (though no decision has been made yet).
...
Non-nil if we need to inhibit SXEmacs from loading custom.el after init.el.")

(when Init-inhibit-custom-file-p
  ;; This is the default custom-file.
  (let ((file (expand-file-name (concat "custom-" (user-login-name) ".el")
				user-init-directory)))
...
;; `ignore-errors' is a handy way to wrapping code that may be prone to
;; error (for example, loading a library that you don't have) without
;; the error getting in your way.  Here is its documentation...
;;
;; ,----[ C-h f ignore-errors RET ]
...
;; That's because I use them for _, {, and } -- see below. (Ben Wing)

;; f1 is bound to the help prefix (C-h) by default, and because f1 ==
;; help is so common I think it makes more sense to leave it at the
;; default. --SY.
;;
...
(global-set-key 'f8 'kill-entire-line)
...
;; The definitions of these switch-to-* functions are further down.
...
;; I've commented this section out, simply because I disagree with the
;; proposed bindings.  I've never had any of the problems that these
;; bindings are supposed to address.  Yeah, I'm one of these "odd"
;; folk who actually _like_ most of the default bindings. :-) --SY.
...
;; Changes the text in the window title bar.

;; Commented out.  Even though this works fine in SXEmacs, the title
;; bar text you'd end up with would be...
;;
;; "sample.init.el - SXEmacs steve@sxemacs.org--2007/sxemacs--main--22.1.8--patch-66 [/usr/local/bin/sxemacs]"
;;
...
;; SXEmacs multi-media is threaded (you can play an MP3 or OGG (or
;; any other supported format) and not have to wait for it to finish
;; playback before your SXEmacs is usable again)
(init-workers 6)
...
;; You get this by default with big-menubar.el now. --SY.
...
;;; ********************
;;; Load gnuserv, which will allow you to connect to SXEmacs sessions
;;; using `gnuclient'.

;; If you never run more than one SXEmacs at a time, you might want to
...
;;; is fast fast fast!)
(setq auto-save-directory (file-name-as-directory
			   (expand-file-name ".autosave" (user-home-directory)))
...
;; 4 spaces? you gotta be kidding me!  a TAB is 8 in my book --SY.
;; Also cc-mode is much smarter and easier to get along with these
;; days.  Here are my settings.  Note that I set up 3 separate "styles"
;; at once.  They are dependent on the directory the C source is in. --SY.
...
;; crypt has its drawbacks (like it doesn't work all that well), IMO, a
;; better approach is jka-compr plus EasyPG.  The former for handling
;; compressed files, and the latter for encrypted files. --SY.

;; Handle automatic (de)compresion of [bg]zip files
(Init-safe-require 'jka-compr)
(jka-compr-install)
;; EasyPG
(Init-safe-require 'epa-setup)
...
;;; ********************
;;; scroll-in-place is a package that keeps the cursor on the same
;;; line (and in the same column) when scrolling by a page using
;;; PgUp/PgDn.
