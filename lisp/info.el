;;; info.el --- info package for SXEmacs.
;; Keywords: help
...
(defconst Info-emacs-info-file-name "sxemacs.info"
  "The filename of the SXEmacs info for `Info-goto-emacs-command-node'
...
further (recursive) error recovery.  TRYFILE is ??"
  ...
  (cond
   ...
   ((string-match  (concat #r"^\(wais\|solo\|x-exec\|newspost\|www\|"
			   #r"mailto\|news\|tn3270\|ftp\|http\|file\|"
			   #r"telnet\|gopher\):")
		   filename)
    (if-fboundp 'browse-url
	(browse-url filename)
      (error "Cannot follow URLs in this SXEmacs")))
   (t
	  ...
	  (let* ((qnode (regexp-quote nodename))
		...
		(let ((beg (point)))
	      ...
	      (let ((bufs (delq nil (mapcar 'get-file-buffer
		    ...
		    (pattern (if (string-match #r"\`<<.*>>\'" qnode) qnode
			       (format "\"%s\"\\|<<%s>>" qnode qnode)))
		    (pat2 (concat #r"------ *File: *\([^ ].*[^ ]\) *Node: "
				  #r"*\([^ ].*[^ ]\) *Line: *\([0-9]+\)"))
...
invoke \"sxemacs -batch -f Info-batch-rebuild-dir /usr/local/info\"."
...
annotation for any node of any file.  (See `a' and `x' commands.)"
  ...
  (let (filename)
    ...
    (let ((trim (string-match #r"\s *\'" filename)))
      (if trim (setq filename (substring filename 0 trim))))
    (let ((trim (string-match #r"\s *\'" nodename)))
...
(defun Info-find-nearest-node (point)
  (let (node)
    (cond
     ...
     ((setq node (Info-get-token point "\\* " #r"\* \([^:]*\)::"))
...
(defun Info-find-emacs-command-nodes (command)
  "Return a list of locations documenting COMMAND in the SXEmacs Info manual.
...
\(FILENAME NODENAME BUFFERPOS\)."
  (let ((where '())
	(cmd-desc (concat "^\\* " (regexp-quote (symbol-name command))
			  #r":\s *\(.*\)\.")))
    (save-excursion
      (Info-find-node "SXEmacs" "Command Index")
