...
perform the migration at any time with M-x migrate-user-init-file.")
	...
	(declare-fboundp (customize-save-variable 'load-home-init-file t)))))
...
(defun migrate-user-init-file ()
  ...
  (declare-fboundp (customize-set-value 'load-home-init-file nil))
  (declare-fboundp (custom-migrate-custom-file (make-custom-file-name user-init-file
								      'force-new)))
  (message "Moving init file...")
  (let ((new-user-init-file (expand-file-name user-init-file-base
					      user-init-directory)))
    (with-temp-file (concat user-init-file ".migrated")
      (insert ";; This is a backup of your old .emacs\n\n")
...
(defun unmigrate-user-init-file (&optional target-file-name)
  ...
  (let ((target-file-name
    ...
    (let ((old-custom-file custom-file))
      (declare-fboundp (custom-migrate-custom-file target-file-name))
      (declare-fboundp (customize-save-variable 'load-home-init-file t))
