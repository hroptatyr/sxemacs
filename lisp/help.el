;;; help.el --- help commands for SXEmacs.
...
;; This code implements SXEmacs's on-line help system, the one invoked by
;;`M-x help-for-help'.
...
(define-key help-map "F" 'sxemacs-local-faq)
...
(globally-declare-fboundp 'view-major-mode)
...
(defun describe-distribution ()
  "Display info on how to obtain the latest version of SXEmacs."
  (interactive)
...
(defun describe-copying ()
  "Display info on how you may redistribute copies of SXEmacs."
  (interactive)
...
(defun describe-no-warranty ()
  "Display info on all the kinds of warranty SXEmacs does NOT have."
  (interactive)
  (describe-copying)
...
(defun describe-installation ()
  "Display a buffer showing information about this SXEmacs was compiled."
  (interactive)
...
(defun view-emacs-news ()
  "Display info on recent changes to SXEmacs."
  (interactive)
  (Help-find-file (expand-file-name "NEWS" data-directory)))

(defun sxemacs-www-page ()
...
(defun sxemacs-local-faq ()
  "View the local copy of the SXEmacs FAQ.
If you have access to the World Wide Web, you should use `sxemacs-www-faq'
instead, to ensure that you get the most up-to-date information."
  (interactive)
  (save-window-excursion
    (declare-fboundp (info))
    (declare-fboundp (Info-find-node "sxemacs-faq" "Top")))
  (switch-to-buffer "*info*"))

(defalias 'xemacs-local-faq 'sxemacs-local-faq)
...
\\[Info-elisp-ref]	Type a function name; it jumps to the full documentation
	in the SXEmacs Lisp Programmer's Manual.
\\[sxemacs-local-faq]	Local copy of the SXEmacs FAQ.
...
\\[help-with-tutorial]	SXEmacs learn-by-doing tutorial.
...
\\[describe-distribution]	SXEmacs ordering information.
\\[describe-no-warranty]	Information on absence of warranty for SXEmacs.
\\[describe-copying]	SXEmacs copying permission (General Public License)."
  help-map)
...
(defvar help-symbol-regexp
  (let ((sym-char "[+a-zA-Z0-9_:*]")
    ...
    (concat "\\("
	    ...
	    #r"\<\(" sym-char-no-dash "+\\(-" sym-char-no-dash #r"+\)+\)\>"
	    "\\|"
	    "`\\(" sym-char "+\\)'"
	    "\\)")))
...

;; Functions ported from C into Lisp in SXEmacs
