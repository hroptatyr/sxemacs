;;; menubar-items.el --- Menubar and popup-menu content for SXEmacs.
...
;; This file is dumped with SXEmacs (when window system and menubar support is
;; compiled in).
...
; (purecopy-menubar ;purespace is dead
   ...
   `(
     ("%_File"
      ...
      "-----"
      ["E%_xit SXEmacs" save-buffers-kill-emacs]
      )
	...
	:selected (and (boundp 'fast-lock-mode) fast-lock-mode)]
     ...
     ("%_Help"
      ["%_About SXEmacs..." about-sxemacs]
      "-----"
      ["What's %_New in SXEmacs" view-emacs-news]
      ["%_Obtaining SXEmacs" describe-distribution]
      "-----"
      ("%_Info (Online Docs)"
       ["%_Info Contents" info]
	...
	:active (boundp 'Installation-string)]
       ...
       ["SXEmacs %_License" describe-copying]
       ["Find %_Packages" finder-by-keyword]
       ["View %_Splash Screen" sxemacs-splash-buffer]
       ["%_Unix Manual..." manual-entry])
      ["Send %_Bug Report..." report-sxemacs-bug
       :active (fboundp 'report-sxemacs-bug)])))
...
(defun sxemacs-splash-buffer ()
  "Redisplay SXEmacs splash screen in a buffer."
  (interactive)
