;;; register.el --- register commands for SXEmacs.
...
Interactively, second arg is non-nil if prefix arg is supplied."
  ...
  (let ((val (get-register register)))
    (cond
     ((consp val)
      (declare-fboundp (insert-rectangle val)))
     ((stringp val)
      (insert val))
     ((numberp val)
...
START and END are buffer positions giving two corners of rectangle."
  ...
  (set-register register
		(if delete-flag
		    (declare-fboundp (delete-extract-rectangle start end))
		  (declare-fboundp (extract-rectangle start end)))))
