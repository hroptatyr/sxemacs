;;; files.el --- file input and output commands for SXEmacs.
...
(eval-when-compile
  (globally-declare-fboundp '(dired-noselect dired)))
...
unlike `file-truename'."
  (let (tem (count 100) (newname filename))
	...
	(while (string-match #r"\`\.\./" tem) ;#### Unix specific
...
(defun find-file-try-dired-noselect (filename)
...
(defvar auto-mode-alist
  '((#r"\.te?xt\'" . text-mode)
    (#r"\.el\'" . emacs-lisp-mode)
    (#r"\.c?l\(?:i?sp\)?\'" . lisp-mode)
    (#r"\.article\'" . text-mode)
    (#r"\.letter\'" . text-mode)
...
(defun hack-one-local-variable (var val)
  (cond ((eq var 'mode)
	...
	((or (get var 'risky-local-variable)
	     (and
	      (string-match #r"-hooks?$\|-functions?$\|-forms?$\|-program$\|-command$"
			    (symbol-name var))
	 ...
	 (files-done
	  (map-y-or-n-p
	   ...
	   (list (list ?\C-r (lambda (buf)
			       ...
			       ;; to `view-buffer'.
			       (declare-fboundp (view-buffer buf))
	    ...
	    (setq
	     ...
	     (concat result
			 ...
			 (concat
		      ...
		      ((eq ch ?^)  "\\^")
		      ((eq ch ?$)  "\\$")
		      ((eq ch ?\\) #r"\\") ; probably cannot happen...
		      ((eq ch ??)  "[^\000]")
		      (t (char-to-string ch)))))
	    (setq i (1+ i)))))
