...
(defconst build-report-installation-version-regexp
  "SXEmacs\\s-+\\([0-9]+\\)\\.\\([0-9]+\\)\\(\\(-b\\|\\.\\)\\([0-9]+\\)\\)?\\s-+\\\\?\"\\([^\\\"]+\\)\\\\?\"\\s-+configured\\s-+for\\s-+`\\(.+\\)'\\."
  "*REGEXP matching SXEmacs Beta Version string in
...
sxemacs_codename\\s-*=\\s-*\"\\([^\"]+\\)\""
  "*REGEXP matching SXEmacs Beta Version variable assignments in
...
(defconst build-report-installation-srcdir-regexp
  ...
  "REGEXP matching SXEmacs Beta srcdir as the first substring match in
`build-report-installation-file' file.  This variable is used by
`build-report-installation-data'.")
...
(defcustom build-report-version-file
  (expand-file-name
   "version.sh"
   (gethash 'blddir (config-value-hash-table)))
  "*version.sh file identifying SXEmacs (Beta) Distribution."
  :type '(file
          :custom-show t
          :documentation-shown t)
  :group 'build-report)
...
build directory of the running SXEmacs by default (customizable via
...
(defun build-report-installation-data (&optional file)
  "Return a list of SXEmacs installation data containing MAJOR_NUMBER
...
(defun build-report-version-file-data (&optional file)
  "Return a list of SXEmacs version information containing
