...
streams will be passed to the `play-media-stream-asynchronously'
function.
...
See `play-media-stream-synchronously' and
`play-media-stream-asynchronously'."
  (if (and (fboundp #'play-media-stream-asynchronously)
           (not synchronous-sounds))
      (play-media-stream-asynchronously stream device sentinel volume)
    (play-media-stream-synchronously stream device sentinel volume)))
  ...
  (let ((data (cdr-safe (assq sound sound-alist))))
    (when (and data default-audio-device)
      (let ((ms (or (plist-get data :stream)
            ...
            (vol (plist-get data :volume (or volume bell-volume))))
        (when ms
