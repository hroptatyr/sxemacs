...
;; Therefore always tag their usage using (featurep 'number-types)
;; or the like 
;; - hroptatyr
    ...
    (while (cdr (setq rest (cdr rest)))
	  ...
	  (let (constants)
	    ...
	    (while (setq rest (cdr rest))
	      (cond ((and (featurep 'number-types)
                          (rationalp (car rest)))
                     (setq constants (cons (car rest) constants))
                     (setcar rest nil))
                    ((integerp (car rest))
                    ...
                    ((and (featurep 'bigfr)
                          (realp (car rest)))
                     (setq constants (cons (bigfr (car rest)) constants))
                     (setcar rest nil))
                    ((and (featurep 'bigf)
                          (realp (car rest)))
                     (setq constants (cons (bigf (car rest)) constants))
                     (setcar rest nil))
                    ((and (featurep 'lisp-float-type)
                          (realp (car rest)))
                     (setq constants (cons (float (car rest)) constants))
                     (setcar rest nil))
