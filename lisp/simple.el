;;; simple.el --- basic editing commands for SXEmacs
...
;; A grab-bag of basic SXEmacs commands not specifically related to some
...
(defun sendmail-user-agent-compose (&optional to subject other-headers continue
  ...
  (let ((cc (cdr (assoc-ignore-case "cc" other-headers)))
    ...
    (or (declare-fboundp (mail continue to subject in-reply-to cc yank-action send-actions))
	continue
	(error "Message aborted"))
    (save-excursion
      (goto-char (point-min))
      (with-boundp 'mail-header-separator
...

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;		   hash-table to vector conversion                       ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defsubst hash-values-to-vector (hash)
  "Return a vector from the values of hash-table HASH."
  (unless (hash-table-p hash)
    (wrong-type-argument 'hash-table-p hash))
  (let ((vect (make-vector (hash-table-count hash) 0))
	(i 0))
    (maphash
     #'(lambda (key val)
	 (aset vect i val)
...
(defsubst hash-keys-to-vector (hash)
  "Return a vector from the keys of hash-table HASH."
  (unless (hash-table-p hash)
    (wrong-type-argument 'hash-table-p hash))
  (let ((vect (make-vector (hash-table-count hash) 0))
	(i 0))
