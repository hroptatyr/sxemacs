...
(defun x-nearest-color (color colorlist) "
...
close enough are found."
  (when (stringp color)
    (setq color (x-rgb-to-8bits (find-color-rgb color))))
  (let ((favor-non-gray (>= (apply 'x-color-off-gray-diag color) 
    ...
    (mapc
     #'(lambda (candidate)
	 (when candidate
	   (let ((distance (x-color-distance color candidate)))
	     (if (and distance (< distance best-distance)
		      ;; The candidate color is on the gray diagonal
		      ;; if its RGB components are all equal.
		      (or (/= (x-rgb-color-red candidate) 
			      (x-rgb-color-green candidate)) 
			  (/= (x-rgb-color-green candidate)
			      (x-rgb-color-blue candidate))
			  (not favor-non-gray)))
