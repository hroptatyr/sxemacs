...

static int
sound_oss_open_device(sound_oss_data *sod)
{
	if (sod->device_fd < 0) {
		/* open /dev/dsp */
...
static int
sound_oss_close_device(sound_oss_data *sod)
{
	sod->lock = 0;
	if (sod->device_fd < 0)
		return 0;
...
static int
sound_oss_init_device(sound_oss_data *sod, sound_oss_subthread_data *sosd)
{
	int tmp[] = {
...
sound_oss_create(Lisp_Object oss_options)
{
	...
	/* initialise and fill */
	sod = xnew_and_zero(sound_oss_data);
...

static int
sound_oss_play(media_subthread *mst)
{
	/* stream stuff */
	...
	int resolution, i;
	/* subthread stuff */
	sound_oss_subthread_data _sosd, *sosd = &_sosd;
	sxe_mse_volume_args _volargs, *volargs = &_volargs;
	sxe_mse_rerate_args _rrargs, *rrargs = &_rrargs;

	SOUND_UNPACK_MT(mt, mst, device, ms, mss, lad, sod, sosd->mtap);
