...
#define REAL_INC_CHARPTR(ptr)						\
	((void) ((ptr) +=						\
		 REP_BYTES_BY_FIRST_BYTE (* (unsigned char *) (ptr))))
...
INLINE_HEADER const Bufbyte *charptr_n_addr(const Bufbyte * ptr,
					    Charcount offset);
INLINE_HEADER const Bufbyte *charptr_n_addr(const Bufbyte * ptr,
					    Charcount offset)
{
...
INLINE_HEADER Bytecount charptr_copy_char(const Bufbyte * src, Bufbyte * dst);
INLINE_HEADER Bytecount charptr_copy_char(const Bufbyte * src, Bufbyte * dst)
{
...
INLINE_HEADER Bytind BI_BUF_PTR_BYTE_POS(struct buffer * buf, const Bufbyte * ptr);
INLINE_HEADER Bytind BI_BUF_PTR_BYTE_POS(struct buffer *buf, const Bufbyte * ptr)
{
...
INLINE_HEADER Bufbyte *BI_BUF_BYTE_ADDRESS_BEFORE(struct buffer * buf,
						  Bytind pos);
INLINE_HEADER Bufbyte *BI_BUF_BYTE_ADDRESS_BEFORE(struct buffer *buf,
						  Bytind pos)
{
...
INLINE_HEADER Emchar TRT_TABLE_CHAR_1(Lisp_Object table, Emchar ch)
{
...
INLINE_HEADER int bufbyte_strcmp(Bufbyte * bp, const char *ascii_string);
INLINE_HEADER int bufbyte_strcmp(Bufbyte * bp, const char *ascii_string)
{
#ifdef MULE
	while (1) {
