...
float_to_int(fpfloat x, const char *name, Lisp_Object num, Lisp_Object num2)
{
	...
	return Fcanonicalize_number(make_bigz_bz(ent_scratch_bigz));
...
#if defined(HAVE_MPFR) || defined(HAVE_FPFLOAT)
      ...
      (number, precision))
{
	...
	ENT_OPTABLE_LIFT_TO_FLOAT(number);
      ...
      (number, number2, precision))
{
	...
	if (NILP(number2) && FLOATP(number)) {
		...
		ENT_OPTABLE_LIFT_TO_FLOAT(number2);
...
#if defined(HAVE_FPFLOAT) || defined(HAVE_MPFR)
...
(an integer) as precision.
      ...
      (number, precision))
{
#if defined(HAVE_MPFR) || defined(HAVE_MPC) || defined(HAVE_PSEUC)
      ...
      (number, divisor))
{
	...
	divisor = ent_normalise_number(divisor);
	ntnum = get_number_type(number);
	...
	if (INTEGERP(number) && INTEGERP(divisor)) {
		 ...
		 */
		number = ent_optable_lift[ntnum][BIGZ_T](
			number, 0UL);
		divisor = ent_optable_lift[ntdiv][BIGZ_T](
			divisor, 0UL);
		...
		return Fcanonicalize_number(
			make_bigz_bz(ent_scratch_bigz));
#else
		number = ent_optable_lift[ntnum][FLOAT_T](
			number, 0UL);
		divisor = ent_optable_lift[ntdiv][FLOAT_T](
			divisor, 0UL);
	...
	quo = ent_optable_quo[ntnum][ntdiv](number, divisor);
	ntquo = get_number_type(quo);
	...
	if (INTP(number))
		...
		if (bigz_divisible_p(XBIGQ_NUMERATOR(number),
			...
			bigz_add(ent_scratch_bigz,
				 ...
				 XBIGQ_DENOMINATOR(number));
			bigz_div(ent_scratch_bigz,
				 ent_scratch_bigz, XBIGQ_DENOMINATOR(number));
		}
		return Fcanonicalize_number(make_bigz_bz(ent_scratch_bigz));
	}
...
#if defined(HAVE_FPFLOAT) || HAVE_MPFR
	DEFSUBR(Facos);
	DEFSUBR(Fasin);
	DEFSUBR(Fatan);
	DEFSUBR(Fcos);
...
#if defined(HAVE_FPFLOAT) || defined(HAVE_MPF) || defined(HAVE_MPFR)
	DEFSUBR(Ffceiling);
	DEFSUBR(Fffloor);
	DEFSUBR(Ffround);
	DEFSUBR(Fftruncate);
