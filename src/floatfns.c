...
{
#ifdef HAVE_MPZ
	bigz_set_double(ent_scratch_bigz, x);
...

/* now that's the right place, Steve, isn't it? :) */
...
double extract_float(Lisp_Object num)
{
	...
	return XFLOAT_DATA(ent_optable_lift[nt][FLOAT_T](num, 0UL));
}
      ...
      (number, precision))
{
	...
	if (COMPARABLEP(number)) {
		...
		bigc_sqrt(ent_scratch_bigc, XBIGC_DATA(number));
		return make_bigc_bc(ent_scratch_bigc);
#endif	/* HAVE_MPC */
	} 
