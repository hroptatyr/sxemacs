...
except under Linux where WAV files are also supported.
...
parse_sound_alist_elt(Lisp_Object elt,
		      Lisp_Object * volume,
		      Lisp_Object * pitch,
		      Lisp_Object * duration, Lisp_Object * sound)
{
	*volume = Qnil;
	*pitch = Qnil;
	*duration = Qnil;
	...
	if (SYMBOLP(elt) || STRINGP(elt)) {	/* ( name . <sound> ) */
		*sound = elt;
		...
		return;
		   ...
		   (SYMBOLP(XCAR(elt)) || STRINGP(XCAR(elt)))) {
		*sound = XCAR(elt);
	} else if (INT_OR_FLOATP(XCAR(elt)) &&	/* ( name <vol> . <sound> ) */
		   (SYMBOLP(XCDR(elt)) || STRINGP(XCDR(elt)))) {
		*volume = XCAR(elt);
		*sound = XCDR(elt);
	} else if (INT_OR_FLOATP(XCAR(elt)) &&	/* ( name <vol> <sound> ) */
		   CONSP(XCDR(elt)) &&
		   NILP(XCDR(XCDR(elt))) &&
		   (SYMBOLP(XCAR(XCDR(elt))) || STRINGP(XCAR(XCDR(elt))))) {
		*volume = XCAR(elt);
		*sound = XCAR(XCDR(elt));
	} else if ((SYMBOLP(XCAR(elt)) ||	/* ( name <sound> . <vol> ) */
		    STRINGP(XCAR(elt))) && INT_OR_FLOATP(XCDR(elt))) {
		*sound = XCAR(elt);
		*volume = XCDR(elt);
	}
		  ...
		  STRINGP(XCAR(elt))) &&
		 CONSP(XCDR(elt)) &&
		 NILP(XCDR(XCDR(elt))) && INT_OR_FLOATP(XCAR(XCDR(elt)))) {
		*sound = XCAR(elt);
      ...
      (sound, volume, device))
{
	...
	while (1) {
		...
		parse_sound_alist_elt(sound, &new_volume, &pitch, &duration,
				      &data);
		sound = data;
		if (NILP(volume))
			volume = new_volume;
	...
	if (DEVICE_CONNECTED_TO_NAS_P(d) && STRINGP(sound)) {
		...
		TO_EXTERNAL_FORMAT(LISP_STRING, sound,
				   ALLOCA, (soundext, soundextlen), Qbinary);
		if (nas_play_sound_data
		    ((unsigned char *)soundext, soundextlen, vol))
			return Qnil;
	}
#endif				/* HAVE_NAS_SOUND */
	...
	if (DEVICE_CONNECTED_TO_ESD_P(d) && STRINGP(sound)) {
		...
		TO_EXTERNAL_FORMAT(LISP_STRING, sound, ALLOCA,
				   (soundext, soundextlen), Qbinary);

		/* #### ESD uses alarm(). But why should we also stop SIGIO? */
		stop_interrupts();
		succes =
		    esd_play_sound_data((unsigned char *)soundext, soundextlen,
					vol);
		start_interrupts();
		QUIT;
		if (succes)
	...
	if ((NILP(Vnative_sound_only_on_console) || DEVICE_ON_CONSOLE_P(d))
		...
		stop_interrupts();
		succes =
		    play_sound_data((unsigned char *)soundext, soundextlen,
				    vol);
		start_interrupts();
		QUIT;
		if (succes)
...
static void init_native_sound(struct device *d)
{
...
Caveats:
- SXEmacs must be built with sound support for your system.  Not all
