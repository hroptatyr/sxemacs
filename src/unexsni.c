 ...
 *
 */
void unexec(new_name, old_name, data_start, bss_start, entry_address)
...
unsigned data_start, bss_start, entry_address;
{
	...
	old_base =
	    mmap(0, stat_buf.st_size, PROT_READ, MAP_SHARED, old_file, 0);
	...
	old_program_h =
	    (Elf32_Phdr *) ((byte *) old_base + old_file_h->e_phoff);
	old_section_h =
	    (Elf32_Shdr *) ((byte *) old_base + old_file_h->e_shoff);
	old_section_names =
	...
	if (!strcmp
	    (old_section_names + OLD_SECTION_H(old_sbss_index - 1).sh_name,
	     ".sdata")) {
	...
	new_data2_align = new_data2_align ?
	    OLD_SECTION_H(old_bss_index).sh_addralign - new_data2_align : 0;
	...
	new_section_h =
	    (Elf32_Shdr *) ((byte *) new_base + old_file_h->e_shoff +
			    new_data2_size + new_data2_align + new_data3_size);
	...
	for (n = new_file_h->e_phnum - 1; n >= 0; n--) {
		if (NEW_PROGRAM_H(n).p_vaddr
			...
			NEW_PROGRAM_H(n).p_vaddr +=
			    new_data2_size - old_bss_size + new_data3_size -
			    old_sbss_size;

		if (NEW_PROGRAM_H(n).p_offset >= new_data3_offset)
			NEW_PROGRAM_H(n).p_offset +=
			    new_data2_size + new_data2_align + new_data3_size;
	}
#endif
	...
	for (n = 1, nn = 1; n < old_file_h->e_shnum; n++, nn++) {
			...
			/* If it is sbss section, insert the new data3 section before it. */
		{
			...
			if (!old_sdata_index) {
				...
				NEW_SECTION_H(nn).sh_flags =
				    OLD_SECTION_H(n).sh_flags;
		...
		/* Any section that was original placed AFTER the bss section should now
		   be off by NEW_DATA2_SIZE. */
			...
			NEW_SECTION_H(nn).sh_offset += new_data2_size +
			    new_data2_align + new_data3_size - old_bss_padding;
		 ...
		 */
		if (!strcmp
		    (old_section_names + OLD_SECTION_H(n).sh_name, ".data")
		    || !strcmp(old_section_names + OLD_SECTION_H(n).sh_name,
			       ".data1") || (old_sdata_index
					     && (n == old_sdata_index)))
		...
		memcpy(NEW_SECTION_H(nn).sh_offset + new_base, src,
		       ((n == old_sdata_index) ?
			old_sdata_size : NEW_SECTION_H(nn).sh_size));
		...
		if (NEW_SECTION_H(nn).sh_type == SHT_SYMTAB
			...
			Elf32_Sym *sym =
			    (Elf32_Sym *) (NEW_SECTION_H(nn).sh_offset +
					   new_base);
			for (; num--; sym++) {
