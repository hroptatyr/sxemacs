...
#undef LDAV_SYMBOL
	...
	if (pstat(PSTAT_DYNAMIC, statbuf, sizeof(struct pst_dynamic), 0, 0) ==
	    -1)
		return (-1);
...
#define NETBSD_LDAV_FILE "/kern/loadavg"
	...
	count = fscanf(fp, "%lu %lu %lu %lu\n",
		       &load_ave[0], &load_ave[1], &load_ave[2], &scale);
	(void)fclose(fp);
	if (count != 4)
		return -1;
...
#define LDAV_DONE
	...
	if (!getloadavg_initialized) {
		if (processor_set_default(host_self(), &default_set) ==
		    KERN_SUCCESS)
	...
	if (getloadavg_initialized) {
		...
		if (processor_set_info
		    (default_set, PROCESSOR_SET_BASIC_INFO, &host,
			...
			if (nelem > 0)
				loadavg[elem++] =
				    (double)info.load_average / LOAD_SCALE;
		}
	}
...
#define LDAV_DONE
	...
	for (elem = 0; elem < nelem; elem++)
		loadavg[elem]
		    = (load_ave.tl_lscale == 0 ? load_ave.tl_avenrun.d[elem]
		       : (load_ave.tl_avenrun.l[elem] /
			  (double)load_ave.tl_lscale));
#endif				/* OSF_ALPHA */
