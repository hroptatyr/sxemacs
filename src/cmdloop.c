...
static Lisp_Object default_error_handler(Lisp_Object data)
{
      ...
      (x))
{
	/* This is an error handler used when we're running temacs and when
	   we're in the early stages of SXEmacs.  No errors ought to be
	...
	stderr_out("*** Error in SXEmacs initialization");
	Fprint(x, Qexternal_debugging_output);
	stderr_out("*** Backtrace\n");
	Fbacktrace(Qexternal_debugging_output, Qt);
	stderr_out("*** Killing SXEmacs\n");
...
static Lisp_Object cmd_error(Lisp_Object data, Lisp_Object dummy)
{
...
static DOESNT_RETURN command_loop_3(void)
{
...
DOESNT_RETURN initial_command_loop(Lisp_Object load_me)
{
	/* This function can GC */
	if (!NILP(load_me)) {
...
static Lisp_Object load1(Lisp_Object name)
{
	/* This function can GC */
...
static Lisp_Object cold_load_command_loop(Lisp_Object dummy)
{
	/* This function can GC */
...
Lisp_Object call_command_loop(Lisp_Object catch_errors)
{
	...
	if (NILP(catch_errors))
		...
		internal_catch(Qtop_level, cold_load_command_loop, Qnil, 0);
	goto loop;
	return Qnil;
}

static Lisp_Object initial_error_handler(Lisp_Object datum, Lisp_Object ignored)
{
	/* This function can GC */
	Vcommand_loop = Qnil;
	Fding(Qnil, Qnil, Qnil);
...
#if defined LISP_COMMAND_LOOP || !defined HAVE_BDWGC || !defined EF_USE_BDWGC
