...
#ifdef WIN32_NATIVE
		{
			...
			if (pHandle == NULL)
				warn_when_safe(Qprocess, Qwarning,
					       "cannot open process (PID %d) for cleanup",
					       pid);
			else
...
void init_callproc(void)
{
...

	{
		...
		if (!shell)
			shell = egetenv("COMSPEC");
		/* Should never happen! */
		if (!shell)
			shell = (GetVersion() & 0x80000000 ? "command" : "cmd");
#else				/* not WIN32_NATIVE */
		const char *shell = egetenv("SHELL");
		if (!shell)
			shell = "/bin/sh";
		...
		if (!egetenv("SHELL")) {
			CBufbyte *faux_var =
			    alloca_array(CBufbyte, 7 + strlen(shell));
