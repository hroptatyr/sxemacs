...
static int event_equal(Lisp_Object obj1, Lisp_Object obj2, int depth)
{
...
{
	sxe_event_t *res = xnew_and_zero(sxe_event_t);
  ...
  syntax_error_2 ("Invalid property for event type", prop, event_type)
	...
	case key_press_event:
		if (UNBOUNDP(e->event.key.keysym))
			syntax_error
			    ("A key must be specified to make a keypress event",
			     plist);
		break;
	case button_press_event:
	...
	case misc_user_event:
		if (NILP(e->event.misc.function))
			syntax_error
			    ("A function must be specified to make a misc-user event",
			     plist);
		break;
	default:
		break;
	...
	case magic_event:
		{
			...
			{
				Lisp_Object console =
				    CDFW_CONSOLE(EVENT_CHANNEL(event));
				if (CONSOLE_GTK_P(XCONSOLE(console)))
					name =
					    gtk_event_name(event->event.magic.
...
#ifdef HAVE_X_WINDOWS
			{
				...
				if (CONSOLE_X_P(XCONSOLE(console)))
					name =
					    x_event_name(event->event.magic.
							 underlying_x_event.
							 type);
			}
