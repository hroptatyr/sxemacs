...
static void
gap_array_adjust_markers(Gap_Array * ga, Memind from, Memind to, int amount)
{
	Gap_Array_Marker *m;
...
static void gap_array_move_gap(Gap_Array * ga, int pos)
{
	int gap = ga->gap;
...
static void gap_array_make_gap(Gap_Array * ga, int increment)
{
	char *ptr = ga->array;
	int real_gap_loc;
	int old_gap_size;
...
static void gap_array_delete_els(Gap_Array * ga, int from, int numdel)
{
	int to = from + numdel;
	int gapsize = ga->gapsize;
...
static Gap_Array_Marker *gap_array_make_marker(Gap_Array * ga, int pos)
{
	Gap_Array_Marker *m;
...
static void gap_array_delete_marker(Gap_Array * ga, Gap_Array_Marker * m)
{
...
static void gap_array_delete_all_markers(Gap_Array * ga)
{
...
static void gap_array_move_marker(Gap_Array * ga, Gap_Array_Marker * m, int pos)
{
	assert(pos >= 0 && pos <= ga->numels);
	m->pos = GAP_ARRAY_ARRAY_TO_MEMORY_POS(ga, pos);
}
...
static Gap_Array *make_gap_array(int elsize)
{
...
static void free_gap_array(Gap_Array * ga)
{
	if (ga->array)
		xfree(ga->array);
...
static int extent_list_locate_from_pos(Extent_List * el, Memind pos, int endp)
{
...
static EXTENT extent_list_at(Extent_List * el, Memind pos, int endp)
{
	Gap_Array *ga = endp ? el->end : el->start;
...
static void extent_list_insert(Extent_List * el, EXTENT extent)
{
	int pos, foundp;
...
static void extent_list_delete(Extent_List * el, EXTENT extent)
{
	int pos, foundp;
...
static void extent_list_delete_all(Extent_List * el)
{
	gap_array_delete_els(el->start, 0, GAP_ARRAY_NUM_ELS(el->start));
	gap_array_delete_els(el->end, 0, GAP_ARRAY_NUM_ELS(el->end));
}

static Extent_List_Marker *extent_list_make_marker(Extent_List * el, int pos,
						   int endp)
{
...
static void extent_list_delete_marker(Extent_List * el, Extent_List_Marker * m)
{
...
static Extent_List *allocate_extent_list(void)
{
...
static void free_extent_list(Extent_List * el)
{
...
DEFINE_LRECORD_IMPLEMENTATION("extent-info", extent_info,
			      ...
			      struct extent_info);

static Lisp_Object allocate_extent_info(void)
{
...
static Lisp_Object decode_buffer_or_string(Lisp_Object object)
{
	if (NILP(object))
...
static struct extent_info *buffer_or_string_extent_info(Lisp_Object object)
{
	if (STRINGP(object)) {
...
static Extent_List *buffer_or_string_extent_list(Lisp_Object object)
{
...
static struct extent_info *buffer_or_string_extent_info_force(Lisp_Object
							      object)
{
	struct extent_info *info = buffer_or_string_extent_info(object);
...
void sledgehammer_extent_check(Lisp_Object object)
{
	int i;
	int endp;
...
static Stack_Of_Extents *buffer_or_string_stack_of_extents(Lisp_Object object)
{
	struct extent_info *info = buffer_or_string_extent_info(object);
	if (!info)
		return 0;
...
static Stack_Of_Extents *buffer_or_string_stack_of_extents_force(Lisp_Object
								 object)
{
...
static void print_extent_2(EXTENT e)
{
	Lisp_Object extent;
	char buf[200];
...
static void soe_dump(Lisp_Object obj)
{
	int i;
...
static void soe_insert(Lisp_Object obj, EXTENT extent)
{
	Stack_Of_Extents *soe = buffer_or_string_stack_of_extents(obj);
...
static void soe_delete(Lisp_Object obj, EXTENT extent)
{
	Stack_Of_Extents *soe = buffer_or_string_stack_of_extents(obj);
...
static void soe_move(Lisp_Object obj, Memind pos)
{
...

	{
		...
		if (direction < 0)
			start_pos =
			    extent_list_locate_from_pos(bel, soe->pos,
							endp) - 1;
		else
			start_pos =
			    extent_list_locate_from_pos(bel, soe->pos + 1,
							endp);
...
static EXTENT make_extent_detached(Lisp_Object object)
{
...
static EXTENT real_extent_at_forward(Extent_List * el, int pos, int endp)
{
...
static EXTENT real_extent_at_backward(Extent_List * el, int pos, int endp)
{
	for (; pos >= 0; pos--) {
		EXTENT e = extent_list_at(el, pos, endp);
		if (!extent_internal_p(e))
...
static EXTENT extent_first(Lisp_Object obj)
{
...
#ifdef DEBUG_SXEMACS
static EXTENT extent_e_first(Lisp_Object obj)
{
	Extent_List *el = buffer_or_string_extent_list(obj);
...
static EXTENT extent_next(EXTENT e)
{
...
#ifdef DEBUG_SXEMACS
static EXTENT extent_e_next(EXTENT e)
{
	Extent_List *el = extent_extent_list(e);
	int foundp;
...
static EXTENT extent_last(Lisp_Object obj)
{
	Extent_List *el = buffer_or_string_extent_list(obj);
...
#ifdef DEBUG_SXEMACS
static EXTENT extent_e_last(Lisp_Object obj)
{
	Extent_List *el = buffer_or_string_extent_list(obj);
...
static EXTENT extent_previous(EXTENT e)
{
	Extent_List *el = extent_extent_list(e);
	int foundp;
	int pos = extent_list_locate(el, e, 0, &foundp);
...
#ifdef DEBUG_SXEMACS
static EXTENT extent_e_previous(EXTENT e)
{
	Extent_List *el = extent_extent_list(e);
	int foundp;
	int pos = extent_list_locate(el, e, 1, &foundp);
...
static void extent_attach(EXTENT extent)
{
...
static void extent_detach(EXTENT extent)
{
	Extent_List *el;
...
extent_in_region_p(EXTENT extent, Bytind from, Bytind to, unsigned int flags)
{
	...
	start = buffer_or_string_bytind_to_startind(obj, from,
	    ...
	    buffer_or_string_bytind_to_endind(obj, to,
					      !(flags & ME_END_CLOSED));
	...
	if (in_region_flags == 0)
		...
		case ME_START_OR_END_IN_REGION:
			retval = (start <= exs && exs <= end) || (start <= exe
								  && exe <=
								  end);
			break;
		default:
...
static Lisp_Object map_extents_unwind(Lisp_Object obj)
{
...

	{
			...
			if (numsoe) {
				if (flags & ME_MIGHT_MOVE_SOE) {
					...
					/* Can't map over SOE, so just extend range to cover the
					   SOE. */
					EXTENT e =
					    extent_list_at(soe->extents, 0, 0);
					range_start_pos =
					    extent_list_locate
					    (buffer_or_string_extent_list(obj),
					     e, 0, &foundp);
					assert(foundp);
					range_start_type = 2;
				} else {
		...
		for (stage = 0; stage < 2; stage++) {
			...
			if (stage == 0) {	/* The SOE stage */
				...
				case 0:
					pos =
					    extent_list_locate_from_pos(el, st,
									range_endp);
					break;
				case 1:
					pos =
					    extent_list_locate_from_pos(el,
									st + 1,
									range_endp);
					break;
				case 2:
				...
				if (posm)
					...
					extent_list_delete_marker
					    (buffer_or_string_stack_of_extents_force
					     (obj)->extents, posm);
				posm =
				    extent_list_make_marker(el, pos,
							    range_endp);
...
void adjust_extents(Lisp_Object obj, Memind from, Memind to, int amount)
{
	int endp;
	int pos;
	int startpos[2];
	...
	for (endp = 0; endp < 2; endp++) {
		for (pos = startpos[endp]; pos < extent_list_num_els(el); pos++) {
			EXTENT e = extent_list_at(el, pos, endp);
			if (extent_endpoint(e, endp) > to)
				break;
			set_extent_endpoint(e,
					    do_marker_adjustment(extent_endpoint
								 (e, endp),
								 from, to,
								 amount), endp);
		}
	...
	for (i = 0; i < Dynarr_length(closure.list); i++) {
		...
		new_start =
		    do_marker_adjustment(new_start,
					 adjust_to, adjust_to, amount);
		new_end =
		    do_marker_adjustment(new_end, adjust_to, adjust_to, amount);
...
static int extent_priority_sort_function(const void *humpty, const void *dumpty)
{
...
static void extent_fragment_sort_by_priority(EXTENT_dynarr * extarr)
{
	int i;
...
static int invisible_p(REGISTER Lisp_Object propval, Lisp_Object list)
{
...
static int invisible_ellipsis_p(REGISTER Lisp_Object propval, Lisp_Object list)
{
	REGISTER Lisp_Object tail, proptail;
	for (tail = list; CONSP(tail); tail = XCDR(tail)) {
		REGISTER Lisp_Object tem;
...
print_extent(Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
{
	if (escapeflag) {
		...
		if (print_readably) {
			if (!EXTENT_LIVE_P(XEXTENT(obj)))
				error
				    ("printing unreadable object #<destroyed extent>");
			else
				error
				    ("printing unreadable object #<extent 0x%lx>",
				     (long)XEXTENT(obj));
		}
		...
		if (!EXTENT_LIVE_P(XEXTENT(obj)))
			...
			char *buf = (char *)
			    alloca(strlen(title) + strlen(name) +
				   strlen(posttitle) + 1);
