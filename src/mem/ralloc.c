...
static heap_ptr find_heap(POINTER address)
{
	heap_ptr heap;
...
static POINTER obtain(POINTER address, size_t size)
{
	heap_ptr heap;
...
static POINTER get_more_space(size_t size)
{
...
static void relinquish(void)
{
	...
	for (h = last_heap; h && break_value < h->end; h = h->prev) {
		excess +=
		    (char *)h->end - (char *)((break_value < h->bloc_start)
	...
	if (excess > extra_bytes * 2 && (*real_morecore) (0) == last_heap->end) {
		...
		if ((char *)last_heap->end - (char *)last_heap->bloc_start <=
		    excess) {
...
static bloc_ptr find_bloc(POINTER * ptr)
{
...
static bloc_ptr get_bloc(size_t size)
{
...
static int relocate_blocs(bloc_ptr bloc, heap_ptr heap, POINTER address)
{
...
static void reorder_bloc(bloc_ptr bloc, bloc_ptr before)
{
	bloc_ptr prev, next;
...
static void update_heap_bloc_correspondence(bloc_ptr bloc, heap_ptr heap)
{
	register bloc_ptr b;
...
static int resize_bloc(bloc_ptr bloc, size_t size)
{
	register bloc_ptr b;
	heap_ptr heap;
	POINTER address;
...
static void free_bloc(bloc_ptr bloc)
{
...
POINTER r_alloc_sbrk(ptrdiff_t size)
{
	...
	if (size > 0) {
		...
		while ((char *)h->end <
		       (char *)MEM_ROUNDUP((char *)address + get)) {
			h = h->next;
			if (h == NIL_HEAP)
				break;
		...
		if (h == NIL_HEAP) {
			...
			if (first_heap == last_heap)
				address =
				    (POINTER) ROUNDUP(virtual_break_value);
			else
		...
		if (r_alloc_freeze_level == 0 && excess > 2 * extra_bytes) {
			...
			first_heap->bloc_start
			    =
			    (POINTER) MEM_ROUNDUP((char *)first_heap->
						  bloc_start - excess);

			relocate_blocs(first_bloc, first_heap,
				       first_heap->bloc_start);
		...
		if ((char *)virtual_break_value + size <
		    (char *)first_heap->start) {
			/* We found an additional space below the first heap */
			first_heap->start =
			    (POINTER) ((char *)virtual_break_value + size);
		}
	}
		...
		if (pb && pb->data + pb->size != b->data) {
			...
			while (ph) {
				if (ph->bloc_start <= pb->data
					...
					assert(pb->data + pb->size + b->size >
					       ph->end);
					break;
				} else {
					assert(ph->bloc_start + b->size >
					       ph->end);
				}
				ph = ph->prev;
			}
...
/* We actually need a data structure that can be usefully structured
   ...
   address, i.e. a hash table.  Maybe the SXEmacs hash table can be
...
static MMAP_HANDLE new_mmap_handle(size_t nsiz)
{
	MMAP_HANDLE h =
	    (MMAP_HANDLE) UNDERLYING_MALLOC(sizeof(struct alloc_dll));
	if (h == 0)
		return 0;
	h->size = nsiz;
	if (mmap_start == 0) {
...
static MMAP_HANDLE find_mmap_handle_lsearch(POINTER * alias)
{
...
/* Free a handle. */
static void free_mmap_handle(MMAP_HANDLE h)
{
	MMAP_HANDLE prev = h->left;
	MMAP_HANDLE nex = h->right;
	if (prev == h || nex == h) {	/* In fact, this should be && *//* We're the singleton dll */
...
static void init_MHASH_table(void)
{
	int i = 0;
	for (; i < MHASH_PRIME; i++) {
...
static int MHASH(VM_ADDR addr)
{
#if (SXE_LONGBITS == 64)
	unsigned long int addr_shift =
	    (unsigned long int)(addr) >> USELESS_LOWER_ADDRESS_BITS;
#else
	unsigned int addr_shift =
	    (unsigned int)(addr) >> USELESS_LOWER_ADDRESS_BITS;
#endif
...
static void MHASH_ADD(VM_ADDR addr, MMAP_HANDLE h)
{
	int kVal = MHASH(addr);
	if (MHASH_HITS[kVal].n_hits++ == 0) {	/* Only overwrite the table if there were no hits so far. */
...
static void MHASH_DEL(VM_ADDR addr)
{
	int kVal = MHASH(addr);
...
/* If we're metering, we introduce some extra symbols to aid the noble
   cause of bloating SXEmacs core size. */
...
#define M_Addrlist_Size 11	/* What is the size of the SXEmacs memory map? */
...
static MMAP_HANDLE find_mmap_handle(POINTER * alias)
{
...
/*
   ...
   address map which is managed local to the SXEmacs process.  The
...
#ifdef MMAP_GENERATE_ADDRESSES
/* Implementation of the three calls for address picking when SXEmacs is incharge */
...
static void Addr_Block_initialize(void)
{
	MEMMETER(MVAL(M_Addrlist_Size)++)
	    addr_chain =
	    (ADDRESS_CHAIN) UNDERLYING_MALLOC(sizeof(ADDRESS_BLOCK));
...
static void Coalesce_Addr_Blocks(void)
{
	ADDRESS_CHAIN p;
...
static VM_ADDR New_Addr_Block(size_t sz)
{
	...
	for (; p; p = p->next) {
		if (p->flag == empty && p->sz > sz) {
			...
			ADDRESS_CHAIN remainder =
			    (ADDRESS_CHAIN)
			    UNDERLYING_MALLOC(sizeof(ADDRESS_BLOCK));
			...
			p->sz = sz;
			new_addr =
			    mmap((VM_ADDR) p->addr, p->sz,
				 PROT_READ | PROT_WRITE, MAP_FLAGS, DEV_ZERO_FD,
				 0);
...
static void Free_Addr_Block(VM_ADDR addr, size_t sz)
{
	...
	for (; p; p = p->next) {
		if (p->addr == addr) {
			if (p->sz != sz)
				abort();	/* ACK! Shouldn't happen at all. */
	...
	if (!p)
		abort();	/* Can't happen... we've got a block to free which is not in
				   the address list. */
	Coalesce_Addr_Blocks();
}
#else				/* !MMAP_GENERATE_ADDRESSES */
...
static VM_ADDR New_Addr_Block(size_t sz)
{
	return mmap(0, sz, PROT_READ | PROT_WRITE, MAP_FLAGS, DEV_ZERO_FD, 0);
}
	...
	default:
		{
			...
			if (dead_handle == 0) {	/* Didn't find it in the list of mmap handles */
				...
				MEMMETER(MVAL(M_Wastage) -=
					 (dead_handle->space_for -
					  dead_handle->size))
				    MEMMETER(MVAL(M_Live_Pages) -=
					     (dead_handle->space_for /
					      page_size))
				...
				Free_Addr_Block(dead_handle->vm_addr,
						dead_handle->space_for);
...
POINTER r_re_alloc(POINTER * ptr, size_t sz)
{
	if (r_alloc_initialized == 0) {
		...
		MEMMETER(MVAL(M_Average_Bumpval) =
			 (((double)MVAL(M_Remap) * MVAL(M_Average_Bumpval)) +
			  (sz - h->size))
			 / (double)(MVAL(M_Remap) + 1))
		    MEMMETER(MVAL(M_Remap)++)
		    if (h->space_for > sz) {	/* We've got some more room *//* Also, if a shrinkage was asked for. */
		...
		if (new_vm_addr == VM_FAILURE_ADDR) {	/* Failed to realloc. */
			/* *ptr = 0; */
			return 0;
		}
