...

#define snsd_t	sound_nas_subthread_data
static int
nas_fill(media_subthread *mst, int nframes)
{
	size_t len = 0, tmplen = 0;
	sxe_media_sample_t *tmpbuf;
	snsd_t *snsd = media_subthread_data(mst);
	int i;

	tmpbuf = (sxe_media_sample_t*)(mst->buffer + snsd->writepos);
	len = media_stream_meth(mst->substream->up, read)(
		mst->substream, mst->buffer + snsd->writepos,
		nframes);
	...
	snsd->rrargs->tweak = mst->up->ratetrafo;
...
static int
nas_read(media_subthread *mst, int nbytes)
{
	AuStatus as;
	size_t tmplen = 0;
	snsd_t *snsd = media_subthread_data(mst);
	...
	if (snsd->writepos < mst->buffer_alloc_size >> 2 &&
	    snsd->underrun < 4) {
		tmplen = nas_fill(mst, nbytes*snsd->channels/snsd->framesize);
	} else if (snsd->overfill > nbytes * 8 || snsd->underrun >= 4) {
		NAS_DEBUG_S("having a rest\n");
	} else {
		NAS_DEBUG_S("resetting write position.\n");
		memcpy(mst->buffer,
		       mst->buffer+snsd->readpos,
	...
	AuWriteElement(snsd->snd->aud, snsd->flow, 0, nbytes,
		       mst->buffer+snsd->readpos, AuFalse, &as);
	if (as != AuSuccess)
...
nas_event_handler(AuServer *aud, AuEvent *ev, AuEventHandlerRec *hnd)
{
	...
	case AuReasonWatermark:
		nas_read(mst, event->num_bytes);
		break;
	case AuReasonUnderrun:
		/* buffer underrun -> refill buffer */
		if (nas_read(mst, event->num_bytes) != 0) {
...
static int
sound_nas_play(media_subthread *mst)
{
	/* stream stuff */
	...
	int bsize = 0;
	/* subthread stuff */
	sound_nas_subthread_data _snsd, *snsd = &_snsd;
	sxe_mse_volume_args _volargs, *volargs = &_volargs;
	sxe_mse_rerate_args _rrargs, *rrargs = &_rrargs;

	SOUND_UNPACK_MT(mt, mst, device, ms, mss, lad, snd, snsd->mtap);
	...
	AuRegisterEventHandler(snd->aud, AuEventHandlerIDMask |
			       ...
			       nas_event_handler, (AuPointer)mst);
