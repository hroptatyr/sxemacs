...
static int pdump_file_get(const char *path)
{
	...
	pdump_hFile = CreateFile(path, GENERIC_READ + GENERIC_WRITE,	/* Required for copy on write */
				 0,	/* Not shared */
	...
	pdump_hMap = CreateFileMapping(pdump_hFile, NULL,	/* No security attributes */
	...
	pdump_start = MapViewOfFile(pdump_hMap, FILE_MAP_COPY,	/* Copy on write */
				    0,	/* Start at zero */
				    0, 0);	/* Map all of it */
...
static int pdump_resource_get(void)
{
	...
	/* See Q126630 which describes how Windows NT and 95 trap writes to
	   ...
	   that is running SXEmacs.  Since the resource section contains little
