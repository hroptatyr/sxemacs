...
/* convert them to internal format */
	...
	while (bufseek <= length*framesize &&
		...
		declen = avcodec_decode_audio2(
			avcc, outbuf+bufseek, &size, pkt.data, pkt.size);
#elif HAVE_AVCODEC_DECODE_AUDIO
...
static VideoState *
stream_open(const char *filename, size_t filelen)
{
	VideoState *is = xnew(VideoState);
	AVFormatParameters params, *ap = &params;
	int err = 0;

	is->filename = (char*)filename;
	is->filelen = filelen;
