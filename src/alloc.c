...
#endif				/* NEED_STRDUP */

static void *allocate_lisp_storage(size_t size)
{
	return xmalloc(size);
}
 ...
 */
void free_lcrecord(struct lcrecord_header *lcrecord)
{
...
Lisp_Object
make_bigf (double float_value, unsigned long precision)
{
	Lisp_Bigf *f;
	...
	bigf_set_double(bigf_data(f), float_value);
	return wrap_bigf(f);
}
...
/* This function creates a bigfloat with the default precision if the
   PRECISION argument is zero. */
Lisp_Object
make_bigfr (double float_value, unsigned long precision)
{
	Lisp_Bigfr *f;
	...
	bigfr_set_double(bigfr_data(f), float_value);
	return wrap_bigfr(f);
}
...
/* This function creates a bigfloat with the default precision if the
   PRECISION argument is zero. */
Lisp_Object
make_bigc(double re_value, double im_value, unsigned long precision)
{
	Lisp_Bigc *c;
	...
	bigc_set_double_double(bigc_data(c), re_value, im_value);
	return wrap_bigc(c);
}
...
Lisp_Object
make_quatern(long z, long i, long j, long k)
{
	Lisp_Quatern *g;
...
#ifdef HAVE_RESCLASS
DECLARE_FIXED_TYPE_ALLOC(resc_rng, Lisp_Resc_Rng);
#define MINIMUM_ALLOWED_FIXED_TYPE_CELLS_resc_rng 250
DECLARE_FIXED_TYPE_ALLOC(resc_elm, Lisp_Resc_Elm);
#define MINIMUM_ALLOWED_FIXED_TYPE_CELLS_resc_elm 250

Lisp_Object
make_resc_rng(unsigned long modulus)
{
	Lisp_Resc_Rng *r;
	
	ALLOCATE_FIXED_TYPE(resc_rng, Lisp_Resc_Rng, r);
	set_lheader_implementation(&r->lheader, &lrecord_resc_rng);
	resc_rng_init(resc_rng_data(r));
	bigz_set_long(resc_rng_data(r), modulus);
	return wrap_resc_rng(r);
}

Lisp_Object
make_resc_rng_bz(bigz modulus)
{
	Lisp_Resc_Rng *r;
	...
	set_lheader_implementation(&r->lheader, &lrecord_resc_rng);
	resc_rng_init(resc_rng_data(r));
	bigz_set(resc_rng_data(r), modulus);
	return wrap_resc_rng(r);
}

Lisp_Object
make_resc_elm(long element, Lisp_Object r)
{
	Lisp_Resc_Elm *e;
	
	ALLOCATE_FIXED_TYPE(resc_elm, Lisp_Resc_Elm, e);
	set_lheader_implementation(&e->lheader, &lrecord_resc_elm);
	resc_elm_init(resc_elm_data(e));
	bigz_set_long(resc_elm_data(e), element);
	bigz_mod(resc_elm_data(e), resc_elm_data(e), XRESC_RNG_DATA(r));
	resc_elm_ring(e) = r;
	return wrap_resc_rng(e);
}

Lisp_Object
make_resc_elm_bz(bigz element, Lisp_Object r)
{
	Lisp_Resc_Elm *e;
	...
	set_lheader_implementation(&e->lheader, &lrecord_resc_elm);
	resc_elm_init(resc_elm_data(e));
	bigz_set(resc_elm_data(e), element);
	bigz_mod(resc_elm_data(e), resc_elm_data(e), XRESC_RNG_DATA(r));
	resc_elm_ring(e) = r;
	return wrap_resc_rng(e);
}
...
DECLARE_FIXED_TYPE_ALLOC(indef, Lisp_Indef);
#define MINIMUM_ALLOWED_FIXED_TYPE_CELLS_indef 250
...
make_indef_internal(indef sym)
{
	...
	ALLOCATE_FIXED_TYPE(indef, Lisp_Indef, i);
...
static hashcode_t vector_hash(Lisp_Object obj, int depth)
{
...
static struct string_chars *allocate_string_chars_struct(Lisp_String *
							 string_it_goes_with,
							 EMACS_INT fullsize)
{
	...
	if (fullsize <= (countof(current_string_chars_block->string_chars)
		...
		struct string_chars_block *new_scb =
		    xnew(struct string_chars_block);
...
void resize_string(Lisp_String * s, Bytecount pos, Bytecount delta)
{
	...
	if (BIG_STRING_FULLSIZE_P(oldfullsize)) {
		if (BIG_STRING_FULLSIZE_P(newfullsize)) {
			...
			set_string_data(s,
					(Bufbyte *) xrealloc(string_data(s),
							     string_length(s) +
							     delta + 1));
			if (delta > 0 && pos >= 0)
				memmove(string_data(s) + pos + delta,
		...
		if (oldfullsize == newfullsize) {
			...
			Bufbyte *new_data = BIG_STRING_FULLSIZE_P(newfullsize)
			    ...
			    : allocate_string_chars_struct(s,
							   newfullsize)->chars;
...
Lisp_Object make_string(const Bufbyte * contents, Bytecount length)
{
	Lisp_Object val;
...
Lisp_Object make_string_nocopy(const Bufbyte * contents, Bytecount length)
{
	Lisp_String *s;
	Lisp_Object val;
...
static Lisp_Object mark_lcrecord_list(Lisp_Object obj)
{
	...
	while (!NILP(chain)) {
		...
		gc_checking_assert(	/* There should be no other pointers to the free list. */
...
static void sweep_lcrecords_1(struct lcrecord_header **prev, int *used)
{
	struct lcrecord_header *header;
	int num_used = 0;
	/* int total_size = 0; */
...
#ifdef HAVE_RESCLASS
static void
sweep_resc_rngs (void)
{
#define UNMARK_resc_rng(ptr) UNMARK_RECORD_HEADER(&((ptr)->lheader))
#define ADDITIONAL_FREE_resc_rng(ptr) resc_rng_fini(ptr->data)

	SWEEP_FIXED_TYPE_BLOCK(resc_rng, Lisp_Resc_Rng);
}
static void
sweep_resc_elms (void)
{
#define UNMARK_resc_elm(ptr) UNMARK_RECORD_HEADER(&((ptr)->lheader))
#define ADDITIONAL_FREE_resc_elm(ptr) resc_elm_fini(ptr->data)

	SWEEP_FIXED_TYPE_BLOCK(resc_elm, Lisp_Resc_Elm);
}
#endif /* HAVE_RESCLASS */

#ifdef WITH_NUMBER_TYPES
static void sweep_indefs(void)
{
#define UNMARK_indef(ptr) UNMARK_RECORD_HEADER(&((ptr)->lheader))
#define ADDITIONAL_FREE_indef(ptr)

	SWEEP_FIXED_TYPE_BLOCK(indef, Lisp_Indef);
}
...
#ifdef HAVE_RESCLASS
	/* Put all unmarked resclass rings on free list */
	sweep_resc_rngs();
	sweep_resc_elms();
#endif	/* HAVE_RESCLASS */

#ifdef WITH_NUMBER_TYPES
	/* Put all unmarked indefinite symbols on free list */
	sweep_indefs();
#endif	/* WITH_NUMBER_TYPES */
...
void garbage_collect_1(void)
{
	...
	if (!noninteractive) {
		...
		if (STRINGP(Vgc_message) && !cursor_changed && !FRAME_STREAM_P(f)) {
...
#endif /* HAVE_QUATERN */
#ifdef HAVE_RESCLASS
	HACK_O_MATIC(resc_rng, "resc_rng-storage", pl);
	pl = gc_plist_hack("resc_rngs-free", gc_count_num_resc_rng_freelist, pl);
	pl = gc_plist_hack("resc_rngs-used", gc_count_num_resc_rng_in_use, pl);
	HACK_O_MATIC(resc_elm, "resc_elm-storage", pl);
	pl = gc_plist_hack("resc_elms-free", gc_count_num_resc_elm_freelist, pl);
	pl = gc_plist_hack("resc_elms-used", gc_count_num_resc_elm_in_use, pl);
#endif /* HAVE_RESCLASS */
#ifdef WITH_NUMBER_TYPES
	HACK_O_MATIC(indef, "indef-storage", pl);
	pl = gc_plist_hack("indefs-free", gc_count_num_indef_freelist, pl);
	pl = gc_plist_hack("indefs-used", gc_count_num_indef_in_use, pl);
#endif /* WITH_NUMBER_TYPES */
