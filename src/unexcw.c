...
static void copy_executable_and_dump_data_section(int a_out, int a_new)
{
	...
	if (f_nextdata.s_lnnoptr != 0)
		f_nextdata.s_lnnoptr += file_sz_change;
	if (f_nextdata.s_relptr != 0)
		f_nextdata.s_relptr += file_sz_change;
	...
	for (i = 0; i < (f_hdr.f_nscns - 3); i++) {
		...
		if (section.s_lnnoptr != 0)
			section.s_lnnoptr += file_sz_change;
		if (section.s_relptr != 0)
			section.s_relptr += file_sz_change;
	...
	if (!sections_reversed) {
		...
		if (lseek(a_new, 0, SEEK_CUR) != f_nextdata.s_scnptr) {
			printf("at %lx should be at %lx\n",
			       lseek(a_new, 0, SEEK_CUR), f_nextdata.s_scnptr);
