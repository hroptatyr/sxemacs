...
static Lisp_Object traverse_keymaps_noop(Lisp_Object keymap, void *arg)
{
	return Qnil;
}
...
static Lisp_Object keymap_prompt_mapper(Lisp_Object keymap, void *arg)
{
...
ensure_meta_prefix_char_keymapp(Lisp_Object keys, int indx, Lisp_Object keymap)
{
	...
	if (EQ(keys, new_keys))
		...
		error_with_frob(mpc_binding,
				...
				(char *)XSTRING_DATA(Fkey_description(keys)),
				(char *)
				XSTRING_DATA(Fkey_description(new_keys)),
				(char *)
				XSTRING_DATA(Fsingle_key_description
					     (Vmeta_prefix_char)));
}
...
static Lisp_Object raw_lookup_key_mapper(Lisp_Object k, void *arg)
{
	/* This function can GC */
	struct raw_lookup_key_mapper_closure *c =
...
static Lisp_Object where_is_recursive_mapper(Lisp_Object map, void *arg)
{
	...
	for (submaps = keymap_submaps(map);
...

		{
			...
			lower =
			    traverse_keymaps(submap, Qnil,
					     where_is_recursive_mapper, c);
		  ...
		  Lisp_Object firstonly, char *target_buffer)
{
	...
	for (i = 0; i < nmaps; i++) {
		...
		this_result =
		    traverse_keymaps(maps[i], Qnil, where_is_recursive_mapper,
				     &c);
		if (!NILP(firstonly)) {
			result = this_result;
			if (!NILP(result))
...
static Lisp_Object describe_map_mapper_shadow_search(Lisp_Object map, void *arg)
{
...
static Lisp_Object keymap_lookup_inherited_mapper(Lisp_Object km, void *arg)
{
...
describe_map_mapper(const struct key_data *key,
		    Lisp_Object binding, void *describe_map_closure)
{
	/* This function can GC */
	...
	if (closure->mice_only_p &&
	    (!(EQ(keysym, Qbutton0) ||
	       ...
	       EQ(keysym, Qbutton6up) || EQ(keysym, Qbutton7up))))
		return;
...
static Lisp_Object describe_map_parent_mapper(Lisp_Object keymap, void *arg)
{
	/* This function can GC */
	struct describe_map_closure *describe_map_closure =
	    (struct describe_map_closure *)arg;
	describe_map_closure->self = keymap;
	map_keymap(XKEYMAP(keymap)->table, 0,	/* don't sort: we'll do it later */
