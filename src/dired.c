...
be returned.
	...
	if (!NILP(handler)) {
		UNGCPRO;
		if (!NILP(files_only))
			return call6(handler, Qdirectory_files, directory, full,
				     match, nosort, files_only);
		else
			return call5(handler, Qdirectory_files, directory, full,
				     match, nosort);
	}
      ...
      (filename))
{
	...
	{
		char *tmpnam =
		    (char *)XSTRING_DATA(Ffile_name_nondirectory(filename));
		int l = strlen(tmpnam);

		if (l >= 5 && S_ISREG(s.st_mode)
