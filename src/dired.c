...
Lisp_Object Qsorted_list;
Lisp_Object Qdesc_sorted_list;
...
#define dired_stack_t		Lisp_Dllist
#define new_dired_stack()	noseeum_make_dllist()
#define free_dired_stack(ds)	noseeum_free_dllist(ds)
...

static void
dfr_inner(dirent_t *res, unsigned int maxdepth,
	  Lisp_Object fulldir, Lisp_Object dir, Lisp_Object compbf,
	  int want_full_p, int symlink_file_p, Lisp_Object files_only,
	  ...
	  Lisp_Object bloom_filter)
{
	...
	if (!DIRENTRY_NONEMPTY(res) ||
	    (TRIVIAL_DIRECTORY_ENTRY(res->d_name) &&
	     !(NILP(Vdirectory_files_no_trivial_p) && maxdepth == 0)))
		return;
	...
	if (res->d_type == DT_DIR)
		dir_p = 1;
	else if (res->d_type == DT_LNK && !symlink_file_p) {
		...
		if (sxemacs_stat(statnam, &st) == 0 &&
		    ...
		    !NILP(compbf) && !(bloom_owns_p(XBLOOM(compbf), fullname)))
			dir_p = 1;
	...
	if (dir_p) {
		...
		dsi = xnew_and_zero(dired_stack_item_t);
		dsi->dir = name;
...
static void
dfr_outer(Lisp_Object directory, dirent_t *ent, unsigned int maxdepth,
	  Lisp_Object compbf,
	  int want_full_p, int symlink_file_p, Lisp_Object files_only,
	  dired_stack_t *ds, Lisp_Object match,
	  struct re_pattern_buffer *bufp, Lisp_Object result,
	  Lisp_Object bloom_filter)
{
	...
	while (DFR_READDIR(d, ent, &res) == 0 && res != NULL) {
		dfr_inner(res, maxdepth, fulldir, dir,
			  compbf, want_full_p, symlink_file_p,
...
static Lisp_Object
directory_files_magic(Lisp_Object directory, int full_p,
		      Lisp_Object match, Lisp_Object files_only,
		      unsigned int maxdepth,
		      Lisp_Object symlink_is_file, Lisp_Object bloom_filter)
{
	/* This function can GC */
	Lisp_Object result = wrap_dllist(make_dllist());
	Lisp_Object list = Qnil;
	dired_stack_t *ds = new_dired_stack();
	...
	GCPRO5(directory, result, list, compbf, bloom_filter);
	...
	ds_item = xnew_and_zero(dired_stack_item_t);
	ds_item->dir = make_string((Bufbyte*)"", 0);
	ds_item->depth = 0;
	dired_stack_push(ds, ds_item);

	/* malloc the directory entry pointer */
	ent = xnew_and_zero(dirent_t);

	while (dired_stack_size(ds) > 0) {
		dfr_outer(directory, ent, maxdepth, compbf,
			  full_p, !NILP(symlink_is_file),
			  files_only, ds, match,
...
Lisp_Object directory_files_canonicalise_dn(Lisp_Object directory)
{
	struct gcpro gcpro1;
	GCPRO1(directory);
...
Lisp_Object directory_files_resultify(Lisp_Object result,
				      Lisp_Object result_type)
{
	/* This function can GC */
      ...
      Lisp_Object arg6, Lisp_Object arg7, Lisp_Object arg8)
{
	...
	GCPRO1(args[0]);
	gcpro1.nvars = 10;
	RETURN_UNGCPRO(Ffuncall(10, args));
}
      ...
      (directory, full, match, result_type, files_only))
{
	...
	result = directory_files_magic(directory, !NILP(full), match,
				       files_only, 0, Qnil, Qnil);
       ...
       symlink_is_file, bloom_filter))
{
	Lisp_Object handler;
	Lisp_Object result = Qnil;
	unsigned int maxdepth_c = 64;
	struct gcpro gcpro1, gcpro2, gcpro3, gcpro4, gcpro5,
		gcpro6, gcpro7, gcpro8;
	GCPRO8(directory, full, match, result_type, files_only,
	...
	if (!NILP(handler)) {
		UNGCPRO;
		return call9(handler, Qdirectory_files_recur,
			     directory, full, match, result_type, files_only,
			     maxdepth, symlink_is_file, bloom_filter);
	}
	...
	result = directory_files_magic(directory, !NILP(full), match,
				       files_only, maxdepth_c, symlink_is_file,
				       bloom_filter);
