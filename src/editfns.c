      ...
      ())
{
	...
	if (!tmpdir) {
		struct stat st;
		int myuid = getuid();
		static char path[5 /* strlen ("/tmp/") */  + 1 +
				 _POSIX_PATH_MAX];
...
# if 0				/* changed by ben.  This behavior absolutely stinks, and the
				...
				if (initial_directory[0] != '\0') {
					cached_home_directory =
					    (Extbyte *) initial_directory;
				} else {
					/* This will probably give the wrong value */
					cached_home_directory =
					    (Extbyte *) getcwd(NULL, 0);
				}
# else
				/*
      ...
      (buffer, start, end))
{
	...
	bp = XBUFFER(emacs_get_buffer(buffer, 1));
...
If SXEmacs was compiled with --enable-ent the following additional
