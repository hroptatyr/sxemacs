       ...
       unsigned int emacs_edata, unsigned int dummy1, unsigned int dummy2)
{
	...
	if (ld)
		...
		dynamic_beg =
		    (caddr_t) EXEC_ALIGN(old_hdr.a_text) + old_hdr.a_data;
	   ...
	   corrections that will need to be made. */
	{
		...
		if (new_hdr.a_extension > 0) {
			new_hdr.a_extension +=
			    LESYM_OFFSET(new_hdr) - LESYM_OFFSET(old_hdr);
		}
...
#ifdef DEBUG
		printf
		    ("copying %#lx bytes of new text from %#lx to position %#lx\n",
		     newtext_size, old_dataddr,
		     TEXT_OFFSET(new_hdr) + old_dataddr);
#endif
...
#ifdef DEBUG
		printf
		    ("copying %#lx bytes of data from %#lx to position %#lx\n",
		     newdata1_size, new_dataddr, new_datoff);
#endif
		unexec_write(new_fd, new_datoff, (caddr_t) new_dataddr,
			     newdata1_size);
		...
		if (dyn_size) {
			...
			printf("copying %#lx bytes of dyn data from executable"
			       ...
			       dyn_size, dynamic_beg,
			       new_datoff + newdata1_size);
#endif
			unexec_copy(new_fd, old_fd,
				    old_datoff + newtext_size + newdata1_size,
...
#ifdef DEBUG
		printf
		    ("Copying %#lx bytes of junk from %#lx (old) to %#lx (new)\n",
...

		{
			...
			if ((new_header_delta > 0)
				...
				if (read(old_fd, &dhdr, sizeof(dhdr)) ==
				    sizeof(dhdr)) {
					...
					lseek(new_fd, EXT_OFFSET(new_hdr),
					      SEEK_SET);
					if (write(new_fd, &dhdr, sizeof(dhdr))
					    != sizeof(dhdr)) {
						unexec_error
						    ("Unable to write debug information to \"%s\"\n",
						     1, new_name);
					}
					lseek(new_fd, curpos, SEEK_SET);
				} else {
					unexec_error
					    ("Unable to read debug information from \"%s\"\n",
					     1, old_name);
				}
			}
		}
...
int run_time_remap(char *dummy)
{
	...
	if (sbrk_of_0_at_unexec < current_sbrk)
		...
		if (brk((caddr_t) sbrk_of_0_at_unexec))
			fprintf(stderr,
				"failed to change brk addr to 0x%x: %s\n",
