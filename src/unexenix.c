...
unsigned data_start, bss_start, entry_address;
{
	...
	*u_xsegp = &u_xseg;
	...
	READ(infd, u_xexecp, sizeof(struct xexec), "error reading %s", a_name);
	check_exec(u_xexecp);
	READ(infd, u_xextp, sizeof(struct xext), "error reading %s", a_name);
		...
		case XS_TTEXT:
			{
				...
				fatal_unexec("invalid text segment in %s",
					     a_name);
			}
		case XS_TDATA:
			{
				if (i == 1) {
					...
					datalen = datalim - (data_org =
							     (char *)(u_xsegp->
								      xs_rbase));
					datadiff = datalen - u_xsegp->xs_psize;
					break;
				}
				fatal_unexec("invalid data segment in %s",
					     a_name);
			}
		default:
			{
				if (i > 1)
					break;
				fatal_unexec("invalid segment record in %s",
					     a_name);
			}
		}
	}
	...
	READ(infd, u_xsegp, sizeof(struct xseg), "error reading %s", a_name);
	...
	WRITE(outfd, data_org, datalen, "write error on %s", new_name);

	for (i = 2, segpos += (2 * sizeof(struct xseg));
	     i < nsegs; i++, segpos += sizeof(struct xseg)) {
		SEEK(infd, segpos, "seek error on %s", a_name);
		READ(infd, u_xsegp, sizeof(struct xseg),
		...
		SEEK(outfd, u_xsegp->xs_filpos + datadiff, "seek error on %s",
		     new_name);
