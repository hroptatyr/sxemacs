...
static void *check_realloc(void *ptr, size_t size)
{
	...
	if (!ptr)
		return result;
	present =
	    (EMACS_INT) gethash(ptr, pointer_table, (const void **)&old_size);
	if (!present) {
...
static void log_gcpro(char *file, int line, struct gcpro *value, blocktype type)
{
	if (type == ungcpro_type) {
		...
		if (!gcprolist)
			abort();
		if (value == gcprolist->next)
			goto OK;
		if (!gcprolist->next)
			abort();
		if (value == gcprolist->next->next)
			goto OK;
		if (!gcprolist->next->next)
			abort();
		if (value == gcprolist->next->next->next)
			goto OK;
		if (!gcprolist->next->next->next)
			abort();
		if (value == gcprolist->next->next->next->next)
			goto OK;
		abort();
	      OK:;
	}
...
void debug_gcpro1(char *file, int line, struct gcpro *gcpro1, Lisp_Object * var)
{
...
void debug_ungcpro(char *file, int line, struct gcpro *gcpro1)
{
...
{
	int i, j;
	for (i = 0, j = gcprohistptr; i < GCPROHISTLIMIT; i++, j++) {
		if (j >= GCPROHISTLIMIT)
			j = 0;
		printf("%3d  %s		%d	%s	0x%x\n",
		       j, gcprohist[j].file, gcprohist[j].line,
		       (gcprohist[j].type ==
			gcpro1_type ? "GCPRO1" : gcprohist[j].
			type ==
			gcpro2_type ? "GCPRO2" : gcprohist[j].
			type ==
			gcpro3_type ? "GCPRO3" : gcprohist[j].
			type ==
			gcpro4_type ? "GCPRO4" : gcprohist[j].
			type ==
			gcpro5_type ? "GCPRO5" : gcprohist[j].
			type ==
			ungcpro_type ? "UNGCPRO" : "???"), gcprohist[j].value);
	}
	fflush(stdout);
}
