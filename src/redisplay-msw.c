 ...
 */
static HDC get_frame_dc(struct frame *f, int start_page_p)
{
 ...
 */
static HDC get_frame_compdc(struct frame *f)
{
		    ...
		    Lisp_Object bg_pmap)
{
	...
	if (!NILP(bg)) {
		...
		SetBkColor(hdc,
			   COLOR_INSTANCE_MSWINDOWS_COLOR(XCOLOR_INSTANCE(bg)));
	} else {
 ...
 ****************************************************************************/
static void
mswindows_output_hline(struct window *w, struct display_line *dl,
		       struct rune *rb)
{				/* XXX Implement me */
}
		       ...
		       struct rune *rb, int start_pixpos)
{
	...
	Lisp_Object bg_pmap =
	    WINDOW_FACE_CACHEL_BACKGROUND_PIXMAP(w, rb->findex);
			...
			int width, face_index findex, Emchar ch, int image_p)
{
	...
	if (focus && bar_p) {
		...
		mswindows_update_dc(hdc, Qnil, cachel->background, Qnil);
		ExtTextOut(hdc, xpos, dl->ypos, ETO_OPAQUE, &rect, NULL, 0,
			   NULL);
	} else if (!focus) {
		...
		cachel =
		    WINDOW_FACE_CACHEL(w,
				       (real_char_p ? findex :
					local_face_index));
		mswindows_update_dc(hdc, cachel->foreground, cachel->background,
				    Qnil);
		ExtTextOut(hdc, xpos, dl->ypos, ETO_OPAQUE | ETO_CLIPPED, &rect,
			   p_char, n_char, NULL);
	}
			...
			int cursor_height)
{
	...
	if (width < 0)
		width =
		    mswindows_text_width(cachel, Dynarr_atp(buf, 0),
					 Dynarr_length(buf));
#else
	assert(width >= 0);
#endif
	...
	if (!NILP(bg_pmap)) {
		...
		mswindows_update_dc(hdc,
				    cachel->foreground, cachel->background,
				    Qnil);
		redisplay_output_pixmap(w, bg_pmap, &db, &dga, findex, 0, 0, 0,
					TRUE);
	...
	for (i = 0; i < nruns; i++) {
		...
		mswindows_update_dc(hdc, cachel->foreground,
				    ...
				    Qnil);
		mswindows_set_dc_font(hdc, font, cachel->underline,
				      cachel->strikethru);
		...
		ExtTextOut(hdc, xpos, dl->ypos,
			   NILP(bg_pmap) ? ETO_CLIPPED | ETO_OPAQUE :
			   ETO_CLIPPED, &rect, (char *)runs[i].ptr, runs[i].len,
			   NULL);
			  ...
			  struct display_glyph_area *dga)
{
	...
	if (IMAGE_INSTANCE_MSWINDOWS_MASK(p)) {
		...
		StretchBlt(hdc,
			   ...
			   MulDiv(dga->height, real_y, surface_y), SRCCOPY);
				 ...
				 struct display_glyph_area *dga)
{
	...
	struct display_glyph_area xdga = { 0, 0, IMAGE_INSTANCE_PIXMAP_WIDTH(p),
	...
	while (xdga.height > 0) {
		...
		while (xdga.width > 0) {
			...
			xdga.yoffset =
			    xdb.ypos % IMAGE_INSTANCE_PIXMAP_HEIGHT(p);
			xdga.xoffset =
			    xdb.xpos % IMAGE_INSTANCE_PIXMAP_WIDTH(p);
				...
				int height)
{
	...
	if (!NILP(w->vchild)) {
		mswindows_redraw_exposed_windows(w->vchild, x, y, width,
						 height);
		return;
	} else if (!NILP(w->hchild)) {
		mswindows_redraw_exposed_windows(w->hchild, x, y, width,
						 height);
		return;
	}
	...
	if (Dynarr_largest(w->face_cachels))
		redisplay_clear_bottom_of_window(w, cdla, rect_draw.top,
						 rect_draw.bottom);
				 ...
				 int height)
{
	for (; !NILP(window); window = XWINDOW(window)->next)
		mswindows_redraw_exposed_window(XWINDOW(window), x, y, width,
						height);
}
 ...
 ****************************************************************************/
void
mswindows_redraw_exposed_area(struct frame *f, int x, int y, int width,
			      int height)
{
	...
	if (!f->window_face_cache_reset) {
		mswindows_redraw_exposed_windows(f->root_window, x, y, width,
						 height);
		GdiFlush();
	} else
 ...
 ****************************************************************************/
static int mswindows_divider_height(void)
{
 ...
 ****************************************************************************/
static int mswindows_eol_cursor_width(void)
{
 ...
 ****************************************************************************/
static void mswindows_frame_output_begin(struct frame *f)
{
}
 ...
 ****************************************************************************/
static void mswindows_frame_output_end(struct frame *f)
{
...
static void msprinter_frame_output_end(struct frame *f)
{
	GdiFlush();
}

static int mswindows_flash(struct device *d)
{
	...
	while (elt < end) {
		...
		if (rb->findex == findex && rb->type == RUNE_CHAR
			...
			if (Dynarr_length(buf)) {
				mswindows_output_string(w, dl, buf, xpos, 0,
							start_pixpos, width,
							findex, 0, 0, 0, 0);
				xpos = rb->xpos;
				width = 0;
			...
			if (rb->type == RUNE_CHAR) {
				...
				if (rb->cursor_type == CURSOR_ON)
					mswindows_output_cursor(w, dl, xpos,
								cursor_width,
								rb->findex, 0,
								0);
...
mswindows_output_vertical_divider(struct window *w, int clear_unused)
{
	...
	if (spacing) {
		...
		mswindows_update_dc(hdc, Qnil,
				    WINDOW_FACE_CACHEL_BACKGROUND(w,
								  DEFAULT_INDEX),
				    Qnil);
	...
	if (rect.left < rect.right) {
		...
		mswindows_update_dc(hdc, Qnil,
				    WINDOW_FACE_CACHEL_BACKGROUND(w, div_face),
				    Qnil);
		ExtTextOut(hdc, 0, 0, ETO_OPAQUE, &rect, NULL, 0, NULL);
	}
		       ...
		       Lisp_Object bcolor, Lisp_Object background_pixmap)
{
	...
	if (!NILP(background_pixmap)) {
		...
		mswindows_update_dc(hdc, fcolor, bcolor, background_pixmap);
...
/* XXX Implement me! */
static void mswindows_clear_frame(struct frame *f)
{
	GdiFlush();
}

