...
static void normalize_filename(char *fp, char path_sep)
{
	char sep;
	char *elem;
...
int crlf_to_lf(int n, unsigned char *buf, unsigned *lf_count)
{
...
static int parse_root(char *name, char **pPath)
{
	char *start = name;
...
static int get_long_basename(char *name, char *buf, int size)
{
...
BOOL win32_get_long_filename(char *name, char *buf, int size)
{
	char *o = buf;
	char *p;
	char *q;
...
LPBYTE nt_get_resource(char *key, LPDWORD lpdwtype)
{
	LPBYTE lpvalue;
	HKEY hrootkey = NULL;
	DWORD cbData;
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_ROOT, 0, KEY_READ, &hrootkey) ==
		...
		if (RegQueryValueEx(hrootkey, key, NULL, NULL, NULL, &cbData) ==
		    ...
		    && RegQueryValueEx(hrootkey, key, NULL, lpdwtype, lpvalue,
	...
	/* Check for environment variables and use registry if they don't exist */
	{
		...
		for (i = 0; i < countof(env_vars); i++) {
			if (!getenv(env_vars[i]) &&
			    (lpval =
			     nt_get_resource(env_vars[i], &dwType)) != NULL) {
				if (dwType == REG_EXPAND_SZ) {
					char buf1[500], buf2[500];

					ExpandEnvironmentStrings((LPSTR) lpval,
								 buf1, 500);
					_snprintf(buf2, 499, "%s=%s",
						  env_vars[i], buf1);
					putenv(strdup(buf2));
				} else if (dwType == REG_SZ) {
					char buf[500];

					_snprintf(buf, 499, "%s=%s",
						  env_vars[i], lpval);
					putenv(strdup(buf));
				}
...
static volume_info_data *lookup_volume_info(char *root_dir)
{
...
static void add_volume_info(char *root_dir, volume_info_data * info)
{
...
volume_info_data *GetCachedVolumeInformation(char *root_dir)
{
	volume_info_data *info;
	...
	if (info == NULL || !VOLINFO_STILL_VALID(root_dir, info)) {
		...
		if (!GetVolumeInformation(root_dir,
					  name, sizeof(name),
					  &serialnum,
					  &maxcomp, &flags, type, sizeof(type)))
			return NULL;
...
int get_volume_info(const char *name, const char **pPath)
{
	char temp[MAX_PATH];
...
int is_fat_volume(const char *name, const char **pPath)
{
...
const char *map_win32_filename(const char *name, const char **pPath)
{
...
struct direct *readdir(DIR * dirp)
{
	...
	/* Emacs never uses this value, so don't bother making it match
	   value returned by sxemacs_stat().  */
	dir_static.d_ino = 1;
...
time_t convert_time(FILETIME ft)
{
	...
	ret =
	    (long double)ft.dwHighDateTime * 4096 * 1024 * 1024 +
	    ft.dwLowDateTime;
	ret -= utc_base;
...
time_t convert_time(FILETIME uft)
{
	time_t ret;
#ifndef MAXLONGLONG
	SYSTEMTIME st;
...
void convert_from_time_t(time_t time, FILETIME * pft)
{
	long double tmp;
...
static unsigned hashval(const unsigned char *str)
{
	unsigned h = 0;
	while (*str) {
		h = (h << 4) + *str++;
...
static DWORD generate_inode_val(const char *name)
{
...
int mswindows_stat(const char *path, struct stat *buf)
{
	...
	errm = SetErrorMode(SEM_FAILCRITICALERRORS | SEM_NOOPENFILEERRORBOX);
	if (rootdir) {
	...
	if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
		...
		fh = CreateFile(name, 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
static void (__cdecl * signal_handlers[SIG_MAX]) (int) = {
0};
...
int open_input_file(file_data * p_file, const char *filename)
{
...
static IMAGE_SECTION_HEADER *rva_to_section(DWORD rva,
			  ...
			  int *is_cygnus_app)
{
	...
	*is_cygnus_app = FALSE;
	...
	if (p && stricmp(p, ".com") == 0)
		*is_dos_app = TRUE;
	else if (p && (stricmp(p, ".bat") == 0 || stricmp(p, ".cmd") == 0)) {
		...
		nt_header =
		    (PIMAGE_NT_HEADERS) ((char *)dos_header +
					 dos_header->e_lfanew);
			...
			*is_dos_app = TRUE;
			...
			section =
			    rva_to_section(import_dir.VirtualAddress,
					   nt_header);
			imports =
			    (IMAGE_IMPORT_DESCRIPTOR *) RVA_TO_PTR(import_dir.
								   VirtualAddress,
								   section,
								   executable);
			...
			for (; imports->Name; imports++) {
				char *dllname =
				    (char *)RVA_TO_PTR(imports->Name, section,
						       executable);
...
static void convert_from_time_t(time_t time, FILETIME * pft)
{
	long double tmp;
