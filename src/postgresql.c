/*
  ...
  Copyright (C) 2005 Sebastian Freundt <hroptatyr@sxemacs.org>
...
KNOWN PROBLEMS (Last update 2005/05/03) -- 'kay, let's gather them
+  None.
...
/* trans statuses */
static Lisp_Object Qpg_trans_idle, Qpg_trans_active, Qpg_trans_intrans;
static Lisp_Object Qpg_trans_inerror, Qpg_trans_unknown;

static Lisp_Object Qpqdb, Qpquser, Qpqpass, Qpqhost, Qpqport, Qpqtty;
static Lisp_Object Qpqoptions, Qpqerrormessage, Qpqbackendpid;
static Lisp_Object Qpqstatus, Qpqtransactionstatus, Qpqparameterstatus;
static Lisp_Object Qpqprotocolversion, Qpqserverversion;
#ifdef HAVE_OPENSSL
static Lisp_Object Qpqgetssl;
#endif
...
static Lisp_PGconn *allocate_pgconn(void)
{
#ifdef RUNNING_XEMACS_21_1
...
static void finalize_pgconn(void *header, int for_disksave)
{
...
static Lisp_Object make_pgresult(Lisp_PGresult * pgresult)
{
...
static Lisp_PGresult *allocate_pgresult(void)
{
#ifdef RUNNING_XEMACS_21_1
...
static void finalize_pgresult(void *header, int for_disksave)
{
 ...
 */
static void
sxemacs_notice_processor(Lisp_Object conn, const char *msg)
{
	/* void *arg is my sentinel function */
	Lisp_Object sentinel = XPGCONN(conn)->notice_processor;
	...
	if (NILP(sentinel))
		...
		call2_trapping_errors("Error in notice processor",
				      sentinel, conn, build_string(msg));
      ...
      (conn, field))
{
	...
	if (EQ(field, Qpqdb))
		...
		case CONNECTION_OK:
			return Qpg_connection_ok;
		case CONNECTION_BAD:
			return Qpg_connection_bad;
#ifdef HAVE_POSTGRESQLV7
		case CONNECTION_STARTED:
			return Qpg_connection_started;
		case CONNECTION_MADE:
			return Qpg_connection_made;
		case CONNECTION_AWAITING_RESPONSE:
			return Qpg_connection_awaiting_response;
		case CONNECTION_AUTH_OK:
			return Qpg_connection_auth_ok;
		case CONNECTION_SETENV:
			return Qpg_connection_setenv;
#endif				/* HAVE_POSTGRESQLV7 */
		default:
			/* they've added a new field we don't know about */
	...
	} else if (EQ(field, Qpqtransactionstatus)) {
		PGTransactionStatusType ts;
		switch ((ts = PQtransactionStatus(P))) {
		case PQTRANS_IDLE:
			return Qpg_trans_idle;
		case PQTRANS_ACTIVE:
			return Qpg_trans_active;
		case PQTRANS_INTRANS:
			return Qpg_trans_intrans;
		case PQTRANS_INERROR:
			return Qpg_trans_inerror;
		case PQTRANS_UNKNOWN:
			return Qpg_trans_unknown;
		default:
			/* they've added a new field we don't know about */
	...
	} else if (EQ(field, Qpqparameterstatus)) {
		return Qnil;
	} else if (EQ(field, Qpqprotocolversion))
		return make_int(PQprotocolVersion(P));
	else if (EQ(field, Qpqserverversion))
      ...
      (result))
{
	...
	case PGRES_EMPTY_QUERY:
		return Qpgres_empty_query;
	case PGRES_COMMAND_OK:
		return Qpgres_command_ok;
	case PGRES_TUPLES_OK:
		return Qpgres_tuples_ok;
	case PGRES_COPY_OUT:
		return Qpgres_copy_out;
	case PGRES_COPY_IN:
		return Qpgres_copy_in;
	case PGRES_BAD_RESPONSE:
		return Qpgres_bad_response;
	case PGRES_NONFATAL_ERROR:
		return Qpgres_nonfatal_error;
	case PGRES_FATAL_ERROR:
		return Qpgres_fatal_error;
	default:
		/* they've added a new field we don't know about */
      ...
      (conn))
{
	...
	if (!PGN)
		...
		temp =
		    list2(build_ext_string(PGN->relname, PG_OS_CODING),
			  make_int(PGN->be_pid));
		free((void *)PGN);
		return temp;
	}
      ...
      (conn, status))
{
	...
	if (EQ(status, Qpgres_empty_query))
		est = PGRES_EMPTY_QUERY;
	else if (EQ(status, Qpgres_command_ok))
		est = PGRES_COMMAND_OK;
	else if (EQ(status, Qpgres_tuples_ok))
		est = PGRES_TUPLES_OK;
	else if (EQ(status, Qpgres_copy_out))
		est = PGRES_COPY_OUT;
	else if (EQ(status, Qpgres_copy_in))
		est = PGRES_COPY_IN;
	else if (EQ(status, Qpgres_bad_response))
		est = PGRES_BAD_RESPONSE;
	else if (EQ(status, Qpgres_nonfatal_error))
		est = PGRES_NONFATAL_ERROR;
	else if (EQ(status, Qpgres_fatal_error))
      ...
      (string))
{
	...
	result = (char *)malloc(4*XSTRING_LENGTH(string));
...
void syms_of_postgresql(void)
{
	...
	defsymbol(&Qpg_connection_awaiting_response,
	...
	defsymbol(&Qpg_trans_idle, "pg::trans-idle");
	defsymbol(&Qpg_trans_active, "pg::trans-active");
	defsymbol(&Qpg_trans_intrans, "pg::trans-in-trans");
	defsymbol(&Qpg_trans_inerror, "pg::trans-in-error");
	defsymbol(&Qpg_trans_unknown, "pg::trans-unknown");
	...
	defsymbol(&Qpqtransactionstatus, "pq::transaction-status");
	defsymbol(&Qpqparameterstatus, "pq::parameter-status");
	defsymbol(&Qpqprotocolversion, "pq::protocol-version");
	defsymbol(&Qpqserverversion, "pq::server-version");
	defsymbol(&Qpqerrormessage, "pq::error-message");
	defsymbol(&Qpqbackendpid, "pq::backend-pid");
#ifdef HAVE_OPENSSL
	defsymbol(&Qpqgetssl, "pq::getssl");
#endif
