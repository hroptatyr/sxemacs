			...
			int output_is_std_handle, int must_flush)
{
	if (stream) {
		...
		int no_useful_stderr = errhand == 0
		    || errhand == INVALID_HANDLE_VALUE;

		if (!no_useful_stderr)
			no_useful_stderr =
			    !PeekNamedPipe(errhand, 0, 0, 0, 0, 0);
...
static void print_error_message(Lisp_Object error_object, Lisp_Object stream)
{
	/* This function can GC */
...
void print_internal(Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
{
	/* This function can GC */
	...
	case Lisp_Type_Record:
		{
			...
			if (LHEADER_IMPLEMENTATION(lheader)->printer)
				...
				default_object_printer(obj, printcharfun,
						       escapeflag);
			break;
		}
