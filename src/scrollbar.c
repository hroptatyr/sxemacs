...
static void free_window_mirror_scrollbars(struct window_mirror *mir)
{
...
static struct window_mirror *free_scrollbars_loop(Lisp_Object window,
...

static struct scrollbar_instance *create_scrollbar_instance(struct frame *f,
							    int vertical)
{
...
static struct scrollbar_instance *get_scrollbar_instance(struct frame *f,
							 int vertical)
{
 ...
 */
static Bufpos scrollbar_point(struct window *w, int flag)
{
	...
	buf = emacs_get_buffer(w->buffer, 0);
	if (!NILP(buf))
		b = XBUFFER(buf);
	else
		return start_pos;
			  ...
			  struct scrollbar_instance *instance)
{
	...
	{
		/* fuck, just for an assertion? */
		Lisp_Object tmp = real_window(instance->mirror, 0);
		assert(instance->mirror && XWINDOW(tmp) == w);
	}
	...
	if (!MINI_WINDOW_P(w) && vertical) {
		if (!DEVMETH_OR_GIVEN
		    (d, inhibit_scrollbar_slider_size_change, (), 0)) {
...
void init_frame_scrollbars(struct frame *f)
{
	...
	if (HAS_DEVMETH_P(d, create_scrollbar_instance)) {
		...
		call_critical_lisp_code(XDEVICE(FRAME_DEVICE(f)),
					Qinit_scrollbar_from_resources, frame);
...
void init_device_scrollbars(struct device *d)
{
	if (HAS_DEVMETH_P(d, create_scrollbar_instance)) {
		...
		call_critical_lisp_code(d,
					Qinit_scrollbar_from_resources, device);
		unbind_to(depth, Qnil);
	}
}
...
static void scrollbar_reset_cursor(Lisp_Object win, Lisp_Object orig_pt)
{
	...
	int selected = ((w == XWINDOW(tmp_win)) ? 1 : 0);
      ...
      (object))
{
	...
	/* Motif and Athena scrollbars behave differently, but in accordance
	   ...
	   differences down in lwlib because knowledge of SXEmacs buffer and
      ...
      (window, value))
{
	...
	if (hscroll < 0)
		hscroll = 0;
	else if (hscroll >= (1 << (SXE_SHORTBITS - 1)) - 1)
		hscroll = (1 << (SXE_SHORTBITS - 1)) - 1;
...
This is a specifier; use `set-specifier' to change it.
	...
	set_specifier_fallback(Vscrollbar_on_top_p, list1(Fcons(Qnil, Qnil)));
...
void complex_vars_of_scrollbar(void)
{
	...
	set_specifier_caching(XGLYPH(Vscrollbar_pointer_glyph)->image,
			      ...
			      scrollbar_pointer_changed_in_window, 0, 0, 0);
}
