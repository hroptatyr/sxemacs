...
static int allocate_pty(void)
{
	...
	/* Master clone devices are available on most systems */
	{
		...
		for (i = 0; i < countof(clones); i++) {
			clone = clones[i];
			master_fd =
			    open(clone, O_RDWR | O_NONBLOCK | OPEN_BINARY, 0);
			if (master_fd >= 0)
				goto have_master;
		}
		clone = NULL;
...
static void unix_send_process(Lisp_Object proc, struct lstream *lstream)
{
	/* Use volatile to protect variables from being clobbered by longjmp.  */
...
#if defined(HAVE_OPENSSL) && defined(OPENSSL_SSL)
					if (SSLCONNP(p->process_type_data)) {
						Lisp_Object
							c = p
							->process_type_data;
						writeret = SSL_write(XSSLCONN(c)
								     ->ssl_conn,
								     chunkbuf,
								     chunklen);
					break;
...
static Lisp_Object unix_network_process_listener(Lisp_Object process)
{
        ...
        int           sock = 0;
        struct  gcpro ngcpro,ngcpro1,ngcpro2;
...
static void unix_network_server_accept(Lisp_Object process)
{
	...
	struct sockaddr_in sa;
        ...
        ns = accept(UNIX_DATA(p)->infd, (struct sockaddr*)(&sa), &sa_size);
        if ( ns < 0 ) 
                return;
...

	{
		...
		if (INTP(service))
			...
			port = svc_info->s_port;
                ...
                if (SYMBOLP(host)) {
                        if (EQ(host,Qip_any)) {
                                address.sin_addr = IPADDR_ANY;
                        } else if (EQ(host,Qlocalhost)) {
                                address.sin_addr = IPADDR_LOOPBACK;
                        } else {
                                invalid_argument("invalid host ",host);
                        }
                } else {
