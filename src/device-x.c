...
#if defined WITH_EMODULES && defined HAVE_EMODULES &&	\
	defined(LWLIB_USES_ATHENA) && !defined(HAVE_ATHENA_3D)
...

	{
		...
		if (xaw_dll_handle != NULL) {
			...
			void *xaw_function_handle =
			    lt_dlsym(xaw_dll_handle,
				     "Xaw3dComputeTopShadowRGB");

			/* If we found it, warn the user in big, nasty,
			   unfriendly letters */
			if (xaw_function_handle != NULL) {
				warn_when_safe(
					Qdevice, Qerror,
					big_messy_error_string);
			}
