...
static void sanity_check_geometry_resource(Display * dpy)
{
	...
	strncpy(buf1, app_name, sizeof(buf1)-1);
        buf1[sizeof(buf1)-1]='\0';
	strncpy(buf2, app_class, sizeof(buf2)-1);
        buf2[sizeof(buf2)-1]='\0';

	for (s = buf1; *s; s++)
		if (*s == '.')
			*s = '_';
	strncat(buf1, "._no_._such_._resource_.geometry", sizeof(buf1) - strlen(buf1) - 1);
        buf1[sizeof(buf1)-1]='\0';
	strncat(buf2, "._no_._such_._resource_.Geometry",  sizeof(buf2) - strlen(buf2) - 1);
	...
	if (XrmGetResource(XtDatabase(dpy), buf1, buf2, &type, &value) == True) {
		warn_when_safe(Qgeometry, Qerror,
			       ...
			       "SXEmacs (and most other X programs) malfunction in obscure ways. (i.e.\n"
...
static void x_init_device_class(struct device *d)
{
 ...
 */
static Extbyte *compute_x_app_name(int argc, Extbyte ** argv)
{
	int i;
	Extbyte *ptr;
...
static void validify_resource_component(char *str, size_t len)
{
	for (; len; len--, str++)
...
static void x_init_device(struct device *d, Lisp_Object props)
{
		...
		if (xaw_dll_handle != NULL) {
			...
			if (xaw_function_handle != NULL) {
				warn_when_safe(Qdevice, Qerror, "\n"
					       "It seems that SXEmacs is built dynamically linked to the flat Athena widget\n"
					       "library but it finds a 3D Athena variant with the same name at runtime.\n"
					       "\n"
					       "This WILL cause your SXEmacs process to dump core at some point.\n"
					       ...
					       "(register SXEmacs as incompatible with all 3d widget sets, see\n"
					       ...
					       "using `ldd /path/to/sxemacs' under other Linux distributions.  One\n"
...
#if defined(LWLIB_MENUBARS_MOTIF) || defined(HAVE_XIM) || defined (USE_XFONTSET)
	{
		...
		if (STRINGP(Vx_app_defaults_directory) &&
			...
			LISP_STRING_TO_EXTERNAL(Vx_app_defaults_directory,
						data_dir, Qfile_name);
			path =
			    (char *)alloca(strlen(data_dir) + strlen(locale) +
					   7);
			...
			LISP_STRING_TO_EXTERNAL(Vdata_directory, data_dir,
						Qfile_name);
			path =
			    (char *)alloca(strlen(data_dir) + 13 +
					   strlen(locale) + 7);
			sprintf(path, "%sapp-defaults/%s/Emacs", data_dir,
				locale);
			if (!access(path, R_OK))
				XrmCombineFileDatabase(path, &db, False);
		}
	...
	XtGetApplicationNameAndClass(dpy, (char **)&app_name,
				     (char **)&app_class);
	/* search for a matching visual if requested by the user, or setup the display default */
	{
		...
		char *buf1 =
		    alloca_array(char, strlen(app_name) + resource_name_length);
		char *buf2 =
		    alloca_array(char,
				 strlen(app_class) + resource_name_length);
		char *type;
		XrmValue value;
...
#define CHECK_VIS_CLASS(visual_class)					\
	...
	cnt = sizeof (#visual_class) - 1, vis_class = visual_class
			...
			if (cnt) {
				...
				if (depth == 0) {
					stderr_out
					    ("Invalid Depth specification in %s... ignoring...\n",
					     str);
				} else {
					if (XMatchVisualInfo
						...
						stderr_out
						    ("Can't match the requested visual %s... using defaults\n",
						     str);
					}
				}
			} else {
				stderr_out
				    ("Invalid Visual specification in %s... ignoring.\n",
				     str);
			}
		}
		if (visual == NULL) {
      ...
      (font_path, device))
{
	...
	EXTERNAL_LIST_LOOP(path_entry, font_path) {
		LISP_STRING_TO_EXTERNAL(XCAR(path_entry), directories[i++],
					Qfile_name);
	}
