...
static int emacs_tty_add_timeout(EMACS_TIME thyme)
{
	return add_low_level_timeout(&tty_timer_queue, thyme);
}

static void emacs_tty_remove_timeout(int id)
{
	remove_low_level_timeout(&tty_timer_queue, id);
}

static void tty_timeout_to_emacs_event(Lisp_Event * emacs_event)
{
...

static int emacs_tty_event_pending_p(int user_p)
{
	if (!user_p) {
		EMACS_TIME sometime;
...
static void emacs_tty_next_event(Lisp_Event * emacs_event)
{
	while (1) {
		int ndesc;
		int i;
		...
		if (!get_low_level_timeout_interval
		    (tty_timer_queue, &time_to_block))
			/* no timer events; block indefinitely */
			pointer_to_this = 0;
		else {
			EMACS_TIME_TO_SELECT_TIME(time_to_block,
		...
		if (ndesc > 0) {
			...
			for (i = 0; i < MAXDESC; i++) {
				if (FD_ISSET(i, &temp_mask)
				    && FD_ISSET(i, &process_only_mask)) {
					Lisp_Object process;
					Lisp_Process *p =
					    get_process_from_usid(FD_TO_USID
								  (i));
					...
					emacs_event->timestamp = 0;	/* #### */
					emacs_event->event.process.process =
					    process;
					return;
				}
			}
...
static void emacs_tty_handle_magic_event(Lisp_Event * emacs_event)
{
	/* Nothing to do currently */
}

static void emacs_tty_select_process(Lisp_Process * process)
{
	event_stream_unixoid_select_process(process);
}

static void emacs_tty_unselect_process(Lisp_Process * process)
{
	event_stream_unixoid_unselect_process(process);
}

static void emacs_tty_select_console(struct console *con)
{
	event_stream_unixoid_select_console(con);
}

static void emacs_tty_unselect_console(struct console *con)
{
	event_stream_unixoid_unselect_console(con);
}

static void emacs_tty_quit_p(void)
{
