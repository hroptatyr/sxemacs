...
static Lisp_Object mswindows_key_to_emacs_keysym(int mswindows_key, int mods,
						 int extendedp);
static int mswindows_modifier_state(BYTE * keymap, DWORD fwKeys, int has_AltGr);
 ...
 * one.
 */
static Lisp_Object mswindows_u_dispatch_event_queue,
...
static DWORD WINAPI slurp_thread(LPVOID vparam)
{
...
static Lisp_Object make_ntpipe_input_stream(HANDLE hpipe, LPARAM param)
{
	Lisp_Object obj;
...
static LPARAM get_ntpipe_input_stream_param(Lstream * stream)
{
	struct ntpipe_slurp_stream *s = NTPIPE_SLURP_STREAM_DATA(stream);
	return s->user_data;
}

static HANDLE get_ntpipe_input_stream_waitable(Lstream * stream)
{
	struct ntpipe_slurp_stream *s = NTPIPE_SLURP_STREAM_DATA(stream);
	...
	*(data++) = s->onebyte;
...

	{
		...
		if (size > 0) {
			...
			if (PeekNamedPipe
			    (s->hpipe, NULL, 0, NULL, &bytes_available, NULL)) {
				...
				if (bytes_available)
					ReadFile(s->hpipe, data,
						 min(bytes_available, size),
						 &bytes_read, NULL);
			}
...
static int ntpipe_slurp_closer(Lstream * stream)
{
	/* This function must be called from the main thread only */
	struct ntpipe_slurp_stream_shared_data *s =
	    NTPIPE_SLURP_STREAM_DATA(stream)->thread_data;
...
static void init_slurp_stream(void)
{
...
#ifndef HAVE_MSG_SELECT
static DWORD WINAPI shove_thread(LPVOID vparam)
{
	...
	for (;;) {
		...
		if (s->size > 0) {
			if (!WriteFile
			    (s->hpipe, s->buffer, s->size, &bytes_written, NULL)
...
static Lisp_Object make_ntpipe_output_stream(HANDLE hpipe, LPARAM param)
{
	Lisp_Object obj;
...
static LPARAM get_ntpipe_output_stream_param(Lstream * stream)
{
...
static int ntpipe_shove_was_blocked_p(Lstream * stream)
{
	struct ntpipe_shove_stream *s = NTPIPE_SHOVE_STREAM_DATA(stream);
	return s->blocking_p;
}

static int ntpipe_shove_closer(Lstream * stream)
{
	struct ntpipe_shove_stream *s = NTPIPE_SHOVE_STREAM_DATA(stream);
...
static void init_shove_stream(void)
{
...
static void winsock_initiate_read(struct winsock_stream *str)
{
	...
	if (str->pending_p) {
		if (WaitForSingleObject(str->ov.hEvent, 0) == WAIT_TIMEOUT) {
			...
			if (!GetOverlappedResult
			    ((HANDLE) str->s, &str->ov, &str->bufsize, TRUE)) {
	       ...
	       Lstream_data_count size)
{
	...
	if (str->pending_p) {
		if (WaitForSingleObject(str->ov.hEvent, 0) == WAIT_TIMEOUT) {
			...
			DWORD dw_unused;
			if (!GetOverlappedResult
			    ((HANDLE) str->s, &str->ov, &dw_unused, TRUE))
				str->error_p = 1;
			str->pending_p = 0;
		}
	}
...
static int winsock_closer(Lstream * lstr)
{
...
static int winsock_was_blocked_p(Lstream * stream)
{
	struct winsock_stream *str = WINSOCK_STREAM_DATA(stream);
...
static Lisp_Object make_winsock_input_stream(SOCKET s, LPARAM param)
{
	return make_winsock_stream_1(s, param, "r");
}

static Lisp_Object make_winsock_output_stream(SOCKET s, LPARAM param)
{
	return make_winsock_stream_1(s, param, "w");
}

static HANDLE get_winsock_stream_waitable(Lstream * lstr)
{
	struct winsock_stream *str = WINSOCK_STREAM_DATA(lstr);
	return str->ov.hEvent;
}

static LPARAM get_winsock_stream_param(Lstream * lstr)
{
	struct winsock_stream *str = WINSOCK_STREAM_DATA(lstr);
	return str->user_data;
}

static void init_winsock_stream(void)
{
...
static int mswindows_user_event_p(Lisp_Event * sevt)
{
...
static void mswindows_enqueue_process_event(Lisp_Process * p)
{
	Lisp_Object emacs_event = Fmake_event(Qnil, Qnil);
	Lisp_Event *event = XEVENT(emacs_event);
	Lisp_Object process;
...
static Lisp_Object mswindows_dequeue_dispatch_event(void)
{
	...
	event = dequeue_event(NILP(mswindows_u_dispatch_event_queue) ?
			      &mswindows_s_dispatch_event_queue :
...
Lisp_Object mswindows_cancel_dispatch_event(Lisp_Event * match)
{
	Lisp_Object event;
	...
	EVENT_CHAIN_LOOP(event, *head) {
		...
		if ((e->event_type == match->event_type) &&
		    ((e->event_type == timeout_event) ?
		     (e->event.timeout.interval_id ==
		      match->event.timeout.interval_id) :
		     /* Must be key_press_event */
		     ((e->event.key.modifiers & match->event.key.modifiers) !=
		      0))) {
			if (NILP(previous_event))
				dequeue_event(head, tail);
			else {
				XSET_EVENT_NEXT(previous_event,
						XEVENT_NEXT(event));
				if (EQ(*tail, event))
					*tail = previous_event;
			}
...
/************************************************************************/
static int find_waitable_handle(HANDLE h)
{
	int i;
...
static BOOL add_waitable_handle(HANDLE h)
{
...
static void remove_waitable_handle(HANDLE h)
{
 ...
 */
static Lisp_Object mswindows_unsafe_pump_events(Lisp_Object u_n_u_s_e_d)
{
...
Lisp_Object mswindows_pump_outstanding_events(void)
{
	...
	if (NILP(mswindows_error_caught_in_modal_loop))
		result =
		    mswindows_protect_modal_loop(mswindows_unsafe_pump_events,
						 Qnil);
	UNGCPRO;
	return result;
}
 ...
 */
static void mswindows_drain_windows_queue(void)
{
	MSG msg;
	...
	while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
		...
		GetClassName(msg.hwnd, class_name_buf,
			     sizeof(class_name_buf) - 1);
		if (stricmp(class_name_buf, XEMACS_CLASS) != 0) {
			...
			msframe =
			    FRAME_MSWINDOWS_DATA(XFRAME
						 (mswindows_find_frame
						  (msg.hwnd)));
			if (!msframe->paint_pending) {
				msframe->paint_pending = 1;
				mswindows_enqueue_magic_event(msg.hwnd,
							      WM_PAINT);
			}
 ...
 */
static void mswindows_need_event_in_modal_loop(int badly_p)
{
	MSG msg;
 ...
 */
static void mswindows_need_event(int badly_p)
{
	int active;
	...
	while (NILP(mswindows_u_dispatch_event_queue)
		...
		if (badly_p)
			...
			EMACS_TIME_TO_SELECT_TIME(sometime,
		...
		if (active == 0) {
			...
			if (FD_ISSET(windows_fd, &temp_mask)) {
				if (mswindows_in_modal_loop)
					mswindows_need_event_in_modal_loop
					    (badly_p);
				else
				...
				for (i = 0; i < MAXDESC - 1; i++) {
					...
					if (FD_ISSET(i, &temp_mask)
					    && FD_ISSET(i, &tty_only_mask)) {
						struct console *c =
						    tty_find_console_from_fd(i);
				...
				for (i = 0; i < MAXDESC - 1; i++) {
					if (FD_ISSET(i, &temp_mask)) {
						if (FD_ISSET
							...
							mswindows_enqueue_process_event
							    (p);
						} else {
		...
		if (what_events == QS_ALLINPUT && badly_p &&
			...
			active =
			    MsgWaitForMultipleObjects(mswindows_waitable_count,
		...
		if (active == WAIT_TIMEOUT) {
			...
			if (p != NULL) {
				...
				/* None. This means that the process handle itself has signaled.
				   ...
				   possible. */
				LIST_LOOP_3(vaffanculo, Vprocess_list,
					    vproctail)
				    if (get_nt_process_handle
					(XPROCESS(vaffanculo)) ==
					mswindows_waitable_handles[ix])
					break;
				mswindows_waitable_handles[ix] =
				 ...
				 */
				if (!NILP(vproctail)) {
					mswindows_enqueue_process_event(XPROCESS
									(vaffanculo));
				} else	/* trash me soon. */
					/* Have to return something: there may be no accompanying
					   process event */
					mswindows_enqueue_magic_event(NULL,
								      XM_BUMPQUEUE);
			}
		}
#endif
	...
	case XTYP_WILDCONNECT:
		{
			...
			if (!
			    (hszItem
			     || DdeCmpStringHandles(hszItem,
						    mswindows_dde_service))
			    && !(hszTopic
				 || DdeCmpStringHandles(hszTopic,
							mswindows_dde_topic_system)))
				return (DdeCreateDataHandle
					(mswindows_dde_mlid, (LPBYTE) pairs,
...
#ifdef CYGWIN
			filename =
			    alloca(cygwin_win32_to_posix_path_list_buf_size(cmd)
				   + 5);
 ...
 */
static void mswindows_handle_paint(struct frame *frame)
{
	...
	if (GetUpdateRect(hwnd, NULL, FALSE)) {
		...
		if (!check_for_ignored_expose(frame, x, y, width, height)) {
			...
			mswindows_redraw_exposed_area(frame, x, y, width,
						      height);
 ...
 */
static int key_needs_default_processing_p(UINT vkey)
{
				  ...
				  int downp, int keyp)
{
	...
	if (!(keyp && (wParam == VK_CONTROL || wParam == VK_LCONTROL || wParam == VK_RCONTROL || wParam == VK_MENU || wParam == VK_LMENU || wParam == VK_RMENU || wParam == VK_SHIFT || wParam == VK_LSHIFT || wParam == VK_RSHIFT))) {	/* Not a modifier key */
...
static void clear_sticky_modifiers(void)
{
	need_to_add_mask = 0;
	last_downkey = 0;
	down_mask = 0;
...
static void output_modifier_keyboard_state(void)
{
	BYTE keymap[256];
	...
	stderr_out
	    ("GetKeyboardState VK_MENU %d %d VK_LMENU %d %d VK_RMENU %d %d\n",
	     keymap[VK_MENU] & 0x80 ? 1 : 0, keymap[VK_MENU] & 0x1 ? 1 : 0,
	     keymap[VK_LMENU] & 0x80 ? 1 : 0, keymap[VK_LMENU] & 0x1 ? 1 : 0,
	     keymap[VK_RMENU] & 0x80 ? 1 : 0, keymap[VK_RMENU] & 0x1 ? 1 : 0);
	stderr_out
	    ("GetKeyboardState VK_CONTROL %d %d VK_LCONTROL %d %d VK_RCONTROL %d %d\n",
	...
	stderr_out
	    ("GetKeyboardState VK_SHIFT %d %d VK_LSHIFT %d %d VK_RSHIFT %d %d\n",
	     keymap[VK_SHIFT] & 0x80 ? 1 : 0, keymap[VK_SHIFT] & 0x1 ? 1 : 0,
	     keymap[VK_LSHIFT] & 0x80 ? 1 : 0, keymap[VK_LSHIFT] & 0x1 ? 1 : 0,
	     keymap[VK_RSHIFT] & 0x80 ? 1 : 0, keymap[VK_RSHIFT] & 0x1 ? 1 : 0);
}
...
static void output_alt_keyboard_state(void)
{
	BYTE keymap[256];
	SHORT keystate[3];
	// SHORT asyncstate[3];
...
/*   asyncstate[2] = GetAsyncKeyState (VK_RMENU); */
	...
	stderr_out("GetKeyState VK_MENU %d %d VK_LMENU %d %d VK_RMENU %d %d\n",
		   keystate[0] & 0x8000 ? 1 : 0, keystate[0] & 0x1 ? 1 : 0,
		   keystate[1] & 0x8000 ? 1 : 0, keystate[1] & 0x1 ? 1 : 0,
		   keystate[2] & 0x8000 ? 1 : 0, keystate[2] & 0x1 ? 1 : 0);
...
mswindows_wnd_proc(HWND hwnd, UINT message_, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		mswindows_enqueue_misc_user_event(fobj, Qeval,
						  list3(Qdelete_frame, fobj,
							Qt));
		break;
	...
	case WM_SYSKEYUP:
		...
		/* See Win95 comment under WM_KEYDOWN */
		{
			...
			if (wParam == VK_CONTROL) {
				GetKeyboardState(keymap);
				keymap[(lParam & 0x1000000) ? VK_RCONTROL :
				       VK_LCONTROL] &= ~0x80;
				should_set_keymap = 1;
			} else if (wParam == VK_MENU) {
				GetKeyboardState(keymap);
				keymap[(lParam & 0x1000000) ? VK_RMENU :
				       VK_LMENU] &= ~0x80;
				should_set_keymap = 1;
			}
	...
	case WM_SYSKEYDOWN:
		 ...
		 * calling TranslateMessage() unless AltGr is *really* down. */
		{
			...
			if ((sticky_changed =
			     mswindows_handle_sticky_modifiers(wParam, lParam,
							       1, 1))) {
			...
			mods =
			    mswindows_modifier_state(keymap_sticky, (DWORD) - 1,
						     has_AltGr);
			...
			if (!NILP
			    (keysym =
			     mswindows_key_to_emacs_keysym(wParam, mods,
							   extendedp))) {
				mswindows_enqueue_keypress_event(hwnd, keysym,
								 mods);
				if (sticky_changed)
				...
				Emchar quit_ch =
				    CONSOLE_QUIT_CHAR(XCONSOLE
						      (mswindows_find_console
						       (hwnd)));
				POINT pnt =
				...
				if (wParam == VK_CONTROL) {
					keymap_orig[extendedp ? VK_RCONTROL :
						    VK_LCONTROL] |= 0x80;
					keymap_sticky[extendedp ? VK_RCONTROL :
						      VK_LCONTROL] |= 0x80;
				} else if (wParam == VK_MENU) {
					keymap_orig[extendedp ? VK_RMENU :
						    VK_LMENU] |= 0x80;
					keymap_sticky[extendedp ? VK_RMENU :
						      VK_LMENU] |= 0x80;
				}
				...
				while (PeekMessage
				       (&tranmsg, hwnd, WM_CHAR, WM_CHAR,
					PM_REMOVE)
				       || PeekMessage(&tranmsg, hwnd,
					...
					} else if (potential_accelerator
						   && !got_accelerator
						   &&
						   mswindows_char_is_accelerator
						   (frame, ch)) {
						got_accelerator = 1;
						break;
					}
					mswindows_enqueue_keypress_event(hwnd,
									 make_char
									 (ch),
									 mods_with_quit);
				}	/* while */
	...
	case WM_LBUTTONUP:
		...
		if (msframe->ignore_next_lbutton_up) {
			...
			if (msframe->button2_need_rbutton) {
				...
				mswindows_enqueue_mouse_button_event(hwnd,
								     WM_LBUTTONDOWN,
								     MAKEPOINTS
								     (lParam),
								     wParam &
	...
	case WM_LBUTTONDOWN:
		...
		if (msframe->button2_need_lbutton) {
			...
			msframe->button2_need_lbutton = 0;
			msframe->button2_need_rbutton = 0;
			if (mswindows_button2_near_enough
	...
	case WM_MOUSEMOVE:
		...
		if (!msframe->sizing) {
			...
			if ((msframe->button2_need_lbutton
			     || msframe->button2_need_rbutton)
			    && !mswindows_button2_near_enough(msframe->
							      last_click_point,
							      MAKEPOINTS
							      (lParam))) {
				KillTimer(hwnd, BUTTON_2_TIMER_ID);
				SendMessage(hwnd, WM_TIMER, BUTTON_2_TIMER_ID,
					    0);
			}
	...
	case WM_NOTIFY:
		{
			...
			/* handle tree view callbacks */
				...
				frame = XFRAME(mswindows_find_frame(hwnd));
				mswindows_handle_gui_wm_command(frame, 0,
								ptree->itemNew.
								lParam);
			}
			/* handle tab control callbacks */
				...
				TC_ITEM item;
				int idx =
				    SendMessage(nmhdr->hwndFrom, TCM_GETCURSEL,
						0, 0);
				frame = XFRAME(mswindows_find_frame(hwnd));

				item.mask = TCIF_PARAM;
				SendMessage(nmhdr->hwndFrom, TCM_GETITEM,
					    (WPARAM) idx, (LPARAM) & item);
	...
	case WM_SHOWWINDOW:
		 ...
		 */
		{
			...
			frame = XFRAME(fobj);
			if (wParam == TRUE) {
				mswindows_enqueue_magic_event(hwnd,
							      XM_MAPFRAME);
				FRAME_VISIBLE_P(frame) = 1;
			} else {
				mswindows_enqueue_magic_event(hwnd,
	...
	case WM_SIZE:
		...
		if (wParam == SIZE_RESTORED || wParam == SIZE_MAXIMIZED
			...
			if (wParam == SIZE_MINIMIZED) {
				...
				pixel_to_real_char_size(frame, rect.right,
							rect.bottom,
							&FRAME_MSWINDOWS_CHARWIDTH
							(frame),
							&FRAME_MSWINDOWS_CHARHEIGHT
							(frame));

				pixel_to_char_size(frame, rect.right,
						   rect.bottom, &columns,
						   &rows);
	...
	case WM_WINDOWPOSCHANGING:
		{
			...
			if (wpl.showCmd != SW_SHOWMINIMIZED
				...
				round_size_to_real_char(XFRAME
							(mswindows_find_frame
							 (hwnd)),
							wp->cx - (ncsize.right -
								  ncsize.left),
	...
	case WM_HSCROLL:
		{
			...
			mswindows_handle_scrollbar_event(hwndScrollBar, code,
							 pos);
	...
	case WM_MOUSEWHEEL:
		{
			...
			/* enqueue button4/5 events if mswindows_handle_mousewheel_event
			   ...
			   displayed */
			if (!mswindows_handle_mousewheel_event
			    (mswindows_find_frame(hwnd), keys, delta,
			     MAKEPOINTS(lParam)))
				mswindows_enqueue_mouse_button_event(hwnd,
								     message_,
								     MAKEPOINTS
...
#ifdef HAVE_MENUBARS
	case WM_INITMENU:
		if (UNBOUNDP(mswindows_handle_wm_initmenu((HMENU) wParam,
							  XFRAME
							  (mswindows_find_frame
							   (hwnd)))))
	...
	case WM_INITMENUPOPUP:
		if (!HIWORD(lParam)) {
			if (UNBOUNDP(mswindows_handle_wm_initmenupopup((HMENU)
								       wParam,
								       XFRAME
								       (mswindows_find_frame
									(hwnd)))))
...
#ifdef HAVE_TOOLBARS
			if (!NILP
			    (mswindows_handle_toolbar_wm_command
			     (frame, cid, id)))
				break;
#endif
	...
	case WM_DROPFILES:	/* implementation ripped-off from event-Xt.c */
		{
			...
			filecount =
			    DragQueryFile((HDROP) wParam, 0xffffffff, NULL, 0);
			for (i = 0; i < filecount; i++) {
				...
				DragQueryFile((HANDLE) wParam, i, fname,
					      len + 1);
				...
				filename =
				    ...
				    (cygwin_win32_to_posix_path_list_buf_size
				     (fname) + 5);
				strcpy(filename, "file:");
				cygwin_win32_to_posix_path_list(fname,
								filename + 5);
#else
				...
				xfree(fname);
				l_item =
				    make_string(filename, strlen(filename));
				l_dndlist = Fcons(l_item, l_dndlist);
				xfree(filename);
			}
			DragFinish((HDROP) wParam);

			event->event.misc.object =
					  ...
					  int extendedp)
{
	if (extendedp) {	/* Keys not present on a 82 key keyboard */
		...
		case VK_SELECT:
			return KEYSYM("select");
		case VK_PRINT:
			return KEYSYM("print");
		case VK_EXECUTE:
			return KEYSYM("execute");
		case VK_SNAPSHOT:
			return KEYSYM("print");
		case VK_INSERT:
		...
		case VK_NUMPAD9:
			return KEYSYM("kp-9");
		case VK_MULTIPLY:
			return KEYSYM("kp-multiply");
		case VK_ADD:
			return KEYSYM("kp-add");
		case VK_SEPARATOR:
			return KEYSYM("kp-separator");
		case VK_SUBTRACT:
			return KEYSYM("kp-subtract");
		case VK_DECIMAL:
			return KEYSYM("kp-decimal");
		case VK_DIVIDE:
			return KEYSYM("kp-divide");
		case VK_F1:
			return KEYSYM("f1");
		case VK_F2:
			return KEYSYM("f2");
 ...
 */
Lisp_Object mswindows_find_console(HWND hwnd)
{
...
static int emacs_mswindows_add_timeout(EMACS_TIME thyme)
{
	int milliseconds;
...
static void emacs_mswindows_remove_timeout(int id)
{
 ...
 */
static int emacs_mswindows_event_pending_p(int user_p)
{
 ...
 */
static void emacs_mswindows_next_event(Lisp_Event * emacs_event)
{
 ...
 */
static void emacs_mswindows_handle_magic_event(Lisp_Event * emacs_event)
{
	...
	case WM_KILLFOCUS:
		{
			...
			conser =
			    Fcons(frame,
				  Fcons(FRAME_DEVICE(f), in_p ? Qt : Qnil));
			GCPRO1(conser);
	...
	case XM_UNMAPFRAME:
		{
			Lisp_Object frame = EVENT_CHANNEL(emacs_event);
			va_run_hook_with_args(EVENT_MSWINDOWS_MAGIC_TYPE
					      (emacs_event)
					      == XM_MAPFRAME ? Qmap_frame_hook :
					      Qunmap_frame_hook, 1, frame);
		}
		break;
...
#ifndef HAVE_MSG_SELECT
static HANDLE get_process_input_waitable(Lisp_Process * process)
{
...
static void emacs_mswindows_select_process(Lisp_Process * process)
{
...
static void emacs_mswindows_unselect_process(Lisp_Process * process)
{
...
static void emacs_mswindows_select_console(struct console *con)
{
#ifdef HAVE_MSG_SELECT
...
static void emacs_mswindows_unselect_console(struct console *con)
{
#ifdef HAVE_MSG_SELECT
	if (CONSOLE_MSWINDOWS_P(con))
		return;		/* mswindows consoles are automatically selected */
...
static void emacs_mswindows_quit_p(void)
{
	...
	if (mswindows_quit_chars_count > 0) {
		...
		while (mswindows_quit_chars_count > 0) {
			emacs_event =
			    mswindows_cancel_dispatch_event(&match_against);
				   ...
				   Lisp_Object * outstream, int flags)
{
	...
	hin =
	    inhandle >=
	    0 ? (HANDLE) get_osfhandle((int)inhandle) : INVALID_HANDLE_VALUE;
	hout =
	    outhandle >=
	    0 ? (HANDLE) get_osfhandle((int)outhandle) : INVALID_HANDLE_VALUE;
	fdi = (int)inhandle;
	fdo = (int)outhandle;
#else
#error "So, WHICH kind of processes do you want?"
	...
	*instream = (hin == INVALID_HANDLE_VALUE ? Qnil
#if defined (HAVE_SOCKETS) && !defined (HAVE_MSG_SELECT)
...
#ifdef HAVE_WIN32_PROCESSES
	*outstream = (hout == INVALID_HANDLE_VALUE ? Qnil
#if defined (HAVE_SOCKETS) && !defined (HAVE_MSG_SELECT)
		      : flags & STREAM_NETWORK_CONNECTION
...
#endif
		:
		HANDLE_TO_USID(get_ntpipe_input_stream_waitable
			       (XLSTREAM(*instream))));
}

USID
emacs_mswindows_delete_stream_pair(Lisp_Object instream, Lisp_Object outstream)
{
...
static int emacs_mswindows_current_event_timestamp(struct console *c)
{
	return GetTickCount();
}
...
struct mswin_message_debug debug_mswin_messages[] = {
	FROB(WM_NULL)
	    FROB(WM_CREATE) FROB(WM_DESTROY) FROB(WM_MOVE) FROB(WM_SIZE)
	     FROB(WM_ACTIVATE)
	     FROB(WM_SETFOCUS)
	    FROB(WM_KILLFOCUS)
	    FROB(WM_ENABLE)
	    ...
	    FROB(WM_ENDSESSION) FROB(WM_SHOWWINDOW) FROB(WM_WININICHANGE)
#if(WINVER >= 0x0400)
	    ...
	    FROB(WM_CANCELMODE)
	    FROB(WM_SETCURSOR)
	    FROB(WM_MOUSEACTIVATE) FROB(WM_CHILDACTIVATE) FROB(WM_QUEUESYNC)
	    ...
	    FROB(WM_GETFONT)
	    FROB(WM_SETHOTKEY)
	    FROB(WM_GETHOTKEY) FROB(WM_QUERYDRAGICON) FROB(WM_COMPAREITEM)
#if(WINVER >= 0x0500) && defined(WM_GETOBJECT)
	    FROB(WM_GETOBJECT)
#endif				/* WINVER >= 0x0500 */
	    FROB(WM_COMPACTING)
	    FROB(WM_COMMNOTIFY)
	    FROB(WM_WINDOWPOSCHANGING) FROB(WM_WINDOWPOSCHANGED)
	     FROB(WM_POWER)
	     FROB(WM_COPYDATA) FROB(WM_CANCELJOURNAL)
#if(WINVER >= 0x0400)
	    FROB(WM_NOTIFY)
	    FROB(WM_INPUTLANGCHANGEREQUEST)
	    FROB(WM_INPUTLANGCHANGE)
	    FROB(WM_TCARD)
	    FROB(WM_HELP) FROB(WM_USERCHANGED) FROB(WM_NOTIFYFORMAT)
	     FROB(WM_CONTEXTMENU)
	    FROB(WM_STYLECHANGING)
	    FROB(WM_STYLECHANGED)
	    FROB(WM_DISPLAYCHANGE) FROB(WM_GETICON) FROB(WM_SETICON)
#endif				/* WINVER >= 0x0400 */
	    FROB(WM_NCCREATE)
	    FROB(WM_NCDESTROY)
	    FROB(WM_NCCALCSIZE)
	    FROB(WM_NCHITTEST)
	    FROB(WM_NCPAINT) FROB(WM_NCACTIVATE) FROB(WM_GETDLGCODE)
	    ...
	    FROB(WM_NCMBUTTONDOWN) FROB(WM_NCMBUTTONUP) FROB(WM_NCMBUTTONDBLCLK)
...
/* FROB (WM_KEYFIRST) */
	    ...
	    FROB(WM_MENUSELECT) FROB(WM_MENUCHAR) FROB(WM_ENTERIDLE)
#if(WINVER >= 0x0500)
	    ...
	    FROB(WM_CTLCOLORBTN)
	    FROB(WM_CTLCOLORDLG)
	    FROB(WM_CTLCOLORSCROLLBAR) FROB(WM_CTLCOLORSTATIC)
...
/* FROB (WM_MOUSEFIRST) */
	    ...
	    FROB(WM_MBUTTONDOWN) FROB(WM_MBUTTONUP) FROB(WM_MBUTTONDBLCLK)
#if (_WIN32_WINNT >= 0x0400) || (_WIN32_WINDOWS > 0x0400)
	    FROB(WM_MOUSEWHEEL) FROB(WM_MOUSELAST)
#else
	    FROB(WM_MOUSELAST)
#endif				/* if (_WIN32_WINNT < 0x0400) */
	    FROB(WM_PARENTNOTIFY) FROB(WM_ENTERMENULOOP) FROB(WM_EXITMENULOOP)
#if(WINVER >= 0x0400)
	    FROB(WM_NEXTMENU)
	     FROB(WM_SIZING)
	    FROB(WM_CAPTURECHANGED) FROB(WM_MOVING) FROB(WM_POWERBROADCAST)
	    ...
	    FROB(WM_MDIMAXIMIZE)
	    FROB(WM_MDITILE)
	    FROB(WM_MDICASCADE) FROB(WM_MDIICONARRANGE) FROB(WM_MDIGETACTIVE)

	     FROB(WM_MDISETMENU)
	    FROB(WM_ENTERSIZEMOVE)
	    FROB(WM_EXITSIZEMOVE) FROB(WM_DROPFILES) FROB(WM_MDIREFRESHMENU)
...
#endif				/* WINVER >= 0x0500 */
#if(WINVER >= 0x0400) && !defined(CYGWIN)
	    FROB(WM_IME_KEYDOWN) FROB(WM_IME_KEYUP)
#endif				/* WINVER >= 0x0400 */
#endif				/* WM_IME_SETCONTEXT */
#if(_WIN32_WINNT >= 0x0400)
	    FROB(WM_MOUSEHOVER) FROB(WM_MOUSELEAVE)
	    ...
	    FROB(WM_PALETTEISCHANGING) FROB(WM_PALETTECHANGED) FROB(WM_HOTKEY)
#if(WINVER >= 0x0400)
	    FROB(WM_PRINT) FROB(WM_PRINTCLIENT)
	     FROB(WM_HANDHELDFIRST) FROB(WM_HANDHELDLAST)
	     FROB(WM_AFXFIRST) FROB(WM_AFXLAST)
#endif				/* WINVER >= 0x0400 */
	    FROB(WM_PENWINFIRST) FROB(WM_PENWINLAST)
};
...
void reinit_vars_of_event_mswindows(void)
{
	...
	mswindows_event_stream->event_pending_p =
	    emacs_mswindows_event_pending_p;
	...
	mswindows_event_stream->remove_timeout_cb =
	    emacs_mswindows_remove_timeout;
	mswindows_event_stream->quit_p_cb = emacs_mswindows_quit_p;
	mswindows_event_stream->select_console_cb =
	    emacs_mswindows_select_console;
	mswindows_event_stream->unselect_console_cb =
	    emacs_mswindows_unselect_console;
#ifdef HAVE_MSG_SELECT
...
#else
	mswindows_event_stream->select_process_cb =
	    emacs_mswindows_select_process;
	mswindows_event_stream->unselect_process_cb =
	    emacs_mswindows_unselect_process;
	mswindows_event_stream->create_stream_pair_cb =
...
void vars_of_event_mswindows(void)
{
	...
	DEFVAR_BOOL("mswindows-alt-by-itself-activates-menu", &mswindows_alt_by_itself_activates_menu	/*
