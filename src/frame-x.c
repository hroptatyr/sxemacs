...
static Lisp_Object x_frame_property(struct frame *f, Lisp_Object property)
{
	...
	if (EQ(Qwindow_id, property))
        ...
        if (EQ(Qwait_for_wm, property))
                return (FRAME_X_WAIT_FOR_WM(f) ? Qt : Qnil);
...
x_frame_properties(struct frame *f)
{
	...
	props = cons3(Qborder_width, make_int(w->core.border_width), props);
        ...
        props = cons3(Qwait_for_wm, FRAME_X_WAIT_FOR_WM(f) ? Qt : Qnil, props);
		       ...
		       String Xt_resource_encoding_name)
{
	...
	for (ptr = value; *ptr; ptr++)
		if (!BYTE_ASCII_P(*ptr)) {
			const char *tmp;
			encoding =
			    DEVICE_XATOM_COMPOUND_TEXT(XDEVICE
						       (FRAME_DEVICE(f)));
		...
		if (STRINGP(prop)) {
			...
			if (!strcmp((char *)XSTRING_DATA(str),
				...
				internal_border_width_specified = True;
                        ...
                        if (!strcmp((char*)XSTRING_DATA(str), "waitforwm")) {
                                FRAME_X_WAIT_FOR_WM(f) = !NILP(val);
                                continue;
                        }
		...
		if (!f->init_finished) {
			...
			if (size_specified_p
				...
				x_set_initial_frame_size(f, flags, x, y, width,
                        ...
                        /*
                         * Workaround Xt blocking waiting for ConfigureNotify.
                         */
                        XtVaSetValues(FRAME_X_SHELL_WIDGET(f), XtNwaitForWm,
                                      (XtArgVal)FRAME_X_WAIT_FOR_WM(f), NULL);
		...
		if (dnd_typ == DndFiles) {
			...
			*dnd_data = 0;
			while (!NILP(run)) {
				...
				dnd_data =
				    (char *)xrealloc(dnd_data, dnd_len + len);
				strcpy(dnd_data + dnd_len - 1,
				       (const char *)XSTRING_DATA(XCAR(run)));
				dnd_len += len;
				run = XCDR(run);
			}
