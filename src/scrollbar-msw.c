				    ...
				    struct scrollbar_instance *sb)
{
	...
	SCROLLBAR_MSW_HANDLE(sb) =
	    CreateWindowEx(0, "SCROLLBAR", 0, orientation | WS_CHILD,
			   ...
			   CW_USEDEFAULT, CW_USEDEFAULT,
			   FRAME_MSWINDOWS_HANDLE(f), NULL, NULL, NULL);
...
#if 0
	{
		...
		struct scrollbar_instance *z =
		    ...
		    GetWindowLong(SCROLLBAR_MSW_HANDLE(sb),
				  GWL_USERDATA);
		*z = *z;
	}
#endif
...
static void mswindows_free_scrollbar_instance(struct scrollbar_instance *sb)
{
...
static void unshow_that_mofo(void *handle)
{
	ShowScrollBar((HWND) handle, SB_CTL, 0);
}

static void mswindows_release_scrollbar_instance(struct scrollbar_instance *sb)
{
	if (gc_in_progress)
					   ...
					   int new_scrollbar_y)
{
	...
	stderr_out("[%d, %d], page = %d, pos = %d, inhibit = %d\n", new_minimum,
		   new_maximum, new_slider_size, new_slider_position,
		   inhibit_slider_size_change);
#endif
	...
	if (pos_changed) {
		MoveWindow(SCROLLBAR_MSW_HANDLE(sb),
			   ...
			   new_scrollbar_width, new_scrollbar_height, TRUE);
	}
}
...
void mswindows_handle_scrollbar_event(HWND hwnd, int code, int pos)
{
	...
	case SB_LINEDOWN:
		mswindows_enqueue_misc_user_event
		    (frame, vert ? Qscrollbar_line_down : Qscrollbar_char_right,
		     win);
		break;

	case SB_LINEUP:
		mswindows_enqueue_misc_user_event
		    (frame, vert ? Qscrollbar_line_up : Qscrollbar_char_left,
		     win);
		break;
	...
	case SB_BOTTOM:
		mswindows_enqueue_misc_user_event
		    (frame, vert ? Qscrollbar_to_bottom : Qscrollbar_to_right,
		     win);
		break;
	...
	case SB_THUMBPOSITION:
		{
			...
			mswindows_enqueue_misc_user_event
			    (frame,
			     ...
			     Qscrollbar_horizontal_drag, Fcons(win,
							       make_int(pos)));
		}
		break;
...
static int can_scroll(struct scrollbar_instance *scrollbar)
{
				  ...
				  POINTS where)
{
	...
	if (ScreenToClient(FRAME_MSWINDOWS_HANDLE(XFRAME(frame)),
		...
		pixel_to_glyph_translation(XFRAME(frame), donde_esta.x,
					   donde_esta.y, &mene, &_mene, &tekel,
					   &upharsin, &needle_in_haystack,
	...
	if (wheelScrollLines == WHEEL_PAGESCROLL) {
		...
		if (hasVertBar)
			function =
			    delta >
			    0 ? Qscrollbar_page_up : Qscrollbar_page_down;
		else
			function =
			    delta >
			    0 ? Qscrollbar_page_left : Qscrollbar_page_right;
		mswindows_enqueue_misc_user_event(frame, function,
						  Fcons(win, Qnil));
		...
		if (hasVertBar)
			function =
			    delta >
			    0 ? Qscrollbar_line_up : Qscrollbar_line_down;
		else
			function =
			    delta >
			    0 ? Qscrollbar_char_left : Qscrollbar_char_right;
		if (toScroll < 0)
			toScroll = -toScroll;
		while (toScroll--)
