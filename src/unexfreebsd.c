		     ...
		     caddr_t to_base_addr, struct translation_struct *ts)
{
	...
	if (offset == -1)
		unexec_error("bad relocation address 0x%x (0x%x)", 0,
			     ri->r_address, ((long *)ri)[1]);
...
*/
		if (!(type & N_STAB)) {
			...
			if (shlib_image &&
				...
				unexec_error
				    ("unexec'd shlib image has sbrk() ref -- we quit!",
				     0);
		}
		tmp++;
	}
       ...
       unsigned int emacs_edata, unsigned int dummy1, unsigned int dummy2)
{
	...
	if (dynamic_addr)
		plt_end =
		    (caddr_t) MASK_UP(ld2->sdt_plt + ld2->sdt_plt_sz,
				      sizeof(double));
	else
	...
	reset_symtab((struct nlist *)(new_base_addr + N_SYMOFF(new_hdr)),
		     ...
		     new_edata, new_edata, !!dynamic_addr);
	...
	if (dynamic_addr) {
		...
		syms =
		    new_base_addr + unexec_addr_to_offset(ld2->sdt_nzlist, &ts);
		strings =
		    new_base_addr + unexec_addr_to_offset(ld2->sdt_strings,
							  &ts);
		reset_ldso_symtab((struct nzlist *)syms,
				  (struct nzlist *)strings, (char *)strings,
				  new_edata, new_edata, !!dynamic_addr);
...
int run_time_remap(char *dummy)
{
	...
	if (sbrk_of_0_at_unexec < current_sbrk) {
		if (sbrk_of_0_at_unexec != 0)
			fprintf(stderr,
				"Absurd new brk addr = %lx (current = %lx)\n",
	...
	if (sbrk_of_0_at_unexec > current_sbrk) {
		...
		if (brk((caddr_t) sbrk_of_0_at_unexec))
			fprintf(stderr,
				"failed to change brk addr to %lx: %s\n",
...
/* with proper COW, i don't think we really need to do this... */
	{
		...
		unsigned long base_addr =
		    MASK_UP(mprotect_bottom_addr, page_size);
		unsigned long top_addr =
		    MASK_DOWN(mprotect_top_addr, page_size);
		long len = top_addr - base_addr;

		if (len > 0) {
			errno = 0;
			if (mprotect
			    ((caddr_t) base_addr, len, PROT_READ | PROT_EXEC))
				fprintf(stderr,
