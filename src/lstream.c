...
inline static size_t aligned_sizeof_lstream(size_t lstream_type_specific_size)
{
	return ALIGN_SIZE(offsetof(Lstream, data) + lstream_type_specific_size,
			  ALIGNOF(max_align_t));
}

static size_t sizeof_lstream(const void *header)
{
		      ...
		      int buffering_size)
{
	...
	case LSTREAM_BLOCKN_BUFFERED:
		lstr->buffering_size = buffering_size;
		break;
...
Lstream *Lstream_new(const Lstream_implementation * imp, const char *mode)
{
	Lstream *p;
	int i;
...
void Lstream_set_character_mode(Lstream * lstr)
{
...
Lstream_adding(Lstream * lstr, Lstream_data_count num, int force)
{
	...
	DO_REALLOC(lstr->out_buffer, lstr->out_buffer_size, size,
		   unsigned char);
...
static Lstream_data_count
Lstream_raw_read(Lstream * lstr, unsigned char *buffer, Lstream_data_count size)
{
	if (!(lstr->flags & LSTREAM_FL_IS_OPEN))
		Lstream_internal_error("lstream not open", lstr);
...
static Lstream_data_count Lstream_read_more(Lstream * lstr)
{
#if 0
...
Lstream_read(Lstream * lstr, void *data, Lstream_data_count size)
{
	...
	while (size > 0) {
		...
		chunk =
		    min(size, lstr->in_buffer_current - lstr->in_buffer_ind);
		if (chunk > 0) {
...
static int Lstream_pseudo_close(Lstream * lstr)
{
	if (!(lstr->flags & LSTREAM_FL_IS_OPEN))
...
int Lstream_get_fd(Lstream * lstr)
{
	int rc = -1;
...
static int stdio_seekable_p(Lstream * stream)
{
	struct stat lestat;
	struct stdio_stream *str = STDIO_STREAM_DATA(stream);
...
static int stdio_flusher(Lstream * stream)
{
	struct stdio_stream *str = STDIO_STREAM_DATA(stream);
...
filedesc_reader(Lstream * stream, unsigned char *data, Lstream_data_count size)
{
	...
	if (str->end_pos >= 0)
		size =
		    min(size,
			(Lstream_data_count) (str->end_pos - str->current_pos));
	nread =
	    str->allow_quit ? read_allowing_quit(str->fd, data,
						 size) : read(str->fd, data,
							      size);
	if (nread > 0)
...
static int errno_would_block_p(int val)
{
#ifdef EWOULDBLOCK
	if (val == EWOULDBLOCK)
		return 1;
		...
		Lstream_data_count size)
{
	...
	if (str->pty_flushing) {
		...
		if (ptr - data >= str->pty_max_bytes - str->chars_sans_newline) {
			ptr =
			    data + str->pty_max_bytes - str->chars_sans_newline;
			need_newline = 0;
		}
		size = ptr - data;
	}
...
static int filedesc_seekable_p(Lstream * stream)
{
	struct filedesc_stream *str = FILEDESC_STREAM_DATA(stream);
	if (str->starting_pos < 0)
		return 0;
...
static int filedesc_was_blocked_p(Lstream * stream)
{
	struct filedesc_stream *str = FILEDESC_STREAM_DATA(stream);
...
static int filedesc_get_fd(Lstream * stream)
{
	struct filedesc_stream *str = FILEDESC_STREAM_DATA(stream);
	return str->fd;
}
...
{
	struct lisp_string_stream *str =
	    LISP_STRING_STREAM_DATA(XLSTREAM(stream));
	return str->obj;
}
		       ...
		       Lstream_data_count size)
{
	struct resizing_buffer_stream *str =
	    RESIZING_BUFFER_STREAM_DATA(stream);
...
static int dynarr_rewinder(Lstream * stream)
{
	Dynarr_reset(DYNARR_STREAM_DATA(stream)->dyn);
	return 0;
}

static int dynarr_closer(Lstream * stream)
{
	return 0;
}
...
static int lisp_buffer_rewinder(Lstream * stream)
{
	struct lisp_buffer_stream *str = LISP_BUFFER_STREAM_DATA(stream);
	struct buffer *buf = XBUFFER(str->buffer);
...
DEFINE_LSTREAM_IMPLEMENTATION("ssl", lstream_ssl,
			      sizeof(struct ssl_stream));

static Lisp_Object
make_ssl_stream_1(SSL *conn, int flags, const char *mode)
{
	Lisp_Object obj;
	Lstream *lstr = Lstream_new(lstream_ssl, mode);
	struct ssl_stream *str = SSL_STREAM_DATA(lstr);
	str->conn = conn;
	XSETLSTREAM(obj, lstr);
	return obj;
}

Lisp_Object make_ssl_input_stream(SSL *conn, int flags)
{
	return make_ssl_stream_1(conn, flags, "r");
}

Lisp_Object make_ssl_output_stream(SSL *conn, int flags)
{
	return make_ssl_stream_1(conn, flags, "w");
}

static Lstream_data_count
ssl_reader(Lstream *stream, unsigned char *data, Lstream_data_count size)
{
	Lstream_data_count nread;
	struct ssl_stream *str = SSL_STREAM_DATA(stream);
...
static Lstream_data_count
ssl_writer(Lstream *stream, const unsigned char *data,
	   Lstream_data_count size)
{
...
static int ssl_closer(Lstream *stream)
{
	struct ssl_stream *str = SSL_STREAM_DATA(stream);
	if (str->closing)
...
static int ssl_get_fd(Lstream *stream)
{
	struct ssl_stream *str = SSL_STREAM_DATA(stream);
