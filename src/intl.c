/* Various functions for internationalizing SXEmacs
...
*/
void init_input(const char *res_name, const char *res_class, Display * display)
{
	XIMStyles *styles;
	unsigned short i;
	...
	XGetICValues(initial_input_context,
		     XNFilterEvents, &input_method_event_mask, NULL);
...
static XIM_Composed_Text composed_input_buf =
    { XIM_Composed_Text_BUFSIZE, {0} };
...
{
	if (CONSP(object)) {
		Lisp_Object tmp = Fcar(object);
		if (SYMBOLP(tmp) && EQ(tmp, Qdefer_gettext)) {
