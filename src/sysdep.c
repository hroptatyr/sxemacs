...
#define PATHNAME_CONVERT_OUT(path, pathout)		\
	TO_EXTERNAL_FORMAT(C_STRING, (path),		\
			   C_STRING_ALLOCA, (pathout),	\
			   Qfile_name);
...
int sys_open(const char *path, int oflag, ...)
{
	...
	{
		int rtnval;
		while ((rtnval = open(pout, oflag, mode)) == -1
		       && (errno == EINTR))
			DO_NOTHING;
		return rtnval;
	}
...
int interruptible_open(const char *path, int oflag, int mode)
{
	...
	PATHNAME_CONVERT_OUT(path, nrout);

	for (;;) {
		int rtnval = open(nrout, oflag, mode);
...
FILE *sys_fopen(const char *path, const char *type)
{
	...
	{
		FILE *rtnval;
		while (!(rtnval = fopen(pout, type)) && (errno == EINTR))
			DO_NOTHING;
		return rtnval;
	}
#else
	return fopen(pout, type);
#endif
}
...
DIR *sys_opendir(const char *filename)
{
	DIR *rtnval;
	char *fout;
	PATHNAME_CONVERT_OUT(filename, fout);

	while (!(rtnval = opendir(fout))
	       && (errno == EINTR)) ;
	return rtnval;
}
...
int sys_readlink(const char *path, char *buf, size_t bufsiz)
{
	...
	return readlink(pout, buf, bufsiz);
}
...
{
	char *eout, *nout;
	PATHNAME_CONVERT_OUT(existing, eout);
	PATHNAME_CONVERT_OUT(new, nout);
	return link(eout, nout);
}
...
{
	char *oout, *nout;
	PATHNAME_CONVERT_OUT(old, oout);
	PATHNAME_CONVERT_OUT(new, nout);
	return rename(oout, nout);
}
...
{
	char *n1out, *n2out;
	PATHNAME_CONVERT_OUT(name1, n1out);
	PATHNAME_CONVERT_OUT(name2, n2out);
	return symlink(n1out, n2out);
}
...
int sys_execvp(const char *path, char *const *argv)
{
	...
	for (argc = 0; argv[argc]; argc++);
	{
		...
		for (i = 0; i < argc; i++) {
			PATHNAME_CONVERT_OUT(argv[i], new_argv[i]);
		}
		new_argv[argc] = NULL;
...
#endif
#if defined HAVE_MPZ && (defined WITH_GMP || defined WITH_MP)
	bigz_random_seed((unsigned long)arg);
#endif
}
...
/* some coreutils borrows here */
void
x__dirname(restrict char *res, const char *file, size_t len);
{
	/* assumes res is malloc'd of size LEN (and >= xstrlen(file)) */
	size_t file_len = xstrlen(file);
	size_t offs = x__dirlen(file, file_len);
	int append_dot = (offs == 0);

	xmemcpy(dir, file, offs);
	if (append_dot)
		res[offs++] = '.';
	res[offs] = '\0';
	return res;
