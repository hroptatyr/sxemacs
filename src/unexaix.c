...
static void report_error(char *file, int fd)
{
	if (fd)
		close(fd);
...
static void report_error_1(int fd, char *msg, int a1, int a2)
{
	close(fd);
#ifdef emacs
	error(msg, a1, a2);
...
#define CHECK_SCNHDR(ptr, name, flags) \
  if (strcmp(s->s_name, name) == 0) { \
    ...
    ptr = s; \
		...
		if (f_thdr == 0) {
			ERROR1("unexec: couldn't find \"%s\" section",
			       (int)_TEXT);
		}
		if (f_dhdr == 0) {
			ERROR1("unexec: couldn't find \"%s\" section",
...
static void write_segment(int new, char *ptr, char *end)
{
	int i, nwrite, ret;
	char buf[80];
...

static int adjust_lnnoptrs(int writedesc, int readdesc, char *new_name)
{
	int nsyms;
	int naux;
	int new;
	...
	for (nsyms = 0; nsyms < f_hdr.f_nsyms; nsyms++) {
		...
		if (symentry.n_sclass == C_BINCL
		    || symentry.n_sclass == C_EINCL) {
		...
		for (naux = symentry.n_numaux; naux-- != 0;) {
			...
			if (naux != 0	/* skip csect auxentry (last entry) */
			    && (symentry.n_sclass == C_EXT
				|| symentry.n_sclass == C_HIDEXT)) {
...
static int unrelocate_symbols(int new, int a_out, char *a_name, char *new_name)
{
	int i;
	LDHDR ldhdr;
	LDREL ldrel;
...
#define SYMNDX_BSS	2
	...
	for (i = 0; i < ldhdr.l_nreloc; i++) {
		...
		if (ldrel.l_symndx == SYMNDX_BSS) {
			...
			lseek(new,
			      load_scnptr + LDHDRSZ + LDSYMSZ * ldhdr.l_nsyms +
			      LDRELSZ * i, SEEK_SET);
		...
		if (ldrel.l_rsecnm == f_ohdr.o_sndata) {
			...
			lseek(a_out,
			      orig_data_scnptr + (ldrel.l_vaddr -
						  f_ohdr.data_start), SEEK_SET);
			...
			if (orig_int != *p) {
				lseek(new,
				      data_scnptr + (ldrel.l_vaddr -
						     f_ohdr.data_start),
				      SEEK_SET);
				if (write
				    (new, (void *)&orig_int, sizeof(orig_int))
				    != sizeof(orig_int)) {
					PERROR(new_name);
				}
			}
