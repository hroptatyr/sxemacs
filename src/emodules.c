...
static int find_make_module(const char *mod, const char *name, const char *ver,
			    int make_or_find);
	...
	for (i = 0; i < modnum; i++) {
		if (modules[i].used == 1)
			mlist = Fcons(list4(build_string(modules[i].soname),
					    ...
					    build_string(modules[i].modtitle)),
				      mlist);
	}
...
find_make_module(const char *mod, const char *name, const char *ver, int mof)
{
	...
	modnum++;
	modules =
	    (emodules_list *) xrealloc(modules, modnum * sizeof(emodules_list));
...
static Lisp_Object module_load_unwind(Lisp_Object upto)
{
	int x, l = 0;
 ...
 * defined within SXEmacs itself.  It is up to the module to declare any
 ...
 * the cleaning up.
 */
void emodules_load(const char *module, const char *modname, const char *modver)
{
	...
	if ((ellcc_rev == (const long *)0) || (*ellcc_rev <= 0))
		error
		    ("Missing symbol `emodule_compiler': Invalid dynamic module");
	...
	if ((f == (const char **)0) || (*f == (const char *)0))
		error
		    ("Missing symbol `emodule_version': Invalid dynamic module");
...
void emodules_doc_subr(const char *symname, const char *doc)
{
...
void emodules_doc_sym(const char *symname, const char *doc)
{
	Bytecount len = strlen(symname);
	Lisp_Object sym = oblookup(Vobarray, (const Bufbyte *)symname, len);
	Lisp_Object docstr;
