	...
	if (size < 0) {
		...
		if (static_heap_ptr + size >=
			...
			printf("\nRequested %d bytes, static heap exhausted! "
			       "base is %p, current ptr is %p. "
			       "You have exhausted the static heap.\n\n"
			       "If you are simply trying to compile, remove "
			       "sheap-adjust.h and recompile from the top "
			       "level.\nIf this doesn't work then "
			       "STATIC_HEAP_SLOP (defined in this file) is "
			       "too small.\n\n"
			       "If you want to run temacs, change "
			       "SHEAP_ADJUSTMENT in sheap-adjust.h\nto 0 or "
			       "a +ve number. Generally you should *not* try "
