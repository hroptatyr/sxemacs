/* media-internal.c - general routines related to playing sounds
 **
...
#define MYSELF MDRIVER_INTERNAL
...
static inline int waverequire(void **data, size_t * sz, size_t rq)
{
	...
	memcpy(parsestate.wave.leftover + parsestate.wave.left, *data, rq);
...
static inline void waveremove(size_t rq)
{
	if (parsestate.wave.left <= rq)
		...
		memmove(parsestate.wave.leftover,
			parsestate.wave.leftover + rq, parsestate.wave.left);
	}
	return;
}
	...
	for (;;)
		...
		case wvSubchunk:
			if (!waverequire(data, sz, parsestate.wave.chunklength))
				return (0);
			parsestate.wave.align =
			    parsestate.wave.chunklength <
			    14 ? 1 : parsestate.wave.leftover[12];
		...
		case wvSkipChunk:
			if (parsestate.wave.chunklength > 0 && *sz > 0 &&
			    (signed long)*sz <
			    (signed long)parsestate.wave.chunklength) {
...
static size_t parsesundecaudio(void **data, size_t * sz, void **outbuf)
{
	...
	if (parsestate.audio.left) {
		if (parsestate.audio.left + *sz >
		    (size_t) parsestate.audio.align) {
			int count;
			memmove(parsestate.audio.leftover +
				parsestate.audio.left, *data, count =
				parsestate.audio.align - parsestate.audio.left);
	...
	if (!parsestate.audio.skipping) {
		...
		/* Keep compatibility with Linux 68k, etc. by not relying on
		   byte-sex  */
		if (header[3]) {	/* Sun audio (big endian) */
			parsestate.audio.align =
			    ((header[15] > 2) + 1) * header[23];
			parsestate.audio.skipping =
			    header[7] + 256 * (header[6] +
					       256 * (header[5] +
						      256 * header[4]));
		} else {	/* DEC audio (little endian) */
			parsestate.audio.align =
			    ((header[12] > 2) + 1) * header[20];
			parsestate.audio.skipping =
			    header[4] + 256 * (header[5] +
					       256 * (header[6] +
						      256 * header[7]));
		}
	}

	/* We are skipping extra data that has been attached to header; most
	   usually this will be just a comment, such as the original filename
	   and/or the creation date. Make sure that we do not return less than
	   one single sound sample entry to the caller; if this happens, rather
	   decide to move those few bytes into the leftover buffer and deal with
	   it later */
...
/* If the soundcard could not be set to internally support the data format, we
...
size_t sndcnv8U_2mono(void **data, size_t * sz, void **outbuf)
{
	...
	*outbuf = dest = miscplay_sndbuf;
	while (count--) {
		*dest++ = (unsigned char)(((int)*(src) + (int)*(src + 1)) / 2);
		src += 2;
	}
	*data = src;
	return (rc);
...
size_t sndcnvULaw_2mono(void **data, size_t * sz, void **outbuf)
{
	...
	*outbuf = dest = miscplay_sndbuf;
	while (count--) {
		...
		*dest++ = int2ulaw(ulaw2int[*src] + ulaw2int[*(src + 1)]);
		src += 2;
	}
	*data = src;
	return (rc);
...
size_t sndcnv2byteLE(void **data, size_t * sz, void **outbuf)
{
	...
	*outbuf = dest = miscplay_sndbuf;
	while (count--) {
		*dest++ =
		    (unsigned char)(((signed char *)src)[1] ^ (signed char)
				    0x80);
		src += 2;
	}
	*data = src;
...
size_t sndcnv2byteBE(void **data, size_t * sz, void **outbuf)
{
	...
	*outbuf = dest = miscplay_sndbuf;
	while (count--) {
		*dest++ =
		    (unsigned char)(((signed char *)src)[0] ^ (signed char)
				    0x80);
		src += 2;
	}
	*data = src;
...

static void
media_internal_analyse_data(Lisp_Media_Stream *ms, char *data)
{
	/* we always assume to deal with audio data */
	media_substream *mss = media_stream_nsub(ms);
	mtype_audio_properties *mtap;
	char *name;

	/* initialise */
	mtap = xnew_and_zero(mtype_audio_properties);
	name = xmalloc_atomic(20);

	mtap->samplerate = 0;

	/* Keep compatibility with Linux 68k, etc. by not relying on byte-sex */
	if (!memcmp(data, "Creative Voice File\x1A\x1A\x00", 22) &&
	    (data[22] + 256 * data[23]) ==
	    ((0x1233 - data[24] - 256 * data[25]) & 0xFFFF)) { /* VOC */
		memcpy(name, "VOC", 3);
		name[3] = '\0';
		mtap->name = name;
		mtap->channels = 2;
		mtap->samplerate = 8000;
		mtap->samplewidth = 16;
		mtap->framesize = 4;
		mtap->endianness = 0;
	} else if (!memcmp(data, "RIFF", 4) &&
		   !memcmp(data + 8, "WAVEfmt ", 8)) { /* WAVE */
		memcpy(name, "WAV", 3);
		name[3] = '\0';
		mtap->name = name;
		mtap->channels = data[22];
		mtap->samplerate = data[24] + 256 *
			(data[25] + 256 * (data[26] + 256 * data[27]));
		mtap->samplewidth = 16;
		mtap->framesize = mtap->channels * 2;
		mtap->endianness = 0;
	} else if (!memcmp(data, ".snd", 4)) {	/* Sun Audio (big endian) */
		if (data[7] + 256 * (data[6] + 256 *
				     (data[5] + 256 * data[4])) < 24) {
			memcpy(name, "SunAudio", 8);
			name[8] = '\0';
			mtap->name = name;
			mtap->channels = 1;
			mtap->samplerate = 8000;
			...
			mtap->framesize = 2;
			mtap->endianness = 0;
		} else if (!memcmp(data + 12, "\000\000\000\001", 4) ||
			   !memcmp(data + 12, "\000\000\000\002", 4) ||
			   !memcmp(data + 12, "\000\000\000\003", 4)) {
			memcpy(name, "SunAudio", 8);
			name[8] = '\0';
			mtap->name = name;
			mtap->channels = data[23];
			mtap->samplerate = data[19] +
				256 * (data[18] + 256 *
				       (data[17] + 256 * data[16]));
			mtap->samplewidth = 16;
			mtap->framesize = mtap->channels * 2;
	...
	} else if (!memcmp(data, ".sd", 4)) { /* DEC Audio (little endian) */
		if (data[4] + 256 * (data[5] + 256 *
				     (data[6] + 256 * data[7])) < 24) {
			memcpy(name, "SunAudio", 8);
			name[8] = '\0';
			mtap->name = name;
			mtap->channels = 1;
			...
			mtap->framesize = 2;
			mtap->endianness = 0;
		} else if (!memcmp(data + 12, "\001\000\000", 4) ||
			   !memcmp(data + 12, "\002\000\000", 4) ||
			   !memcmp(data + 12, "\003\000\000", 4)) {
			memcpy(name, "SunAudio", 8);
			name[8] = '\0';
			mtap->name = name;
			mtap->channels = data[20];
			mtap->samplerate = data[16] +
				256 * (data[17] + 256 *
				       (data[18] + 256 * data[19]));
			mtap->samplewidth = 16;
			mtap->framesize = mtap->channels * 2;
...
void media_internal_analyse_stream(Lisp_Media_Stream *ms)
{
	/* stuff for file handling */
	int fd = -1, len;
	/* data probe for the internal analyser */
	char *data;
	/* stream stuff */
	media_substream *mss;

	data = xmalloc_atomic(48);
	...
	case MKIND_STRING: {
		...
		memcpy(data, mksp->stream_data, 44);
		break;
	}
	case MKIND_UNKNOWN:
	case MKIND_FIFO:
	...
	media_internal_analyse_data(ms, data);
		       ...
		       size_t(**parsesndfile) (void **, size_t * sz, void **))
{
	...
	if (!memcmp(format, "Creative Voice File\x1A\x1A\x00", 22) &&
		...
		*fmt = (format[32] / (*tracks = format[22])) == 1 ?
		    AFMT_U8 : AFMT_S16_LE;
		/* Keep compatibility with Linux 68k, etc.
		 * by not relying on byte-sex
		 */
		...
		*parsesndfile = parsewave;
		...
		if (format[7] +
		    256 * (format[6] + 256 * (format[5] + 256 * format[4])) <
		    24) {
			*fmt = AFMT_MU_LAW;
			*speed = 8000;
			*tracks = 1;
		...
		if (!memcmp(format + 12, "\000\000\000\001", 4))
			*fmt = AFMT_MU_LAW;
		else if (!memcmp(format + 12, "\000\000\000\002", 4))
			*fmt = AFMT_S8;
		else if (!memcmp(format + 12, "\000\000\000\003", 4))
			*fmt = AFMT_S16_BE;
		else
			return (fmtIllegal);
		/* Keep compatibility with Linux 68k, etc.
		...
		*parsesndfile = parsesundecaudio;
		...
		if (format[4] +
		    256 * (format[5] + 256 * (format[6] + 256 * format[7])) <
		    24) {
			*fmt = AFMT_MU_LAW;
			*speed = 8000;
			*tracks = 1;
		...
		if (!memcmp(format + 12, "\001\000\000", 4))
			*fmt = AFMT_MU_LAW;
		else if (!memcmp(format + 12, "\002\000\000", 4))
			*fmt = AFMT_S8;
		else if (!memcmp(format + 12, "\003\000\000", 4))
			*fmt = AFMT_S16_LE;
		else
			return (fmtIllegal);
		/* Keep compatibility with Linux 68k, etc.
