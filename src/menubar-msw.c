					...
					Lisp_Object error_name)
{
	...
	while ((ptr = (Bufbyte *) memchr(ptr, '&', len - (ptr - item))) != NULL) {
		if (len + 2 > maxlen)
			syntax_error
			    ("Menu item produces too long displayable string",
			     error_name);
...
/* #### This is junk.  Need correct handling of sizes.  Use a Bufbyte_dynarr,
   not a static buffer. */
static char *displayable_menu_item(Lisp_Object gui_item, int bar_p,
				   Emchar * accel)
{
	unsigned int ll;
 ...
 */
static Lisp_Object hmenu_to_lisp_object(HMENU hmenu)
{
...
static HMENU create_empty_popup_menu(void)
{
	return CreatePopupMenu();
}

static void empty_menu(HMENU menu, int add_empty_p)
{
	while (DeleteMenu(menu, 0, MF_BYPOSITION)) ;
	if (add_empty_p)
		AppendMenu(menu, MF_STRING | MF_GRAYED, EMPTY_ITEM_ID,
			   EMPTY_ITEM_NAME);
}
 ...
 * in depth.
 */
static unsigned long checksum_menu_item(Lisp_Object item)
{
	if (STRINGP(item)) {
...
populate_menu_add_item(HMENU menu, Lisp_Object path,
		       ...
		       Lisp_Object * accel_list, int flush_right, int bar_p)
{
	...
	if (xInsertMenuItemA)
		...
		InsertMenu(menu, UINT_MAX, oldflags, olduidnewitem,
			   oldlpnewitem);
			    ...
			    Lisp_Object hash_tab, int bar_p, int populate_p)
{
	...
	EXTERNAL_LIST_LOOP(item_desc, desc) {
		if (NILP(XCAR(item_desc))) {
			...
			if (bar_p
			    &&
			    !REPLACE_ME_WITH_GLOBAL_VARIABLE_WHICH_CONTROLS_RIGHT_FLUSH)
				flush_right = 1;
			if (!populate_p)
	...
	if (populate_p) {
		...
		if (!bar_p && !deep_p && popup_menu_titles
		    && !NILP(pgui_item->name)) {
			CHECK_STRING(pgui_item->name);
			InsertMenu(menu, 0,
				   MF_BYPOSITION | MF_STRING | MF_DISABLED, 0,
				   displayable_menu_item(gui_item, bar_p,
							 NULL));
...
static void update_frame_menubar_maybe(struct frame *f)
{
	...
	if (NILP(FRAME_MSWINDOWS_MENU_HASH_TABLE(f)))
		FRAME_MSWINDOWS_MENU_HASH_TABLE(f) =
		    make_lisp_hash_table(50, HASH_TABLE_NON_WEAK,
					 HASH_TABLE_EQUAL);
	else
...
static void prune_menubar(struct frame *f)
{
	HMENU menubar = GetMenu(FRAME_MSWINDOWS_HANDLE(f));
 ...
 */
static void menu_cleanup(struct frame *f)
{
...
/* Message handlers                                                       */
/*------------------------------------------------------------------------*/
static Lisp_Object unsafe_handle_wm_initmenupopup_1(HMENU menu, struct frame *f)
{
	...
	path =
	    Fgethash(hmenu_to_lisp_object(menu), current_hash_table, Qunbound);
...
static Lisp_Object unsafe_handle_wm_initmenu_1(struct frame *f)
{
 ...
 */
Lisp_Object mswindows_handle_wm_command(struct frame * f, WORD id)
{
...
static Lisp_Object unsafe_handle_wm_initmenupopup(Lisp_Object u_n_u_s_e_d)
{
	return unsafe_handle_wm_initmenupopup_1(wm_initmenu_menu,
						wm_initmenu_frame);
}

static Lisp_Object unsafe_handle_wm_initmenu(Lisp_Object u_n_u_s_e_d)
{
...
Lisp_Object mswindows_handle_wm_initmenupopup(HMENU hmenu, struct frame * frm)
{
	...
	return mswindows_protect_modal_loop(unsafe_handle_wm_initmenupopup,
					    Qnil);
}
...
static void mswindows_update_frame_menubars(struct frame *f)
{
	update_frame_menubar_maybe(f);
}

static void mswindows_free_frame_menubars(struct frame *f)
{
	FRAME_MSWINDOWS_MENU_HASH_TABLE(f) = Qnil;
}

static void mswindows_popup_menu(Lisp_Object menu_desc, Lisp_Object event)
{
	...
	ok = TrackPopupMenu(menu,
			    ...
			    pt.x, pt.y, 0, FRAME_MSWINDOWS_HANDLE(f), NULL);
