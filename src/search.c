...
#define COMPREP(obj)		EMODPTRP(obj)
#define COMPRE_GET(obj)	(COMPRE_T*)XEMODPTR(obj)->ptr
#define COMPRE_PUT(obj, rec)	do {				\
		obj = make_emodptr((void*)rec);			\
		set_emodptr_printer(obj, compre_prfun);	\
		set_emodptr_finaliser(obj, compre_finfun);	\
	} while (0)
#define XCOMPRE_GET(obj)	Fget(obj, Qcompre, Qnil)
#define XCOMPRE_PUT(obj, b)	Fput(obj, Qcompre, b)
#define XCOMPRE_REM(obj)	Fremprop(obj, Qcompre)
...
static void free_compre(COMPRE_T*);
#endif
		       ...
		       Error_behavior errb)
{
	...
	if (!NILP(Fstring_equal(cp->regexp, pattern))
		...
		if (!compile_pattern_1
		    (cp, pattern, translate, regp, posix, errb))
			return 0;
		REGEXP_DEBUG_COMPRE_H("collision.\n", place);
	}
...
#ifdef EF_USE_COMPRE
	if (!NILP(rc = XCOMPRE_GET(pattern)) && COMPREP(rc) &&
...
compre_prfun(Lisp_Object obj, Lisp_Object pcfun, int escflag)
{
	...
	snprintf(buf, 17, "%x>", (unsigned int)((COMPRE_GET(obj))->buffer));
...
static void
compre_finfun(Lisp_Object obj, int ds)
{
...
make_compre(void)
{
	...
	result->fastmap = xmalloc(REGEXP_FASTMAP_SIZE);
...
clone_compre(COMPRE_T *src)
{
	...
	result->fastmap = (char*)xmalloc(REGEXP_FASTMAP_SIZE);
	memcpy(result->fastmap, src->fastmap, REGEXP_FASTMAP_SIZE);

	/* alloc and clone buffer */
	result->buffer = (unsigned char *)xmalloc(src->allocated);
...
DEFUN("defrexp", Fdefrexp, 2, UNEVALLED, 0, /*
\(defrexp SYMBOL REGEXP DOCSTRING\)
Like `defconst' but for forcibly compiled regexps.
					     */
      (args))
{
	/* This function can GC */
	Lisp_Object pat = XCAR(XCDR(args));
	struct gcpro gcpro1;

	CHECK_STRING(pat);

	GCPRO1(pat);
	XCAR(XCDR(args)) = Fcompile_regexp(pat);
	UNGCPRO;

	return Fdefconst(args);
}
#endif
