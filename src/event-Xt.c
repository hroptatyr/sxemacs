...
/* yeah? look closer, mate! */
struct c_doesnt_have_closures {	/* #### not yet used */
	bool warned_about_overlapping_modifiers;
	bool warned_about_predefined_modifiers;
	bool warned_about_duplicate_modifiers;
	int meta_bit;
	int hyper_bit;
	int super_bit;
	int alt_bit;
...
x_reset_modifier_mapping(struct device *d)
{
	...
	bool warned_about_overlapping_modifiers = false;
	bool warned_about_predefined_modifiers = false;
	bool warned_about_duplicate_modifiers = false;
	int meta_bit = 0;
	int hyper_bit = 0;
	int super_bit = 0;
	int alt_bit = 0;
	...
	/* Boy, I really wish C had local functions...
	 * Watch and learn, boy!  This is still ugly, I don't claim
	 * otherwise, but it proves you wrong! -hrop */
	auto void whatever()
	{
		KeySym last_sym = 0;
		...
		auto inline void modbarf(const char *name, const char *other)
		{
			warn_when_safe(
				...
				name, code, other);
			warned_about_predefined_modifiers = true;
			return;
		}
					 ...
					 int old, const char *other)
		{
			...
			warned_about_overlapping_modifiers = true;
			return;
		}
		...
		auto inline void store_modifier(const char *name, int *old)
		{
			if (*old && *old != modifier_index) {
				...
				warned_about_duplicate_modifiers = true;
			}
		...
		for (int column = 0; column < 4; column += 2) {
			KeySym sym = code
				? XKeycodeToKeysym(display, code, column)
				: 0;
	...
	if (warned_about_overlapping_modifiers) {
		warn_when_safe(Qkey_mapping, Qwarning, err_overlap_mods);
	}

	if (warned_about_predefined_modifiers) {
		warn_when_safe(Qkey_mapping, Qwarning, err_predef_mods);
	}

	/* No need to say anything more for warned_about_duplicate_modifiers. */
	if (warned_about_overlapping_modifiers
	    || warned_about_predefined_modifiers)
		warn_when_safe(Qkey_mapping, Qwarning, err_msg_else);
}
