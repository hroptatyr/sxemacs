      ...
      (locale))
{
	...
	return ((DEVICEP(locale) && DEVICE_LIVE_P(XDEVICE(locale))) ||
		...
		WINDOWP(locale) || EQ(locale, Qglobal))
	    ? Qt : Qnil;
}
...
static void check_valid_locale_or_locale_type(Lisp_Object locale)
{
	/* This cannot GC. */
	if (EQ(locale, Qall) ||
...
static Lisp_Object decode_locale(Lisp_Object locale)
{
	/* This cannot GC. */
	if (NILP(locale))
		return Qglobal;
...
static enum spec_locale_type decode_locale_type(Lisp_Object locale_type)
{
	/* This cannot GC. */
	...
	signal_type_error(Qspecifier_argument_error,
			  "Invalid specifier locale type", locale_type);
...
static enum spec_locale_type locale_type_from_locale(Lisp_Object locale)
{
	return decode_locale_type(Fspecifier_locale_type_from_locale(locale));
}

static void check_valid_domain(Lisp_Object domain)
{
	if (NILP(Fvalid_specifier_domain_p(domain)))
		signal_type_error(Qspecifier_argument_error,
				  "Invalid specifier domain", domain);
}
...
each of which is a symbol that is recognized by SXEmacs as a tag.
...
Lisp_Object decode_specifier_tag_set(Lisp_Object tag_set)
{
	...
	if (NILP(Fvalid_specifier_tag_set_p(tag_set)))
		signal_type_error(Qspecifier_argument_error,
				  "Invalid specifier tag-set", tag_set);
	return tag_set;
}

static Lisp_Object canonicalize_tag_set(Lisp_Object tag_set)
{
	...
	for (i = 1; i < len; i++) {
		...
		while (j >= 0 &&
		       strcmp((char *)string_data(XSYMBOL(tags[j])->name),
			      (char *)string_data(XSYMBOL(tags[j + 1])->name)) >
		       0) {
...
{
	if (NILP(Fvalid_specifier_tag_set_p(tag_set)))
		signal_type_error(Qspecifier_argument_error, "Invalid tag set",
				  tag_set);
      ...
      (tag, predicate))
{
	...
	CHECK_SYMBOL(tag);
	if (valid_device_class_p(tag) || valid_console_type_p(tag))
		signal_type_error(Qspecifier_change_error,
				  "Cannot redefine built-in specifier tags",
				  tag);
	...
	if (NILP(tag) || EQ(tag, Qt) || EQ(tag, Qall) || EQ(tag, Qglobal))
		signal_type_error(Qspecifier_change_error,
				  "Cannot define nil, t, 'all, or 'global",
				  tag);
	assoc = assq_no_quit(tag, Vuser_defined_tags);
	if (NILP(assoc)) {
		recompute = 1;
		Vuser_defined_tags =
		    Fcons(Fcons(tag, predicate), Vuser_defined_tags);
	...
	if (recompute) {
		DEVICE_LOOP_NO_BREAK(devcons, concons) {
			...
			assoc = assq_no_quit(tag,
					     DEVICE_USER_DEFINED_TAGS(XDEVICE
								      (device)));
			assert(CONSP(assoc));
			if (NILP(predicate))
				XCDR(assoc) = Qt;
				...
				XCDR(assoc) =
				    !NILP(call1(predicate, device)) ? Qt : Qnil;
		}
	}
      ...
      (tag))
{
	...
	{
		Lisp_Object tmp = assq_no_quit(tag, Vuser_defined_tags);
		return XCDR(tmp);
	}
}
...
static int tag_sets_match_p(Lisp_Object a, Lisp_Object b, int exact_p)
{
	if (!exact_p) {
		while (!NILP(a) && !NILP(b)) {
			if (EQ(XCAR(a), XCAR(b)))
...
check_valid_instantiator(Lisp_Object instantiator,
			 struct specifier_methods *meths, Error_behavior errb)
{
	...
	LIST_LOOP(rest, inst_list) {
		...
		if (!CONSP(rest)) {
			maybe_signal_type_error(Qspecifier_syntax_error,
						"Invalid instantiator list",
						inst_list, Qspecifier, errb);
			return Qnil;
		}
		if (!CONSP(inst_pair = XCAR(rest))) {
			maybe_signal_type_error(Qspecifier_syntax_error,
						"Invalid instantiator pair",
						inst_pair, Qspecifier, errb);
			return Qnil;
		}
		if (NILP(Fvalid_specifier_tag_set_p(tag_set = XCAR(inst_pair)))) {
			maybe_signal_type_error(Qspecifier_syntax_error,
						"Invalid specifier tag",
						tag_set, Qspecifier, errb);
			return Qnil;
		}

		if (NILP
		    (check_valid_instantiator(XCDR(inst_pair), meths, errb)))
			return Qnil;
	}
		      ...
		      Error_behavior errb)
{
	...
	LIST_LOOP(rest, spec_list) {
		...
		if (!CONSP(rest) || !CONSP(spec = XCAR(rest))) {
			maybe_signal_type_error(Qspecifier_syntax_error,
						"Invalid specification list",
						spec_list, Qspecifier, errb);
			return Qnil;
		}
		if (NILP(Fvalid_specifier_locale_p(locale = XCAR(spec)))) {
			maybe_signal_type_error(Qspecifier_syntax_error,
						"Invalid specifier locale",
						locale, Qspecifier, errb);
			return Qnil;
		}
...
enum spec_add_meth decode_how_to_add_specification(Lisp_Object how_to_add)
{
	...
	signal_type_error(Qspecifier_argument_error,
			  "Invalid `how-to-add' flag", how_to_add);
...
*/
static Lisp_Object bodily_specifier(Lisp_Object spec)
{
...
*/
static void check_modifiable_specifier(Lisp_Object spec)
{
...
static Lisp_Object restore_unlock_value(Lisp_Object val)
{
...
int unlock_ghost_specifiers_protected(void)
{
	...
	record_unwind_protect(restore_unlock_value, Vunlock_ghost_specifiers);
...
static Lisp_Object *specifier_get_inst_list(Lisp_Object specifier,
					    Lisp_Object locale,
		...
		if (tag_sets_match_p(tag_set, tagged_inst_tag, exact_p)) {
			if (short_p && NILP(tagged_inst_tag))
				retval = Fcons(copy_tree_p ?
					       Fcopy_tree(XCDR(tagged_inst),
							  Qt) :
					       XCDR(tagged_inst), retval);
			else
				retval =
				    Fcons(copy_tree_p ?
					  Fcopy_tree(tagged_inst,
						     Qt) : tagged_inst, retval);
		}
	}
	...
	*spec_list = cleanup_assoc_list(*spec_list);
	...
	LIST_LOOP(rest, *spec_list) {
		...
		Lisp_Object inst_list =
		    specifier_process_inst_list(XCDR(spec), tag_set, exact_p, 0,
						1);
		if (!NILP(inst_list))
...
static Lisp_Object *specifier_new_spec(Lisp_Object specifier,
				       Lisp_Object locale,
				       enum spec_locale_type type)
{
	Lisp_Object *spec_list = SPECIFIER_GET_SPEC_LIST(specifier, type);
		      ...
		      Lisp_Object tag_set, int exact_p)
{
	...
	if (type == LOCALE_GLOBAL)
		*spec_list =
		    specifier_process_remove_inst_list(*spec_list, tag_set,
						       exact_p, &was_removed);
	else {
			     ...
			     Lisp_Object tag_set, int exact_p)
{
	...
	LIST_LOOP(rest, *spec_list) {
		...
		if (!WINDOWP(XCAR(spec)) && object_dead_p(XCAR(spec))) {
			...
			XCDR(spec) =
			    specifier_process_remove_inst_list(XCDR(spec),
							       tag_set, exact_p,
							       &was_removed);
			if (NILP(XCDR(spec)))
				remove_spec = 1;
...
handle_multiple_add_insts(Lisp_Object * inst_list,
			  Lisp_Object new_list, enum spec_add_meth add_meth)
{
	switch (add_meth) {
	...
	case SPEC_REMOVE_TAG_SET_PREPEND: {
		...
		LIST_LOOP(rest, new_list) {
			Lisp_Object canontag =
				canonicalize_tag_set(XCAR(XCAR(rest)));
			struct gcpro gcpro1;
	...
	case SPEC_PREPEND:
	case SPEC_REMOVE_LOCALE_TYPE:
	case SPEC_REMOVE_ALL:
	default:
		return SPEC_PREPEND;
	}
}
	...
	LIST_LOOP(rest, inst_list) {
		...
		sub_inst_list = HAS_SPECMETH_P(sp, going_to_add) ?
		    SPECMETH(sp, going_to_add,
			     (bodily_specifier(specifier), locale,
			      tag_set, instantiator)) : Qt;
		if (EQ(sub_inst_list, Qt))
			/* no change here. */
			sub_inst_list =
			    list1(Fcons
				  (canonicalize_tag_set(tag_set),
				   instantiator));
		else {
			...
			LIST_LOOP(rest2, sub_inst_list)
			    XCAR(XCAR(rest2)) =
			    canonicalize_tag_set(XCAR(XCAR(rest2)));
		}
		   ...
		   Lisp_Object inst_list, enum spec_add_meth add_meth)
{
	...
	GCPRO1(list_to_build_up);
	list_to_build_up =
	    build_up_processed_list(specifier, locale, inst_list);
	...
	if (add_meth == SPEC_REMOVE_LOCALE_TYPE)
		...
		specifier_remove_spec(specifier, Qglobal, LOCALE_GLOBAL, Qnil,
				      0);
	}
	...
	*orig_inst_list = tem;
	...
	MAYBE_SPECMETH(sp, after_change, (bodily_specifier(specifier), locale));
}
			   ...
			   enum spec_add_meth add_meth)
{
	...
	LIST_LOOP(rest, *src_list) {
		...
		if (WINDOWP(XCAR(spec)) || !object_dead_p(XCAR(spec)))
			specifier_add_spec
			    (dest, XCAR(spec),
			     specifier_process_inst_list(XCDR(spec), tag_set,
							 exact_p, 0, 0),
			     add_meth);
	}
}
...
map_specifier(Lisp_Object specifier, Lisp_Object locale,
	      int (*mapfun) (Lisp_Object specifier,
			     ...
			     int exact_p,
			     void *closure),
	      Lisp_Object tag_set, Lisp_Object exact_p, void *closure)
{
	int retval = 0;
	Lisp_Object rest;
	...
	LIST_LOOP(rest, locale) {
		...
		if (!NILP(Fvalid_specifier_locale_p(theloc))) {
			...
			retval =
			    (*mapfun) (specifier, Qnil, LOCALE_BUFFER, tag_set,
				       !NILP(exact_p), closure);
			if (retval)
				break;
			retval =
			    (*mapfun) (specifier, Qnil, LOCALE_WINDOW, tag_set,
				       !NILP(exact_p), closure);
			if (retval)
				break;
			retval =
			    (*mapfun) (specifier, Qnil, LOCALE_FRAME, tag_set,
				       !NILP(exact_p), closure);
			if (retval)
				break;
			retval =
			    (*mapfun) (specifier, Qnil, LOCALE_DEVICE, tag_set,
				       !NILP(exact_p), closure);
			if (retval)
				break;
			retval =
			    (*mapfun) (specifier, Qglobal, LOCALE_GLOBAL,
				       tag_set, !NILP(exact_p), closure);
			if (retval)
				break;
		}
      ...
      (specifier, spec_list, how_to_add))
{
	...
	check_valid_spec_list(spec_list,
			      decode_specifier_type
			      (Fspecifier_type(specifier), ERROR_ME), ERROR_ME);
	add_meth = decode_how_to_add_specification(how_to_add);
...
specifier_spec_list_mapfun(Lisp_Object specifier,
			   Lisp_Object locale,
			   enum spec_locale_type locale_type,
			   Lisp_Object tag_set, int exact_p, void *closure)
{
	...
	if (!NILP(Fvalid_specifier_locale_p(locale)) ||
		...
		RETURN_UNGCPRO
		    (specifier_get_external_inst_list(specifier, locale,
						      locale_type_from_locale
						      (locale), tag_set,
						      !NILP(exact_p), 1, 1));
	} else
		return Fspecifier_spec_list(specifier, locale, tag_set,
					    exact_p);
}
...
remove_specifier_mapfun(Lisp_Object specifier,
			Lisp_Object locale,
			enum spec_locale_type locale_type,
			Lisp_Object tag_set, int exact_p, void *ignored_closure)
{
	if (NILP(locale))
		specifier_remove_locale_type(specifier, locale_type, tag_set,
					     exact_p);
	else
		specifier_remove_spec(specifier, locale, locale_type, tag_set,
				      exact_p);
	return 0;
}
...
static Lisp_Object
call_validate_matchspec_method(Lisp_Object boxed_method, Lisp_Object matchspec)
{
...
void set_specifier_fallback(Lisp_Object specifier, Lisp_Object fallback)
{
	Lisp_Specifier *sp = XSPECIFIER(specifier);
	...
	if (SPECIFIERP(fallback))
		assert(EQ
		       (Fspecifier_type(specifier), Fspecifier_type(fallback)));
...
DEFUN("specifier-instance-from-inst-list", Fspecifier_instance_from_inst_list, 3, 4, 0,	/*
...
>>>>>>> origin/master
	...
	if (!NILP(built_up_list))
		val =
		    specifier_instance_from_inst_list(specifier, Qunbound,
						      domain, built_up_list,
						      ERROR_ME, 0, Qzero);
	UNGCPRO;
	return UNBOUNDP(val) ? default_ : val;
}

DEFUN("specifier-matching-instance-from-inst-list", Fspecifier_matching_instance_from_inst_list, 4, 5, 0,	/*
Attempt to convert a particular inst-list into an instance.
This attempts to instantiate INST-LIST in the given DOMAIN
...
set_specifier_caching(Lisp_Object specifier, int struct_window_offset,
			...
			Lisp_Object oldval),
		      int struct_frame_offset, void (*value_changed_in_frame)
		       (Lisp_Object specifier, struct frame * f,
			Lisp_Object oldval), int always_recompute)
{
	Lisp_Specifier *sp = XSPECIFIER(specifier);
					 ...
					 struct window *w)
{
	...
	location = (Lisp_Object *)
	    ((char *)w +
	     XSPECIFIER(specifier)->caching->offset_into_struct_window);
	...
	if (!EQ(newval, *location)
	    || XSPECIFIER(specifier)->caching->always_recompute) {
		oldval = *location;
		*location = newval;
...
static void
recompute_one_cached_specifier_in_frame(Lisp_Object specifier, struct frame *f)
{
	Lisp_Object frame;
	Lisp_Object newval, *location, oldval;
	...
	location = (Lisp_Object *)
	    ((char *)f +
	     XSPECIFIER(specifier)->caching->offset_into_struct_frame);
	if (!EQ(newval, *location)
	    || XSPECIFIER(specifier)->caching->always_recompute) {
		oldval = *location;
		*location = newval;
...
static int
recompute_cached_specifier_everywhere_mapfun(struct window *w, void *closure)
{
...
static void integer_validate(Lisp_Object instantiator)
{
...
static void natnum_validate(Lisp_Object instantiator)
{
...
static void boolean_validate(Lisp_Object instantiator)
{
...
static void display_table_validate(Lisp_Object instantiator)
{
...
void specifier_type_create(void)
{
	...
	dump_add_root_struct_ptr(&the_specifier_type_entry_dynarr,
				 &sted_description);
