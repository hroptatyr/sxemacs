...
/* Control whether sxemacs_stat() attempts to determine file type and link count
...
static Lisp_Object build_syscolor_string(int idx)
{
	return (idx < 0 ? Qnil : mswindows_color_to_string(GetSysColor(idx)));
}

static Lisp_Object build_syscolor_cons(int index1, int index2)
{
...
static Lisp_Object build_sysmetrics_cons(int index1, int index2)
{
	return Fcons(index1 < 0 ? Qnil : make_int(GetSystemMetrics(index1)),
		     index2 < 0 ? Qnil : make_int(GetSystemMetrics(index2)));
}

static Lisp_Object build_devicecaps_cons(HDC hdc, int index1, int index2)
{
...
static void mswindows_init_device(struct device *d, Lisp_Object props)
{
	WNDCLASSEX wc;
	HDC hdc;
	...
	if (xLoadImageA)	/* not in NT 3.5 */
		wc.hIconSm =
		    (HICON) xLoadImageA(GetModuleHandle(NULL), XEMACS_CLASS,
					IMAGE_ICON, 16, 16, 0);
	else
		wc.hIconSm = 0;
...
#ifdef HAVE_DRAGNDROP
static void mswindows_init_dde()
{
...
# endif
	...
	mswindows_dde_item_open = DdeCreateStringHandle(mswindows_dde_mlid,
							...
							0);
	DdeNameService(mswindows_dde_mlid, mswindows_dde_service, 0L,
		       DNS_REGISTER);
}
#endif

void init_mswindows_very_early()
{
#ifdef HAVE_DRAGNDROP
...
static void mswindows_finish_init_device(struct device *d, Lisp_Object props)
{
#ifdef HAVE_DRAGNDROP
...
static void mswindows_delete_device(struct device *d)
{
#ifdef HAVE_DRAGNDROP
...
static void mswindows_mark_device(struct device *d)
{
	mark_object(DEVICE_MSWINDOWS_FONTLIST(d));
}

static Lisp_Object
mswindows_device_system_metrics(struct device *d, enum device_metrics m)
{
...
#define FROB(met, fore, back)				\
    case DM_##met:					\
      return build_syscolor_cons (fore, back);
		...
		FROB(color_menu_highlight, COLOR_HIGHLIGHTTEXT,
		     COLOR_HIGHLIGHT);
...
static void signal_open_printer_error(struct device *d)
{
	invalid_operation("Failed to open printer", DEVICE_CONNECTION(d));
}

/* Helper function */
static int msprinter_init_device_internal(struct device *d, char *printer_name)
{
	...
	DEVICE_MSPRINTER_HCDC(d) = CreateCompatibleDC(DEVICE_MSPRINTER_HDC(d));
...
static void msprinter_delete_device_internal(struct device *d)
{
...
static void msprinter_init_device(struct device *d, Lisp_Object props)
{
	char *printer_name;
	DEVMODE *pdm;
	size_t dm_size;
	...
	TO_EXTERNAL_FORMAT(LISP_STRING, DEVICE_CONNECTION(d),
			   C_STRING_ALLOCA, printer_name, Qmswindows_tstr);
	...
	DocumentProperties(NULL, DEVICE_MSPRINTER_HPRINTER(d),
			   printer_name, pdm, NULL, DM_OUT_BUFFER);

	assert(DEVMODE_SIZE(pdm) <= dm_size);

	DEVICE_MSPRINTER_DEVMODE(d) = allocate_devmode(pdm, 0, printer_name, d);

}

static void msprinter_delete_device(struct device *d)
{
	if (d->device_data) {
		msprinter_delete_device_internal(d);
...
static Lisp_Object
msprinter_device_system_metrics(struct device *d, enum device_metrics m)
{
	switch (m) {
...
#undef FROB
	...
	case DM_num_bit_planes:
		/* this is what X means by bitplanes therefore we ought to be
		   ...
		   therefore useless */
		return
		    make_int(GetDeviceCaps(DEVICE_MSPRINTER_HDC(d), BITSPIXEL));
...
static void msprinter_mark_device(struct device *d)
{
...
static void global_free_2_maybe(HGLOBAL hg1, HGLOBAL hg2)
{
	if (hg1 != NULL)
		GlobalFree(hg1);
	if (hg2 != NULL)
...
static HGLOBAL devmode_to_hglobal(Lisp_Devmode * ldm)
{
			  ...
			  DEVMODE * devmode_out, char *devname)
{
	...
	if (devname != NULL && stricmp(devname, DEVICE_MSPRINTER_NAME(d)) != 0) {
		Lisp_Object new_connection =
		    build_ext_string(devname, Qmswindows_tstr);
		struct gcpro gcpro1;
	...
	if (ResetDC(DEVICE_MSPRINTER_HDC(d), devmode_out) == NULL) {
		...
		DEVICE_MSPRINTER_HDC(d) =
		    CreateDC("WINSPOOL", DEVICE_MSPRINTER_NAME(d), NULL,
			     devmode_out);
	}
...
handle_devmode_changes(Lisp_Devmode * ldm, HGLOBAL hDevNames, HGLOBAL hDevMode)
{
	...
	char *new_name =
	    devnames ? (char *)devnames + devnames->wDeviceOffset : NULL;
	...
	if (!NILP(ldm->device)) {
		...
		if (!sync_printer_with_devmode
		    (d, devmode, ldm->devmode, new_name)) {
...
static void ensure_not_printing(struct device *d)
{
	if (!NILP(DEVICE_FRAME_LIST(d))) {
		Lisp_Object device;
		XSETDEVICE(device, d);
		invalid_operation
		    ("Cannot change settings while print job is active",
		     device);
	}
}

static Lisp_Devmode *decode_devmode(Lisp_Object dev)
{
	if (DEVMODEP(dev))
		return XDEVMODE(dev);
	else {
 ...
 */
static Lisp_Object print_dialog_worker(Lisp_Object dev, DWORD flags)
{
	...
	/* Finally, build the resulting plist */
	{
		...
		if (pd.Flags & PD_PAGENUMS) {
			result =
			    Fcons(Qto_page,
				  Fcons(make_int(pd.nToPage), result));
			result =
			    Fcons(Qfrom_page,
				  Fcons(make_int(pd.nFromPage), result));
			result =
			    Fcons(Qselected_page_button, Fcons(Qpages, result));
		} else if (pd.Flags & PD_SELECTION)
			result =
			    Fcons(Qselected_page_button,
				  Fcons(Qselection, result));
		else
			result =
			    Fcons(Qselected_page_button, Fcons(Qall, result));
...
mswindows_handle_print_dialog_box(struct frame * f, Lisp_Object keys)
{
		...
		EXTERNAL_PROPERTY_LIST_LOOP_3(key, value, keys) {
			if (EQ(key, Q_device)) {
				...
				if (EQ(value, Qselection))
					...
					invalid_argument
					    ("Invalid value for :selected-page-button",
					     value);
			} else
				syntax_error
				    ("Unrecognized print-dialog keyword", key);
		}
	}
	...
	return print_dialog_worker(!UNBOUNDP(device) ? device : settings,
				   flags);
}
...
int mswindows_get_default_margin(Lisp_Object prop)
{
	...
	if (EQ(prop, Qright_margin))
		return 1440;
	if (EQ(prop, Qtop_margin))
		return 720;
	if (EQ(prop, Qbottom_margin))
		return 720;
	abort();
	return 0;
}

static int plist_get_margin(Lisp_Object plist, Lisp_Object prop, int mm_p)
{
	Lisp_Object val =
	    Fplist_get(plist, prop,
		       make_int(mswindows_get_default_margin(prop)));
	if (!INTP(val))
...
mswindows_handle_page_setup_dialog_box(struct frame * f, Lisp_Object keys)
{
...

	{
		EXTERNAL_PROPERTY_LIST_LOOP_3(key, value, keys) {
			if (EQ(key, Q_device)) {
				...
				syntax_error
				    ("Unrecognized page-setup dialog keyword",
				     key);
		}
	}
...

	{
		...
		pd.rtMargin.right =
		    plist_get_margin(plist, Qright_margin, !data);
		pd.rtMargin.bottom =
		    plist_get_margin(plist, Qbottom_margin, !data);
		pd.hDevMode = devmode_to_hglobal(ldm);
		...
		/* Finally, build the resulting plist */
		{
			...
			result =
			    plist_set_margin(result, Qbottom_margin,
					     pd.rtMargin.bottom, mm_p);
			result =
			    plist_set_margin(result, Qright_margin,
					     pd.rtMargin.right, mm_p);
			result =
			    plist_set_margin(result, Qtop_margin,
					     pd.rtMargin.top, mm_p);
			result =
			    plist_set_margin(result, Qleft_margin,
					     pd.rtMargin.left, mm_p);
			return result;
		}
	}
}
      ...
      (device, settings))
{
	...
	if (!NILP(ldm->device))
		invalid_operation
		    ("The object is currently selected into a device",
		     settings);
...
static void
print_devmode(Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
{
	char buf[100];
...
static int equal_devmode(Lisp_Object obj1, Lisp_Object obj2, int depth)
{
...
static unsigned long hash_devmode(Lisp_Object obj, int depth)
{
	Lisp_Devmode *dm = XDEVMODE(obj);

	return HASH3(XDEVMODE_SIZE(dm),
		     dm->devmode ? memory_hash(dm->devmode, XDEVMODE_SIZE(dm))
		     : 0, dm->printer_name ? string_hash(dm->printer_name) : 0);
}
...
static void signal_enum_printer_error(void)
{
	invalid_operation("Error enumerating printers",
      ...
      ())
{
	...
	enum_flags =
	    PRINTER_ENUM_LOCAL | (have_nt ? PRINTER_ENUM_CONNECTIONS : 0);
	enum_level = have_nt ? 4 : 5;
	enum_entry_size =
	    have_nt ? sizeof(PRINTER_INFO_4) : sizeof(PRINTER_INFO_5);
