...
static Lisp_Object mark_mswindows_dialog_id(Lisp_Object obj)
{
	...
	case WM_DESTROY:
		{
			...
			Vdialog_data_list =
			    delq_no_quit(data, Vdialog_data_list);
		}
		break;
	...
	case WM_COMMAND:
		{
			...
			if (w_param != IDCANCEL) {	/* user pressed escape */
				assert(w_param >= ID_ITEM_BIAS
				       && w_param
				       <
				       XVECTOR_LENGTH(did->callbacks) +
				       ID_ITEM_BIAS);

				get_gui_callback(XVECTOR_DATA(did->callbacks)
						 [w_param - ID_ITEM_BIAS],
						 &fn, &arg);
				mswindows_enqueue_misc_user_event(did->frame,
								  fn, arg);
			} else
				mswindows_enqueue_misc_user_event(did->frame,
								  Qrun_hooks,
...
push_lisp_string_as_unicode(unsigned_char_dynarr * dynarr, Lisp_Object string)
{
	...
	TO_EXTERNAL_FORMAT(LISP_STRING, string,
			   C_STRING_ALLOCA, mbcs_string, Qnative);
...
static unsigned int button_width(Lisp_Object text)
{
...
static Lisp_Object free_dynarr_opaque_ptr(Lisp_Object arg)
{
...
static Lisp_Object dialog_popped_down(Lisp_Object arg)
{
	popup_up_p--;
	return Qnil;
}
...
handle_directory_dialog_box(struct frame *f, Lisp_Object keys)
{
...

	{
		EXTERNAL_PROPERTY_LIST_LOOP_3(key, value, keys) {
			if (EQ(key, Q_title)) {
				CHECK_STRING(value);
				LISP_STRING_TO_EXTERNAL(value, bi.lpszTitle,
							Qmswindows_tstr);
			} else if (EQ(key, Q_initial_directory))
				LOCAL_FILE_FORMAT_TO_TSTR(Fexpand_file_name
							  (value, Qnil),
							  pd.fname);
			else if (EQ(key, Q_initial_filename)) ;	/* do nothing */
				...
				syntax_error
				    ("Unrecognized directory dialog keyword",
				     key);
		}
	}
...
static Lisp_Object handle_file_dialog_box(struct frame *f, Lisp_Object keys)
{
	OPENFILENAME ofn;
...

	{
		EXTERNAL_PROPERTY_LIST_LOOP_3(key, value, keys) {
			if (EQ(key, Q_initial_filename)) {
				...
				CHECK_STRING(value);
				LISP_STRING_TO_EXTERNAL(value, ofn.lpstrTitle,
							Qmswindows_tstr);
			} else if (EQ(key, Q_initial_directory))
				LOCAL_FILE_FORMAT_TO_TSTR(Fexpand_file_name
							  (value, Qnil),
				...
				syntax_error("Unrecognized file-dialog keyword",
					     key);
		}
	}
...
static Lisp_Object handle_question_dialog_box(struct frame *f, Lisp_Object keys)
{
	...
	/* Determine the final width layout */
	{
		...
		if (string_max * X_DLU_PER_CHAR >
		    max(X_MAX_TEXT, button_row_width))
	...
	/* Ok, now we are ready to stuff the dialog template and lay out controls */
	{
		...
		dlg_tem.cy =
		    (Y_TEXT_FROM_EDGE + text_height + Y_TEXT_FROM_BUTTON +
		...
		Dynarr_add_many(template, &static_class_id,
				sizeof(static_class_id));
		...
		item_tem.y =
		    Y_TEXT_FROM_EDGE + text_height + Y_TEXT_FROM_BUTTON;
		item_tem.x =
		    X_BUTTON_FROM_EDGE + (button_row_width <
		...
		for (i = 0; i < Dynarr_length(dialog_items); ++i) {
			...
			Dynarr_add_many(template, &ones, 2);
			Dynarr_add_many(template, &button_class_id,
			...
			/* Next thing to add is control text, as Unicode string */
			{
				...
				memcpy(trans, XSTRING_DATA(ctext),
				       XSTRING_LENGTH(ctext) + 1);
				translen =
				    mswindows_translate_menu_or_dialog_item
				    (trans, XSTRING_LENGTH(ctext),
				     2 * XSTRING_LENGTH(ctext) + 3,
				     &accel_unused, ctext);
				push_bufbyte_string_as_unicode(template, trans,
							       translen);
			}
	   ...
	   list. */
	{
		...
		did->callbacks =
		    make_vector(Dynarr_length(dialog_items), Qunbound);
		for (i = 0; i < Dynarr_length(dialog_items); i++)
		...
		did->hwnd =
		    CreateDialogIndirectParam(NULL,
					      (LPDLGTEMPLATE)
					      Dynarr_atp(template, 0),
					      FRAME_MSWINDOWS_HANDLE(f),
					      dialog_proc,
					      (LPARAM)
					      LISP_TO_VOID(dialog_data));
		if (!did->hwnd)
			/* Something went wrong creating the dialog */
			signal_type_error(Qdialog_box_error, "Creating dialog",
					  keys);
...
void vars_of_dialog_mswindows(void)
{
	...
	DEFVAR_LISP("default-file-dialog-filter-alist", &Vdefault_file_dialog_filter_alist	/*
												 */ );
	Vdefault_file_dialog_filter_alist =
	    list5(Fcons(build_string("Text Files"), build_string("*.txt")),
		  ...
		  Fcons(build_string("HTML Files"),
			build_string("*.html;*.html")),
