	...
	*sndcnv = sndcnvnop;
	...
	if (fmt != the_fmt) {
		if (fmt == AFMT_S16_LE || fmt == AFMT_S16_BE) {
			*sndcnv =
			    fmt == AFMT_S16_BE ? sndcnv2byteBE : sndcnv2byteLE;
			if (((i = fmt =
			      AFMT_U8), ioctl(audio_fd, SNDCTL_DSP_SETFMT,
					      &i)) < 0 || fmt != i
			    || ioctl(audio_fd, SNDCTL_DSP_SETFMT, &the_fmt) < 0
			    || fmt != the_fmt) {
				perror("SNDCTL_DSP_SETFMT");
				return (0);
			}
		} else if (fmt == AFMT_MU_LAW && the_fmt == AFMT_U8) {
			/* the kernel will convert for us */ } else {
			perror("SNDCTL_DSP_SETFMT");
			return (0);
		}
	} else if (fmt == AFMT_S8) {
		*sndcnv = sndcnv2unsigned;
		if (((i = fmt =
		      AFMT_U8), ioctl(audio_fd, SNDCTL_DSP_SETFMT, &i)) < 0
		    || fmt != i
		    || ioctl(audio_fd, SNDCTL_DSP_SETFMT, &the_fmt) < 0
		    || fmt != the_fmt) {
	...
	the_speed = speed;
	ioctl(audio_fd, SNDCTL_DSP_SPEED, &the_speed);
	...
	if (((i =
	      (the_stereo =
	       tracks) - 1), ioctl(audio_fd, SNDCTL_DSP_STEREO, &i)) < 0)
		the_stereo = 1;
	...
	if (tracks != the_stereo) {
		if (tracks == 2) {
			...
			*sndcnv = *sndcnv == sndcnv2byteLE ? sndcnv2monobyteLE :
			...
			/* Switch to mono mode */
			if (((i =
			      0), ioctl(audio_fd, SNDCTL_DSP_STEREO, &i)) < 0
			    || i) {
				perror("SNDCTL_DSP_STEREO");
				return (0);
			}
			...
			if (((i =
			      AFMT_QUERY), ioctl(audio_fd, SNDCTL_DSP_SETFMT,
						 &i)) < 0 || (i != the_fmt
							      &&
							      (((i =
								 the_fmt),
								...
								ioctl(audio_fd,
								      ...
								      &i)) < 0
							       || i != the_fmt))
			    || (ioctl(audio_fd, SOUND_PCM_READ_CHANNELS, &i) >=
				0 && i != 1)) {
				/* There was no way that we could set the soundcard to a meaningful
				   mode */
				perror
				    ("SNDCTL_DSP_SETFMT and SNDCTL_DSP_STEREO");
				return (0);
			}
		} else {
			...
			if (((i = the_stereo =
			      tracks), ioctl(audio_fd, SNDCTL_DSP_STEREO,
					     &i)) < 0 || i != the_stereo - 1) {
				perror("SNDCTL_DSP_STEREO");
				return (0);
			}
	...
	if (speed * 14 < the_speed * 10 || speed * 6 > the_speed * 10) {
		char buffer[256];
		sprintf(buffer, "SNDCTL_DSP_SPEED (req: %d, rtn: %d)", speed,
			the_speed);
		perror(buffer);
		return (0);
	}
	...
	/* Debugging output is displayed only when compiled as stand-alone version */
	{
		...
		fprintf(stderr, "%s, %s, %dHz, L:%d/R:%d\n",
			...
			the_speed, the_volume / 256, the_volume % 256);
	}
#endif
				   ...
				   int length, int volume)
{
	...
	do {
		for (pptr = data;
		     (prtn =
		      parsesndfile((void **)&pptr, (size_t *) & rrtn,
				   (void **)&optr)) > 0;)
			for (cptr = optr;
			     (crtn =
			      sndcnv((void **)&cptr, (size_t *) & prtn,
				     (void **)&sptr)) > 0;) {
				for (;;) {
					if ((wrtn =
