...
#define XIC_Value(Get_Set, xic, name, attr, value)			\
	do {								\
		char *bad_arg;						\
		XVaNestedList list = XVaCreateNestedList(		\
			0, attr, value, NULL);				\
		if ((bad_arg = X##Get_Set##ICValues(			\
			     xic, name, list, NULL)) != NULL)		\
			stderr_out ("X" #Get_Set "ICValues "		\
				    "bad Arg: %s\n", bad_arg);		\
		XFree (list);						\
	} while (0)
#else /* ! DEBUG_SXEMACS */
...
Initialize_Locale (void)
{
	...
	/* dverna - Nov. 98: #### DON'T DO THIS !!! The default XtLanguageProc
	   ...
	   locale management, and especially the value of LC_NUMERIC. Anyway,
	   since at this point, we don't know yet whether we're gonna need an
	   X11 frame, we should really do it manually and not use Xlib's dumb
	   default routine */
	...
	if (!XSupportsLocale ()) {
		...
		if (!XSupportsLocale ()) {
			xim_warn ("X Windows does not support locale `C'!\n");
			return;
		}
	}
	...
	if (XSetLocaleModifiers ("") == NULL) {
		xim_warn ("XSetLocaleModifiers(\"\") failed\n"
			  "Check the value of the XMODIFIERS "
...
IMInstantiateCallback (Display *dpy, XPointer client_data, XPointer call_data)
{
	...
	if ( xim_initted == False ) {
		...
		DEVICE_X_XIM (d) = xim =
			XOpenIM (dpy, XtDatabase (dpy), name, class);
...
#define res(name, class, representation, field, default_value)		\
	{ name, class, representation, sizeof(xic_vars.field),		\
			XtOffsetOf(xic_vars_t, field), XtRString,	\
			default_value }
	...
	FRAME_X_XIC_STYLE (f) = style =
		best_style (&xic_vars.styles,
			    (XIMStyles *)DEVICE_X_XIM_STYLES(d));
...
XIM_SetGeometry (struct frame *f)
{
	...
	if (style & XIMStatusArea) {
		...
		/* If input method has existing status area, use its current
		   size */
		...
		if (needed->width == 0) {
			/* Use XNArea instead of XNAreaNeeded */
...
retry:
	...
	for (i = 0; i < len; i++) {
		...
		XSendEvent (display, main_window, False, 0L,
			    (XEvent *) &new_event);
	}
}
#endif /* 0 */
...
#undef STYLE_INFO
...
#ifdef DEBUG_XIM
	stderr_out ("EmacsCvtStringToXIMStyles called with "
		    "size=%d, string=\"%s\"\n",
	...
	if (*num_args != 0) {
		...
		XtAppWarningMsg(the_app_con, "wrongParameters",
				"cvtStringToXIMStyle",
				"XtToolkitError",
				"String to XIMStyle conversion requires "
	...
	p->count_styles = 0;
	p->supported_styles = xnew_atomic_array(XIMStyle, max_styles);
	...
	while (c < end) {
		for(i=0 ; i<max_styles ; i++) {
			...
			if(!strncmp(c, rec->name, rec->namelen - 1)) {
				p->supported_styles[p->count_styles] =
					rec->style;
				p->count_styles++;
				break;
			}
	...
	Cardinal    *num_args)
{
	...
	if (*num_args != 0) {
		XtAppWarningMsg(app, "wrongParameters", "freeXIMStyles",
				"XtToolkitError",
				"Freeing an XIMStyles requires that "
				"zero arguments be passwd",
				(String *)NULL, (Cardinal *)NULL);
		return;
	}
	...
	if (converter_data) {
		...
		XIMStyles *free_me = (XIMStyles *) toVal->addr;
		if (free_p) {
			/* use our free function */
			xfree(free_me->supported_styles);
		}
	}
}
 ...
 * - user preferences (already checked to be supported by SXEmacs)
...
best_style (XIMStyles *user, XIMStyles *xim)
{
	...
	for (i=0 ; i<user->count_styles ; i++) {
		for (j=0 ; j<xim->count_styles ; j++) {
			if (user->supported_styles[i] ==
			    xim->supported_styles[j]) {
       ...
       (frame))
{
	...
	f = XFRAME (frame);
	if (!FRAME_X_P (f)) {
		return signal_simple_error(
			"This frame is not on X device", frame);
	}
	XIM_init_frame (f);
...
x_init_fontset (struct device *d)
{
	...
	char * default_string;
	/*  char * font_set_string = "-dt-interface user-medium-r-normal\
	    -s*-*-*-*-*-*-*-*-*";*/
	char *font_set_string =
	...
	DEVICE_X_FONTSET (d) = fontset =
		XCreateFontSet (dpy, font_set_string,
				&missing_charsets,
