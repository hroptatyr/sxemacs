...
} arg;
{
  ...
  /********************************/
	...
	/* descriptor & save            */
	if ((fd_flag = fcntl(fd, F_GETFL, 0)) == -1)
		return (-1);
	if (ioctl(fd, TCGETS, &term_old) == -1)
		return (-1);
...
/*************** HFSKBD  FUNCTION ******************************/
static int hfskbd(fd, request, arg)
int fd;
int request;
struct hfbuf *arg;
{
...
/*************** HFQUERY FUNCTION ******************************/
static int hfqry(fd, request, arg)
int fd;
int request;
struct hfquery *arg;
{
...
char *buf;
{
	...
	if (SETJMP(hftenv)) {	/* set environment in case     *//* of time out                 */
	...
	while (!is_ack_vtd) {	/* do until valid ACK VTD      */
		...
		if (!memcmp(&ack, &ACK, sizeof(HFINTROSZ))	/* the ACK intro &  */
		    &&(ack.hf_request == req)) {	/* is it the response we want ? *//* yes, ACK VTD found          */
	...
	if (i = ack.hf_arg_len) {	/* any data following ?        *//* yes,                        */
...
/*************** HFT_ALRM FUNCTION ******************************/
static void hft_alrm(sig)	/* Function hft_alrm - handle  */
...
static int WR_REQ(fd, request, cmdlen, cmd, resplen)
int fd;
int request;
int cmdlen;
char *cmd;
