		...
		if (load_ignore_elc_files) {
			...
			if (!fstat(fd, &s1)) {	/* can't fail, right? */
				...
				if (result >= 0 &&
					...
					NNGCPRO1(newer_name);
					newer =
					    Ffile_name_nondirectory(newer_name);
					NNUNGCPRO;
				}
  ...
  if (load_ignore_elc_files)						\
...

	{
		...
		if (reading_elc) {
			...
			num_read =
			    Lstream_read(XLSTREAM(lispstream), elc_header, 8);
			if (num_read < 8 || strncmp(elc_header, ";ELC", 4)) {
				...
				Lstream_unread(XLSTREAM(lispstream), elc_header,
					       num_read);
			} else
...
#endif
#if defined(HAVE_FPFLOAT) || defined(HAVE_MPFR) || defined(HAVE_MPF)
#ifdef HAVE_MPFR
...
#endif	/* HAVE_MPC */
#ifdef HAVE_RESCLASS
		if (isresclassring_string(read_ptr))
			return read_resclassring_string(read_ptr);

		if (isresclass_string(read_ptr))
			return read_resclass_string(read_ptr);
#endif	/* HAVE_RESCLASS */
#ifdef HAVE_QUATERN
...
static Lisp_Object parse_integer(const Bufbyte * buf, Bytecount len, int base)
{
...

	{
      ...
      overflow:
#ifdef HAVE_MPZ
	return read_bigz_string((char*)buf);
#elif 0
...
static Lisp_Object read_integer(Lisp_Object readcharfun, int base)
{
	/* This function can GC */
	int saw_a_backslash;
...
static Lisp_Object read_bit_vector(Lisp_Object readcharfun)
{
...
static struct structure_type *recognized_structure_type(Lisp_Object type)
{
	int i;
...
static Lisp_Object read_structure(Lisp_Object readcharfun)
{
	Emchar c = readchar(readcharfun);
...
static Emchar reader_nextchar(Lisp_Object readcharfun)
{
	/* This function can GC */
	Emchar c;
...
static Lisp_Object read_string(Lisp_Object readcharfun, Emchar delim, int raw)
{
#ifdef I18N3
	...
	while ((c = readchar(readcharfun)) >= 0 && c != delim) {
		if (c == '\\') {
			if (raw) {
				...
				/* otherwise, backslash escapes the
				   next char. */
				c = read_escape(readcharfun);
		}
...
static Lisp_Object read_raw_string(Lisp_Object readcharfun)
{
	Emchar c;
						...
						if (invalid)
							RETURN_UNGCPRO
							    (Fsignal
							     (Qinvalid_read_syntax,
							      list2(build_string
								    ("invalid string property list"),
								    XCDR
								    (plist))));
						Fset_text_properties(beg, end,
								     plist,
								     tmp);
					}
					UNGCPRO;
...
int isfloat_string(const char *cp)
{
	int state = 0;
...
#ifdef HAVE_RESCLASS
int
isresclass_string (const char *cp)
{
	int state;
	const Bufbyte *ucp = (const Bufbyte *)cp;
...
int
isresclassring_string (const char *cp)
{
	int state;
	const Bufbyte *ucp = (const Bufbyte *)cp;
...

static void *sequence_reader(Lisp_Object readcharfun,
			     Emchar terminator,
			     void *state,
...
static void *read_list_conser(Lisp_Object readcharfun, void *state,
			      Charcount len)
{
