...
unexec(char *new_name, char *a_name,
       ...
       unsigned long bss_start, unsigned long entry_address)
{
	int new, old;
	char *oldptr;
...
#define BUFSIZE 8192
	...
	oldptr =
	    (char *)mmap(0, stat.st_size, PROT_READ, MAP_FILE | MAP_SHARED, old,
			 0);
	...
	if (nhdr.fhdr.f_magic != ALPHAMAGIC && nhdr.fhdr.f_magic != ALPHAUMAGIC) {
		fprintf(stderr,
	...
	if (nhdr.fhdr.f_opthdr != sizeof(nhdr.aout)) {
		fprintf(stderr,
			"unexec: input a.out header is %d bytes, not %ld.\n",
...
#define CHECK_SCNHDR(ptr, name, flags)					\
  ...
  for (i = 0; i < nhdr.fhdr.f_nscns && !ptr; i++)			\
	...
	if (rdata_section != NULL) {
		...
		rdata_section->s_scnptr =
		    text_section->s_scnptr + nhdr.aout.tsize;
	}
...
/* Not needed for now */
	...
	update_dynamic_symbols(oldptr, new_name, new, newsyms,
			       ((pHDRR) (oldptr + ohdr.fhdr.f_symptr))->
			       issExtMax,
			       ((pHDRR) (oldptr + ohdr.fhdr.f_symptr))->
			       cbExtOffset,
			       ((pHDRR) (oldptr + ohdr.fhdr.f_symptr))->
...
#undef symhdr
	...
	WRITE(new, &nhdr, sizeof(nhdr), "writing header of %s", new_name);
...
int update_dynamic_symbols(char *old,	/* Pointer to old executable */
	...
	for (i = 0; i < nsyms && found < 2; i += cbEXTR) {
		...
		if (!strcmp(s, "_edata")) {
			...
			WRITE(new, &n_edata, cbEXTR,
			      "writing symbol table entry for _edata into %s",
			      new_name);
			...
			WRITE(new, &n_end, cbEXTR,
			      "writing symbol table entry for _end into %s",
			      new_name);
		}
	}
...
static void fatal_unexec(char *s, char *arg)
{
	if (errno == EEOF)
