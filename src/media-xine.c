...

DECLARE_MEDIA_DRIVER_OPEN_METH(media_xine);
DECLARE_MEDIA_DRIVER_CLOSE_METH(media_xine);
DECLARE_MEDIA_DRIVER_READ_METH(media_xine);
DECLARE_MEDIA_DRIVER_REWIND_METH(media_xine);

DEFINE_MEDIA_DRIVER_CUSTOM(media_xine,
			   media_xine_open, media_xine_close,
			   NULL, NULL,
			   media_xine_read, NULL,
			   media_xine_rewind, NULL);


static void
media_xine_close(ms_driver_data *data)
{
	media_xine_context *mxc = (media_xine_context*)data;
	if (mxc) {
		if (mxc->stream)
...
media_xine_open(Lisp_Media_Stream *ms)
{
	/* stream stuff */
	media_substream *mss;
...

static size_t
media_xine_read(media_substream *mss, void *outbuf, size_t length)
{
...
static void
media_xine_rewind(media_substream *mss)
{
