 ...
 ****************************************************************************/
static void tty_output_vertical_divider(struct window *w, int clear)
{
...
static void set_foreground_to(struct console *c, Lisp_Object sym)
{
	Lisp_Object result;
	Bufbyte *escseq = 0;
...
static void set_background_to(struct console *c, Lisp_Object sym)
{
	Lisp_Object result;
	Bufbyte *escseq = 0;
	Bytecount escseqlen = 0;
...
int init_tty_for_redisplay(struct device *d, char *terminal_type)
{
	...
	*/
	...
	/* What we should really do is allocate just enough space for
	   ...
	   pointers. */
	CONSOLE_TTY_DATA(c)->term_entry_buffer = (char *)xmalloc(sizeof(entry_buffer));
	...
	TTY_SD(c).set_aback = tgetstr("AB", &bufptr);
	TTY_SD(c).set_afore = tgetstr("AF", &bufptr);
	TTY_SD(c).set_back = tgetstr("Sb", &bufptr);
	TTY_SD(c).set_fore = tgetstr("Sf", &bufptr);
	...
	
	{
		...
		if ((TTY_SD(c).set_aback && TTY_SD(c).set_afore) ||
		    (TTY_SD(c).set_back && TTY_SD(c).set_fore) ||
		    ((colors > 0) || (pairs  > 0))) {
			...
			if ( pairs == 0 && colors > 0 )
				...
				pairs = colors * colors;
			CONSOLE_TTY_DATA(c)->max_colors = colors;
			CONSOLE_TTY_DATA(c)->max_pairs = pairs;

			/* Most terminals that report 8 colors will
                           make bold brighter */
			CONSOLE_TTY_DATA(c)->is_bold_brighter = 
				(colors == 8 && TTY_SD(c).turn_on_bold ) 
                                ? 1 : 0; 
			...
			*/
			CONSOLE_TTY_DATA(c)->is_reverse_brighter = 
                                (colors == 8 && TTY_SD(c).turn_on_reverse) 
                                ? 1 : 0; 
		}
		else {
			DEVICE_CLASS(d) = Qmono;
			CONSOLE_TTY_DATA(c)->max_colors = 2;
			CONSOLE_TTY_DATA(c)->max_pairs = 4;
