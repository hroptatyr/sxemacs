		...
		int cursor_start, int cursor_width, int cursor_height)
{
	...
	for (i = 0; i < nruns; i++) {
		...
		if (cachel->underline) {
			...
			if (!XGetFontProperty
			    (xfont, XA_UNDERLINE_POSITION, &upos))
				upos = dl->descent / 2;
			if (!XGetFontProperty
			    (xfont, XA_UNDERLINE_THICKNESS, &uthick))
				uthick = 1;
		...
		if (cachel->strikethru) {
			...
			if (!XGetFontProperty
			    (xfont, XA_STRIKEOUT_ASCENT, &ascent))
				ascent = xfont->ascent;
			if (!XGetFontProperty
			    (xfont, XA_STRIKEOUT_DESCENT, &descent))
...
static void x_output_vertical_divider(struct window *w, int clear)
{
	...
	XFillRectangle(dpy, x_win, background_gc,
		       x + spacing + shadow_thickness, y1, line_width, y2 - y1);
