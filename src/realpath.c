...
/* Emulate readlink on win32 - finds real name (i.e. correct case) of
   ...
   reparse points. */
static int win32_readlink(const char *name, char *buf, int size)
{
...
static int cygwin_readlink(const char *name, char *buf, int size)
{
	...
	if (n < 0 && errno == EINVAL) {
		/* The file may exist, but isn't a symlink. Try to find the
		   right name. */
		char *tmp =
		    alloca(cygwin_posix_to_win32_path_list_buf_size(name));
