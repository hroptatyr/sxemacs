...
/* Control whether sxemacs_stat() attempts to generate fake but hopefully
...
child_process *new_child(void)
{
	...
	if (cp->char_avail) {
		...
		if (cp->char_consumed) {
			cp->thrd =
			    CreateThread(NULL, 1024, reader_thread, cp, 0, &id);
			if (cp->thrd)
				return cp;
		}
	}
...
void delete_child(child_process * cp)
{
	int i;
	...
	if (cp->thrd) {
		...
		if (GetExitCodeThread(cp->thrd, &rc) && rc == STILL_ACTIVE) {
			...
			if (WaitForSingleObject(cp->thrd, 1000) !=
			    WAIT_OBJECT_0) {
				DebPrint(("delete_child.WaitForSingleObject (thread) failed " "with %lu for fd %ld\n", GetLastError(), cp->fd));
...
/* Find a child by pid.  */
static child_process *find_child_pid(DWORD pid)
{
	child_process *cp;
...
static int _sys_read_ahead(int fd)
{
	child_process *cp;
	int rc = 0;
...
DWORD WINAPI reader_thread(void *arg)
{
	...
	for (;;) {
		...
		if (WaitForSingleObject(cp->char_consumed, INFINITE) !=
		    WAIT_OBJECT_0) {
			DebPrint(("reader_thread.WaitForSingleObject failed with " "%lu for fd %ld\n", GetLastError(), cp->fd));
			break;
		}
	}
	     ...
	     int *pPid, child_process * cp)
{
	...
	if (!InitializeSecurityDescriptor
	    (&sec_desc, SECURITY_DESCRIPTOR_REVISION))
		goto EH_Fail;
	/* #### not supported under win98, but will go away */
...
void merge_and_sort_env(char **envp1, char **envp2, char **new_envp)
{
	char **optr, **nptr;
	int num;
	    ...
	    const char *const *argv, const char *const *envp)
{
	...
	if (NILP(Ffile_executable_p(program))) {
		...
		TO_EXTERNAL_FORMAT(LISP_STRING, full,
				   C_STRING_ALLOCA, cmdname, Qfile_name);
	} else {
...
static BOOL CALLBACK find_child_console(HWND hwnd, child_process * cp)
{
	DWORD thread_id;
	DWORD process_id;
	...
	if (sig == SIGINT) {
		if (NILP(Vwin32_start_process_share_console) && cp && cp->hwnd) {
			...
			if (foreground_window && SetForegroundWindow(cp->hwnd)) {
				...
				keybd_event(VK_CONTROL, control_scan_code, 0,
					    0);
				keybd_event(vk_break_code, break_scan_code, 0,
					    0);
...
void prepare_standard_handles(int in, int out, int err, HANDLE handles[3])
{
	HANDLE parent;
...
void reset_standard_handles(int in, int out, int err, HANDLE handles[3])
{
      ...
      (filename))
{
	...
	if (!win32_get_long_filename
	    (XSTRING_DATA(filename), longname, MAX_PATH))
		return Qnil;
...
/* Sync with FSF Emacs 19.34.6 note: dwWinThreadId declared in
   ...
   XEmacs.  #### Check what we lose by ifdef'ing out these. --marcpa */
	...
	if (dwWinThreadId)
		/* Reply is not needed.  */
		PostThreadMessage(dwWinThreadId, WM_EMACS_SETLOCALE, XINT(lcid),
				  0);
#endif
...
void vars_of_ntproc(void)
{
	...
	DEFVAR_LISP("win32-start-process-show-window", &Vwin32_start_process_show_window	/*
												   When nil, processes started via start-process hide their windows.
												   When non-nil, they show their window in the method of their choice.
												 */ );
	Vwin32_start_process_show_window = Qnil;

	DEFVAR_LISP("win32-start-process-share-console", &Vwin32_start_process_share_console	/*
