...
Lisp_Object *execute_rare_opcode(Lisp_Object * stack_ptr,
				 const Opbyte * program_ptr, Opcode opcode);
...
static void meter_code(Opcode prev_opcode, Opcode this_opcode)
{
	if (byte_metering_on) {
		Lisp_Object *p =
		    XVECTOR_DATA(XVECTOR_DATA(Vbyte_code_meter)[this_opcode]);
		p[0] = INT_PLUS1(p[0]);
		if (prev_opcode)
...

static Lisp_Object bytecode_negate(Lisp_Object obj)
{
      retry:

	if (INTP(obj)) {
#ifdef WITH_NUMBER_TYPES
		return ent_optable_neg[INT_T](obj);
#else
		return make_integer(-XINT(obj));
#endif
	}
#ifdef LISP_FLOAT_TYPE
	if (FLOATP(obj))
		return make_float(-XFLOAT_DATA(obj));
#endif
	if (CHARP(obj))
		return make_integer(-((int)XCHAR(obj)));
	if (MARKERP(obj))
		return make_integer(-((int)marker_position(obj)));

#ifdef WITH_NUMBER_TYPES
	return ent_optable_neg[get_number_type(obj)](obj);
#endif
...
static Lisp_Object bytecode_nreverse(Lisp_Object list)
{
...
/* such a stupid function :(
   how can someone assume that there cannot be numbers which are less and
   greater than the other simultaneously
   stupid, stupid, stupid
   -hroptatyr
*/
static int bytecode_arithcompare(Lisp_Object obj1, Lisp_Object obj2)
{
...
#ifdef LISP_FLOAT_TYPE
	{
		...
		if (INTP(obj1))
			...
			goto arithcompare_float;

		if (INTP(obj2))
			ival2 = XINT(obj2);
		else if (CHARP(obj2))
			ival2 = XCHAR(obj2);
		else if (MARKERP(obj2))
			ival2 = marker_position(obj2);
		else
			goto arithcompare_float;
...

	{
		...
		if (FLOATP(obj1))
			dval1 = XFLOAT_DATA(obj1);
		else if (INTP(obj1))
			dval1 = (double)XINT(obj1);
		else if (CHARP(obj1))
			dval1 = (double)XCHAR(obj1);
		else if (MARKERP(obj1))
			dval1 = (double)marker_position(obj1);
		else {
			obj1 =
			    wrong_type_argument(Qnumber_char_or_marker_p, obj1);
			goto retry;
		}

		if (FLOATP(obj2))
			dval2 = XFLOAT_DATA(obj2);
		else if (INTP(obj2))
			dval2 = (double)XINT(obj2);
		else if (CHARP(obj2))
			dval2 = (double)XCHAR(obj2);
		else if (MARKERP(obj2))
			dval2 = (double)marker_position(obj2);
		else {
			obj2 =
			    wrong_type_argument(Qnumber_char_or_marker_p, obj2);
			goto retry;
		}
...
#ifdef WITH_NUMBER_TYPES
#define BYTECODE_ARITHCOMPARE_OP(op, obj1, obj2) do			\
{									\
	number_type nt;							\
									\
	...
	case IMPOSSIBLE:						\
		...
		break;							\
	default:							\
		obj1 = ent_optable_##op[nt][nt](obj1, obj2);		\
		break;							\
	}								\
} while (0)
#define BYTECODE_ARITHCOMPARE_OP2(op1, op2, obj1, obj2) do		\
{									\
	number_type nt;							\
									\
	switch (nt = promote_args(&obj1, &obj2)) {			\
	case IMPOSSIBLE:						\
		...
		break;							\
	default:							\
		if (NILP(ent_optable_##op1[nt][nt](obj1, obj2)))	\
			obj1 = ent_optable_##op2[nt][nt](obj1, obj2);	\
		else							\
			obj1 = Qt;					\
		break;							\
	}								\
...
bytecode_arithop(Lisp_Object obj1, Lisp_Object obj2, Opcode opcode)
{
	...
	nt = promote_args(&obj1, &obj2);

	switch (opcode) {
	case Bplus:
		result = ent_optable_sum[nt][nt](obj1, obj2);
		break;
	case Bdiff:
		result = ent_optable_diff[nt][nt](obj1, obj2);
		break;
	case Bmult:
		result = ent_optable_prod[nt][nt](obj1, obj2);
		break;
	case Bquo:
		result = ent_optable_div[nt][nt](obj1, obj2);
		break;
	case Bmax:
		result = (!NILP(ent_optable_gt[nt][nt](obj1, obj2)))
			? obj1 : obj2;
		break;
	case Bmin:
		result = (!NILP(ent_optable_lt[nt][nt](obj1, obj2)))
			? obj1 : obj2;
		break;
	default:
		result = Qzero;
...
funcall_compiled_function(Lisp_Object fun, int nargs, Lisp_Object args[])
{
	...
	{
		Lisp_Object value =
		    execute_optimized_program((Opbyte *)
					      XOPAQUE_DATA(f->instructions),
					      f->stack_depth,
	...
	while (1) {
		...
		default:
			if (opcode >= Bconstant)
				...
				stack_ptr =
				    execute_rare_opcode(stack_ptr, program_ptr,
							opcode);
			break;
		...
		case Bcall + 7:
			...
			if (byte_metering_on && SYMBOLP(TOP)) {
				Lisp_Object val =
				    Fget(TOP, Qbyte_code_meter, Qnil);
				if (INTP(val))
					Fput(TOP, Qbyte_code_meter,
					     make_int(XINT(val) + 1));
			}
#endif
		...
		case Bunbind + 7:
			UNBIND_TO(specpdl_depth() -
				  (opcode < Bunbind + 6 ? opcode - Bunbind :
				   opcode ==
				   Bunbind + 6 ? READ_UINT_1 : READ_UINT_2));
			break;
		...
		case Beq:
			{
				Lisp_Object arg = POP;
				TOP =
				    EQ_WITH_EBOLA_NOTICE(TOP, arg) ? Qt : Qnil;
				break;
			}
		...
		case Beqlsign:
			{
				Lisp_Object arg = POP;
#ifdef WITH_NUMBER_TYPES
				BYTECODE_ARITHCOMPARE_OP(eq, TOP, arg);
#else
				TOP = bytecode_arithcompare(TOP, arg) == 0
					? Qt : Qnil;
#endif
				break;
			}
		...
		case Bgtr:
			{
				Lisp_Object arg = POP;
#ifdef WITH_NUMBER_TYPES
				BYTECODE_ARITHCOMPARE_OP(gt, TOP, arg);
#else
				TOP = bytecode_arithcompare(TOP, arg) > 0
					? Qt : Qnil;
#endif
		...
		case Blss:
			{
				Lisp_Object arg = POP;
#ifdef WITH_NUMBER_TYPES
				BYTECODE_ARITHCOMPARE_OP(lt, TOP, arg);
#else
				TOP = bytecode_arithcompare(TOP, arg) < 0
					? Qt : Qnil;
#endif
		...
		case Bleq:
			{
				Lisp_Object arg = POP;
#ifdef WITH_NUMBER_TYPES
				BYTECODE_ARITHCOMPARE_OP2(lt, eq, TOP, arg);
#else
				TOP = bytecode_arithcompare(TOP, arg) <= 0
					? Qt : Qnil;
#endif
		...
		case Bgeq:
			{
				Lisp_Object arg = POP;
#ifdef WITH_NUMBER_TYPES
				BYTECODE_ARITHCOMPARE_OP2(gt, eq, TOP, arg);
#else
				TOP = bytecode_arithcompare(TOP, arg) >= 0
					? Qt : Qnil;
#endif
...
{
	if ((opcode < Bvarref) ||
	    (opcode == 0251) || (opcode > Bassq && opcode < Bconstant))
		invalid_byte_code_error
		    ("invalid opcode %d in instruction stream", opcode);
}

/* Check that IDX is a valid offset into the `constants' vector */
static void check_constants_index(int idx, Lisp_Object constants)
{
...
static void optimize_byte_code(	/* in */
	...
	*varbind_count = 0;
	...
	while (ptr < end) {
		...
		case Bvarref + 5:
			...
			if (!SYMBOLP(val))
				invalid_byte_code_error
				    ("variable reference to non-symbol %S",
				     val);
			if (EQ(val, Qnil) || EQ(val, Qt)
			    || (SYMBOL_IS_KEYWORD(val)))
				invalid_byte_code_error
				    ("variable reference to constant symbol %s",
		...
		case Bvarset + 5:
			...
			if (!SYMBOLP(val))
				invalid_byte_code_error
				    ("attempt to set non-symbol %S", val);
			if (EQ(val, Qnil) || EQ(val, Qt))
				invalid_byte_code_error
				    ("attempt to set constant symbol %s",
				     string_data(XSYMBOL(val)->name));
		...
		case Bvarbind + 5:
			...
			if (!SYMBOLP(val))
				invalid_byte_code_error
				    ("attempt to let-bind non-symbol %S", val);
			if (EQ(val, Qnil) || EQ(val, Qt)
			    || (SYMBOL_IS_KEYWORD(val)))
				invalid_byte_code_error
				    ("attempt to let-bind constant symbol %s",
				     string_data(XSYMBOL(val)->name));
...
void optimize_compiled_function(Lisp_Object compiled_function)
{
	...
	if (STRINGP(f->instructions)) {
		...
		program =
		    alloca_array(Opbyte,
				 1 + 2 * XSTRING_LENGTH(f->instructions));
		optimize_byte_code(f->instructions, f->constants, program,
...
{
	/* This function can GC */
	Lisp_Compiled_Function *f = XCOMPILED_FUNCTION(obj);	/* GC doesn't relocate */
	...
	write_c_string(" ", printcharfun);
	print_internal(compiled_function_constants(f), printcharfun,
		       escapeflag);
...
static Lisp_Object mark_compiled_function(Lisp_Object obj)
{
...
compiled_function_equal(Lisp_Object obj1, Lisp_Object obj2, int depth)
{
	...
	return (f1->flags.documentationp == f2->flags.documentationp && f1->flags.interactivep == f2->flags.interactivep && f1->flags.domainp == f2->flags.domainp &&	/* I18N3 */
...
Lisp_Object compiled_function_instructions(Lisp_Compiled_Function * f)
{
...

	{
		...
		const Opbyte *const program =
		    (const Opbyte *)OPAQUE_DATA(opaque);
		const Opbyte *program_ptr = program;
		const Opbyte *const program_end =
		    program_ptr + OPAQUE_SIZE(opaque);
      ...
      (function))
{
	CHECK_COMPILED_FUNCTION(function);
	return
	    make_int(compiled_function_stack_depth
      ...
      (function))
{
	...
	if (CONSP(f->instructions)) {
		...
		if (!CONSP(tem))
			signal_simple_error("Invalid lazy-loaded byte code",
					    tem);
