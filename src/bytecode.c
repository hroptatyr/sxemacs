...
enum Opcode {			/* Byte codes */
	...
	Bcl_macro = 0270,		/* only if modules/cl is there */

	BLAST_BEFORE_THREE_O_O = Bcl_macro,
...
static Lisp_Object*
execute_rare_opcode(Lisp_Object *stk, const Opbyte *prg, Opcode opcode)
...
{
	/* This function can GC */
	REGISTER const Opbyte *program_ptr = program;
	/* C99 here we come */
	Lisp_Object stack_beg[stack_depth + 1];
	...
	*/
	...
	while (1) {
		REGISTER Opcode opcode = READ_UINT_1;

		/* Get nvars right before maybe signaling. */
		GCPRO_STACK;
#ifdef ERROR_CHECK_BYTE_CODE
		...
		default:
			if (opcode >= Bconstant)
				...
				stack_ptr = execute_rare_opcode(
					stack_ptr, program_ptr, opcode);
			break;
		...
		case Bunbind_all:
			/* To unbind back to the beginning of this frame.  Not
			   used yet, but will be needed for tail-recursion
			   elimination. */
		...
		case Beqlsign: {
			Lisp_Object arg = POP;
			if (ent_binrel(ASE_BINARY_REL_EQUALP, TOP, arg))
				TOP = Qt;
			else
				TOP = Qnil;
			break;
		...
		case Bgtr: {
			Lisp_Object arg = POP;
			if (ent_binrel(ASE_BINARY_REL_GREATERP, TOP, arg))
				TOP = Qt;
			else
				TOP = Qnil;
			break;
		...
		case Blss: {
			Lisp_Object arg = POP;
			if (ent_binrel(ASE_BINARY_REL_LESSP, TOP, arg))
				TOP = Qt;
			else
				TOP = Qnil;
			break;
		...
		case Bleq: {
			Lisp_Object arg = POP;
			if (ent_binrel2(ASE_BINARY_REL_LESSP,
					ASE_BINARY_REL_EQUALP, TOP, arg))
				TOP = Qt;
			else
				TOP = Qnil;
			break;
		...
		case Bgeq: {
			Lisp_Object arg = POP;
			if (ent_binrel2(ASE_BINARY_REL_GREATERP,
					ASE_BINARY_REL_EQUALP, TOP, arg))
				TOP = Qt;
			else
				TOP = Qnil;
		...
		case Bplus: {
			Lisp_Object arg = POP;
			TOP = ent_binop(ASE_BINARY_OP_SUM, TOP, arg);
			break;
		}
		case Bdiff: {
			Lisp_Object arg = POP;
			TOP = ent_binop(ASE_BINARY_OP_DIFF, TOP, arg);
			break;
		}
		case Bmult: {
			Lisp_Object arg = POP;
			TOP = ent_binop(ASE_BINARY_OP_PROD, TOP, arg);
			break;
		}
		case Bquo: {
			Lisp_Object arg = POP;
			TOP = ent_binop(ASE_BINARY_OP_DIV, TOP, arg);
			break;
		}
		case Bmax: {
			Lisp_Object arg = POP;
		...
		case Bcl_macro:	{
			...
			if (SUBRP(fun) && XSUBR(fun)->max_args == UNEVALLED) {
				Lisp_Object(*subr)(Lisp_Object) =
					(Lisp_Object(*)(Lisp_Object))
					subr_function(XSUBR(fun));
				TOP = subr(args);
			} else {
				fprintf(stderr, "Uh-oh!\nSuicide?\n");
				if (random() & 1) {
					fprintf(stderr, "YESSSSSS!\n");
					abort();
				}
				fprintf(stderr, "tomorrow maybe\n");
			}
			break;
		}
		}
...
/* It makes a worthwhile performance difference (5%) to shunt
   ...
   How about __attribute__((noinline)) then? -hrop */
static Lisp_Object*
		    ...
		    const Opbyte *program_ptr, Opcode opcode)
{
	...
	case Brem: {
		Lisp_Object arg = POP;
		TOP = ent_binop(ASE_BINARY_OP_REM, TOP, arg);
		break;
	}
...
static void check_opcode(Opcode opcode)
{
	if ((opcode < Bvarref) ||
	    (opcode == 0251) ||
	    (opcode > BLAST_BEFORE_THREE_O_O && opcode < Bconstant))
		invalid_byte_code_error
...
static void optimize_byte_code(
	/* in */
	...
	int *const varbind_count)
{
	...
	Bufbyte *ptr = XSTRING_DATA(instructions);
	...
	*varbind_count = 0;
	...
	for (struct jump *j = jumps; j < jumps_ptr; j++) {
#ifdef ERROR_CHECK_BYTE_CODE
	...
	for (bool more_fixups_needed = true; more_fixups_needed; ) {
		struct jump *j;

		/* assume we don't need more hiccups */
		more_fixups_needed = false;
		for (j = jumps; j < jumps_ptr; j++) {
			...
			case BRgotoifnilelsepop:
			case BRgotoifnonnilelsepop:
				if (jump > SCHAR_MIN && jump <= SCHAR_MAX) {
					WRITE_INT8(jump, p);
				} else {
					/* barf */
					struct jump *jj;
