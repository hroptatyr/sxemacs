...
static Lisp_Object ent_inv_INT_T(Lisp_Object l)
{
	if (XREALINT(l) == 0)
		Fsignal(Qarith_error, Qnil);
	return make_bigq(1L, XREALINT(l));
}
#else
static Lisp_Object ent_inv_INT_T(Lisp_Object l)
{
...
{
	return ent_normalise_number(number);
}
...
void init_optables_INT_T(void)
{
	...
	ent_optable_lift[INT_T][INT_T] = ent_lift_INT_T_INT_T;
	ent_optable_lift[INDEF_T][INT_T] = ent_lift_INDEF_T_COMPARABLE;
}
