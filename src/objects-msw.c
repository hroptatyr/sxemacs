 ...
 */
static int match_font(char *pattern1, char *pattern2, char *fontname)
{
	...
	if (FontType == 0 /*vector */  || FontType == TRUETYPE_FONTTYPE)
		...
		sprintf(fontname, "%s::%d::", lpelfe->elfLogFont.lfFaceName,
			MulDiv(lpntme->ntmTm.tmHeight -
			       lpntme->ntmTm.tmInternalLeading, 72,
			       GetDeviceCaps(font_enum->hdc, LOGPIXELSY)));
...
Lisp_Object mswindows_enumerate_fonts(HDC hdc)
{
	...
	if (xEnumFontFamiliesExA)
		xEnumFontFamiliesExA(hdc, &logfont,
				     (FONTENUMPROC) font_enum_callback_1,
				     (LPARAM) (&font_enum), 0);
	else			/* NT 3.5x */
		EnumFontFamilies(hdc, 0,
				 (FONTENUMPROC) old_font_enum_callback_1,
				 (LPARAM) (&font_enum));
...
static HFONT
mswindows_create_font_variant(Lisp_Font_Instance * f, int under, int strike)
{
	/* Cannot GC */
				    ...
				    Lisp_Object device, Error_behavior errb)
{
	...
	TO_EXTERNAL_FORMAT(LISP_STRING, name, C_STRING_ALLOCA, extname, Qctext);
...
#if 0
static void mswindows_mark_color_instance(Lisp_Color_Instance * c)
{
}
#endif
...
static void mswindows_finalize_color_instance(Lisp_Color_Instance * c)
{
	if (c->data) {
		xfree(c->data);
		c->data = 0;
...
static int mswindows_valid_color_name_p(struct device *d, Lisp_Object color)
{
	const char *extname;

	TO_EXTERNAL_FORMAT(LISP_STRING, color,
			   C_STRING_ALLOCA, extname, Qctext);
	return (mswindows_string_to_color(extname) != -1);
}

static void mswindows_finalize_font_instance(Lisp_Font_Instance * f);
			 ...
			 Error_behavior errb)
{
	...
	if (fields > 0 && strlen(fontname)) {
		...
		maybe_signal_simple_error("Must specify a font name", name,
					  Qfont, errb);
		return (0);
	}
	...
	if (i == countof(fontweight_map)) {	/* No matching weight */
		if (!style) {
			...
			maybe_signal_simple_error("Invalid font weight", name,
						  Qfont, errb);
			return (0);
		}
	}
	...
	if (fields < 3)
		...
		maybe_signal_simple_error("Invalid font pointsize", name, Qfont,
					  errb);
		return (0);
	}
	...
	if (fields >= 4 && effects[0] != '\0') {
		...
		if (effects2 && effects2[0] != '\0') {
			if (stricmp(effects2, "underline") == 0)
				...
				maybe_signal_simple_error("Invalid font effect",
							  name, Qfont, errb);
				return (0);
			}
		}
	...
	if (!NILP(device_font_list)) {
		...
		sprintf(truename, "%s:%s:%d:%s:%s", fontname, weight, pt,
			effects, charset);
		LIST_LOOP(fonttail, device_font_list) {
			if (match_font
			    (XSTRING_DATA(XCAR(fonttail)), truename, NULL))
				break;
		}
		if (NILP(fonttail)) {
			maybe_signal_simple_error("No matching font", name,
						  Qfont, errb);
			return 0;
		}
	}
	...
	if ((hfont = CreateFontIndirect(&logfont)) == NULL) {
		maybe_signal_simple_error("Couldn't create font", name, Qfont,
					  errb);
		return 0;
	}
	...
	if (!hfont2) {
		...
		maybe_signal_simple_error("Couldn't map font", name, Qfont,
					  errb);
		return 0;
	}
	...
	if (f->data) {
		for (i = 0; i < MSWINDOWS_NUM_FONT_VARIANTS; i++) {
			if (FONT_INSTANCE_MSWINDOWS_HFONT_I(f, i) != NULL
			    && FONT_INSTANCE_MSWINDOWS_HFONT_I(f,
							       i) !=
			    MSWINDOWS_BAD_HFONT)
				DeleteObject(FONT_INSTANCE_MSWINDOWS_HFONT_I
					     (f, i));
		}
...
#if 0
static void mswindows_mark_font_instance(Lisp_Font_Instance * f)
{
}
#endif
...
static Lisp_Object mswindows_list_fonts(Lisp_Object pattern, Lisp_Object device)
{
	Lisp_Object fonttail, result = Qnil;
	char *extpattern;

	TO_EXTERNAL_FORMAT(LISP_STRING, pattern,
			   C_STRING_ALLOCA, extpattern, Qctext);
      ...
      ())
{
	...
	for (i = 0; i < countof(mswindows_X_color_map); i++)
		result =
		    Fcons(build_string(mswindows_X_color_map[i].name), result);
