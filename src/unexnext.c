		  ...
		  unsigned long fpos, unsigned long tpos, unsigned long len)
{
	if ((ffd == -1) && (tfd == -1)) {
		...
		while ((len > 0)
		       && (bread = read(ffd, buf, MIN(1 << 16, len))) > 0) {
			if (bread < 0)
				fatal_unexec("cannot read source");
...
static void unexec_doit(int infd, int outfd)
{
	...
	for (pregions = &regions; *pregions;) {
		...
		for (sp = lc, i = 0;
		     i < mh.ncmds;
		     i++, sp =
		     (struct segment_command *)(((char *)sp) + sp->cmdsize)) {
			unsigned long ob, oe;
			if (sp->cmd != LC_SEGMENT
			    || (strcmp(sp->segname, SEG_DATA) == 0))
				continue;
			ob = MAX((*pregions)->addr, sp->vmaddr);
			oe = MIN((*pregions)->addr + (*pregions)->size,
				 sp->vmaddr + sp->vmsize);
			if (ob >= oe)
				continue;
			if (ob == (*pregions)->addr)
				if (oe == (*pregions)->addr + (*pregions)->size) {
					...
					(*pregions)->addr = oe;
					(*pregions)->size -= (oe - ob);
			} else if (oe == (*pregions)->addr + (*pregions)->size) {
				(*pregions)->size -= (oe - ob);
			} else {
				cregion = alloca(sizeof(*cregion));
				cregion->addr = oe;
				cregion->size =
				    ((*pregions)->addr + (*pregions)->size) -
				    oe;
	...
	for (sp = lc, i = mh.ncmds, hpos = sizeof(mh), opos = 0;
		...
		case LC_SEGMENT:
			if (strcmp(sp->segname, SEG_DATA) == 0) {
				...
				j = sp->cmdsize;
				while (regions) {
					mcopy(-1, outfd, regions->addr, opos,
					      regions->size);
					sp->cmd = LC_SEGMENT;
					sp->cmdsize = sizeof(*sp);
					strncpy(sp->segname, SEG_DATA,
						sizeof(sp->segname));
					...
					sp->fileoff = opos;
					opos += sp->filesize;
					mcopy(-1, outfd, (unsigned long)sp,
					      hpos, sp->cmdsize);
					hpos += sp->cmdsize;
					mh.ncmds++;
				...
				mcopy(infd, outfd, sp->fileoff, opos,
				      sp->filesize);
				sect =
				    (struct section *)(((char *)sp) +
						       sizeof(*sp));
				...
				sp->fileoff = opos;
				opos += sp->filesize;
				mcopy(-1, outfd, (unsigned long)sp, hpos,
				      sp->cmdsize);
				hpos += sp->cmdsize;
			}
			break;
		case LC_SYMTAB:
			...
			mcopy(infd, outfd, st->symoff, opos,
			      st->nsyms * sizeof(struct nlist));
			st->symoff = opos;
...
void unexec(char *outfile, char *infile)
{
	char tmpfile[MAXPATHLEN];
        int  remain = sizeof(tmpfile)-1;
	int infd, outfd;

	if ((infd = open(infile, O_RDONLY, 0)) < 0)
		fatal_unexec("cannot open input file `%s'", infile);

	strncpy(tmpfile, outfile, remain);
        tmpfile[remain]='\0';
        remain -= strlen(tmpfile);
	strncat(tmpfile, "-temp",remain);
