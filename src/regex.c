...
#if (defined (DEBUG_SXEMACS) && !defined (DEBUG))
#define DEBUG
#endif
...
static void print_compiled_pattern(struct re_pattern_buffer *bufp)
{
		...
		case '*':
...

			{
				...
				if (minimal) {
					...
					if (many_times_ok) {	/* More than one repetition is allowed, so put in
	   ...
	   the search and match routines.  */
	{
		...
		if (fail_stack.size < (2 * re_max_failures * MAX_FAILURE_ITEMS)) {
			...
			if (!fail_stack.stack)
				fail_stack.stack
				    =
				    (fail_stack_elt_t *) xmalloc(fail_stack.
								 size *
								 sizeof
								 (fail_stack_elt_t));
			else
				fail_stack.stack
				    =
				    (fail_stack_elt_t *) xrealloc(fail_stack.
								  stack,
								  (fail_stack.
								   size *
								   sizeof
			...
			if (!fail_stack.stack)
				fail_stack.stack
				    =
				    (fail_stack_elt_t *) malloc(fail_stack.
								size *
								sizeof
								(fail_stack_elt_t));
			else
				fail_stack.stack
				    =
				    (fail_stack_elt_t *) realloc(fail_stack.
								 stack,
								 (fail_stack.
								  size *
								  sizeof
		    ...
		    struct re_registers *regs, int stop)
{
	...
	for (;;) {
		...
		if (p == pend) {	/* End of pattern means we might have succeeded.  */
			...
			/* If caller wants register contents data back, do it.  */
			{
				...
				if (regs && !bufp->no_sub) {
					...
					for (mcnt = 1;
					     mcnt < MIN(num_nonshy_regs,
							regs->num_regs);
					     mcnt++) {
						int ireg =
						    bufp->
						    external_to_internal_register[mcnt];

						if (REG_UNSET(regstart[ireg])
						    || REG_UNSET(regend[ireg]))
							regs->start[mcnt] =
							    regs->end[mcnt] =
							    -1;
						else {
			...
			if (regs && regs->num_regs > 0)
				for (mcnt = num_regs; mcnt < regs->num_regs;
				     mcnt++)
		...
		case dummy_failure_jump:
			...
			PUSH_FAILURE_POINT((unsigned char *)0,
					   (unsigned char *)0, -2);
			goto unconditional_jump;
			       ...
			       struct re_pattern_buffer *bufp)
{
	...
	ret =
	    regex_compile((unsigned char *)pattern, length, re_syntax_options,
			  bufp);
...
int regcomp(regex_t * preg, const char *pattern, int cflags)
{
	...
	ret =
	    regex_compile((unsigned char *)pattern, strlen(pattern), syntax,
			  preg);
