...
static void create_left_glyph_block(struct window *w,
				    struct display_line *dl, int overlay_width);
static void create_right_glyph_block(struct window *w, struct display_line *dl);
			    ...
			    Bytecount offset, Bytecount len)
{
	...
	convert_bufbyte_string_into_emchar_dynarr(nonreloc, len,
						  rtw_emchar_dynarr);
	return redisplay_text_width_emchar_string(w, findex,
						  Dynarr_atp(rtw_emchar_dynarr,
							     0),
						  Dynarr_length
						  (rtw_emchar_dynarr));
}
				  ...
				  Bytecount offset, Bytecount len)
{
	...
	return DEVMETH(XDEVICE(FRAME_DEVICE(f)),
		       text_width, (f, &cachel,
				    Dynarr_atp(rtw_emchar_dynarr, 0),
						  ...
						  enum display_type type)
{
	...
	if (dl->display_blocks) {
		...
		if (elt < Dynarr_largest(dl->display_blocks)) {
			struct display_block *dbp =
			    Dynarr_atp(dl->display_blocks, elt);
...
static int tab_char_width(struct window *w)
{
	struct buffer *b = XBUFFER(w->buffer);
	int char_tab_width = XINT(b->tab_width);

	if (char_tab_width <= 0 || char_tab_width > 1000)
		char_tab_width = 8;

	return char_tab_width;
}

static int space_width(struct window *w)
{
...
static int tab_pix_width(struct window *w)
{
	...
	for (i = 0; i < Dynarr_length(fixup->runes); i++) {
		...
		if (r->type == RUNE_DGLYPH) {
			if (r->object.dglyph.ascent < dl->ascent)
				r->object.dglyph.yoffset =
				    dl->ascent - r->object.dglyph.ascent +
...
static void calculate_baseline(pos_data * data)
{
	...
	if (data->max_pixmap_height > data->new_ascent + data->new_descent) {
		int default_font_ascent, default_font_descent,
		    default_font_height;
		...
		if (data->max_pixmap_height >
			...
			scaled_default_font_descent =
			    (data->max_pixmap_height * default_font_descent /
			     default_font_height) + 1;

			data->new_descent =
			    max(data->new_descent, scaled_default_font_descent);
		}
	}
}
...
generate_display_line(struct window *w, struct display_line *dl, int bounds,
		      Bufpos start_pos, prop_block_dynarr ** prop, int type)
{
	Bufpos ret_bufpos;
	int overlay_width;
...
static prop_block_dynarr *add_hscroll_rune(pos_data * data)
{
	struct glyph_block gb;
	...
	gb.glyph = Vhscroll_glyph;
	{
		...
		data->hscroll_glyph_width_adjust =
		    data->pixpos - oldpixpos -
		    space_width(XWINDOW(data->window));
	}
	data->bi_endpos = 0;
...
static prop_block_dynarr *add_emchar_rune_1(pos_data * data,
					    int no_contribute_to_line_height)
{
	...
	if (data->ch == '\n') {
		...
		if (!EQ(charset, data->last_charset) ||
			...
			struct face_cachel *cachel =
			    WINDOW_FACE_CACHEL(w, data->findex);
			Lisp_Object font_instance =
			    ensure_face_cachel_contains_charset(cachel,
								data->window,
								charset);
			...
			if (EQ(font_instance, Vthe_null_font_instance)) {
				font_instance =
				    FACE_CACHEL_FONT(cachel, Vcharset_ascii);
			...
			if (!no_contribute_to_line_height) {
				data->new_ascent =
				    max(data->new_ascent, (int)fi->ascent);
				data->new_descent =
				    max(data->new_descent, (int)fi->descent);
			}
		...
		if (width < 0) {
			...
			width =
			    redisplay_text_width_emchar_string(XWINDOW
							       (data->window),
							       data->findex,
							       &data->ch, 1);
		}
	...
	if (Dynarr_length(data->db->runes) < Dynarr_largest(data->db->runes)) {
		crb =
		    Dynarr_atp(data->db->runes, Dynarr_length(data->db->runes));
		local = 0;
	} else {
		crb = &rb;
		local = 1;
					 ...
					 int char_tab_width)
{
	...
	if (data->bi_bufpos)
		rb.bufpos =
		    bytind_to_bufpos(XBUFFER
				     (WINDOW_BUFFER(XWINDOW(data->window))),
				     data->bi_bufpos);
	else
...
static prop_block_dynarr *add_disp_table_entry_runes_1(pos_data * data,
						       Lisp_Object entry)
{
		...
		if (EQ(XCAR(entry), Qformat)
			...
			while (src < end) {
				...
				if (c != '%' || src == end) {
					...
					switch (c) {
						/*case 'x':
						   dst += long_to_string_base
						   ((char *)dst, data->ch, 16);
						   break; */
					case '%':
						dst += set_charptr_emchar(
							dst, '%');
						break;
						/* #### unimplemented */
					default:
			...
			prop =
			    add_bufbyte_string_runes(data, result, dst - result,
						     0, 0);
		}
	}
	...
	if (VECTORP(entry)) {
		...
		for (elt = 0; elt < len; elt++) {
			if (NILP(vector_data(de)[elt]))
				...
				prop =
				    add_disp_table_entry_runes_1(data,
								 vector_data(de)
								 [elt]);
						...
						pos_data * data)
{
	...
	for (elt = 0; elt < Dynarr_length(*prop); elt++) {
		...
		case PROP_CHAR:
			...
			data->bi_cursor_bufpos =
			    pb->data.p_char.bi_cursor_bufpos;
		...
		case PROP_MINIBUF_PROMPT: {
			...
			while (pb->data.p_string.len > 0) {
				data->ch =
					charptr_emchar(pb->data.p_string.
						       str);
				add_failed = add_emchar_rune(data);
				...
				if (add_failed) {
					...
					/* Complicated equivalent of ptr++,
					   len-- */
					Bufbyte *oldpos =
						pb->data.p_string.str;
					INC_CHARPTR(pb->data.p_string.
						    str);
					pb->data.p_string.len -=
						pb->data.p_string.str -
						oldpos;
				}
			}
			...
			data->findex = old_findex;
			/* ##### FIXME FIXME FIXME -- Upon successful return
			   from this function, data->bi_bufpos is automatically
			   incremented.  However, we don't want that to happen
			   if we were adding the minibuffer prompt. */
			{
				struct buffer *buf =
					XBUFFER(WINDOW_BUFFER
						(XWINDOW(data->window)));
				/* #### Chuck fix this shit or I'm gonna
				   scream! */
				if (bi_old_bufpos > BI_BUF_BEGV(buf)) {
					...
					/* #### is this correct?
					 * Does anyone know?
					   Does anyone care? Is this a cheesy
					   hack or what? */
					data->bi_bufpos =
						BI_BUF_BEGV(buf) - 1;
				}
			...
			break;
		case PROP_BLANK: {
			/* #### I think it's unnecessary and misleading to
			   preserve the blank_width, as it implies that the
			   value carries over from one rune to the next,
			   which is wrong. */
			...
			if (data->pixpos + data->blank_width >
			    data->max_pixpos)
				data->blank_width =
					data->max_pixpos - data->pixpos;

			/* We pass a bogus value of char_tab_width.  It
			   shouldn't matter because unless something is really
			   screwed up this call won't cause that arg to be
			   used. */
			add_failed = add_blank_rune(
				data, XWINDOW(data->window), 0);

			/* This can happen in the case where we have a tab which
			   is wider than the window. */
			if (data->blank_width != pb->data.p_blank.width) {
				pb->data.p_blank.width -=
					data->blank_width;
		...
		if (glyph_contrib_p(gb->glyph, data->window)) {
			...
			if (NILP(baseline)) {
				...
				data->max_pixmap_height =
				    max(data->max_pixmap_height, height);
			}

			/* A string so determine contribution normally. */
			else if (EQ(baseline, Qt)) {
				data->new_ascent =
				    max(data->new_ascent, ascent);
				data->new_descent =
				    max(data->new_descent, descent);
			}
			...
			/* A pixmap with an explicitly set baseline.  We determine the
			   contribution here. */
				...
				data->new_ascent =
				    max(data->new_ascent, pix_ascent);
				data->new_descent =
				    max(data->new_descent, pix_descent);
				data->max_pixmap_height =
				    max(data->max_pixmap_height, height);
...
static prop_block_dynarr *add_glyph_runes(pos_data * data)
{
	...
	int elt;
	glyph_block_dynarr *glyph_arr = (data->ef->glyphs);
	prop_block_dynarr *prop;

	for (elt = 0; elt < Dynarr_length(glyph_arr); elt++) {
		struct glyph_block *gb = Dynarr_atp(glyph_arr, elt);
		prop = add_glyph_rune(data, gb,
				      gb->active ? END_GLYPHS : BEGIN_GLYPHS,
				      0, 0);
...
create_text_block(struct window *w, struct display_line *dl,
		  Bytind bi_start_pos, prop_block_dynarr ** prop, int type)
{
	...
	while (data.pixpos <= data.max_pixpos
		...
		/* If selective display was an integer and we aren't working on
		   ...
		   supposed to display. */
		if (selective > 0 && (data.bi_bufpos == BI_BUF_BEGV(b)
				      || BUF_FETCH_CHAR(b,
							prev_bytind(b,
								    data.
								    bi_bufpos))
			...
			while (bi_spaces_at_point(b, data.bi_bufpos) >=
			       selective) {
				data.bi_bufpos =
				    bi_find_next_newline_no_quit(b,
								 data.bi_bufpos,
								 1);
				if (data.bi_bufpos >= BI_BUF_ZV(b)) {
		...
		if (initial
		    || (!no_more_frags && data.bi_bufpos == data.ef->end)) {
			...
			get_display_tables(w, data.findex, &face_dt,
					   &window_dt);
		...
		/* If there are glyphs, add them to the line.  These are
		   the end glyphs for the previous run of text.  We add
		   them here rather than doing them at the end of handling
		   the previous run so that glyphs at the beginning and
		   end of a line are handled correctly. */
		if (Dynarr_length (data.ef->glyphs) > 0) {
		...
		/* If the current position is covered by an invisible
		   ...
		   ellipses).  */
			...
			if (data.ef->invisible_ellipses) {
				...
				data.ef->invisible_ellipses_already_displayed =
				    1;
		...
		/* If at end-of-buffer, we've already processed begin and
		   ...
		   so we're done. */
			...
			if (!NILP(entry) && !EQ(entry, make_char(data.ch))) {
				*prop =
				    add_disp_table_entry_runes(&data, entry);
			   ...
			   to the line and end this loop. */
				...
				if (selective > 0
					...
					/* We need to set data.bi_bufpos to the start of the
					   ...
					   correctly. */
					INC_BYTIND(b, data.bi_bufpos);
					while (bi_spaces_at_point
					       (b,
						data.bi_bufpos) >= selective) {
						data.bi_bufpos =
			   ...
			   line is done. */
				...
				bi_next_bufpos =
				    bi_find_next_newline_no_quit(b,
								 ...
								 1);
				if (BI_BUF_FETCH_CHAR
				    (b, prev_bytind(b, bi_next_bufpos))
				    == '\n')
			...
			/* If the current character is a tab, determine the next tab
			   ...
			   current pixel position to that starting position. */
				...
				char_tab_width =
				    (next_tab_start -
				     tab_start_pixpos) / space_width(w);

				*prop =
				    add_blank_rune(&data, w, char_tab_width);
...

	{
		unsigned short ascent =
		    (unsigned short)XINT(w->minimum_line_ascent);
	...
	}
	{
		unsigned short descent =
		    (unsigned short)XINT(w->minimum_line_descent);
...
static int create_overlay_glyph_block(struct window *w, struct display_line *dl)
{
	struct frame *f = XFRAME(w->frame);
	struct device *d = XDEVICE(f->device);
	pos_data data;
	...
	if (STRINGP(Voverlay_arrow_string)) {
		add_bufbyte_string_runes
		    (&data,
		     ...
		     XSTRING_LENGTH(Voverlay_arrow_string), 1, 0);
		 ...
		 Lisp_Object window)
{
	glyph_block_dynarr *gbd = (side == LEFT_GLYPHS
				   ? dl->left_glyphs : dl->right_glyphs);
	int elt, end;
	int reverse;
	...
	while (count && ((!reverse && elt < end) || (reverse && elt >= end))) {
		...
		if (gb->active &&
		    ((side == LEFT_GLYPHS &&
		     ...
		     || (side == RIGHT_GLYPHS &&
			 extent_end_glyph_layout(XEXTENT(gb->extent)) ==
			 layout))) {
			...
			int overlay_width)
{
	...
	if (out_cnt)
		end_xpos =
		    add_margin_runes(dl, odb, dl->bounds.left_out, out_cnt,
	...
	if (!in_out_cnt && white_out_cnt && (white_out_start - end_xpos)) {
		add_margin_blank(dl, odb, w, end_xpos,
				 white_out_start - end_xpos, LEFT_GLYPHS);
	}
	...
	if (white_out_cnt) {
		end_xpos =
		    add_margin_runes(dl, odb, white_out_start, white_out_cnt,
	...
	if (idb && (white_in_start - end_xpos > 0)) {
		add_margin_blank(dl, idb, w, end_xpos,
				 white_in_start - end_xpos, LEFT_GLYPHS);
	}
...
static void create_right_glyph_block(struct window *w, struct display_line *dl)
{
	...
	while (elt < Dynarr_length(dl->right_glyphs)) {
		...
		if (extent_end_glyph_layout(XEXTENT(gb->extent)) ==
		    GL_WHITESPACE) {
			int width = glyph_width(gb->glyph, window);
	   ...
	   determine the needed info. */
	{
		...
		while (elt < Dynarr_length(dl->right_glyphs)) {
			struct glyph_block *gb =
			    Dynarr_atp(dl->right_glyphs, elt);
	...
	if (idb && (dl->bounds.right_in - end_xpos)) {
		add_margin_blank(dl, idb, w, end_xpos,
				 dl->bounds.right_in - end_xpos, RIGHT_GLYPHS);
	}

	/* Next add any whitespace glyphs in the outside margin. */
	if (white_out_cnt) {
		end_xpos =
		    add_margin_runes(dl, odb, dl->bounds.right_in,
				     white_out_cnt, GL_WHITESPACE, RIGHT_GLYPHS,
				     window);
	} else
			     ...
			     int min_pixpos, int max_pixpos, int type)
{
	...
	if (!NILP(result_str)) {
		...
		for (elt = 0;
		     elt < Dynarr_length(formatted_string_extent_dynarr);
		     elt++) {
			Lisp_Object extent = Qnil;
			Lisp_Object child;

			XSETEXTENT(extent,
				   Dynarr_at(formatted_string_extent_dynarr,
					     elt));
			child =
			    Fgethash(extent, buf->modeline_extent_table, Qnil);
			if (NILP(child)) {
				child = Fmake_extent(Qnil, Qnil, result_str);
				Fputhash(extent, child,
					 buf->modeline_extent_table);
			}
			Fset_extent_parent(child, extent);
			set_extent_endpoints
			    (XEXTENT(child),
			     Dynarr_at(formatted_string_extent_start_dynarr,
				       elt),
...
generate_modeline(struct window *w, struct display_line *dl, int type)
{
	...
	generate_formatted_string_db(b->modeline_format,
				     ...
				     MODELINE_INDEX, min_pixpos, max_pixpos,
				     type);
	...
	if (STRINGP(elt)) {
		...
		while ((pos < max_pos || max_pos == -1) && *this) {
			...
			if (this != last) {
				...
				if (size <= *offset)
					*offset -= size;
					...
					Charcount tmp_max =
					    (max_pos ==
					     -1 ? pos + size -
					     *offset : min(pos + size - *offset,
							   max_pos));
					const Bufbyte *tmp_last =
					    charptr_n_addr(last, *offset);

					pos =
					    add_string_to_fstring_db_runes(data,
									   tmp_last,
									   pos,
									   pos,
									   tmp_max);
				...
				if (*this == 'M') {
					pos =
					    generate_fstring_runes(w, data, pos,
								   spec_width,
								   max_pos,
								   Vglobal_mode_string,
								   depth,
		...
		if (SYMBOLP(car)) {
			...
			while (CONSP(elt) && --limit > 0
				...
				pos =
				    generate_fstring_runes(w, data, pos, pos,
							   max_pos, XCAR(elt),
							   depth, max_pixsize,
							   findex, type, offset,
							   cur_ext);
			...
			if (EXTENT_LIVE_P(ext)) {
				...
				Dynarr_add(formatted_string_extent_start_dynarr,
					   start);
				Dynarr_add(formatted_string_extent_end_dynarr,
					   data->bytepos);
			}
		}
	} else if (GLYPHP(elt)) {
		...
		if (*offset > 0)
			*offset -= 1;
			...
			pos =
			    add_glyph_to_fstring_db_runes(data, elt, pos, pos,
							  max_pos, cur_ext);
	} else {
	      invalid:
		{
			...
			if (size <= *offset)
				*offset -= size;
				...
				const Bufbyte *tmp_str =
				    charptr_n_addr((const Bufbyte *)str,
						   *offset);
...
static void regenerate_modeline(struct window *w)
{
...
int real_current_modeline_height(struct window *w)
{
	...
	Fset_marker(w->pointm[CMOTION_DISP], w->pointm[CURRENT_DISP],
		    w->buffer);

	if (ensure_modeline_generated(w, CMOTION_DISP)) {
		display_line_dynarr *dla =
		    window_display_lines(w, CMOTION_DISP);
...
create_string_text_block(struct window *w, Lisp_Object disp_string,
			 struct display_line *dl,
			 Bufpos start_pos,
			 prop_block_dynarr ** prop, face_index default_face)
{
	struct frame *f = XFRAME(w->frame);
	...
	while (data.pixpos <= data.max_pixpos) {
		...
		/* If there are glyphs, add them to the line.  We add the end
		   glyphs for the previous run of text here rather than doing
		   them at the end of handling the previous run so that glyphs
		   at the beginning and end of a line are handled correctly. */
		if (Dynarr_length(data.ef->glyphs) > 0) {
		...
		/* Otherwise we have a buffer line which cannot fit on one display
		   line. */
			...
			if (truncate_win) {
				...
				/* Now find the start of the next line. */
				bi_pos =
				    bi_find_next_emchar_in_string(s, '\n',
								  data.
								  bi_bufpos, 1);
	...
	if (data.bi_bufpos > bi_string_zv)
		dl->end_bufpos =
		    buffer_or_string_bytind_to_bufpos(disp_string,
						      bi_string_zv);
	else
		dl->end_bufpos =
		    buffer_or_string_bytind_to_bufpos(disp_string,
	...
	if (truncate_win)
		data.dl->num_chars =
		    string_column_at_point(s, dl->end_bufpos,
			  ...
			  Bufpos start_pos, face_index default_face)
{
	...
	while (ypos < yend) {
		...
		dlp->offset = 0;
		next_pos =
		    generate_string_display_line(w, disp_string, dlp, start_pos,
						 &prop, default_face);
...
regenerate_window(struct window *w, Bufpos start_pos, Bufpos point, int type)
{
	...
	while (ypos < yend || force) {
		...
		dlp->offset = 0;
		start_pos =
		    generate_display_line(w, dlp, 1, start_pos, &prop, type);
...
static int
regenerate_window_incrementally(struct window *w, Bufpos startp, Bufpos pointm)
{
	struct buffer *b = XBUFFER(w->buffer);
	display_line_dynarr *cdla = window_display_lines(w, CURRENT_DISP);
	display_line_dynarr *ddla = window_display_lines(w, DESIRED_DISP);
	...
	extent_end_unchanged = (BUF_EXTENT_END_UNCHANGED(b) == -1
				? -1 : BUF_Z(b) - BUF_EXTENT_END_UNCHANGED(b));
		   ...
		   so we will actually end up being safe. */
	{
		...
		if (end_unchanged >= ddl->bufpos
		    && end_unchanged <= ddl->end_bufpos) {
			w->last_modified[DESIRED_DISP] =
			    make_int(BUF_MODIFF(b));
			w->last_facechange[DESIRED_DISP] =
			    make_int(BUF_FACECHANGE(b));
			Fset_marker(w->last_start[DESIRED_DISP],
				    make_int(startp), w->buffer);
			Fset_marker(w->last_point[DESIRED_DISP],
				    make_int(pointm), w->buffer);
			...
			if (extent_end_unchanged <= extent_beg_unchanged)
				extent_beg_unchanged = extent_end_unchanged =
				    -1;
			...
			if (extent_beg_unchanged != -1
				...
				return regenerate_window_extents_only_changed(w,
									      startp,
									      pointm,
									      extent_beg_unchanged,
									      extent_end_unchanged);
...
static void redisplay_window(Lisp_Object window, int skip_selected)
{
	...
	if (echo_active) {
		old_pointm = selected_globally ? BUF_PT(b)
	...
	if (XINT(w->last_modified[CURRENT_DISP]) >= BUF_MODIFF(b)
	    ...
	    && !f->frame_changed && !truncation_changed
		...
		if (EQ(Fmarker_buffer(w->last_point[CURRENT_DISP]), w->buffer)
		    ...
		    && !f->point_changed && !f->windows_structure_changed) {
			/* If not, we're done. */
			if (f->modeline_changed)
				regenerate_modeline(w);
			...
			if (!w->windows_changed
				...
				if (point_visible(w, pointm, CURRENT_DISP)
				    ...
				    && w->last_point_y[CURRENT_DISP] != -1) {
					if (redisplay_move_cursor
					    (w, pointm, FRAME_TTY_P(f))) {
	...
	if (scroll_step > 0) {
		...
		for (; scrolled >= 0; scrolled -= scroll_step) {
			startp = vmotion(w, startp,
					 (pointm <
					  startp) ? -scroll_step : scroll_step,
					 0);
			regenerate_window(w, startp, pointm, DESIRED_DISP);
	...
	if (f->frame_changed) {
		display_line_dynarr *cla =
		    window_display_lines(w, CURRENT_DISP);
	...
	if (echo_active) {
		w->buffer = old_buffer;
		Fset_marker(w->pointm[DESIRED_DISP], make_int(old_pointm),
			    old_buffer);
		Fset_marker(w->start[DESIRED_DISP], make_int(old_startp),
			    old_buffer);
	}
	...
	if (!skip_output) {
		...
		Bufpos end = (w->window_end_pos[DESIRED_DISP] == -1 ? BUF_ZV(b)
...
static int reset_buffer_changes_mapfun(struct window *w, void *ignored_closure)
{
	buffer_reset_changes(XBUFFER(w->buffer));
	return 0;
}

static void reset_buffer_changes(void)
{
...
static int call_redisplay_end_triggers(struct window *w, void *closure)
{
...
static int redisplay_device(struct device *d, int automatic)
{
...
static Lisp_Object restore_profiling_redisplay_flag(Lisp_Object val)
{
...
static void redisplay_without_hooks(void)
{
...
static char *window_line_number(struct window *w, int type)
{
...
static void decode_mode_spec(struct window *w, Emchar spec, int type)
{
	...
	case 'C': {
		...
		if (NILP(codesys) || SYMBOLP(codesys)
		    || CODING_SYSTEMP(codesys)) {
	...
	case 'n':
		...
		/* print %, * or hyphen, if buffer is read-only, modified or
		   neither */
	case '*':
	...
	case 't':
		...
		/* print percent of buffer above top of window, or Top, Bot or
		   All */
	case 'p': {
		...
		if (w->window_end_pos[type] <= BUF_Z(b) - BUF_ZV(b)) {
			...
			/* We can't normally display a 3-digit number, so get us
			   a 2-digit number that is close. */
			if (percent == 100)
				percent = 99;
	...
	if (STRINGP(obj)) {
		Dynarr_add_many(mode_spec_bufbyte_string,
				XSTRING_DATA(obj), XSTRING_LENGTH(obj));
	} else if (str) {
		Dynarr_add_many(mode_spec_bufbyte_string,
				(const Bufbyte*)str, strlen(str));
	}
...
static void free_display_line(struct display_line *dl)
{
	...
	if (dl->display_blocks) {
		for (block = 0; block < Dynarr_largest(dl->display_blocks);
		     block++) {
			struct display_block *db =
...

static void mark_glyph_block_dynarr(glyph_block_dynarr * gba)
{
	if (gba) {
...
static void mark_window_mirror(struct window_mirror *mir)
{
...
static void update_internal_cache_list(struct window *w, int type)
{
	int line;
	display_line_dynarr *dla = window_display_lines(w, type);
...
static void validate_line_start_cache(struct window *w)
{
	struct buffer *b = XBUFFER(w->buffer);
	struct frame *f = XFRAME(w->frame);
	...
	if (!w->line_cache_validation_override) {
		...
		if (XINT(w->line_cache_last_updated) < BUF_MODIFF(b)
		    || f->faces_changed || f->clip_changed) {
			Dynarr_reset(w->line_start_cache);
		}
	}
}
...
int point_in_line_start_cache(struct window *w, Bufpos point, int min_past)
{
	...
	if (!Dynarr_length(cache) || line_start_cache_start(w) > point
		...
		if (!Dynarr_length(cache)) {
			...
			Bufpos to =
			    find_next_newline_no_quit(b, from, win_char_height);
		...
		while (line_start_cache_start(w) > point
			...
			from =
			    find_next_newline_no_quit(b, from,
						      -win_char_height);
		...
		if (line_start_cache_start(w) > point) {
			...
			if (from >= BUF_ZV(b)) {
				to = find_next_newline_no_quit(b, from,
							       -win_char_height);
				from = to;
				to = BUF_ZV(b);
			} else
	...
	while (1) {
		...
		if (point >= start && point <= end) {
			if (pos < min_past
			    && line_start_cache_start(w) > BUF_BEGV(b)) {
				Bufpos from =
				    find_next_newline_no_quit(b,
...
static Bufpos
start_end_of_last_line(struct window *w, Bufpos startp, int end, int may_error)
{
	struct buffer *b = XBUFFER(w->buffer);
	line_start_cache_dynarr *cache = w->line_start_cache;
	int pixpos = 0;
	...
	while (1) {
		...
		if (start_elt == Dynarr_length(cache)) {
			...
			start_elt =
			    point_in_line_start_cache(w, cur_start, -1) + 1;
		}
	}
}
...
static Bufpos end_of_last_line_may_error(struct window *w, Bufpos startp)
{
...
Bufpos start_with_line_at_pixpos(struct window * w, Bufpos point, int pixpos)
{
	...
	while (1) {
		...
		while (cur_elt < 0) {
			...
			from =
			    find_next_newline_no_quit(b, cur_pos,
						      -win_char_height);
			to = line_start_cache_end(w);
			update_line_start_cache(w, from, to, point, 0);
...
start_with_point_on_display_line(struct window *w, Bufpos point, int line)
{
	...
	if (line >= 0) {
		...
		if (scroll_on_clipped_lines)
			pixpos -=
			    Dynarr_atp(w->line_start_cache, cur_elt)->height;
		else
			pixpos -= 1;
		...
		if (cur_elt + new_line >= Dynarr_length(w->line_start_cache)) {
			...
			int adjustment =
			    (cur_elt + new_line) -
			    Dynarr_length(w->line_start_cache) + 1;
			Lisp_Object window;
			int defheight;
			...
			Bufpos point, int no_regen)
{
	...
	if (no_regen) {
		...
		start = Dynarr_atp(internal_cache, 0)->start;
		    ...
		    Dynarr_atp(internal_cache,
			       Dynarr_length(internal_cache) - 1)->end;
		...
		if (start < low_bound) {
			...
			while (ic_elt >= 0) {
				if (Dynarr_atp(internal_cache, ic_elt)->start <
				    low_bound)
					break;
				else
					ic_elt--;
			...
			if (!(ic_elt >= 0)) {
				Dynarr_reset(cache);
				Dynarr_add_many(cache,
						Dynarr_atp(internal_cache, 0),
						Dynarr_length(internal_cache));
				w->line_cache_validation_override--;
				return;
			}

			Dynarr_insert_many_at_start(cache,
						    Dynarr_atp(internal_cache,
							       0), ic_elt + 1);
		}
		...
		if (end > high_bound) {
			...
			Dynarr_add_many(cache,
					Dynarr_atp(internal_cache, ic_elt),
	...
	if (!Dynarr_length(cache) || from < low_bound) {
		...
		while (startp < old_lb || low_bound == -1) {
			...
			if (low_bound != -1) {
				while (ic_elt >= 0) {
					if (Dynarr_atp(internal_cache, ic_elt)->
					    start < old_lb)
						break;
					else
						ic_elt--;
			...
			new_startp =
			    Dynarr_atp(internal_cache, ic_elt)->end + 1;
	...
	if (to > high_bound) {
		Bufpos startp =
		    Dynarr_atp(cache, Dynarr_length(cache) - 1)->end + 1;
		...
		do {
			...
			Dynarr_add_many(cache, Dynarr_atp(internal_cache, 0),
					Dynarr_length(internal_cache));
			high_bound =
			    Dynarr_atp(cache, Dynarr_length(cache) - 1)->end;
			startp = high_bound + 1;
		}
		while (to > high_bound);
	}
			   ...
			   int *pix_x, int *pix_y)
{
	...
	if (char_y < num_disp_lines) {
		...
		struct display_block *db =
		    get_display_block_from_line(dl, TEXT);

		*pix_y = (dl->ypos - dl->ascent +
			  ((unsigned int)(dl->ascent + dl->descent -
					  dl->clip) >> 1));
		...
		if (Dynarr_length(dla)) {
			struct display_line *dl =
			    Dynarr_atp(dla, Dynarr_length(dla) - 1);
			   ...
			   Lisp_Object * obj1, Lisp_Object * obj2)
{
	...
	if (!device_check_failed
	    ...
	    && cache->low_y_coord <= y_coord && cache->high_y_coord > y_coord) {
		*col = cache->col;
		*row = cache->row;
		*obj_x = cache->obj_x;
		*obj_y = cache->obj_y;
	...
	*w = find_window_by_pixel_pos(x_coord, y_coord, window);
	...
	for (*row = 0; *row < Dynarr_length(dla); (*row)++) {
		...
		if ((int)(dl->ypos - dl->ascent) <= y_coord
			...
			for (*col = 0; *col <= Dynarr_length(db->runes);
			     (*col)++) {
				int past_end =
				    (*col == Dynarr_length(db->runes));
				...
				if (past_end ||
				    (rb->xpos <= x_coord
				     && x_coord < rb->xpos + rb->width)) {
					if (past_end) {
						(*col)--;
						rb = Dynarr_atp(db->runes,
								*col);
					}
...
static Lisp_Object restore_disable_preemption_value(Lisp_Object value)
{
...
{
	if (WINDOWP(locale)) {
		MARK_FRAME_GLYPHS_CHANGED(XFRAME
					  (WINDOW_FRAME(XWINDOW(locale))));
				   ...
				   struct overhead_stats *ovstats)
{
	...
	for (i = 0; i < Dynarr_largest(dyn); i++)
		total +=
		    compute_rune_dynarr_usage(Dynarr_at(dyn, i).runes, ovstats);
				  ...
				  struct overhead_stats *ovstats)
{
	...
	for (i = 0; i < Dynarr_largest(dyn); i++) {
		...
		total +=
		    compute_display_block_dynarr_usage(dl->display_blocks,
						       ovstats);
		total +=
		    compute_glyph_block_dynarr_usage(dl->left_glyphs, ovstats);
		total +=
		    compute_glyph_block_dynarr_usage(dl->right_glyphs, ovstats);
	}
...
void init_redisplay(void)
{
...
#endif
	{
		...
		if (!formatted_string_extent_start_dynarr)
			formatted_string_extent_start_dynarr =
			    Dynarr_new(Bytecount);
	...
	if (!isatty(0)) {
		stderr_out("SXEmacs: standard input is not a tty\n");
		exit(1);
	}
	...
	stderr_out("SXEmacs: Cannot open display.\n\
...
*Scroll up to this many lines, to bring point back on screen.
	...
	DEFVAR_BOOL_MAGIC("truncate-partial-width-windows", &truncate_partial_width_windows	/*
...
This is a specifier; use `set-specifier' to change it.
	...
	set_specifier_caching(Vtext_cursor_visible_p,
			      ...
			      text_cursor_visible_p_changed, 0, 0, 0);
