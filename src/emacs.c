...
-- A time line for Lucid Emacs/SXEmacs is
...
version 19.11 (first SXEmacs) released September 13, 1994.
...
version 20.3 (the first stable version of SXEmacs 20.x) released
  November 30, 1997.
...
#if defined (WIN32_NATIVE) && defined (DEBUG_SXEMACS)
...
static SIGTYPE debug_memory_error(int signum)
{
...
static void ensure_no_quitting_from_now_on(void)
{
...
static Lisp_Object make_arg_list_1(int argc, Extbyte ** argv, int skip_args)
{
	...
	for (i = argc - 1; i >= 0; i--) {
		if (i == 0 || i > skip_args) {
			...
			if (i == 0) {
				...
				GetModuleFileName(NULL, full_exe_path,
						  MAX_PATH);
				fullpath =
				    build_ext_string(full_exe_path,
						     Qmswindows_tstr);
...
#if defined(HAVE_SHLIB)
				{
					...
					LISP_STRING_TO_EXTERNAL(fullpath,
								fullpathext,
...
Lisp_Object make_arg_list(int argc, Extbyte ** argv)
{
...
void make_argc_argv(Lisp_Object argv_list, int *argc, Extbyte *** argv)
{
	Lisp_Object next;
...
# endif
	...
	/* This is Unix only.  MS Windows NT has a library call that does
	   ...
	   called for GNU dld in temacs and sxemacs.  */
	{
...
#if 0				/* CANNOT_DUMP - this can never be right in SXEmacs --andyp */
      ...
      (arg))
{
	...
	/* If we displayed a message on the console, then we must allow the
	   ...
	   hurt, and we can't necessarily check arg; e.g. sxemacs --help
	...
	if (mswindows_message_outputted && !noninteractive)
		Fmswindows_message_box(build_string
				       ("Messages outputted.  SXEmacs is exiting."),
				       Qnil, Qnil);
#endif
...
*Directory of site-specific Lisp files that come with SXEmacs.
							*/ );
