...
static ElfW(Addr) round_up(ElfW(Addr) x, ElfW(Addr) y)
{
	int rem = x % y;
	if (rem == 0)
		return x;
       ...
       uintptr_t bss_start, uintptr_t entry_address)
{
	...
	old_base =
	    (caddr_t) mmap(0, stat_buf.st_size, PROT_READ, MAP_SHARED, old_file,
			   0);
	...
	old_program_h =
	    (ElfW(Phdr) *) ((byte *) old_base + old_file_h->e_phoff);
	old_section_h =
	    (ElfW(Shdr) *) ((byte *) old_base + old_file_h->e_shoff);
	old_section_names =
	...
	for (old_bss_index = 1; old_bss_index < (int)old_file_h->e_shnum;
		...
		fprintf(stderr, "Looking for .bss - found %s\n",
			old_section_names +
			OLD_SECTION_H(old_bss_index).sh_name);
#endif
		if (!strcmp
	...
	for (old_sbss_index = 1; old_sbss_index < (int)old_file_h->e_shnum;
		...
		fprintf(stderr, "Looking for .sbss - found %s\n",
			old_section_names +
			OLD_SECTION_H(old_sbss_index).sh_name);
#endif
		if (!strcmp
	...
	for (old_mdebug_index = 1; old_mdebug_index < (int)old_file_h->e_shnum;
		...
		fprintf(stderr, "Looking for .mdebug - found %s\n",
			old_section_names +
			OLD_SECTION_H(old_mdebug_index).sh_name);
#endif
		if (!strcmp
		    (old_section_names +
		     OLD_SECTION_H(old_mdebug_index).sh_name, ".mdebug"))
			break;
	}
	...
	for (old_data_index = 1; old_data_index < (int)old_file_h->e_shnum;
		...
		fprintf(stderr, "Looking for .data - found %s\n",
			old_section_names +
			OLD_SECTION_H(old_data_index).sh_name);
#endif
		if (!strcmp
	...
	new_file_size =
	    stat_buf.st_size + old_file_h->e_shentsize + new_data2_size;
	...
	new_program_h =
	    (ElfW(Phdr) *) ((byte *) new_base + old_file_h->e_phoff);
	...
	for (n = new_file_h->e_phnum - 1; n >= 0; n--) {
		if (NEW_PROGRAM_H(n).p_vaddr
			...
			NEW_PROGRAM_H(n).p_vaddr +=
			    new_data2_size - old_bss_size;
	...
	for (n = 1, nn = 1; n < (int)old_file_h->e_shnum; n++, nn++) {
		...
		if (n == old_bss_index
			...
			NEW_SECTION_H(nn).sh_offset =
			    NEW_SECTION_H(new_data2_index).sh_offset +
			    new_data2_size;
			NEW_SECTION_H(nn).sh_addr =
			    NEW_SECTION_H(new_data2_index).sh_addr +
			    new_data2_size;
			/* Let the new bss section address alignment be the same as the
			   section address alignment followed the old bss section, so
			   this section will be placed in exactly the same place. */
			NEW_SECTION_H(nn).sh_addralign =
			    OLD_SECTION_H(nn).sh_addralign;
		...
		if (!strcmp
		    (old_section_names + NEW_SECTION_H(n).sh_name, ".data")
		...
		if (strcmp
		    ...
		    == 0) {
			pHDRR symhdr =
			    (pHDRR) (NEW_SECTION_H(nn).sh_offset + new_base);
		...
		if (NEW_SECTION_H(nn).sh_type == SHT_MIPS_DEBUG
			...
			HDRR *phdr =
			    (HDRR *) (NEW_SECTION_H(nn).sh_offset + new_base);
...
#define MDEBUGADJUST(__ct,__fileaddr)		\
  if (n_phdrr->__ct > 0)			\
    {						\
      n_phdrr->__fileaddr += movement;		\
		...
		if (NEW_SECTION_H(nn).sh_type == SHT_SYMTAB
			...
			ElfW(Sym) * sym =
			    (ElfW(Sym) *) (NEW_SECTION_H(nn).sh_offset +
					   new_base);
			for (; num--; sym++) {
	...
	for (n = new_file_h->e_shnum - 1; n; n--) {
		...
		symendp =
		    (ElfW(Sym) *) ((byte *) symp + NEW_SECTION_H(n).sh_size);

		for (; symp < symendp; symp++)
			if (strcmp((char *)(symnames + symp->st_name), "_end")
			    == 0
			    || strcmp((char *)(symnames + symp->st_name),
				      "end") == 0
			    || strcmp((char *)(symnames + symp->st_name),
				      "_edata") == 0
			    || strcmp((char *)(symnames + symp->st_name),
				      "edata") == 0)
				memcpy(&symp->st_value, &new_bss_addr,
				       sizeof(new_bss_addr));
	}
	...
	for (n = new_file_h->e_shnum - 1; n; n--) {
		...
		case SHT_RELA:
			...
			nn = section.sh_info;
			if (!strcmp
			    (old_section_names + NEW_SECTION_H(nn).sh_name,
			     ".data")
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".sdata")
#ifdef __sgi
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".lit4")
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".lit8")
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".got")
#endif
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".sdata1")
			    ||
			    !strcmp((old_section_names +
				     NEW_SECTION_H(nn).sh_name), ".data1")) {
				ElfW(Addr) offset =
				    NEW_SECTION_H(nn).sh_addr -
				    NEW_SECTION_H(nn).sh_offset;
				caddr_t reloc =
				    old_base + section.sh_offset, end;
				for (end = reloc + section.sh_size; reloc < end;
				     reloc += section.sh_entsize) {
					ElfW(Addr) addr =
					    ((ElfW(Rel) *) reloc)->r_offset -
					    offset;
#ifdef __alpha__
					/* The Alpha ELF binutils currently have a bug that
					   sometimes results in relocs that contain all
					   zeroes.  Work around this for now...  */
					if (((ElfW(Rel) *) reloc)->r_offset ==
					    0)
						continue;
#endif
