      ...
      (fo, offset, type))
{
	...
	struct gcpro gcpro1;
        ...
        else if (EQ(type, Q_c_data) || (CONSP(type) && EQ(XCAR(type), Q_c_data))) {
                size_t tlen;
                if (EQ(type, Q_c_data)) {
      ...
      (fo, offset, val_type, val))
{
        ...
        {
                if (!CHARP(val))
                        signal_error(Qwrong_type_argument, list2(Qcharacterp, val));
                *(char*)ptr = XCHAR(val);
		...
		unsigned int val_ext_len;
                if (!STRINGP(val))
                        signal_error(Qwrong_type_argument, list2(Qstringp, val));
		TO_EXTERNAL_FORMAT(LISP_STRING, val, ALLOCA, (val_ext, val_ext_len),
				   Qbinary);
