...
Sixt argument ACCEPTOR is a function which will be called upon connection
 acceptance with two the accepted connection process.
...
Charcount read_process_output(Lisp_Object process)
{
	...
	case PROCESS_TYPE_SSL:
#if defined(HAVE_OPENSSL) && defined(OPENSSL_SSL)
		/* when that funky layer is not connected anymore, fallback
		 * to usual lstream_read calls
		 * it's not my fault!
		 */
		if ((SSLCONNP(p->process_type_data)) &&
		    XSSLCONN(p->process_type_data)->connected_p) {
			nbytes = SSL_read(XSSLCONN(p->process_type_data)
					  ->ssl_conn, chars, sizeof(chars));
