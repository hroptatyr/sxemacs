...
unsigned char *round_to_next(unsigned char *address, unsigned long align)
{
	unsigned long tmp;
...
static unsigned char *allocate_heap(void)
{
...
void *sbrk(unsigned long increment)
{
	...
	if (size < 0) {
		...
		new_data_region_end = (unsigned char *)
		    ((long)(new_data_region_end + syspage_mask) &
		     ~syspage_mask);
		...
		if (new_size > 0) {
			...
			if (!VirtualFree
			    (real_data_region_end, new_size, MEM_DECOMMIT))
				return NULL;
		}
...
void recreate_heap(char *executable_path)
{
	...
	if (info.State != MEM_FREE) {
		...
		wsprintf(buf,
			 ...
			 info.Type == MEM_IMAGE ? "IMAGE" : info.Type ==
			 MEM_MAPPED ? "MAPPED" : "PRIVATE");
	...
	if (!tmp) {
		...
		wsprintf(buf,
			 ...
			 "(VirtualAlloc at 0x%lx of 0x%lx failed (%d))", base,
			 size, GetLastError());
		MessageBox(NULL, buf, "XEmacs", MB_OK | MB_ICONSTOP);
		exit(1);
	}
...
void round_heap(unsigned long align)
{
