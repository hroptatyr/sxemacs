...
static int mswindows_initially_selected_for_input(struct console *con)
{
	return 1;
}
...
static int mswindows_ensure_console_allocated(void)
{
...
static void mswindows_ensure_console_buffered(void)
{
...
int mswindows_output_console_string(CONST Extbyte * str, Extcount len)
{
	...
	return WriteConsole(mswindows_console_buffer, str, len, &num_written,
			    NULL);
}
      ...
      (char_or_string))
{
	...
	if (STRINGP(char_or_string)) {
		TO_EXTERNAL_FORMAT(LISP_STRING, char_or_string,
				   C_STRING_ALLOCA, extstr, Qmswindows_tstr);
...
DEFUN("mswindows-message-box", Fmswindows_message_box, 1, 3, 0,	/*
	...
	TO_EXTERNAL_FORMAT(LISP_STRING, message_,
			   C_STRING_ALLOCA, msgout, Qmswindows_tstr);

	if (!NILP(title)) {
		CHECK_STRING(title);
		TO_EXTERNAL_FORMAT(LISP_STRING, title,
				   C_STRING_ALLOCA, titleout, Qmswindows_tstr);
	}
...
#undef FROB
...

	{
		...
		if (retval == 0)
			error
			    ("Out of memory when calling `mswindows-message-box'");
					  ...
					  Error_behavior errb)
{
	...
	if (NILP(connection)) {
		...
		if (NILP(connection)) {
			if (ERRB_EQ(errb, ERROR_ME))
				error
				    ("There is no default printer in the system");
			else
				return Qunbound;
		}
	}
