/* SXEmacs routines to deal with syntax tables; also word and list parsing.
...
static Bufpos find_defun_start(struct buffer *buf, Bufpos pos)
{
	Bufpos tem;
	...
	while (tem > BUF_BEGV(buf)) {
		...
		if (SYNTAX_FROM_CACHE(mirrortab, BUF_FETCH_CHAR(buf, tem)) ==
		    Sopen)
			break;
...
static Lisp_Object check_syntax_table(Lisp_Object obj, Lisp_Object default_)
{
	if (NILP(obj))
		obj = default_;
...
void update_syntax_cache(int pos, int count)
{
	...
	if (BUFFERP(syntax_cache.object)) {
		...
		syntax_cache.next_change =
		    XINT(Fnext_single_property_change
			 (make_int(pos > 0 ? pos : 1), Qsyntax_table,
			  syntax_cache.object,
			  make_int(BUF_ZV(syntax_cache.buffer))));
#else
		syntax_cache.next_change =
		...
		if (get_change_before < 0)
			get_change_before = 0;
		else if (get_change_before >
			 XSTRING_LENGTH(syntax_cache.object))
		...
		tmp_table = Qnil;
		/* Always aborts.
		 * #### Is there another sensible thing to do here? */
		assert(BUFFERP(syntax_cache.object) ||
	...
	if (EQ(Fsyntax_table_p(tmp_table), Qt)) {
		...
		/* we CANNOT cast the following, XINT goes to long int
		 * while enums are usually just unsigned ints
		 * this'll trigger a warning on 64b machines */
...
#ifdef SYNTAX_CACHE_STATISTICS
	{
		...
		if (scs_statistics.min_length == -1
		    || scs_statistics.min_length > length)
			scs_statistics.min_length = length;
		if (scs_statistics.max_length == -1
		    || scs_statistics.max_length < length)
			scs_statistics.max_length = length;
		scs_statistics.mean_length_on_miss =
		    ((misses - 1) * scs_statistics.mean_length_on_miss +
		     length) / misses;
	}
	...
	if (!
		...
		fprintf(stderr,
			"updates %d, inits %d, misses low %d, misses high %d,",
...
unsigned char syntax_code_spec[] = " .w_()'\"$\\/<>@!|";
      ...
      (character, syntax_table))
{
	...
	syntax_table =
	    check_syntax_table(syntax_table, current_buffer->syntax_table);
	mirrortab = XCHAR_TABLE(XCHAR_TABLE(syntax_table)->mirror_table);
	return
	    make_char(syntax_code_spec
		      [(int)SYNTAX(mirrortab, XCHAR(character))]);
}
...
Lisp_Object syntax_match(Lisp_Object syntax_table, Emchar ch)
{
...
Bufpos scan_words(struct buffer *buf, Bufpos from, int count)
{
		      ...
		      int comstyle)
{
		...
		/* otherwise, is this a 2-char comment end or start sequence? */
			...
			do {
				...
				if (SYNTAX_CODE_START_SECOND_P(syncode)) {
					...
					if (SYNTAX_CODES_START_P
					    (prev_syncode, syncode)) {
						code = Scomment;
						styles_match_p =
						    SYNTAX_CODES_MATCH_START_P
						    (prev_syncode, syncode,
						     mask);
						from--;
						UPDATE_SYNTAX_CACHE_BACKWARD
	...
	/* If the earliest comment starter
	   ...
	   So it does start a comment.  Skip back to it.  */
		...
		scan_sexps_forward(buf, &state,
				   find_defun_start(buf, comment_end),
...
find_end_of_comment(struct buffer *buf, Bufpos from, Bufpos stop, int comstyle)
{
	...
	while (1) {
		...
		if (from < stop
		    && SYNTAX_CODES_MATCH_END_P(syncode, next_code, mask)) {
			from++;
			UPDATE_SYNTAX_CACHE_FORWARD(from);
			break;
		}
...
>>>>>>> master
		...
		while (from < stop) {
			...
			if (code == Scomment) {
				...
				if (SYNTAX_CODES_START_P(syncode, next_syncode)) {
					...
					code = Scomment;
					comstyle =
					    SYNTAX_CODES_COMMENT_MASK_START
					    (syncode, next_syncode)
					    == SYNTAX_COMMENT_STYLE_A ? 0 : 1;
					from++;
				}
			}
			...
			if (code == Scomment) {
				Bufpos newfrom =
				    find_end_of_comment(buf, from, stop,
							comstyle);
				if (newfrom < 0) {
				...
				break;
				   ...
				   && code != Sendcomment && code != Scomment) {
				BUF_SET_PT(buf, from);
				return Qnil;
			}
			from++;
	...
	while (n < 0) {
		...
		while (from > stop) {
			...
			if (code == Sendcomment) {
				from =
				    find_start_of_comment(buf, from, stop,
							  comstyle);
				break;
			}

			else if (code != Swhitespace
				 && code != Scomment && code != Sendcomment) {
	   ...
	   int sexpflag, int noerror)
{
	...
	while (count > 0) {
		...
		while (from < stop) {
			...
			/* else, a 2-char comment start sequence? */
				...
				int next_syncode;
				UPDATE_SYNTAX_CACHE_FORWARD(from);
				next_syncode = SYNTAX_CODE_FROM_CACHE(
					mirrortab, BUF_FETCH_CHAR(buf, from));

				if (SYNTAX_CODES_START_P(syncode,
							 next_syncode)) {
					/* we have encountered a comment start
					   sequence and we are ignoring all text
					   inside comments. we must record the
					   comment style this sequence begins so
					   that later, only a comment end of the
					   same style actually ends the comment
					   section */
					code = Scomment;
					comstyle =
					    SYNTAX_CODES_COMMENT_MASK_START
			...
			case Scharquote:
				if (from == stop)
					goto lose;
				from++;
				/* treat following character as a word
				   constituent */
			case Sword:
			case Ssymbol:
				if (depth || !sexpflag)
					break;
				/* This word counts as a sexp; return at end of
				   it. */
				while (from < stop) {
					UPDATE_SYNTAX_CACHE_FORWARD(from);
					switch ((unsigned int)
				...
				UPDATE_SYNTAX_CACHE_FORWARD(from);
				{
					Bufpos newfrom =
					    find_end_of_comment(buf, from, stop,
								comstyle);
					if (newfrom < 0) {
						/* we stopped because from ==
						   stop in search forward */
						from = stop;
						if (depth == 0)
							goto done;
						goto lose;
			...
			case Sstring:
				{
					/* XEmacs change: call syntax_match on
					   character */
					Emchar ch =
	...
	while (count < 0) {
		...
		stop = BUF_BEGV(buf);
		while (from > stop) {
			/* mask for finding matching comment style */
			int comstyle = 0;
			/* used by case Sstring in switch below */
			Emchar stringterm = '\0';
			...
			if (code == Sendcomment && parse_sexp_ignore_comments) {
				/* we have found a single char end comment. we
				   must record the comment style encountered so
				   that later, we can match only the proper
				   comment begin sequence of the same style */
				comstyle =
				    SYNTAX_CODE_COMMENT_1CHAR_MASK(syncode)
				    == SYNTAX_COMMENT_STYLE_A ? 0 : 1;
			}
				...
				if (SYNTAX_CODES_END_P(prev_syncode, syncode)) {
					/* we must record the comment style
					   encountered so that later, we can
					   match only the proper comment begin
					   sequence of the same style */
					code = Sendcomment;
					comstyle =
					    SYNTAX_CODES_COMMENT_MASK_END
			...
			case Ssymbol:
				if (depth || !sexpflag)
					break;
				/* This word counts as a sexp; count object
				   finished after passing it. */
				while (from > stop) {
			...
			case Sendcomment:
				if (parse_sexp_ignore_comments)
					from =
					    find_start_of_comment(buf, from,
								  stop,
								  comstyle);
				break;
...
int char_quoted(struct buffer *buf, Bufpos pos)
{
	...
	while (pos > beg) {
		...
		code =
		    SYNTAX_FROM_CACHE(mirrortab, BUF_FETCH_CHAR(buf, pos - 1));
      ...
      (buffer))
{
	...
	while (pos > beg && !char_quoted(buf, pos - 1)
	       ...
	       &&
	       (SYNTAX_FROM_CACHE(mirrortab, c = BUF_FETCH_CHAR(buf, pos - 1))
		== Squote
		   ...
		   Lisp_Object oldstate, int commentstop)
{
	...
	while (from < end) {
		...
		if (code == Scomment) {
			state.comstyle = SYNTAX_CODE_COMMENT_1CHAR_MASK(syncode)
			    == SYNTAX_COMMENT_STYLE_A ? 0 : 1;
		...
		/* a generic comment delimiter? */
		...
		else if (from < end && SYNTAX_CODE_START_FIRST_P(syncode)) {
			int next_syncode;
			UPDATE_SYNTAX_CACHE_FORWARD(from);
			next_syncode =
			    SYNTAX_CODE_FROM_CACHE(mirrortab,
			...
			if (SYNTAX_CODES_START_P(syncode, next_syncode)) {
				code = Scomment;
				state.comstyle =
				    SYNTAX_CODES_COMMENT_MASK_START(syncode,
								    next_syncode)
				    == SYNTAX_COMMENT_STYLE_A ? 0 : 1;
				state.comstr_start = from - 1;
				from++;
		...
		case Scharquote:
			if (stopbefore)
				goto stop;	/* this arg means stop at sexp start */
		...
		case Scomment:
			...
			{
				Bufpos newfrom =
				    find_end_of_comment(buf, from, end,
							state.comstyle);
				if (newfrom < 0) {
		...
		startinstring:
			while (1) {
				...
				if (state.instring != ST_STRING_STYLE &&
				    temp_code == Sstring && c == state.instring)
					break;

				switch ((unsigned int)temp_code) {
				case Sstring_fence:
					if (state.instring == ST_STRING_STYLE)
						goto string_end;
					break;
...
>>>>>>> master
	...
	val =
	    Fcons(state.thislevelstart <
		  0 ? Qnil : make_int(state.thislevelstart), val);
	val =
	    Fcons(state.prevlevelstart <
		  0 ? Qnil : make_int(state.prevlevelstart), val);
...
static int cmst_mapfun(struct chartab_range *range, Lisp_Object val, void *arg)
{
	struct cmst_arg *closure = (struct cmst_arg *)arg;

	if (CONSP(val))
		val = XCAR(val);
	if (SYNTAX_FROM_CODE(XINT(val)) == Sinherit && closure->check_inherit) {
...
static void update_just_this_syntax_table(Lisp_Char_Table * ct)
{
...
static void define_standard_syntax(const char *p, enum syntaxcode syn)
{
	for (; *p; p++)
		Fput_char_table(make_char(*p), make_int(syn),
