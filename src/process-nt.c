...
static struct Lisp_Process *find_process_from_pid(DWORD pid)
{
...
static int
alloc_process_memory(HANDLE h_process, size_t size, process_memory * pmc)
{
	LPTHREAD_START_ROUTINE adr_ExitThread = (LPTHREAD_START_ROUTINE)
...
static void free_process_memory(process_memory * pmc)
{
	ResumeThread(pmc->h_thread);
}
		     ...
		     LPVOID data, size_t data_size)
{
	...
	if (data_size) {
		...
		if (!WriteProcessMemory
		    (h_process, remote_data, data, data_size, NULL))
			goto failure;
	} else
		remote_data = NULL;
...
static DWORD WINAPI sigkill_proc(sigkill_data * data)
{
...
static DWORD WINAPI sigint_proc(sigint_data * data)
{
...
static DWORD WINAPI sig_enable_proc(sig_enable_data * data)
{
	...
	case SIGHUP:
		{
			...
			d.adr_ExitProcess =
			    (void (WINAPI *) (UINT))GetProcAddress(h_kernel,
								   "ExitProcess");
 ...
 */
static void enable_child_signals(HANDLE h_process)
{
	HMODULE h_kernel = GetModuleHandle("kernel32");
	sig_enable_data d;

	assert(h_kernel != NULL);
	d.adr_SetConsoleCtrlHandler = (BOOL(WINAPI *) (LPVOID, BOOL))
	    GetProcAddress(h_kernel, "SetConsoleCtrlHandler");
	assert(d.adr_SetConsoleCtrlHandler);
	run_in_other_process(h_process,
			     (LPTHREAD_START_ROUTINE) sig_enable_proc, &d,
			     sizeof(d));
}
...
static BOOL CALLBACK find_child_console(HWND hwnd, long putada)
{
	DWORD thread_id;
	DWORD process_id;
	...
	if (process_id == cp->dwProcessId) {
		...
		if (strcmp(window_class, mswindows_windows9x_p()
			   ? "tty" : "ConsoleWindowClass") == 0) {
			cp->hwnd = hwnd;
			return FALSE;
		}
	}
	...
	if (signo == SIGINT) {
		if (NILP(Vmswindows_start_process_share_console) && cp
			...
			BYTE break_scan_code =
			    (BYTE) MapVirtualKey(vk_break_code, 0);
			...
			if (foreground_window) {
				...
				child_thread =
				    GetWindowThreadProcessId(cp->hwnd, NULL);
				...
				if (SetForegroundWindow(cp->hwnd)) {
					...
					keybd_event(VK_CONTROL,
						    control_scan_code, 0, 0);
					keybd_event(vk_break_code,
						    break_scan_code,
						    (vk_break_code ==
						     'C' ? 0 :
...
#if 0				/* FSF Emacs */
					DebPrint(("sys_kill.TerminateProcess returned %d " "for pid %lu\n", GetLastError(), pid));
					errno = EINVAL;
#endif
					rc = 0;
				}
...
static int send_signal(struct nt_process_data *cp, int pid, int signo)
{
	return (!mswindows_windows9x_p()
		&& send_signal_the_nt_way(cp, pid, signo))
 ...
 */
static void validate_signal_number(int signo)
{
	if (signo != SIGKILL && signo != SIGTERM
	    && signo != SIGQUIT && signo != SIGINT && signo != SIGHUP)
		invalid_argument("Signal number not supported",
				 make_int(signo));
}

/*-----------------------------------------------------------------------*/
...
static void nt_alloc_process_data(Lisp_Process * p)
{
 ...
 */
static void nt_init_process(void)
{
	/* Initialize winsock */
	WSADATA wsa_data;
...
static void signal_cannot_launch(Lisp_Object image_file, DWORD err)
{
	mswindows_set_errno(err);
	report_file_error("Error starting", image_file);
}

static void ensure_console_window_exists(void)
{
	if (mswindows_windows9x_p())
		mswindows_hide_console();
}

int compare_env(const void *strp1, const void *strp2)
{
		  ...
		  Lisp_Object program, Lisp_Object cur_dir)
{
	...
	if (xSHGetFileInfoA) {
		...
		if (p != NULL &&
			...
			image_type =
			    xSHGetFileInfoA((char *)XSTRING_DATA(program), 0,
					    NULL, 0, SHGFI_EXETYPE);
		} else {
			char progname[MAX_PATH];
			sprintf(progname, "%s.exe",
				(char *)XSTRING_DATA(program));
			image_type =
			    xSHGetFileInfoA(progname, 0, NULL, 0,
					    SHGFI_EXETYPE);
		}
		if (image_type == 0)
			signal_cannot_launch(program,
					     (GetLastError() ==
	...
	if (do_io) {
		...
		DuplicateHandle(GetCurrentProcess(), hprocout,
				GetCurrentProcess(), &hprocerr, 0, TRUE,
		...
		DuplicateHandle(GetCurrentProcess(), hmyshove,
				GetCurrentProcess(), &htmp, 0, FALSE,
				DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS);
		hmyshove = htmp;
		DuplicateHandle(GetCurrentProcess(), hmyslurp,
				GetCurrentProcess(), &htmp, 0, FALSE,
				DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS);
		hmyslurp = htmp;
	}
	   ...
	   (in win32-native.el) */
	{
		...
		LISP_STRING_TO_EXTERNAL(args_or_ret, command_line,
					Qmswindows_tstr);
...

	{
		...
		for (tem = Vprocess_environment; (CONSP(tem)
						  && STRINGP(XCAR(tem)));
		     tem = XCDR(tem))
			new_length++;
	...
	if (is_dos_app) {
		...
		if (egetenv("CMDPROXY"))
			...
			strcpy((char *)cmdname,
			       XSTRING_DATA(Vinvocation_directory));
	...
	/* Create process */
	{
		...
		err =
		    (CreateProcess
		     (NULL, command_line, NULL, NULL, TRUE, flags, proc_env,
		      (char *)XSTRING_DATA(cur_dir), &si, &pi)
		     ? 0 : GetLastError());
		...
		if (do_io) {
			...
			init_process_io_handles(p, (void *)hmyslurp,
						(void *)hmyshove, 0);
		} else {
...
static void nt_update_status_if_terminated(Lisp_Process * p)
{
	DWORD exit_code;
...
static void nt_send_process(Lisp_Object proc, struct lstream *lstream)
{
	...
	while (1) {
		...
		if (writeret < 0) {
			...
			invalid_operation
			    ("Broken pipe error sending to process; closed it",
			     p->name);
		}

		{
			...
			while (Lstream_was_blocked_p
				...
				Faccept_process_output(Qnil, Qzero,
						       make_int(wait_ms));
...
static void
nt_kill_child_process(Lisp_Object proc, int signo, int current_group, int nomsg)
{
 ...
 */
static int nt_kill_process_by_pid(int pid, int signo)
{
...
static Lisp_Object nt_canonicalize_host_name(Lisp_Object host)
{
	...
	*vinfd = (void *)s;
	...
	if (INTP(service)) {
		...
		warn_when_safe(Qstream, Qwarning,
			       ...
			       XSTRING_DATA(host), XSTRING_DATA(service));
	}
...
{
	DEFVAR_LISP("mswindows-start-process-share-console", &Vmswindows_start_process_share_console	/*
	...
	DEFVAR_LISP("mswindows-start-process-inherit-error-mode", &Vmswindows_start_process_inherit_error_mode	/*
