...
static Lisp_Object get_doc_string(Lisp_Object filepos)
{
	...
	if (fd < 0) {
		...
		if (purify_flag) {
			/* sizeof ("../lib-src/") == 12 */
			name_nonreloc =
			    (char *)alloca(12 + XSTRING_LENGTH(file) + 8);
      ...
      (filename))
{
	...
	if (!purify_flag)
		error
		    ("Snarf-documentation can only be called in an undumped Emacs");
#endif
			...
			case '[':
				{
					...
					tem =
					    Fintern(make_string(start, length),
						    Qnil);
					tem =
					    Fwhere_is_internal(tem, keymap, Qt,
							       Qnil, Qnil);
			...
			case '<':
				{
					...
					if (NILP(tem)) {
						buffer_insert_c_string(buf_,
								       "(uses keymap \"");
						buffer_insert_lisp_string(buf_,
									  Fsymbol_name
									  (name));
						buffer_insert_c_string(buf_,
