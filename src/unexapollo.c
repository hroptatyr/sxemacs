...
void unexec(target_file_name, source_file_name)
char *target_file_name, *source_file_name;
{
	...
	unsigned long data_size, old_data_section_size,
	...
	if (read(source_file, &file_header, sizeof(file_header)) !=
	    sizeof(file_header))
...

	...
	sections =
	    (struct scnhdr *)malloc(file_header.f_nscns *
	...
	if (read
	    (source_file, sections, file_header.f_nscns * sizeof(struct scnhdr))
	...
	for (rwdi_section = sections; rwdi_section != sections_limit;
	     rwdi_section++)
		if (strcmp(rwdi_section->s_name, ".rwdi") == 0) {
			...
			delta_after_rwdi =
			    delta_before_rwdi - rwdi_section->s_size;
			...
			source_file_offset_past_rwdi =
			    (rwdi_section + 1)->s_scnptr;
			break;
		}
	/* Skip over the Text Section Headers. */
	for (section = sections; (section->s_flags & STYP_TEXT) != 0;
	     section++) ;
	/*
...

	...
	for (; section != sections_limit; section++) {
		long delta =
		    (section <
		     rwdi_section ? delta_before_rwdi : delta_after_rwdi);
	...
	if (write(target_file, &file_header, sizeof(file_header)) !=
	    sizeof(file_header))
	...
	if (write
	    (target_file, sections, file_header.f_nscns * sizeof(struct scnhdr))
	    != file_header.f_nscns * sizeof(struct scnhdr))
		error("cannot write section headers");
	/* Copy the Allocated Sections. */
	for (section = sections; section != first_data_section; section++)
		if (section->s_scnptr != 0)
			CopyData(target_file, source_file,
				 LONG_ALIGN(section->s_size));
	/* Write the Expanded Data Segment. */
	if (write(target_file, first_data_section->s_vaddr, data_size) !=
	    data_size)
...

	...
	for (section = last_data_section + 1; section != rwdi_section;
	     section++)
		if (section->s_scnptr != 0)
			CopyData(target_file, source_file,
				 LONG_ALIGN(section->s_size));
	...
	for (section = first_data_section; section <= last_data_section;
	     section++)
		for (i = 0; i < section->s_nreloc; i++) {
			if (lseek(source_file, section->s_relptr
				  + i * sizeof(struct reloc) - delta_after_rwdi,
				  L_SET) == -1)
			...
			if (lseek
			    (source_file,
			     reloc_entry.r_vaddr - section->s_vaddr +
	...
	if (blocks_section != (struct scnhdr *)0) {
		...
		while (section_offset < blocks_section->s_size) {
			if (lseek(target_file,
				  blocks_section->s_scnptr + section_offset,
				  L_SET) == -1)
			...
			if (dst_record.rec_type == dst_typ_pad)
				...
				if (read
				    (target_file, comp_unit, sizeof(*comp_unit))
				    != sizeof(*comp_unit))
...
#endif
					  + DST_RECORD_HDR_SIZE, L_SET) == -1)
					error("cannot seek to section table");
				if (read
				    (target_file, &number_of_sections,
				     sizeof(number_of_sections))
				    != sizeof(number_of_sections))
					error("cannot read section table size");
				for (i = 0; i < number_of_sections; i++) {
					if (read
					    (target_file, &section_base,
					     sizeof(section_base))
					    != sizeof(section_base))
						error
						    ("cannot read section base value");
					if (section_base < first_changed_vaddr)
						...
						section_base +=
						    delta_after_rwdi;
					if (lseek
					    (target_file, -sizeof(section_base),
					     L_INCR) == -1)
						error
						    ("cannot seek to section base value");
					if (write
					    (target_file, &section_base,
					     sizeof(section_base))
					    != sizeof(section_base))
						error
						    ("cannot write section base");
				}
				section_offset += comp_unit->data_size;
			} else
				error("unexpected dst record type");
		}
	}
...
static CopyData(target_file, source_file, total_byte_count)
