...
/* what's this? what's wrong with uint32_t? */
#if 0
#if SXE_INTBITS == 32
...
static XImage *convert_EImage_to_XImage(Lisp_Object device, int width,
					...
					unsigned long **pixtbl, int *npixels)
{
	Display *dpy;
	Colormap cmap;
	Visual *vis;
	...
	union {
#if 0
/* kicked in favour of C99's uint32_t  */
		FOUR_BYTE_TYPE val;
#endif
		uint32_t val;
		char cp[4];
	...
	data = xmalloc_atomic(outimg->bytes_per_line * height);
	if (!data) {
	...
	if (vis->class == PseudoColor) {
		...
		pixcount = 32;
		*pixtbl = xnew_atomic_array(unsigned long, pixcount);
		*npixels = 0;

		/* #### should implement a sort by popularity to
		 * assure proper allocation */
		n = *npixels;
...
static void x_finalize_image_instance(Lisp_Image_Instance * p)
{
	if (!p->data)
		return;
		...
		if (0) ;
			...
			if (IMAGE_INSTANCE_SUBWINDOW_ID(p)) {
				...
				stderr_out("widget destroyed, %d left\n",
					   debug_widget_instances);
#endif
				lw_destroy_widget(IMAGE_INSTANCE_X_WIDGET_ID
						  (p));
				lw_destroy_widget(IMAGE_INSTANCE_X_CLIPWIDGET
						  (p));
				...
				for (i = 0;
					...
					if (IMAGE_INSTANCE_X_PIXMAP_SLICE(p, i)) {
						XFreePixmap(dpy,
							    ...
							    (p, i));
						IMAGE_INSTANCE_X_PIXMAP_SLICE(p,
									      i)
						    = 0;
					}
		       ...
		       Lisp_Image_Instance * p2, int depth)
{
	...
	case IMAGE_COLOR_PIXMAP:
	case IMAGE_POINTER:
		if (IMAGE_INSTANCE_X_COLORMAP(p1) !=
		    IMAGE_INSTANCE_X_COLORMAP(p2)
		    || IMAGE_INSTANCE_X_NPIXELS(p1) !=
		    IMAGE_INSTANCE_X_NPIXELS(p2))
			return 0;
		break;
...
static unsigned long x_image_instance_hash(Lisp_Image_Instance * p, int depth)
{
	switch (IMAGE_INSTANCE_TYPE(p)) {
	case IMAGE_MONO_PIXMAP:
	case IMAGE_COLOR_PIXMAP:
...
static Lisp_Object x_locate_pixmap_file(Lisp_Object name)
{
	...
	if (NILP(Vx_bitmap_file_path)) {
		...
		if (XrmGetResource(XtDatabase(display),
				   "bitmapFilePath", "BitmapFilePath", &type,
				   &value)
		    && !strcmp(type, "String"))
			Vx_bitmap_file_path =
			    decode_env_path(0, (char *)value.addr);
		Vx_bitmap_file_path =
		    nconc2(Vx_bitmap_file_path, (decode_path(BITMAPDIR)));
	}

	{
		Lisp_Object found;
		if (locate_file(Vx_bitmap_file_path, name, Qnil, &found, R_OK) <
		    0) {
...
image_instance_add_x_image(Lisp_Image_Instance * ii,
			   XImage * ximage, int slice, Lisp_Object instantiator)
{
	Lisp_Object device = IMAGE_INSTANCE_DEVICE(ii);
	Display *dpy;
	GC gc;
...
x_init_image_instance_from_eimage(Lisp_Image_Instance * ii,
				  ...
				  int dest_mask,
				  Lisp_Object instantiator, Lisp_Object domain)
{
	Lisp_Object device = IMAGE_INSTANCE_DEVICE(ii);
...
init_image_instance_from_xbm_inline(Lisp_Image_Instance * ii,
				    ...
				    Lisp_Object pointer_bg,
				    int dest_mask,
				    Pixmap mask, Lisp_Object mask_filename)
{
	Lisp_Object device = IMAGE_INSTANCE_DEVICE(ii);
	...
	case IMAGE_COLOR_PIXMAP: {
		...
		if (COLOR_INSTANCEP(foreground))
			fg = COLOR_INSTANCE_X_COLOR(XCOLOR_INSTANCE
						    (foreground)).pixel;
		...
		if (COLOR_INSTANCEP(background))
			bg = COLOR_INSTANCE_X_COLOR(XCOLOR_INSTANCE
						    (background)).pixel;
		  ...
		  char *bits)
{
	...
	if (!NILP(mask_data)) {
		...
		LISP_STRING_TO_EXTERNAL(XCAR(XCDR(XCDR(mask_data))), ext_data,
					Qbinary);
		mask = pixmap_from_xbm_inline(
			IMAGE_INSTANCE_DEVICE(ii),
			XINT(XCAR(mask_data)),
...
Your version of XPM is too old.You cannot compile with it.
    Upgrade to version 3.2 g or better or compile with-- with - xpm = no.
...
extract_xpm_color_names(XpmAttributes * xpmattrs, Lisp_Object device,
	...
	symbols = xnew_atomic_array(XpmColorSymbol, i);
	...
	while (--i >= 0) {
		...
		/* Duplicate the pixel value so that we still have
		 * a lock on it if the pixel we were passed is later freed. */
		if (!XAllocColor(dpy, cmap, &color)) {
			/* it must be allocable since we're
			 * just duplicating it */
			abort();
		}
...
static void xpm_free(XpmAttributes * xpmattrs)
{
		  ...
		  int dest_mask, Lisp_Object domain)
{
	...
	case XpmColorError:
		{
			...
			if (force_mono) {
				...
				if (!(dest_mask & IMAGE_MONO_PIXMAP_MASK)) {
					/* second time; blow out. */
					signal_double_file_error
					    ("Reading pixmap data",
					     "color allocation failed", data);
				}
				force_mono = 1;
...

	{
		...
		if (npixels != 0) {
			pixels = xnew_atomic_array(Pixel, npixels);
			memcpy(pixels, xpmattrs.pixels,
			       npixels * sizeof(Pixel));
		} else {
			pixels = NULL;
		}
	...
	case IMAGE_POINTER: {
		...
		/* If the loaded pixmap has colors allocated (meaning it came
		   from an XPM file), then use those as the default colors for
		   the cursor we create.  Otherwise, default to pointer_fg and
		   pointer_bg.
		*/
		if (npixels >= 2) {
			/* With an XBM file, it's obvious which bit is
			   foreground and which is background, or rather, it's
			   implicit: in an XBM file, a 1 bit is foreground, and
			   a 0 bit is background.
			   ...
			   pixel values.  So we need to figure out which of
			   those pixels is the foreground color and which is the
			   ...
			   foreground - perhaps with logical color names
			   somehow?

			   Once we have decided which color is the foreground,
			   we need to ensure that that color corresponds to a
			   `1' bit in the Pixmap.  The XPM library wrote into
			   the (1-bit) pixmap with XPutPixel, which will ignore
			   all but the least significant bit.
			   ...
			   color if both `fg' and `bg' are odd or even, but we
			   can safely assume that that won't happen if the XPM
			   file is sensible I think.)
			   ...
			   and the other is always 0.  We also pretty much need
			   to assume that one is even and the other is odd.)
			*/

			fg.pixel = pixels[0];	/* pick a pixel at random. */
			bg.pixel = fg.pixel;
			for (i = 1; i < npixels; i++) {
				/* Look for an "other" pixel value. */
				bg.pixel = pixels[i];
			...
			/* If (fg.pixel == bg.pixel) then probably something has
			   gone wrong, but I don't think signalling an error
			   would be appropriate. */

			XQueryColor(dpy, cmap, &fg);
			XQueryColor(dpy, cmap, &bg);

			/* If the foreground is lighter than the background,
			   swap them.  (This occurs semi-randomly, depending on
			   the ordering of the color list in the XPM file.)
			*/
			{
				unsigned short fg_total =
					((fg.red / 3) + (fg.green / 3)
					 + (fg.blue / 3));
				unsigned short bg_total =
					((bg.red / 3) + (bg.green / 3)
					 + (bg.blue / 3));
			...
			/* If the fg pixel corresponds to a `0' in the bitmap,
			   invert it.  (This occurs (only?) on servers with
			   Black=0, White=1.)
			*/
			if ((fg.pixel & 1) == 0) {
				XGCValues gcv;
				...
				gc = XCreateGC(dpy, pixmap,
					       (GCFunction |
						GCForeground), &gcv);
				XFillRectangle(dpy, pixmap, gc, 0, 0, w,
					       h);
				XFreeGC(dpy, gc);
			}
		} else {
		    ...
		    int dest_mask, Lisp_Object domain)
{
	...
	/* Must use setjmp not SETJMP because we used jmp_buf above not
	   JMP_BUF */
		     ...
		     Lisp_Object console_type, Lisp_Object dest_mask)
{
	...
	if (!NILP(filename)) {
		...
		/* #### Apparently some versions of XpmReadFileToData, which is
		   called by pixmap_to_lisp_data, don't return an error value if
		   the given file is not a valid XPM file.  Instead, they just
		   seg fault.  It is definitely caused by passing a bitmap.  To
		   try and avoid this we check for bitmaps first.  */
...
static Font safe_XLoadFont(Display * dpy, char *name)
{
	Font font;
			  ...
			  Lisp_Object foreground, Lisp_Object background)
{
	...
	{
		Display *dpy =
		    DEVICE_X_DISPLAY(XDEVICE(IMAGE_INSTANCE_DEVICE(p)));
		Drawable draw =
		    XtWindow(DEVICE_XT_APP_SHELL
			     (XDEVICE(IMAGE_INSTANCE_DEVICE(p))));
...
static void x_unmap_subwindow(Lisp_Image_Instance * p)
{
	...
	if (IMAGE_INSTANCE_TYPE(p) == IMAGE_SUBWINDOW) {
		...
		if (IMAGE_INSTANCE_WANTS_INITIAL_FOCUS(p)) {
			/* #### FIXME to pop-up the find dialog we map the text-field
			   seven times! This doesn't show on a fast linux, though. */
...
static void x_redisplay_subwindow(Lisp_Image_Instance * p)
{
...
/* Update all attributes that have changed. Lwlib actually does most
   of this for us. */
static void x_redisplay_widget(Lisp_Image_Instance * p)
{
	/* This function can GC if IN_REDISPLAY is false. */
#ifdef HAVE_WIDGETS
	widget_value *wv = 0;
	...
	if (IMAGE_INSTANCE_SIZE_CHANGED(p)
		...
		if (IMAGE_INSTANCE_X_WIDGET_ID(p)->core.being_destroyed
			...
			signal_simple_error("SXEmacs bug: subwindow is deleted",
					    sw);
		}
...
static int x_widget_border_width(void)
{
      ...
      (subwindow, property, data))
{
	...
	XChangeProperty(dpy, sw->subwindow, property_atom, XA_STRING, 8,
			PropModeReplace,
			XSTRING_DATA(data), XSTRING_LENGTH(data));
		   ...
		   Lisp_Object domain)
{
	...
	Lisp_Object pixel = FACE_FOREGROUND(IMAGE_INSTANCE_WIDGET_FACE(ii),
					    domain);
...
#endif
	{
		Lisp_Object tmp = query_string_font(
			IMAGE_INSTANCE_WIDGET_TEXT(ii),
			IMAGE_INSTANCE_WIDGET_FACE(ii), domain);
		lw_add_widget_value_arg(
			wv, XtNfont,
			(XtArgVal)FONT_INSTANCE_X_FONT(XFONT_INSTANCE(tmp)));
	}
		     ...
		     const char *type, widget_value * wv)
{
	...
	/* Create a clip window to contain the subwidget. Incredibly the
	   SXEmacs manager seems to be the most appropriate widget for
	...
	pixel = FACE_FOREGROUND
	    (IMAGE_INSTANCE_WIDGET_FACE(ii), IMAGE_INSTANCE_FRAME(ii));
	...
	lw_add_widget_value_arg(wv, XtNwidth, IMAGE_INSTANCE_WIDTH(ii));
	lw_add_widget_value_arg(wv, XtNheight, IMAGE_INSTANCE_HEIGHT(ii));
	/* update the font. */
	update_widget_face(wv, ii, domain);

	wid =
	    lw_create_widget(type, wv->name, id, wv,
			     IMAGE_INSTANCE_X_CLIPWIDGET(ii), False, 0,
	...
	ac = 0;
	XtSetArg(al[ac], XtNx, &IMAGE_INSTANCE_X_WIDGET_XOFFSET(ii));
	ac++;
	XtSetArg(al[ac], XtNy, &IMAGE_INSTANCE_X_WIDGET_YOFFSET(ii));
	ac++;
...
x_widget_property(Lisp_Object image_instance, Lisp_Object prop)
{
	...
	if (EQ(prop, Q_text)) {
		widget_value *wv =
		    lw_get_all_values(IMAGE_INSTANCE_X_WIDGET_LWID(ii));
		     ...
		     int dest_mask, Lisp_Object domain)
{
	...
	if (!NILP(glyph)) {
		if (!IMAGE_INSTANCEP(glyph))
			glyph =
			    glyph_image_instance(glyph, domain, ERROR_ME, 1);
	}
	...
	if (!NILP(glyph) && IMAGE_INSTANCEP(glyph)
		...
		XtSetArg(al[ac], XmNlabelPixmap,
			 XIMAGE_INSTANCE_X_PIXMAP(glyph));
		ac++;
#else
		XtSetArg(al[ac], XtNpixmap, XIMAGE_INSTANCE_X_PIXMAP(glyph));
		ac++;
#endif
...
static void x_button_redisplay(Lisp_Object image_instance)
{
	/* This function can GC if IN_REDISPLAY is false. */
	Lisp_Image_Instance *p = XIMAGE_INSTANCE(image_instance);
	widget_value *wv = gui_items_to_widget_values(image_instance,
...
static void x_progress_gauge_redisplay(Lisp_Object image_instance)
{
	Lisp_Image_Instance *ii = XIMAGE_INSTANCE(image_instance);
			...
			int dest_mask, Lisp_Object domain)
{
	...
	/* This is not done generically because of sizing problems under
	   mswindows. REVISE ME: We don't support windows, so? */
...
static void x_tab_control_redisplay(Lisp_Object image_instance)
{
	Lisp_Image_Instance *ii = XIMAGE_INSTANCE(image_instance);
		...
		if (tab_control_order_only_changed(image_instance)) {
			...
			LIST_LOOP(rest, XCDR(IMAGE_INSTANCE_WIDGET_ITEMS(ii))) {
				if (gui_item_equal_sans_selected
					...
					LISP_STRING_TO_EXTERNAL(XGUI_ITEM
								(XCAR(rest))->
								name, name,
								Qnative);
	...
	if (IMAGE_INSTANCE_WIDGET_FACE_CHANGED(ii)
		...
		lw_modify_all_widgets(IMAGE_INSTANCE_X_WIDGET_LWID(ii), wv,
				      True);
		free_widget_value_tree(wv);
	}
}
...
#endif
#if 1
	IIFORMAT_VALID_CONSOLE(x, rawrgb);
	IIFORMAT_VALID_CONSOLE(x, rawrgba);
#endif
