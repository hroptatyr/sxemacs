...
static int x_sym_p(Lisp_Object value)
{
...
static UINT symbol_to_ms_cf(Lisp_Object value)
{
	...
	if (EQ(value, QCF_TEXT))
		return CF_TEXT;
	if (EQ(value, QCF_BITMAP))
		return CF_BITMAP;
	if (EQ(value, QCF_METAFILEPICT))
		return CF_METAFILEPICT;
	if (EQ(value, QCF_SYLK))
		return CF_SYLK;
	if (EQ(value, QCF_DIF))
		return CF_DIF;
	if (EQ(value, QCF_TIFF))
		return CF_TIFF;
	if (EQ(value, QCF_OEMTEXT))
		return CF_OEMTEXT;
	if (EQ(value, QCF_DIB))
		return CF_DIB;
#ifdef CF_DIBV5
	if (EQ(value, QCF_DIBV5))
		return CF_DIBV5;
#endif
	if (EQ(value, QCF_PALETTE))
		return CF_PALETTE;
	if (EQ(value, QCF_PENDATA))
		return CF_PENDATA;
	if (EQ(value, QCF_RIFF))
		return CF_RIFF;
	if (EQ(value, QCF_WAVE))
		return CF_WAVE;
	if (EQ(value, QCF_UNICODETEXT))
		return CF_UNICODETEXT;
	if (EQ(value, QCF_ENHMETAFILE))
		return CF_ENHMETAFILE;
	if (EQ(value, QCF_HDROP))
		return CF_HDROP;
	if (EQ(value, QCF_LOCALE))
		return CF_LOCALE;
	if (EQ(value, QCF_OWNERDISPLAY))
		return CF_OWNERDISPLAY;
	if (EQ(value, QCF_DSPTEXT))
		return CF_DSPTEXT;
	if (EQ(value, QCF_DSPBITMAP))
		return CF_DSPBITMAP;
	if (EQ(value, QCF_DSPMETAFILEPICT))
		return CF_DSPMETAFILEPICT;
	if (EQ(value, QCF_DSPENHMETAFILE))
...
static Lisp_Object ms_cf_to_symbol(UINT format)
{
	switch (format) {
	case CF_TEXT:
		return QCF_TEXT;
	...
	case CF_WAVE:
		return QCF_WAVE;
	case CF_UNICODETEXT:
		return QCF_UNICODETEXT;
	case CF_ENHMETAFILE:
		return QCF_ENHMETAFILE;
	case CF_HDROP:
		return QCF_HDROP;
	case CF_LOCALE:
		return QCF_LOCALE;
	case CF_OWNERDISPLAY:
		return QCF_OWNERDISPLAY;
	case CF_DSPTEXT:
		return QCF_DSPTEXT;
	case CF_DSPBITMAP:
		return QCF_DSPBITMAP;
	case CF_DSPMETAFILEPICT:
		return QCF_DSPMETAFILEPICT;
	case CF_DSPENHMETAFILE:
		return QCF_DSPENHMETAFILE;
	default:
...
static int cf_is_autofreed(UINT format)
{
	switch (format) {
...
mswindows_own_selection(Lisp_Object selection_name,
			...
			Lisp_Object selection_type, int owned_p /* Not used */ )
{
	HGLOBAL hValue = NULL;
	UINT cfType;
	int is_X_type = FALSE;
	...
	data = select_convert_out(selection_name, cfObject, selection_value);
	...
	if (!cf_is_autofreed(cfType)) {
		...
		Vhandle_alist =
		    Fcons(Fcons(cfType_int, make_opaque_ptr(hValue)),
			  Vhandle_alist);
		...
		if (!NILP(alist_elt)) {
			...
			for (rest = Vhandle_alist; !NILP(rest);
			     rest = Fcdr(rest))
...
static Lisp_Object mswindows_register_selection_data_type(Lisp_Object type_name)
{
...
static Lisp_Object mswindows_selection_data_type_name(Lisp_Object type_id)
{
	UINT format;
	int numchars;
	char name_buf[128];
				...
				Lisp_Object target_type)
{
	...
	TO_INTERNAL_FORMAT(DATA, (data, size), LISP_STRING, ret, Qbinary);
	...
	value = select_convert_in(selection_symbol, cfObject, ret);
...
static Lisp_Object
mswindows_selection_exists_p(Lisp_Object selection, Lisp_Object selection_type)
{
	...
	if (EQ(selection, QCLIPBOARD)) {
		if (NILP(selection_type))
			...
			return
			    IsClipboardFormatAvailable(symbol_to_ms_cf
						       (selection_type))
			    ? Qt : Qnil;
	} else
		return Qnil;
