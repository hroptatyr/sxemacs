                            ...
                            unsigned int *size_ret, int *format_ret)
{
        ...
        if (EQ(obj, QNULL) || (EQ(type, QNULL))) {	/* This is not the same as declining */
                ...
                */
                        ...
                        *data_ret = (unsigned char *)xmalloc((*size_ret) * sizeof(Atom));
                        for (i = 0; i < (int)(*size_ret); i++)
                                if (SYMBOLP(XVECTOR_DATA(obj)[i]))
                                        (*(Atom **) data_ret)[i] = symbol_to_x_atom(d, XVECTOR_DATA(obj)[i], 0);
                                else
                                        signal_error(Qerror,	/* Qselection_error */
                                                     list2(build_string ("all elements of the vector must be of the same type"), obj));
                }
                        ...
                        for (i = 0; i < *size_ret; i++)
                                if (VECTORP(XVECTOR_DATA(obj)[i])) {
                                        ...
                                        if (XVECTOR_LENGTH(pair) != 2)
                                                signal_error(Qerror, list2(build_string("elements of the vector must be vectors of exactly two elements"), pair));
                                        (*(Atom **) data_ret)[i * 2] =
                                                symbol_to_x_atom(d, XVECTOR_DATA(pair)[0], 0);
                                        (*(Atom **) data_ret)[(i * 2) + 1] =
                                                symbol_to_x_atom(d, XVECTOR_DATA(pair)[1], 0);
                                } else
                                        signal_error(Qerror, list2(build_string("all elements of the vector must be of the same type"), obj));
                }
#endif
                else {
                                ...
                                if (CONSP(XVECTOR_DATA(obj)[i]))
                                        * format_ret = 32;
                                        ...
                                        if (!INTP(XVECTOR_DATA(obj)[i]))
                                                signal_error(Qerror,	/* Qselection_error */
                                                             list2(build_string ("all elements of the vector must be integers or conses of integers"), obj));
                        *data_ret =
                                (unsigned char *)xmalloc(*size_ret * (*format_ret / 8));
                        for (i = 0; i < (int)(*size_ret); i++)
                                if (*format_ret == 32)
                ...
                signal_error(Qerror,	/* Qselection_error */
                             list2(build_string ("unrecognized selection data"), obj));
