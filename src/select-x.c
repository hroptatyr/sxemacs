 ...
 */
static Lisp_Object x_atom_to_symbol(struct device *d, Atom atom)
{
	Display *display = DEVICE_X_DISPLAY(d);
 ...
 */
static void x_decline_selection_request(XSelectionRequestEvent * event)
{
 ...
 */
static Lisp_Object x_selection_request_lisp_error(Lisp_Object closure)
{
...
static int property_deleted_p(void *tick)
{
...
static void unexpect_property_change(int tick)
{
...
static void wait_for_property_change(long tick)
{
	/* This function can GC */
...

				   ...
				   static Lisp_Object
				   fetch_multiple_target(XSelectionRequestEvent
							 * event) {
				   /* This function can GC */
				   Display * display = event->display;
				   Window window = event->requestor;
				   ...
				   Fcons(QMULTIPLE,
					 ...
					 (display, window, target, QMULTIPLE,
					  selection_atom));}

				   static Lisp_Object
				   copy_multiple_data(Lisp_Object obj) {
				   Lisp_Object vec;
				   int i; int len; if (CONSP(obj))
				   return Fcons(XCAR(obj),
						copy_multiple_data(XCDR(obj)));
				   CHECK_VECTOR(obj); len = XVECTOR_LENGTH(obj);
				   vec = make_vector(len, Qnil); for (i = 0;
								      i < len;
								      i++) {
...

				   ...
				   static Lisp_Object Qx_selection_reply_timeout_internal; DEFUN("x-selection-reply-timeout-internal", Fx_selection_reply_timeout_internal, 1, 1, 0,	/*
																							 */
												 (arg)) {
				   selection_reply_timed_out = 1;
				   reading_selection_reply = 0; return Qnil;}
 ...
 */
				   ...
				   XtWindow(FRAME_X_TEXT_WIDGET(sel_frame));
				   Time requestor_time =
				   DEVICE_X_MOUSE_TIMESTAMP(d);
				   Atom target_property =
				   DEVICE_XATOM_EMACS_TMP(d);
				   Atom selection_atom =
				   symbol_to_x_atom(d, selection_symbol, 0);
				   int speccount;
				   Atom type_atom =
				   symbol_to_x_atom(d,
						    (CONSP(target_type) ?
						     XCAR(target_type) :
						     target_type), 0);
				   XConvertSelection(display, selection_atom,
						     type_atom, target_property,
						     requestor_window,
						     requestor_time);
				   ...
				   Lisp_Object id =
				   Fadd_timeout(make_int(x_selection_timeout),
				   ...
				   if (buffer_size >
				       MAX_SELECTION_QUANTUM) buffer_size =
				   ...
				   XGetWindowProperty(display, window, property,
						      ...
						      &tmp_data);
				   if (result != Success) {
				   *data_ret = 0; *bytes_ret = 0; return;}
				   XFree((char *)tmp_data);
				   if (*actual_type_ret == None
				       || *actual_format_ret == 0) {
				   if (delete_p) XDeleteProperty(display,
								 window,
								 property);
				   *data_ret = 0; *bytes_ret = 0; return;}
				   ...
				   *data_ret =
				   (Extbyte *) xmalloc(*size_bytes_ret);
#if 0
				   stderr_out("\nread INCR %d\n",
					      min_size_bytes);
#endif
				    ...
				    */
				   prop_id = expect_property_change(display, window, property, PropertyNewValue); x_get_window_property(display, window, property, &tmp_data, &tmp_size_bytes, type_ret, format_ret, size_ret, 1); if (tmp_size_bytes == 0) {	/* we're done */
#if 0
				   stderr_out("  read INCR done\n");
#endif
				   unexpect_property_change(prop_id);
				   if (tmp_data) xfree(tmp_data); break;}
#if 0
				   ...
				   *data_ret =
				   ...
				   Lisp_Object val;
				   struct device *d =
				   get_device_from_display(display);
				   ...
				   *((unsigned int *)data); xfree(data);
				   ...
				   /* It's been read.  Now convert it to a lisp object in some semi-rational
				      manner. */
				   val =
				   selection_data_to_lisp_data(d, data, bytes,
							       actual_type,
							       actual_format);
				   xfree(data); return val;}

 ...
 */
				   ...
				   Lisp_Object v =
				   Fmake_vector(make_int(len), Qzero);
				   for (i = 0; i < len; i++)
				   Faset(v, make_int(i),
					 x_atom_to_symbol(d,
							  ((Atom *) data)[i]));
   ...
   */
				   ...
				   return
				   word_to_lisp(((unsigned long *)data)[0]);
				   else
				   if (format == 16 && size == sizeof(short))
				   return
				   make_int((int)(((unsigned short *)data)[0]));
				   ...
				   *size_ret = 0; *data_ret = 0; type = QNULL;}
				   ...
				   TO_EXTERNAL_FORMAT(LISP_STRING, obj,
						      ...
						      (NILP(type) ? Qctext :
						       Qbinary));
				   *format_ret = 8; *size_ret = extvallen;
				   *data_ret =
				   ...
				   *size_ret = 1;
				   *data_ret =
				   (unsigned char *)xmalloc(sizeof(Atom) + 1);
				   ...
				   *data_ret =
				   ...
				   if (INTP(obj) || CONSP(obj)) {
				   *format_ret = 32;
				   *size_ret = 1;
				   *data_ret =
				   (unsigned char *)xmalloc(sizeof(long) + 1);
				   (*data_ret)[sizeof(long)] = 0;
				   (*(unsigned long **)data_ret)[0] =
				   lisp_to_word(obj);
				   if (NILP(type)) type = QINTEGER;}
				   else
				   if (VECTORP(obj)) {
       ...
       */
				   int i; if (SYMBOLP(XVECTOR_DATA(obj)[0]))
				   ...
				   *data_ret = (unsigned char *)
				   ...
				   signal_error(Qerror,
						list2(build_string
						      ...
						      pair));
				   (*(Atom **) data_ret)[i * 2] =
				   symbol_to_x_atom(d, XVECTOR_DATA(pair)[0],
						    0);
				   (*(Atom **) data_ret)[(i * 2) + 1] =
				   symbol_to_x_atom(d, XVECTOR_DATA(pair)[1],
						    0);}
				   else
				   signal_error(Qerror,
						list2(build_string
 ...
 */
				   void
				   x_handle_selection_notify(XSelectionEvent *
							     event) {
				   ...
				   message
				   ("received a SelectionNotify event for the wrong window");
				   else
				   if (event->selection !=
				       reading_which_selection)
				   message
				   ("received the wrong selection type in SelectionNotify!");
				   else
				   reading_selection_reply = 0;	/* we're done now. */
				   }

				   static void
				   x_disown_selection(Lisp_Object selection,
						      Lisp_Object timeval) {
				   struct device *d = decode_x_device(Qnil);
				   Display * display = DEVICE_X_DISPLAY(d);
				   Time timestamp; Atom selection_atom;
				   CHECK_SYMBOL(selection); if (NILP(timeval))
				   ...
				   selection_atom =
				   symbol_to_x_atom(d, selection, 0);
...
/* Ensure that all 8 cut buffers exist.  ICCCM says we gotta... */
				   static void
				   initialize_cut_buffers(Display * display,
							  Window window) {
				   static unsigned const char *const data =
				   (unsigned const char *)"";
#define FROB(atom) XChangeProperty (display, window, atom, XA_STRING, 8, \
				   ...
				   FROB(XA_CUT_BUFFER4);
				   FROB(XA_CUT_BUFFER5);
				   FROB(XA_CUT_BUFFER6); FROB(XA_CUT_BUFFER7);
#undef FROB
...
Return the value of the named CUTBUFFER (typically CUT_BUFFER0).
													 */
					 (cutbuffer)) {
				   struct device *d = decode_x_device(Qnil); Display * display = DEVICE_X_DISPLAY(d); Window window = RootWindow(display, 0);	/* Cutbuffers are on frame 0 */
				   ...
				   x_get_window_property(display, window,
							 cut_buffer_atom, &data,
							 &bytes, &type, &format,
							 &size, 0);
				   if (!data) return Qnil;
				   if (format != 8 || type != XA_STRING)
				   signal_simple_error_2
				   ("Cut buffer doesn't contain 8-bit STRING data",
...
Set the value of the named CUTBUFFER (typically CUT_BUFFER0) to STRING.
				   ...
				   enum { ASCII, LATIN_1, WORLD } chartypes =
				   ASCII;
#endif
				   if (max_bytes > MAX_SELECTION_QUANTUM)
				   max_bytes = MAX_SELECTION_QUANTUM;
				   ...
				   if ((*ptr) == LEADING_BYTE_LATIN_ISO8859_1 ||
				       (*ptr) == LEADING_BYTE_CONTROL_1) {
				   chartypes = LATIN_1; ptr += 2; continue;}

				   chartypes = WORLD; break;}
				   ...
				   int chunk =
				   bytes_remaining <
				   max_bytes ? bytes_remaining : max_bytes;
				   XChangeProperty(display, window,
						   cut_buffer_atom, XA_STRING,
						   8,
						   (bytes_remaining ==
						    bytes ? PropModeReplace :
						    PropModeAppend), data,
						   chunk); data += chunk;
...
positive means move values forward, negative means backward.
				   ...
				   struct device *d = decode_x_device(Qnil); Display * display = DEVICE_X_DISPLAY(d); Window window = RootWindow(display, 0);	/* Cutbuffers are on frame 0 */
				   Atom props[8];
				   CHECK_INT(n); if (XINT(n) == 0)
				   return n; if (!cut_buffers_initialized)
				   initialize_cut_buffers(display, window);
...
it merely informs you that they have happened.
													 */ );
				   Vx_sent_selection_hooks = Qunbound; DEFVAR_INT("x-selection-timeout", &x_selection_timeout	/*
...
\"*selectionTimeout\" resource (which is expressed in milliseconds).
				   ...
				   x_selection_timeout = 0; DEFVAR_BOOL("x-selection-strict-motif-ownership", &x_selection_strict_motif_ownership	/*
