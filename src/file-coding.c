      ...
      (alias))
{
	...
	if (SYMBOLP(aliasee))
		...
		signal_simple_error("Symbol is not a coding system alias",
				    alias);
	...
	while (1) {
		...
		if (Dynarr_length(str->runoff) > 0) {
			Lstream_data_count chunk =
			    min(size,
				(Lstream_data_count) Dynarr_length(str->
								   runoff));
...
static Lstream_data_count
decoding_writer(Lstream * stream, const unsigned char *data,
		Lstream_data_count size)
{
	struct decoding_stream *str = DECODING_STREAM_DATA(stream);
...
static void reset_decoding_stream(struct decoding_stream *str)
{
#ifdef MULE
...
void set_decoding_stream_coding_system(Lstream * lstr, Lisp_Object codesys)
{
...
static Lisp_Object
make_decoding_stream_1(Lstream * stream, Lisp_Object codesys, const char *mode)
{
	    ...
	    unsigned_char_dynarr * dst, Lstream_data_count n)
{
	...
	case CODESYS_CCL:
		...
		/* When applying ccl program to stream, MUST NOT set NULL
		   pointer to src.  */
		ccl_driver(&str->ccl,
			   (src ? (unsigned char *)src : (unsigned char *)""),
...
static Lstream_data_count encoding_reader(Lstream * stream, unsigned char *data,
					  Lstream_data_count size);
static Lstream_data_count encoding_writer(Lstream * stream,
					  const unsigned char *data,
					  Lstream_data_count size);
...
static Lstream_data_count
encoding_writer(Lstream * stream, const unsigned char *data,
		Lstream_data_count size)
{
	struct encoding_stream *str = ENCODING_STREAM_DATA(stream);
	Lstream_data_count retval;
...
static void reset_encoding_stream(struct encoding_stream *str)
{
#ifdef MULE
	switch (CODING_SYSTEM_TYPE(str->codesys)) {
	case CODESYS_ISO2022:
		{
			...
			for (i = 0; i < 4; i++) {
				str->iso2022.charset[i] =
				    CODING_SYSTEM_ISO2022_INITIAL_CHARSET(str->
									  codesys,
									  i);
...
void set_encoding_stream_coding_system(Lstream * lstr, Lisp_Object codesys)
{
	Lisp_Coding_System *cs = XCODING_SYSTEM(codesys);
...
static Lisp_Object
make_encoding_stream_1(Lstream * stream, Lisp_Object codesys, const char *mode)
{
		      ...
		      unsigned_char_dynarr * dst, Lstream_data_count n)
{
	...
	while (n--) {
		unsigned char c = *(unsigned char *)src++;
		if (flags & CODING_STATE_ESCAPE) {	/* Within ESC sequence */
			int retval =
			    parse_iso2022_esc(coding_system, &str->iso2022,
					      c, &flags, 1);
