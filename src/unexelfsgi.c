...
#endif
		if (!strcmp(section_names + OLD_SECTION_H(idx).sh_name, name))
			return idx;
	}
...
unexec(char *new_name,
       char *old_name,
       uintptr_t data_start, uintptr_t bss_start, uintptr_t entry_address)
{
	int old_file;
	...
	int new_bss_addr, new_data2_size, new_data2_offset, new_file,
	    new_file_size;
	...
	new_section_h =
	    (ElfW(Shdr) *) ((byte *) new_base + old_file_h->e_shoff +
			    new_data2_size);
	...
	for (n = new_file_h->e_phnum - 1; n >= 0; n--) {
		...
		printf("%d @ %0x + %0x against %0x + %0x",
		       n, ph->p_vaddr, ph->p_memsz, growme->sh_addr,
		       growme->sh_size);
#endif
		if ((ph->p_type == PT_LOAD) &&
		    (ph->p_vaddr <= growme->sh_addr) &&
		    ((ph->p_vaddr + ph->p_memsz) >=
		     (growme->sh_addr + growme->sh_size))) {
	...
	for (n = 1, nn = 1; n < (int)old_file_h->e_shnum; n++, nn++) {
		...
		if (osec == growme) {
			...
			memcpy(nsec->sh_offset + new_base,
			       (caddr_t) osec->sh_addr, new_data2_size);
			nn++;
			grown = nsec++;
		}
		...
		if ((osec->sh_type == SHT_NOBITS)
		    && (osec->sh_flags & SHF_ALLOC) && (osec != growme)) {
    ...
    {						\
      n_phdrr->__fileaddr += movement;		\
		...
		if (n == old_mdebug_index) {
			HDRR *o_phdrr =
			    (HDRR *) ((byte *) old_base + osec->sh_offset);
			HDRR *n_phdrr =
			    (HDRR *) ((byte *) new_base + nsec->sh_offset);
		...
		if (nsec->sh_type == SHT_SYMTAB || nsec->sh_type == SHT_DYNSYM) {
			...
			ElfW(Sym) * sym =
			    (ElfW(Sym) *) (nsec->sh_offset + new_base);
			byte *symnames =
			    ((byte *) new_base +
			...
			for (; num--; sym++) {
				...
				if (strcmp(symnam, "_end") == 0
					...
					memcpy(&sym->st_value, &new_bss_addr,
					       sizeof(new_bss_addr));
	...
	for (n = new_file_h->e_shnum - 1; n; n--) {
		...
		if (section.sh_type == SHT_REL || section.sh_type == SHT_RELA) {
			...
			if (!strcmp(nm, ".data") || !strcmp(nm, ".sdata")
				...
				ElfW(Addr) offset =
				    info->sh_addr - info->sh_offset;
				caddr_t end, reloc =
				    old_base + section.sh_offset;
