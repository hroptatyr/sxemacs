...
hashcode_t internal_hash(Lisp_Object obj, int depth)
{
	...
	if (CONSP(obj) && !CONSP(XCDR(obj))) {
		...
		Lisp_Object o = obj;
		/* unroll */
		hashcode_t hash = internal_hash(XCAR(o), depth+1);

		o = XCDR(o);
		for (int s = 1; s < 6 && CONSP(o); o = XCDR(o), s++) {
			hashcode_t h = internal_hash(XCAR(o), depth+1);
			hash = HASH3(hash, h, s);
		}
		return hash;
	}
...
static void
ht_iter_next(dict_iter_t di, Lisp_Object *key, Lisp_Object *val)
{
	hentry_t e = di->data;
...
static struct dict_impl_s __hash_table = {
	...
	.iter_next_f = ht_iter_next,
	.iter_fini_f = ht_iter_fini,
};
/* deal with dict interface */
dict_impl_t dict_hash_table = &__hash_table;
