...
#include "ui/device.h"		/* For DEVICE_BAUD_RATE */
#else				/* not emacs */
...
static char *xmalloc_atomic(size)
unsigned int size;
{
...
static const char *find_capability(bp, cap)
const char *bp;
const char *cap;
{
	for (; *bp; bp++)
...
static const char *tgetst1(ptr, area)
const char *ptr;
char **area;
{
	const char *p;
	...
	if (!area) {
		...
		p = ptr;
		while ((c = *p++) && c != ':' && c != '\n') ;
		ret = (char *)xmalloc_atomic(p - ptr + 1);
	} else
		ret = *area;
...
const char *name;
{
	...
	if (tem && !IS_DIRECTORY_SEP(*tem) && !strcmp(name, getenv("TERM"))) {
		...
		if (!indirect) {
			if (!bp)
				...
				strcpy(bp, tem);
			goto ret;
		} else {	/* We will need to read /etc/termcap.  */
			tcenv = tem;
			tem = 0;
		}
	} else
	...
	buf.beg = (char *)xmalloc_atomic(buf.size + 1);
	term = indirect ? indirect : name;

	if (!bp) {
		malloc_size = indirect ? strlen(tcenv) + 1 : buf.size;
		bp = (char *)xmalloc_atomic(malloc_size);
	}
	bp1 = bp;
...
static int scan_file(string, fd, bufp)
char *string;
int fd;
struct buffer *bufp;
{
	...
	*bufp->ptr = 0;
	...
	while (!bufp->ateof) {
		...
		if (*bufp->ptr != '#' && name_match(bufp->ptr, string))
			return 1;
...
char *append_end;
{
	...
	while (1) {
		...
		if (!
		    (nread =
		     read(fd, buf + bufp->full, bufp->size - bufp->full)))
			bufp->ateof = 1;
		bufp->full += nread;
		buf[bufp->full] = 0;
	}
