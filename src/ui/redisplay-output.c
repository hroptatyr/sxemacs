...
static int compare_runes(struct window *w, struct rune *crb, struct rune *drb);
static void redraw_cursor_in_window(struct window *w, int run_end_begin_glyphs);
static void redisplay_output_display_block(struct window *w,
					   struct display_line *dl, int block,
			  ...
			  display_line_dynarr * ddla)
{
	...
	for (db_elt = 0; db_elt < Dynarr_length(dlp->display_blocks); db_elt++) {
		...
		struct display_block *ddb =
		    Dynarr_atp(dlp->display_blocks, db_elt);
		       ...
		       int cursor_height)
{
	...
	if (f->windows_structure_changed ||
	    ...
	    cdl->descent != ddl->descent || cdl->clip != ddl->clip || force) {
		start_pos = 0;
		force = 1;
	} else {
		int elt = 0;
 ...
 ****************************************************************************/
static void clear_left_border(struct window *w, int y, int height)
{
	struct frame *f = XFRAME(w->frame);
	Lisp_Object window;
 ...
 ****************************************************************************/
static void clear_right_border(struct window *w, int y, int height)
{
	struct frame *f = XFRAME(w->frame);
	Lisp_Object window;
		    ...
		    int force_end)
{
	...
	if (ddl->cursor_elt != -1) {
		...
		get_cursor_size_and_location(w, db, ddl->cursor_elt,
					     &cursor_start, &cursor_width,
					     &cursor_height);
	} else {
	...
	if (ddl->modeline) {
		...
		if (cdba && !w->shadow_thickness_changed) {
			...
			redisplay_output_display_block(w, ddl, 0, 0, -1,
						       start_pixpos, 0, 0, 0);
			must_sync = 1;
		}
	...
	while (!ddl->modeline && start_pixpos < end_pixpos) {
		...
		if (block == NO_BLOCK) {
			...
			while (start_pixpos < next_start_pixpos) {
				...
				if (cdba)
					old_block =
					    get_next_display_block(ddl->bounds,
								   cdba,
								   start_pixpos,
								   &block_end);
				else {
				...
				start_pixpos =
				    min(next_start_pixpos, block_end);
			}
		} else {
			struct display_block *cdb, *ddb;
			int block_end = 0;
			...
			if (cdba)
				old_block =
				    get_next_display_block(ddl->bounds, cdba,
							   start_pixpos,
							   &block_end);
			else
				old_block = NO_BLOCK;
			...
			cdb =
			    (old_block !=
			     NO_BLOCK ? Dynarr_atp(cdba, old_block) : 0);
			...
			if (!force && cdb && ddb->type == cdb->type
			    ...
			    && (b == old_b || !old_b)) {
				must_sync |=
				    compare_display_blocks(w, cdl, ddl,
							   old_block, block,
							   start_pixpos,
							   cursor_start,
							   cursor_width,
				...
				for (elt = 0; elt < Dynarr_length(ddb->runes);
				     elt++) {
					struct rune *rb =
					    Dynarr_atp(ddb->runes, elt);
				...
				must_sync = 1;
				redisplay_output_display_block(w, ddl, block,
							       first_elt,
							       last_elt,
							       start_pixpos,
							       cursor_start,
	...
	if (f->windows_structure_changed || f->faces_changed || clear_border
		...
		if (y >= WINDOW_TEXT_TOP(w)
		    && (y + height) <= WINDOW_TEXT_BOTTOM(w)) {
			if (ddl->modeline) {
...
int redisplay_move_cursor(struct window *w, Bufpos new_point, int no_output_end)
{
	struct frame *f = XFRAME(w->frame);
	...
	if (rb->cursor_type == CURSOR_OFF)
		...
		dl->cursor_elt = -1;
		output_display_line(w, 0, cla, y, rb->xpos,
				    rb->xpos + rb->width);
	}
	...
	if (new_point == 0) {
		...
		while (up ? (cur_dl < Dynarr_length(cla)) : (cur_dl >= 0)) {
			...
			while ((!scroll_on_clipped_lines || !dl->clip) &&
			       (up ? (cur_rb < Dynarr_length(db->runes))
				: (cur_rb >= 0))) {
				...
				if (rb->cursor_type != IGNORE_CURSOR
					...
					output_display_line(w, 0, cla, cur_dl,
							    rb->xpos,
							    rb->xpos +
							    rb->width);
...
/*****************************************************************************
 ...
 the window.
 ****************************************************************************/
static void redraw_cursor_in_window(struct window *w, int run_end_begin_meths)
{
	struct frame *f = XFRAME(w->frame);
	struct device *d = XDEVICE(f->device);
	...
	if (rb->cursor_type == CURSOR_ON) {
		...
		output_display_line(w, 0, dla, y, rb->xpos,
				    rb->xpos + rb->width);
 ...
 ****************************************************************************/
void redisplay_redraw_cursor(struct frame *f, int run_end_begin_meths)
{
	Lisp_Object window;
...
static void redisplay_unmap_subwindows_except_us(struct frame *f, int x, int y,
			...
			int cursor_height)
{
	...
	if (!IMAGE_INSTANCE_OPTIMIZE_OUTPUT(p)
		...
		/* First clear the area we are drawing into. This is the easiest
		   ...
		   filled in. */
		redisplay_clear_clipped_region(window, findex, db, dga, 1,
					       Qnil);
		...
		if (!NILP(IMAGE_INSTANCE_LAYOUT_BORDER(p))) {
			...
			if (EQ(IMAGE_INSTANCE_LAYOUT_BORDER(p), Qetched_in))
				style = EDGE_ETCHED_IN;
				 ...
				 (IMAGE_INSTANCE_LAYOUT_BORDER(p), Qetched_out))
				...
				if (edges & EDGE_TOP) {
					ypos +=
					    XINT(IMAGE_INSTANCE_LAYOUT_BORDER
						 (p));
					height -=
					    XINT(IMAGE_INSTANCE_LAYOUT_BORDER
						 (p));
			...
			MAYBE_DEVMETH(d, bevel_area,
				      (w, findex, xpos, ypos, width, height,
				       DEFAULT_WIDGET_SHADOW_WIDTH, edges,
				       style));
		}
	}
	...
	LIST_LOOP(rest, IMAGE_INSTANCE_LAYOUT_CHILDREN(p)) {
		Lisp_Object child =
		    glyph_image_instance(XCAR(rest), image_instance,
					 ERROR_ME_NOT, 1);
		...
		if (IMAGE_INSTANCEP(child)) {
			...
			cdga.xoffset =
			    IMAGE_INSTANCE_XOFFSET(childii) - dga->xoffset;
			cdga.yoffset =
			    IMAGE_INSTANCE_YOFFSET(childii) - dga->yoffset;
			...
			if (redisplay_normalize_glyph_area(&cdb, &cdga)) {
				...
				/* Since the display boxes will now be totally
				   in the window if they are visible at all we
				   can now check this easily. */
				if (cdb.xpos < db->xpos || cdb.ypos < db->ypos
				    ...
				    db->ypos + db->height)
					continue;
				/* We have to invert the offset here as
				   normalization will have made them positive
				   which the output routines will treat as a
				   truly +ve offset. */
				...
				case IMAGE_TEXT: {
					/* #### This is well hacked and
					   could use some generalisation. */
					if (redisplay_normalize_glyph_area(
						...
						find_charsets_in_bufbyte_string
							(charsets,
							 XSTRING_DATA
							 (string),
							 XSTRING_LENGTH
							 (string));
						ensure_face_cachel_complete
							(cachel, window,
							 charsets);
						...
						redisplay_normalize_display_box
							(&cdb, &cdga);
						/* Offsets are now +ve again so
						   be careful when fixing up the
						   display line. */
						xzero(dl);
						/* Munge boxes into display
						   lines. */
						dl.ypos =
							(cdb.ypos -
							 cdga.yoffset)
						...
						dl.clip =
							...
							(cdb.ypos +
							 cdb.height);
						/* output_string doesn't
						   understand offsets in the
						   same way as other routines -
						   we have to add the offset to
						   the width so that we output
						   the full string. */
						MAYBE_DEVMETH(d,
							      output_string,
							      (w, &dl,
				...
				case IMAGE_COLOR_PIXMAP:
					if (!IMAGE_INSTANCE_OPTIMIZE_OUTPUT
					    (childii)
					    || IMAGE_INSTANCE_DIRTYP(childii))
		       ...
		       int width, int height)
{
	...
	DEVMETH(d, clear_region,
		(locale, d, f, findex, x, y, width, height, fcolor, bcolor,
		 background_pixmap));
}
			       ...
			       struct display_glyph_area *glyphsrc)
{
	if (dest->xpos + glyphsrc->xoffset > dest->xpos + dest->width
	    ...
	    (dest->xpos + glyphsrc->xoffset + glyphsrc->width >
	     dest->xpos + dest->width && dest->width - glyphsrc->xoffset <= 0)
	    || (dest->ypos + glyphsrc->yoffset + glyphsrc->height >
		dest->ypos + dest->height
	...
	if (dest->xpos + glyphsrc->xoffset + glyphsrc->width >
	    dest->xpos + dest->width) {
	...
	if (dest->ypos + glyphsrc->yoffset + glyphsrc->height >
	    dest->ypos + dest->height) {
				    ...
				    struct display_glyph_area *dga)
{
	...
	if (db->xpos < left || db->ypos < top
	    || db->xpos + db->width > right || db->ypos + db->height > bottom)
...
void redisplay_clear_top_of_window(struct window *w)
{
	...
	if (!NILP(Fwindow_highest_p(window))) {
		...
		redisplay_clear_region(window, DEFAULT_INDEX, x, y, width,
				       height);
	}
}
...
void redisplay_clear_to_window_end(struct window *w, int ypos1, int ypos2)
{
	...
	if (HAS_DEVMETH_P(d, clear_to_window_end))
		...
		if (height) {
			...
			if (window_is_leftmost(w))
				redisplay_clear_region(window, DEFAULT_INDEX,
						       FRAME_LEFT_BORDER_START
						       (f), ypos1,
						       FRAME_BORDER_WIDTH(f),
						       height);
		      ...
		      int update_values)
{
	...
	while (first_line <= last_line) {
		...
		if (update_values) {
			...
			while (cur_line < Dynarr_length(cdla)) {
				Dynarr_atp(cdla, cur_line)->offset +=
				    (new_len - old_len);
				Dynarr_atp(ddla, cur_line)->offset +=
				    (new_len - old_len);
				cur_line++;
			}
		}
		...
		if (update_values) {
			...
			if (Dynarr_atp(ddla, first_line)->cursor_elt != -1) {
				w->last_point_x[CURRENT_DISP] =
				    w->last_point_x[DESIRED_DISP];
				w->last_point_y[CURRENT_DISP] =
				    w->last_point_y[DESIRED_DISP];
			}
		}
	...
	if (update_values) {
		...
		while (line < Dynarr_length(ddla)) {
			...
			if (!dl->modeline)
				w->max_line_len =
				    max(dl->num_chars, w->max_line_len);
...
#if defined(HAVE_SCROLLBARS) && defined(HAVE_X_WINDOWS)
	{
		...
		update_window_scrollbars(w, NULL, 1,
					 stupid_vertical_scrollbar_drag_hack);
...
void redisplay_output_window(struct window *w)
{
	...
	if (window_needs_vertical_divider(w)
		...
		MAYBE_DEVMETH(d, output_vertical_divider,
			      (w, f->windows_structure_changed));
	}
