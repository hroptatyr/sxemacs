		       ...
		       XColor * color_def)
{
	...
	if (visual->class == DirectColor || visual->class == TrueColor) {
		if (XAllocColor(display, colormap, color_def) != 0) {
			...
			unsigned long rshift, gshift, bshift, rbits, gbits,
			    bbits, junk;
		...
		if (XAllocColor(display, colormap, color_def) != 0)
			...
			while (status != 2) {
				...
				if (cells == NULL) {
					...
					XQueryColors(display, colormap, cells,
						     no_cells);
				}
				...
				if (XAllocColor(display, colormap, color_def) !=
				    0)
					status = 2;
				else
		      ...
		      Error_behavior errb)
{
	...
	if (!result) {
		maybe_signal_simple_error("Unrecognized color", name, Qcolor,
					  errb);
		return 0;
	}
	result = allocate_nearest_color(dpy, cmap, visual, color);
	if (!result) {
		maybe_signal_simple_error("Couldn't allocate color", name,
					  Qcolor, errb);
		return 0;
	}
	...
	if (c->data) {
		if (DEVICE_LIVE_P(XDEVICE(c->device))) {
			if (COLOR_INSTANCE_X_DEALLOC(c)) {
				XFreeColors(DEVICE_X_DISPLAY
					    (XDEVICE(c->device)),
					    DEVICE_X_COLORMAP(XDEVICE
							      (c->device)),
					    &COLOR_INSTANCE_X_COLOR(c).pixel, 1,
					    0);
			}
		}
		xfree(c->data);
...
static unsigned long x_color_instance_hash(Lisp_Color_Instance * c, int depth)
{
	XColor color = COLOR_INSTANCE_X_COLOR(c);
	return HASH3(color.red, color.green, color.blue);
}

static Lisp_Object x_color_instance_rgb_components(Lisp_Color_Instance * c)
{
	XColor color = COLOR_INSTANCE_X_COLOR(c);
...
static int x_valid_color_name_p(struct device *d, Lisp_Object color)
{
	XColor c;
	Display *dpy = DEVICE_X_DISPLAY(d);
	Colormap cmap = DEVICE_X_COLORMAP(d);
...
static void x_mark_font_instance(Lisp_Font_Instance * f)
{
...
static int valid_x_font_name_p(Display * dpy, Extbyte * name)
{
...
static Extbyte *truename_via_FONT_prop(Display * dpy, XFontStruct * font)
{
...
static Extbyte *truename_via_random_props(Display * dpy, XFontStruct * font)
{
...
static Extbyte *truename_via_XListFonts(Display * dpy, Extbyte * font_name)
{
	Extbyte *result = 0;
	Extbyte **names;
	int count = 0;
...
x_font_truename(Display * dpy, Extbyte * name, XFontStruct * font)
{
	...
	if (!truename)
		truename = truename_random =
		    truename_via_random_props(dpy, font);
	if (truename && !valid_x_font_name_p(dpy, truename))
		truename = 0;
	if (!truename && name)
	...
	if (truename) {
		Lisp_Object result =
		    build_ext_string(truename, Qx_font_name_encoding);
		XFree(truename);
		return result;
	} else
		return Qnil;
...
x_font_instance_truename(Lisp_Font_Instance * f, Error_behavior errb)
{
	...
	if (NILP(FONT_INSTANCE_X_TRUENAME(f))) {
		Display *dpy = DEVICE_X_DISPLAY(d);
		{
			...
			LISP_STRING_TO_EXTERNAL(f->name, nameext,
						Qx_font_name_encoding);
			FONT_INSTANCE_X_TRUENAME(f) =
			    x_font_truename(dpy, nameext,
		...
		if (NILP(FONT_INSTANCE_X_TRUENAME(f))) {
			...
			maybe_signal_simple_error
			    ("Couldn't determine font truename", font_instance,
			     Qfont, errb);
...
static Lisp_Object x_font_instance_properties(Lisp_Font_Instance * f)
{
	struct device *d = XDEVICE(f->device);
	int i;
	...
	for (i = FONT_INSTANCE_X_FONT(f)->n_properties - 1; i >= 0; i--) {
		...
		if (name_str &&
			...
			value =
			    (val_str ?
			     build_ext_string(val_str, Qx_atom_name_encoding)
			     : Qnil);
		} else
...
static Lisp_Object x_list_fonts(Lisp_Object pattern, Lisp_Object device)
{
	Extbyte **names;
	int count = 0;
	Lisp_Object result = Qnil;
	...
	while (count--)
		result =
		    Fcons(build_ext_string(names[count], Qx_font_name_encoding),
			  result);
	if (names)
		XFreeFontNames(names);
	return result;
...
{
	DEFVAR_BOOL("x-handle-non-fully-specified-fonts", &x_handle_non_fully_specified_fonts	/*
...
void Xatoms_of_objects_x(struct device *d)
{
	...
	DEVICE_XATOM_ADD_STYLE_NAME(d) =
	    XInternAtom(D, "ADD_STYLE_NAME", False);
	...
	DEVICE_XATOM_CHARSET_REGISTRY(d) =
	    XInternAtom(D, "CHARSET_REGISTRY", False);
	DEVICE_XATOM_CHARSET_ENCODING(d) =
	    XInternAtom(D, "CHARSET_ENCODING", False);
}
