...
static XtResource resources[] = {
	...
	{XtNselectInsensitive, XtCSelectInsensitive, XtRBoolean,
	 sizeof(Boolean),
...
static XtGeometryResult TabsQueryGeometry(Widget,
					  XtWidgetGeometry *,
					  XtWidgetGeometry *);
static XtGeometryResult TabsGeometryManager(Widget, XtWidgetGeometry *,
					    XtWidgetGeometry *);
...
static int TabLayout(TabsWidget, Dimension wid, Dimension hgt,
		     Dimension * r_hgt, Bool query_only);
...
TabsInit(Widget request, Widget new, ArgList args, Cardinal * num_args)
{
	...
	newTw->tabs.foregroundGC =
	    ...
	    newTw->tabs.greyGC = newTw->tabs.topGC = newTw->tabs.botGC = None;
...
static void TabsResize(Widget w)
{
	...
	if (num_children > 0 && tw->composite.children != NULL) {
		...
		(void)TabLayout(tw, tw->core.width, tw->core.height, NULL,
				False);
		...
		for (i = 0, childP = tw->composite.children;
			...
			if (XtIsManaged(*childP)) {
				...
				if (ch >= bw * 2 && ch > 0 && cw >= bw * 2
				    && cw > 0)
					XtConfigureWidget(*childP, SHADWID,
							  tw->tabs.tab_total +
							  SHADWID, cw - bw * 2,
							  ch - bw * 2, bw);
			}
		if (XtIsRealized(w)) {
			XClearWindow(XtDisplay((Widget) tw),
				     XtWindow((Widget) tw));
	      ...
	      ArgList args, Cardinal * num_args)
{
	...
	if (tw->tabs.font != curtw->tabs.font ||
		...
		for (i = 0, childP = tw->composite.children;
		     i < tw->composite.num_children; ++i, ++childP)
			if (XtIsManaged(*childP))
				TabWidth(*childP);
		PreferredSize(tw, &tw->core.width, &tw->core.height, NULL,
			      NULL);
		needRedraw = True;
	...
	if (tw->tabs.topWidget != curtw->tabs.topWidget) {
		if (XtIsRealized(tw->tabs.topWidget)) {
			...
			TabsConstraints tab =
			    (TabsConstraints) w->core.constraints;

			XRaiseWindow(XtDisplay(w), XtWindow(w));
#ifdef	NEED_MOTIF
			XtVaSetValues(curtw->tabs.topWidget, XmNtraversalOn,
				      False, 0);
			XtVaSetValues(w, XmNtraversalOn, True, 0);
#endif

			if ((int)tab->tabs.row != (int)tw->tabs.numRows - 1) {
				TabsShuffleRows(tw);
			}
			needRedraw = True;
		} else
...
static Boolean TabsAcceptFocus(Widget w, Time * t)
{
...
TabsGeometryManager(Widget w, XtWidgetGeometry * req, XtWidgetGeometry * reply)
{
	...
	if (req->request_mode & (CWWidth | CWHeight | CWBorderWidth)) {
		...
		if (wid != tw->core.width || hgt != tw->core.height) {
			Dimension oldWid = tw->core.width, oldHgt =
			    tw->core.height;
			...
			if ((req->request_mode & XtCWQueryOnly) || rw < cw
			    || rh < ch)
				myrequest.request_mode |= XtCWQueryOnly;

			result =
			    XtMakeGeometryRequest((Widget) tw, &myrequest,
						  &myreply);
		...
		if (rw == aw && rh == ah) {
			...
			if (req->request_mode & XtCWQueryOnly)
				...
				for (i = TabsNumChildren(tw); --i >= 0;
				     ++childP)
					if (TabVisible(*childP)) {
						bw = (*childP)->core.
						    border_width;
						XtConfigureWidget(*childP, s,
								  tw->tabs.
								  tab_total + s,
								  rw - 2 * bw,
								  rh - 2 * bw,
				...
				XClearWindow(XtDisplay((Widget) tw),
					     XtWindow((Widget) tw));
				XtClass(tw)->core_class.expose((Widget) tw,
							       NULL, NULL);
#endif				/* COMMENT */
...
static void TabsChangeManaged(Widget w)
{
	...
	if (XtIsRealized(w)) {
		 ...
		 */
		if (tw->tabs.topWidget != NULL
		    && XtIsRealized(tw->tabs.topWidget)) {
...
TabsSelect(Widget w, XEvent * event, String * params, Cardinal * num_params)
{
	 ...
	 */
	for (i = 0, childP = tw->composite.children;
	     i < TabsNumChildren(tw); ++i, ++childP)
		if (TabVisible(*childP)) {
			TabsConstraints tab =
			    (TabsConstraints) (*childP)->core.constraints;
			if (x > tab->tabs.x && x < tab->tabs.x + tab->tabs.width
			    && y > tab->tabs.y && y < tab->tabs.y + h) {
				if (*childP != tw->tabs.topWidget
				    && (XtIsSensitive(*childP)
					|| tw->tabs.selectInsensitive))
...
TabsPage(Widget w, XEvent * event, String * params, Cardinal * num_params)
{
	...
	for (idx = 0, childP = tw->composite.children; idx < nc;
	     ++idx, ++childP)
...
static void
TabsUnhighlight(Widget w, XEvent * event, String * params,
		Cardinal * num_params)
{
	TabsWidget tw = (TabsWidget) w;
...
void XawTabsSetTop(Widget w, Bool callCallbacks)
{
	...
	/* Set the top tab, optionally call all callbacks. */
void XawTabsSetHighlight(Widget t, Widget w)
{
...
static void TabsAllocGCs(TabsWidget tw)
{
	TabsAllocFgGC(tw);
	TabsAllocGreyGC(tw);
	tw->tabs.backgroundGC = AllocBackgroundGC((Widget) tw, None);
	tw->tabs.topGC = AllocTopShadowGC((Widget) tw,
					  tw->tabs.top_shadow_contrast,
					  tw->tabs.be_nice_to_cmap);
	tw->tabs.botGC =
	    AllocBotShadowGC((Widget) tw, tw->tabs.bot_shadow_contrast,
			     tw->tabs.be_nice_to_cmap);
}

static void TabsFreeGCs(TabsWidget tw)
{
	Widget w = (Widget) tw;
		...
		for (j = TabsNumChildren(tw), childP = tw->composite.children;
			...
			if (TabVisible(*childP)) {
				tab =
				    (TabsConstraints) (*childP)->core.
				    constraints;
				if (tab->tabs.row == (int)i
				    && *childP != tw->tabs.topWidget)
					DrawTab(tw, *childP, labels);
			}
		if (i != tw->tabs.numRows - 1)
			DrawTrim(tw, 0, y + th, tw->core.width, th + s, False,
				 False);
	}
	...
	if (labels) {
		...
		if (XtIsSensitive(child)) {
			...
			if (!tab->tabs.greyAlloc) {
				if (tw->tabs.be_nice_to_cmap
				    || tw->core.depth == 1)
					tab->tabs.grey = tab->tabs.foreground;
				else
					tab->tabs.grey =
					    AllocGreyPixel((Widget) tw,
							   tab->tabs.foreground,
							   tw->core.
							   background_pixel,
							   tw->tabs.
							   insensitive_contrast);
	...
	if (ch > 0)
		...
		if (w != NULL) {
			...
			Draw3dBox((Widget) tw, tab->tabs.x + tab->tabs.width,
				  tw->core.height - 2 * s,
				  tw->core.width - tab->tabs.x -
				  tab->tabs.width, 2 * s, s, topgc, botgc);
		} else
			Draw3dBox((Widget) tw, 0, tw->core.height - 2 * s,
...
static void DrawTrim(TabsWidget tw,	/* widget */
		     int x,	/* upper-left corner */
		     int y, int wid,	/* total size */
		     int hgt, Bool bottom,	/* draw bottom? */
		     Bool undraw)
{				/* undraw all */
	Display *dpy = XtDisplay((Widget) tw);
	Window win = XtWindow((Widget) tw);
...
static void TabWidth(Widget w)
{
	TabsConstraints tab = (TabsConstraints) w->core.constraints;
	TabsWidget tw = (TabsWidget) XtParent(w);
	String lbl = tab->tabs.label != NULL ? tab->tabs.label : XtName(w);
	...
	if (tab->tabs.left_bitmap != None) {
		...
		tab->tabs.lbm_y =
		    (tw->tabs.tab_height - tab->tabs.lbm_height) / 2;
	}
...
TabLayout(TabsWidget tw,
	  Dimension wid,
	  Dimension hgt, Dimension * reply_height, Bool query_only)
{
	...
	if (num_children > 0) {
		...
		if (++display_rows == 1) {
			...
			y = TABDELTA;
			if (!query_only)
				for (i = num_children, childP =
				     tw->composite.children; --i >= 0; ++childP)
					if (XtIsManaged(*childP)) {
						tab =
						    (TabsConstraints)
...
static void GetPreferredSizes(TabsWidget tw)
{
...
static void MaxChild(TabsWidget tw, Widget except, Dimension cw, Dimension ch)
{
	int i;
	Widget *childP = tw->composite.children;
	...
	for (i = tw->composite.num_children; --i >= 0; ++childP)
		if (TabVisible(*childP) /*XtIsManaged(*childP) */ &&*childP !=
		    except) {
			(void)XtQueryGeometry(*childP, NULL, &preferred);
			cw = Max(cw,
				 preferred.width + preferred.border_width * 2);
			ch = Max(ch,
				 preferred.height + preferred.border_width * 2);
		}
...
static void TabsShuffleRows(TabsWidget tw)
{
	...
	if (tw->tabs.topWidget != NULL) {
		...
		if (real_rows > 1) {
			tab =
			    (TabsConstraints) tw->tabs.topWidget->core.
			    constraints;
			assert(tab != NULL);
			...
			for (i = tw->composite.num_children, childP =
			     tw->composite.children; --i >= 0; ++childP)
				if (XtIsManaged(*childP)) {
					tab =
					    (TabsConstraints) (*childP)->core.
					    constraints;
					tab->tabs.row =
					    (tab->tabs.row + move) % real_rows;
					tab->tabs.y =
					    bottom - tab->tabs.row * th;
					tab->tabs.visible =
					    (tab->tabs.row < display_rows);
				}
		}
	}
}
	 ...
	 */
static int PreferredSize(TabsWidget tw, Dimension * reply_width,	/* total widget size */
			 Dimension * reply_height, Dimension * reply_cw,	/* child widget size */
			 Dimension * reply_ch)
{
...
static int PreferredSize2(TabsWidget tw, Dimension cw,	/* child width, height */
			  Dimension ch, Dimension * reply_width,	/* total widget size */
...
static int PreferredSize3(TabsWidget tw, Dimension wid,	/* child width, height */
			  Dimension hgt, Dimension * reply_width,	/* total widget size */
			  Dimension * reply_height)
{
	Dimension th;		/* space used by tabs */
	int nrows;
...
static void MakeSizeRequest(TabsWidget tw)
{
	Widget w = (Widget) tw;
	...
	if (result == XtGeometryAlmost) {
		...
		if (reply.width < request.width
			...
			Dimension s = SHADWID;
			ch += s * 2;
			PreferredSize3(tw, reply.width, ch, &request.width,
				       &request.height);
			result = XtMakeGeometryRequest(w, &request, &reply);
			if (result == XtGeometryAlmost)
...
static void TabsAllocGreyGC(TabsWidget tw)
{
	...
	if (tw->tabs.be_nice_to_cmap || w->core.depth == 1) {
		...
		tw->tabs.greyGC =
		    XtAllocateGC(w, w->core.depth,
				 GCBackground | GCFont | GCStipple |
				 GCFillStyle, &values, GCForeground,
