...
static inline void safe_free_str(char *s)
{
	if (s) {
		xfree(s);
	}
...
static void copy_scrollbar_values(widget_value * val, widget_value * copy)
{
	if (!copy->scrollbar_data)
		copy->scrollbar_data =
		    (scrollbar_values *) xmalloc(sizeof(scrollbar_values));
...
void lw_add_widget_value_arg(widget_value * wv, String name, XtArgVal value)
{
	int i = 0;
	if (!wv->args) {
		wv->args = (widget_args *) xmalloc(sizeof(widget_args));
		memset(wv->args, '\0', sizeof(widget_args));
		wv->args->ref_count = 1;
		wv->args->nargs = 0;
		wv->args->args = (ArgList) xmalloc(sizeof(Arg) * 10);
