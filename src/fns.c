...
{
retry:
	if (!(INTEGERP(n) || CHARP(n) || RESC_ELMP(n))) {
	...
	if (LISTP(sequence)) {
		...
		if (XINT(n) * 2 < (EMACS_INT)XDLLIST_SIZE(sequence)) {
			/* start at the front */
			...
			rev = 1;
			i = XDLLIST_SIZE(sequence) - XINT(n) - 1;
		}
...
internal_equalp (Lisp_Object obj1, Lisp_Object obj2, int depth)
{
	...
	if (NUMBERP(obj1) && NUMBERP(obj2)) {
		...
		return ent_optable_eq[nt1][nt2](obj1, obj2);
	}
...

void list_map_inplace(Lisp_Object function, Lisp_Object list)
{
	Lisp_Object args[2];
	struct gcpro gcpro1, gcpro2;
...
void vector_map_inplace(Lisp_Object function, Lisp_Object tuple)
{
...
void string_map_inplace(Lisp_Object function, Lisp_Object string)
{
	Lisp_Object args[2];
...
void bit_vector_map_inplace(Lisp_Object function, Lisp_Object bitvec)
{
	...
	for (i = 0; i < len; i++) {
		args[1] = make_int(bit_vector_bit(v, i));
		args[1] = Ffuncall(2, args);
		if (!NILP(Fzerop(args[1])) || NILP(args[1]))
      ...
      (string))
{
	...
	input = make_lisp_string_input_stream(string, 0, -1);
	XMALLOC_OR_ALLOCA(encoded, 2*length, Bufbyte);
      ...
      (string))
{
	...
	length = XSTRING_CHAR_LENGTH(string);
	/* We need to allocate enough room for decoding the text. */
	XMALLOC_OR_ALLOCA(decoded, length, Bufbyte);
