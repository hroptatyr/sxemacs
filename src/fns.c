      ...
      (sequence))
{
	...
	if (DLLISTP(sequence))
		return copy_dllist(XDLLIST(sequence));
	if (STRINGP(sequence))
       ...
       enum concat_target_type target_type, int last_special)
{
	...
	if (target_type == c_string)
		args_mse =
		    alloca_array(struct merge_string_extents_struct, nargs);
...

	{
		...
		case c_string:
			...
			val = Qnil;
			string_result =
			    (Bufbyte *) alloca(total_length * MAX_EMCHAR_LEN);
      ...
      (list, n))
{
	...
	if (DLLISTP(list))
		return dllist_rac(XDLLIST(list));
