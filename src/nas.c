...
static SndInfo *SndOpenDataForReading(const char *data, int length)
{
	SndInfo *si;
	int size;
	...
	if (size) {
		...
		*(si->comment + size) = 0;
		if (*si->comment == '\0')
			si->comment =
			    NameFromData(data + si->h.dataOffset,
					 length - si->h.dataOffset);
	} else
		si->comment =
		    NameFromData(data + si->h.dataOffset,
				 length - si->h.dataOffset);
...
static WaveInfo *WaveOpenDataForReading(const char *data, int length)
{
	RiffChunk ck;
	RIFF_FOURCC fourcc;
	AuInt32 fileSize;
	...
	if (!readChunk(&ck) ||
	    ...
	    !readFourcc(&fourcc) || cmpID(&fourcc, RIFF_WaveID))
		Err();
		...
		if (!cmpID(&ck.ckID, RIFF_ListID)) {
			...
			if (!cmpID(&fourcc, RIFF_ListInfoID)) {
				...
				while (ck.ckSize) {
					...
					ck.ckSize -=
					    sizeof(RiffChunk) + PAD2(c.ckSize);
				}
			} else
				/* skip unknown chunk */
...
static Sound SoundOpenDataForReading(unsigned char *data, int length)
{
	...
	if ((s->formatInfo =
	     SndOpenDataForReading((char *)data, length)) != NULL) {
#if (AudioLibraryVersionMajor >= 2 ) && (AudioLibraryVersionMinor >= 3)
		...
		if (!
		    ((int (*)(Sound))
		     (SoundFileInfo[SoundFileFormatSnd].toSound)) (s))
#endif
		{
			SndCloseFile((SndInfo *) (s->formatInfo));
			free(s);
	    ...
	    if ((s->formatInfo =
		 WaveOpenDataForReading((char *)data, length)) != NULL) {
#if (AudioLibraryVersionMajor >= 2 ) && (AudioLibraryVersionMinor >= 3)
		...
		if (!
		    ((int (*)(Sound))
		     (SoundFileInfo[SoundFileFormatWave].toSound)) (s))
#endif
		{
			WaveCloseFile((WaveInfo *) (s->formatInfo));
			free(s);
