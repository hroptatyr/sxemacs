...
static void mswindows_init_frame_1(struct frame *f, Lisp_Object props)
{
	...
	FRAME_MSWINDOWS_TARGET_RECT(f)->left =
	    NILP(left) ? -1 : abs(XINT(left));
	...
	hwnd = CreateWindowEx(exstyle,
			      XEMACS_CLASS,
			      STRINGP(f->name) ? (LPCTSTR) XSTRING_DATA(f->
									name)
			      : (STRINGP(name) ? (LPCTSTR) XSTRING_DATA(name) :
				 XEMACS_CLASS), style, rect_default.left,
			      rect_default.top, rect_default.width,
			      rect_default.height, hwnd_parent, NULL, NULL,
			      NULL);
...
static void mswindows_init_frame_2(struct frame *f, Lisp_Object props)
{
...
static void mswindows_init_frame_3(struct frame *f)
{
...
static void mswindows_mark_frame(struct frame *f)
{
...
static void mswindows_focus_on_frame(struct frame *f)
{
	SetForegroundWindow(FRAME_MSWINDOWS_HANDLE(f));
}

static void mswindows_delete_frame(struct frame *f)
{
	if (f->frame_data) {
		Lisp_Object frame;
		XSETFRAME(frame, f);
...
static void mswindows_set_frame_size(struct frame *f, int width, int height)
{
	RECT rect;
	int columns, rows;
	...
	AdjustWindowRectEx(&rect,
			   ...
			   GetWindowLong(FRAME_MSWINDOWS_HANDLE(f),
					 GWL_EXSTYLE));
...
static void mswindows_set_frame_position(struct frame *f, int xoff, int yoff)
{
	SetWindowPos(FRAME_MSWINDOWS_HANDLE(f), NULL,
		     xoff, yoff, 0, 0,
		     SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSENDCHANGING |
...
static void mswindows_make_frame_visible(struct frame *f)
{
...
static void mswindows_make_frame_invisible(struct frame *f)
{
	if (!FRAME_VISIBLE_P(f))
		return;
...
static int mswindows_frame_totally_visible_p(struct frame *f)
{
...
static int mswindows_frame_visible_p(struct frame *f)
{
	return IsWindowVisible(FRAME_MSWINDOWS_HANDLE(f))
	    && !IsIconic(FRAME_MSWINDOWS_HANDLE(f));
}

static void mswindows_iconify_frame(struct frame *f)
{
...
static int mswindows_frame_iconified_p(struct frame *f)
{
	return IsIconic(FRAME_MSWINDOWS_HANDLE(f));
}

static void mswindows_set_frame_icon(struct frame *f)
{
	if (IMAGE_INSTANCEP(f->icon)
	    && IMAGE_INSTANCE_PIXMAP_TYPE_P(XIMAGE_INSTANCE(f->icon))) {
		if (!XIMAGE_INSTANCE_MSWINDOWS_ICON(f->icon)) {
			mswindows_initialize_image_instance_icon(XIMAGE_INSTANCE
								 (f->icon),
								 FALSE);
		}
...
static void mswindows_set_frame_pointer(struct frame *f)
{
	if (IMAGE_INSTANCEP(f->pointer)
	    && IMAGE_INSTANCE_TYPE(XIMAGE_INSTANCE(f->pointer)) ==
	    IMAGE_POINTER) {
...
static void mswindows_set_mouse_position(struct window *w, int x, int y)
{
...
static int
mswindows_get_mouse_position(struct device *d, Lisp_Object * frame, int *x,
			     int *y)
{
	POINT pt;
	HWND hwnd;
...
static void mswindows_raise_frame(struct frame *f)
{
	BringWindowToTop(FRAME_MSWINDOWS_HANDLE(f));
}

static void mswindows_lower_frame(struct frame *f)
{
	SetWindowPos(FRAME_MSWINDOWS_HANDLE(f), HWND_BOTTOM, 0, 0, 0, 0,
		     SWP_NOSIZE | SWP_NOMOVE | SWP_NOSENDCHANGING);
}

static void mswindows_enable_frame(struct frame *f)
{
	EnableWindow(FRAME_MSWINDOWS_HANDLE(f), TRUE);
}

static void mswindows_disable_frame(struct frame *f)
{
	EnableWindow(FRAME_MSWINDOWS_HANDLE(f), FALSE);
}

static void mswindows_set_title_from_bufbyte(struct frame *f, Bufbyte * title)
{
...
static Lisp_Object mswindows_frame_properties(struct frame *f)
{
...
static void mswindows_set_frame_properties(struct frame *f, Lisp_Object plist)
{
	int x = -1, y = -1;
...
static Lisp_Object mswindows_get_frame_parent(struct frame *f)
{
	HWND hwnd = FRAME_MSWINDOWS_HANDLE(f);
	hwnd = GetParent(hwnd);
	if (hwnd) {
...
static int mswindows_frame_size_fixed_p(struct frame *f)
{
 ...
 */
static void apply_dc_geometry(struct frame *f)
{
...
static void error_frame_unsizable(struct frame *f)
{
	Lisp_Object frame;
	XSETFRAME(frame, f);
	invalid_change
	    ("Cannot resize frame (margins) after print job has started.",
	     frame);
}

static void maybe_error_if_job_active(struct frame *f)
{
	if (FRAME_MSPRINTER_JOB_STARTED(f))
		error_frame_unsizable(f);
}

static void msprinter_init_frame_1(struct frame *f, Lisp_Object props)
{
	...
	if (!NILP(DEVICE_FRAME_LIST(XDEVICE(FRAME_DEVICE(f)))))
		invalid_operation
		    ("Only one frame (print job) at a time is allowed on "
...
static void msprinter_init_frame_3(struct frame *f)
{
	DOCINFO di;
	...
	if (!frame_pixsize_valid_p(f, frame_width, frame_height))
		invalid_operation
		    ("Area inside print margins has shrunk to naught",
	...
	/* Apply XEmacs frame geometry and layout windows */
	{
		...
		pixel_to_char_size(f, frame_width, frame_height, &columns,
				   &rows);
		change_frame_size(f, rows, columns, 0);
	}
...
static void msprinter_mark_frame(struct frame *f)
{
}

static void msprinter_delete_frame(struct frame *f)
{
	if (f->frame_data) {
		HDC hdc = DEVICE_MSPRINTER_HDC(XDEVICE(FRAME_DEVICE(f)));
		if (FRAME_MSPRINTER_PAGE_STARTED(f))
...
static Lisp_Object msprinter_frame_properties(struct frame *f)
{
	Lisp_Object props = Qnil;
...
static void msprinter_set_frame_properties(struct frame *f, Lisp_Object plist)
{
	Lisp_Object tail;
...
static void msprinter_set_frame_size(struct frame *f, int width, int height)
{
	/* We're absolutely unsizeable */
	error_frame_unsizable(f);
}

static void msprinter_eject_page(struct frame *f)
{
