			     ...
			     Lisp_Object * function, Lisp_Object * object)
{
	...
	*timeout_list = delq_no_quit_and_free_cons(op, *timeout_list);
	...
	if (timeout->resignal_msecs) {
		...
		if (async_p)
			timeout->interval_id =
			    event_stream_add_async_timeout(timeout->
							   next_signal_time);
		else
			timeout->interval_id =
			    event_stream_add_timeout(timeout->next_signal_time);
...
n the next available event.
this object to `dispatch-event' to handle it.
st cases, you will want to use `next-command-event', which returns
ext available "user" event (i.e. keypress, button-press,
n-release, or menu selection) instead of this function.

ENT is non-nil, it should be an event object and will be filled in
eturned; otherwise a new event object will be created and returned.
OMPT is non-nil, it should be a string and will be displayed in the
area while this function is waiting for an event.

ext available event will be
...
e last case, this function will block until an event is available.

eturned event will be one of the following types:
	...
	case process_event:
		{
			...
			if (readstatus > 0) ;	/* this clauses never gets executed but allows the #ifdefs
						   to work cleanly. */
#ifdef EWOULDBLOCK
