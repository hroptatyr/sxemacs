...
#ifdef HAVE_RESCLASS
	else if (RESC_ELMP(index_))
		idx = bigz_to_long(XRESC_ELM_DATA(index_));
#endif
	else {
...
{									\
	...
	result = Qnil;							\
	for (i = 1; i < nargs; i++) {					\
		obj1 = ent_normalise_number(args[i - 1]);		\
		nt1 = get_number_type(obj1);				\
		obj2 = ent_normalise_number(args[i]);			\
		nt2 = get_number_type(obj2);				\
									\
		result = ent_optable_##op[nt1][nt2](obj1, obj2);	\
									\
		if (NILP(result))					\
			return Qnil;					\
	}								\
...
{									\
	...
	for (i = 1; i < nargs; i++) {					\
		...
		nt2 = get_number_type(obj2);				\
									\
		result1 = ent_optable_##op1[nt1][nt2](obj1, obj2);	\
		result2 = ent_optable_##op2[nt1][nt2](obj1, obj2);	\
									\
      ...
      (int nargs, Lisp_Object * args))
{
	...
	result = Qnil;
	for (i = 0; i < nargs - 1; i++)	{
		obj1 = ent_normalise_number(args[i]);
		nt1 = get_number_type(obj1);
		for (j = i + 1; j < nargs; j++) {
			obj2 = ent_normalise_number(args[j]);
			nt2 = get_number_type(obj2);
			result = ent_optable_ne[nt1][nt2](obj1, obj2);
			if (NILP(result))
				return Qnil;
		}
	}
...
{
	REGISTER int i;
	Lisp_Object accum, addend;
	number_type ntacc, ntadd;

	if (nargs == 0)
		return Qzero;

	accum = ent_normalise_number(args[0]);
	ntacc = get_number_type(accum);

	for (i = 1; i < nargs; i++) {
		addend = ent_normalise_number(args[i]);
		ntadd = get_number_type(addend);
		accum = ent_optable_sum[ntacc][ntadd](accum, addend);
		ntacc = get_number_type(accum);
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	if (nargs == 1) {
		...
		accum = ent_optable_neg[ntacc](accum);

	} else {
		/* Subtract the remaining arguments from accum */
		for (i = 1; i < nargs; i++) {
			subtrahend = ent_normalise_number(args[i]);
			ntsub = get_number_type(subtrahend);
			accum = ent_optable_diff[ntacc][ntsub](
				accum, subtrahend);
			ntacc = get_number_type(accum);
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (i = 1; i < nargs; i++) {
		multiplier = ent_normalise_number(args[i]);
		ntmul = get_number_type(multiplier);
		accum = ent_optable_prod[ntacc][ntmul](accum, multiplier);
		ntacc = get_number_type(accum);
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	if (nargs == 1) {
		/* div 1 by accum */
		accum = ent_optable_div[ntacc][ntacc](Fone(accum), accum);

	} else {
		for (i = 1; i < nargs; i++) {
			divisor = ent_normalise_number(args[i]);
			ntdiv = get_number_type(divisor);
			accum = ent_optable_div[ntacc][ntdiv](accum, divisor);
			ntacc = get_number_type(accum);
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	if (nargs == 1) {
		/* Invert accum */
		accum = ent_optable_inv[ntacc](accum);

	} else {
		for (i = 1; i < nargs; i++) {
			divisor = ent_normalise_number(args[i]);
			ntdiv = get_number_type(divisor);
			accum = ent_optable_quo[ntacc][ntdiv](accum, divisor);
			ntacc = get_number_type(accum);
		}
      ...
      (int nargs, Lisp_Object * args)) 
{
	...
	number_type nti, ntmax;

	maxindex = 0;
	compmax = ent_normalise_number(args[maxindex]);
	ntmax = get_number_type(compmax);

	for (i = 1; i < nargs; i++) {
		compi = ent_normalise_number(args[i]);
		nti = get_number_type(compi);

		if (NILP(ent_optable_gt[ntmax][nti](compmax, compi))) {
			 maxindex = i;
			 ntmax = nti;
			 compmax = compi;
		}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	number_type nti, ntmin;

	minindex = 0;
	compmin = ent_normalise_number(args[minindex]);
	ntmin = get_number_type(compmin);

	for (i = 1; i < nargs; i++) {
		compi = ent_normalise_number(args[i]);
		nti = get_number_type(compi);

		if (NILP(ent_optable_lt[ntmin][nti](compmin, compi))) {
			 minindex = i;
			 ntmin = nti;
			 compmin = compi;
		}
      ...
      (number1, number2))
{
	...
	number1 = ent_normalise_number(number1);
	nt1 = get_number_type(number1);

	number2 = ent_normalise_number(number2);
	nt2 = get_number_type(number2);
	result = ent_optable_rem[nt1][nt2](number1, number2);
      ...
      (number, modulus))
{
	...
	nt1 = get_number_type(number);

	modulus = ent_normalise_number(modulus);
	nt2 = get_number_type(modulus);
	result = ent_optable_mod[nt1][nt2](number, modulus);
      ...
      (number1, number2))
{
	...
	number2 = ent_normalise_number(number2);
	nt2 = get_number_type(number2);
	result = ent_optable_pow[nt1][nt2](number1, number2);
      ...
      (number))
{
	...
	number = ent_optable_sum[nt][INT_T](number, make_int(1));
	return Fcanonicalize_number(number);
}
      ...
      (number))
{
	...
	number = ent_normalise_number(number);
	nt = get_number_type(number);
	number = ent_optable_diff[nt][INT_T](number, make_int(1));
	return Fcanonicalize_number(number);
}
