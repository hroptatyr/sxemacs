...
{									\
	...
	for (i = 1; i < nargs; i++) {					\
		obj1 = args[i - 1];					\
		obj2 = args[i];						\
		switch (nt = promote_args (&obj1, &obj2)) {		\
		case IMPOSSIBLE:					\
			Fsignal(Qrelation_error, Fcons(obj1, obj2));	\
			break;						\
		default:						\
			result = ent_optable_##op[nt][nt](obj1, obj2);	\
			break;						\
		}							\
		if (NILP(result))					\
			return Qnil;					\
...
#define ARITHCOMPARE_MANY2(c_op, op1, op2)				\
{									\
	REGISTER int i;							\
	Lisp_Object obj1, obj2;						\
	Lisp_Object result1, result2;					\
	number_type nt;							\
	result1 = Qnil;							\
	result2 = Qnil;							\
	for (i = 1; i < nargs; i++) {					\
		...
		case IMPOSSIBLE:					\
			...
			break;						\
		default:						\
			result1 = ent_optable_##op1[nt][nt](obj1, obj2);\
			result2 = ent_optable_##op2[nt][nt](obj1, obj2);\
			break;						\
		}							\
		if (NILP(result1) && NILP(result2))			\
			return Qnil;					\
	}								\
	return Qt;							\
}
	...
	for (i = 0; i < nargs - 1; i++)	{
		...
		for (j = i + 1; j < nargs; j++) {
			...
			case IMPOSSIBLE:
				...
				break;
			default:
				result = ent_optable_ne[nt][nt](obj1, obj2);
				break;
#if 0
			case INT_T:
...
#ifdef HAVE_PSEUG
			case BIGG_T:
				dead_wrong_type_argument(Qcomparablep, obj2);
				break;
#endif	/* HAVE_PSEUG */
#ifdef HAVE_MPC
			case BIGC_T:
...
#elif 0
			case BIGC_T:
				if (bigc_eql(XBIGC_DATA(obj1),
					     XBIGC_DATA(obj2)))
					return Qnil;
				break;
#endif	/* HAVE_MPC */
			case INDEF_T:
				if (indef_eql(obj1, obj2))
					return Qnil;
				break;
#endif	/* 0 */
			}
...
#ifndef WITH_NUMBER_TYPES
/* if number types are available, zerop comes from number.c */
      ...
      (string, base))
{
	...
	if (isfloat_string(p) && b == 10) {
		if (ZEROP(Vdefault_real_precision) || Vread_real_as != Qbigfr)
	...
	if (isfloat_string(p) && b == 10) {
		if (ZEROP(Vdefault_real_precision) || Vread_real_as != Qbigf)
			return make_float(atof((const char*)p));
		else
			return read_bigf_string(p);
	}
#elif defined(LISP_FLOAT_TYPE)
	if (isfloat_string(p) && b == 10)
		return make_float(atof(p));
#endif	/* HAVE_MPFR || HAVE_MPFR || LISP_FLOAT_TYPE */

#ifdef HAVE_RESCLASS
	if (isresclass_string(p))
		return read_resclass_string(p);
#endif	/* HAVE_RESCLASS */
...
#ifdef HAVE_MPZ
	{
		...
		*end = '\0';
		if (*p == '\0')
			...
			retval = Fcanonicalize_number(make_bigz_bz(bz));
			bigz_fini(bz);
		}
		*end = save;
		return retval;
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (i = 0; i < nargs; i++) {
		addend = args[i];
		switch (nt = promote_args(&accum, &addend)) {
		case IMPOSSIBLE:
			Fsignal(Qdomain_error, Qnil);
			break;

		default:
			accum = ent_optable_sum[nt][nt](accum, addend);
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	if (nargs == 1) {
		...
		CHECK_NUMBER(accum);
		switch (nt = get_number_type(accum)) {
		case IMPOSSIBLE:
			Fsignal(Qdomain_error, Qnil);
			break;
		default:
			accum = ent_optable_neg[nt](accum);
			break;
		}

	} else {
		/* Subtract the remaining arguments from accum */
		for (i = 1; i < nargs; i++) {
			subtrahend = args[i];
			switch (nt = promote_args (&accum, &subtrahend)) {
			case IMPOSSIBLE:
				Fsignal(Qdomain_error, Qnil);
				break;

			default:
				accum = ent_optable_diff[nt][nt]
					(accum, subtrahend);
				break;
			}
		}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	accum = make_bigz(1L);
#else
	accum = make_integer(1L);
#endif

	for (i = 0; i < nargs; i++) {
		multiplier = args[i];
		switch (nt = promote_args(&accum, &multiplier)) {
		case IMPOSSIBLE:
			Fsignal(Qdomain_error, Qnil);
			break;

		default:
			accum = ent_optable_prod[nt][nt](accum, multiplier);
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (; i < nargs; i++) {
		divisor = args[i];
		switch (nt = promote_args(&accum, &divisor)) {
		case IMPOSSIBLE:
			Fsignal(Qdomain_error, Qnil);
			break;
		default:
			accum = ent_optable_div[nt][nt](accum, divisor);
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (; i < nargs; i++) {
		...
		default:
			accum = ent_optable_quo[nt][nt](accum, divisor);
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args)) 
{
	...
	while (!(CHARP(args[0]) || MARKERP(args[0]) || NUMBERP(args[0])))
		args[0] = wrong_type_argument(Qnumber_char_or_marker_p,
					      args[0]);
	if (CHARP (args[0]))
	...
	for (i = 1; i < nargs; i++) {
		comp1 = args[maxindex];
		comp2 = args[i];
		switch (nt = promote_args(&comp1, &comp2)) {
		case IMPOSSIBLE:
			Fsignal(Qdomain_error, Qnil);
			break;

		default:
			if (!NILP(ent_optable_lt[nt][nt](comp1, comp2)))
				maxindex = i;
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (i = 1; i < nargs; i++) {
		...
		default:
			if (!NILP(ent_optable_gt[nt][nt](comp1, comp2)))
				minindex = i;
			break;
		}
	}
      ...
      (int nargs, Lisp_Object * args))
{
	...
	while (!(CHARP(args[0]) || MARKERP(args[0]) ||
		 INTEGERP(args[0])))
		args[0] = wrong_type_argument(
			Qnumber_char_or_marker_p, args[0]);
	...
	for (i = 1; i < nargs; i++) {
		while (!(CHARP(args[i]) || MARKERP(args[i]) ||
			 INTEGERP (args[i])))
			args[i] = wrong_type_argument(
				Qnumber_char_or_marker_p, args[i]);
		other = args[i];
		...
		case BIGZ_T: {
			...
			bigz_and(bz, XBIGZ_DATA(result), XBIGZ_DATA(other));
      ...
      (int nargs, Lisp_Object * args))
{
	...
	for (i = 1; i < nargs; i++) {
		...
		case BIGZ_T: {
			...
			bigz_ior(bz, XBIGZ_DATA(result), XBIGZ_DATA(other));
			result = make_bigz_bz(bz);
	...
	for (i = 1; i < nargs; i++) {
		while (!(CHARP(args[i]) || MARKERP(args[i]) ||
			...
			args[i] = wrong_type_argument(
				Qnumber_char_or_marker_p, args[i]);
		other = args[i];
		if (promote_args(&result, &other) == INT_T) {
			result = make_int(XREALINT(result) ^ XREALINT(other));
		} else {
			bigz bz;
			...
			bigz_xor(bz, XBIGZ_DATA(result), XBIGZ_DATA(other));
			result = make_bigz_bz(bz);
      ...
      (number1, number2))
{
	...
	while (!(CHARP(number1) || MARKERP(number1) || NUMBERP(number1)))
		number1 = wrong_type_argument(Qnumber_char_or_marker_p,
					      number1);
	while (!(CHARP(number2) || MARKERP(number2) || NUMBERP(number2)))
		number2 = wrong_type_argument(Qnumber_char_or_marker_p,
					      number2);

	switch (nt = promote_args(&number1, &number2)) {
	case IMPOSSIBLE:
		Fsignal(Qdomain_error, Qnil);
		result = Qzero;
		break;
	...
	default:
		result = ent_optable_rem[nt][nt](number1, number2);
		break;
	}
      ...
      (number, modulus))
{
	...
	while (!(CHARP(number) || MARKERP(number) || NUMBERP(number)))
		number = wrong_type_argument(Qnumber_char_or_marker_p, number);
	while (!(CHARP(modulus) || MARKERP(modulus) || NUMBERP(modulus)))
		modulus =
			wrong_type_argument(Qnumber_char_or_marker_p, modulus);
	switch (nt = promote_args(&number, &modulus)) {
	case IMPOSSIBLE:
		Fsignal(Qdomain_error, Qnil);
		result = Qzero;
		break;
	...
	default:
		result = ent_optable_mod[nt][nt](number, modulus);
		break;
	}
	...
	int_or_double iod1, iod2;
	number_char_or_marker_to_int_or_double(number, &iod1);
	number_char_or_marker_to_int_or_double(modulus, &iod2);
      ...
      (number1, number2))
{
	...
	nt2 = get_number_type(number2);
	switch (nt1 = get_number_type(number1)) {
	case IMPOSSIBLE:
		Fsignal(Qdomain_error, Qnil);
		return Qzero;
		break;
	...
	default:
		return Fcanonicalize_number(
			ent_optable_pow[nt1][nt2](number1, number2));
		break;
	}
      ...
      (value, count))
{
	...
	switch (promote_args(&value, &count)) {
	case INT_T:
		if (XREALINT(count) <= 0)
			return make_int(XREALINT(value) >> -XREALINT(count));
		/* Use bignums to avoid overflow */
		bigz_set_long(b2, XREALINT(value));
		bigz_lshift(b1, b2, XREALINT(count));

		result = Fcanonicalize_number(make_bigz_bz(b1));
		break;
	case BIGZ_T:
		if (bigz_sign(XBIGZ_DATA(count)) <= 0) {
			bigz_neg(b1, XBIGZ_DATA(count));
			if (!bigz_fits_ulong_p(b1)) {
				args_out_of_range(
					Qnumber_char_or_marker_p, count);
				return Qzero;
			}
			bigz_rshift(b2, XBIGZ_DATA(value), bigz_to_ulong(b1));
		} else {
			if (!bigz_fits_ulong_p(XBIGZ_DATA(count)))
				args_out_of_range(
					Qnumber_char_or_marker_p, count);
			bigz_lshift(b2, XBIGZ_DATA(value),
				    bigz_to_ulong(XBIGZ_DATA(count)));
		}
		result = Fcanonicalize_number(make_bigz_bz(b2));
		break;
	case INDEF_T:
...
retry:
	...
	if (INTP(number))
		return make_integer(XINT(number) + 1);
	if (CHARP(number))
		return make_integer(XCHAR(number) + 1);
	if (MARKERP(number))
		return make_integer(marker_position(number) + 1);
#ifdef LISP_FLOAT_TYPE
	if (FLOATP(number))
	...
	if (BIGZP(number))
		return Fcanonicalize_number(
			ent_optable_sum[BIGZ_T][INT_T](number, make_int(1)));
#endif
#ifdef HAVE_MPQ
	if (BIGQP(number)) {
		bigq b1, b2;
		...
		bigq_init(b2);
                ...
                /* addition code */
		bigq_set_long(b1, 1L);
		bigq_add(b2, XBIGQ_DATA(number), b1);
		result = make_bigq_bq(b2);
	...
	if (BIGFP(number)) {
		...
		bigf_init_prec(bf, XBIGF_GET_PREC(number));
                ...
                /* addition code */
		bigf_set_long(bf, 1L);
		bigf_add(bf, XBIGF_DATA(number), bf);
	...
	if (BIGFRP(number)) {
		...
		/* addition code */
		bigfr_set_long(bfr, 1L);
		bigfr_add(bfr, XBIGFR_DATA(number), bfr);
	...
	if (BIGGP(number)) {
		...
		bigg_init(bg);
                ...
                /* addition code */
		bigg_set_long(bg, 1L);
		bigg_add(bg, XBIGG_DATA(number), bg);
	...
	if (BIGCP(number)) {
		...
		bigc_init_prec(bc, XBIGC_GET_PREC(number));
                ...
                /* addition code */
		bigc_set_long(bc, 1L);
		bigc_add(bc, XBIGC_DATA(number), bc);
	...
	if (INDEFP(number)) {
		if (INFINITYP(number))
			return ent_optable_sum[INDEF_T][INT_T](
				number, make_int(1));
		else
			return wrong_type_argument(
				Qnumber_char_or_marker_p, number);
	}
#endif
#ifdef HAVE_RESCLASS
	if (RESC_ELMP(number))
		return ent_optable_sum[RESC_ELM_T][INT_T](
			number, make_int(1));
#endif
	number = wrong_type_argument(Qnumber_char_or_marker_p, number);
	goto retry;
...
retry:
	...
	if (BIGZP(number))
		return Fcanonicalize_number(
			ent_optable_diff[BIGZ_T][INT_T](number, make_int(1)));
#endif
#ifdef HAVE_MPQ
	if (BIGQP(number)) {
		bigq b1, b2;
		...
		bigq_init(b2);
                ...
                /* subtraction code */
		bigq_set_long(b1, 1L);
		bigq_sub(b2, XBIGQ_DATA(number), b1);
		result = make_bigq_bq(b2);
	...
	if (BIGFP(number)) {
		...
		bigf_init_prec(bf, XBIGF_GET_PREC(number));
                ...
                /* subtraction code */
		bigf_set_long(bf, 1L);
		bigf_sub(bf, XBIGF_DATA(number), bf);
		result = make_bigf_bf(bf);
	...
	if (BIGFRP(number)) {
		...
		if (bigfr_nan_p(XBIGFR_DATA(number))) {
			bigfr_set_nan(bfr);
		} else if (bigfr_inf_p(XBIGFR_DATA(number))) {
			...
			/* subtraction code */
			bigfr_set_long(bfr, 1L);
			bigfr_sub(bfr, XBIGFR_DATA(number), bfr);
		}
	...
	if (BIGGP(number)) {
		...
		bigg_init(bg);
                ...
                /* addition code */
		bigg_set_long(bg, 1L);
		bigg_sub(bg, XBIGG_DATA(number), bg);
		result = make_bigg_bg(bg);
	...
	if (BIGCP(number)) {
		...
		bigc_init_prec(bc, XBIGC_GET_PREC(number));
                ...
                /* addition code */
		bigc_set_long(bc, 1L);
		bigc_sub(bc, XBIGC_DATA(number), bc);
		result = make_bigc_bc(bc);
	...
	if (INDEFP(number)) {
		if (INFINITYP(number))
			return ent_optable_diff[INDEF_T][INT_T](
				number, make_int(1));
		else
			return wrong_type_argument(
				Qnumber_char_or_marker_p, number);
...
#ifdef HAVE_RESCLASS
	if (RESC_ELMP(number))
		return ent_optable_diff[RESC_ELM_T][INT_T](
			number, make_int(1));
#endif
      ...
      (number, certainty_threshold))
{
	...
	if (result == 0)
		...
		return Fintern(build_string("probably-prime"), Qnil);
	else if (result == 2)
		return Qt;
	else
		return Fintern(build_string("unknown-test-result"), Qnil);
}
      ...
      (number))
{
	...
	bznumber = Fcoerce_number(number, Qbigz, Qnil);
	mpz_nextprime(bz, XBIGZ_DATA(bznumber));
	result = make_bigz_bz(bz);
...
#ifdef HAVE_MPZ
	case BIGZ_T:
		if (bigz_to_long(XBIGZ_DATA(number)) == 1L ||
		    bigz_to_long(XBIGZ_DATA(number)) == -1L)
			result = Qt;
		else
			result = Qnil;
		break;
...
#ifdef HAVE_PSEUG
	case BIGG_T:
		if ((bigz_to_long(XBIGG_RE(number)) == 1L &&
		     bigz_to_long(XBIGG_IM(number)) == 0L) ||
		    (bigz_to_long(XBIGG_RE(number)) == -1L &&
		     bigz_to_long(XBIGG_IM(number)) == 0L) ||
		    (bigz_to_long(XBIGG_RE(number)) == 0L &&
		     bigz_to_long(XBIGG_IM(number)) == 1L) ||
		    (bigz_to_long(XBIGG_RE(number)) == 0L &&
		     bigz_to_long(XBIGG_IM(number)) == -1L))
			result = Qt;
		else
