...
/* **********************
   ...
   significant SXEmacs initialization here!
   ********************** */
...
void _start(void)
{
	...
	if (heap_state == HEAP_UNLOADED) {
		...
		/* #### This is super-bogus. When I rename sxemacs.exe,
		   the renamed file still loads its heap from sxemacs.exe --kkm */
#if 0
		{
	...
	/* #### This is bogus, too. _fmode is set to different values
	   when we run `sxemacs' and `temacs run-emacs'. The sooner we
...
static void get_section_info(file_data * p_infile)
{
	PIMAGE_DOS_HEADER dos_header;
	PIMAGE_NT_HEADERS nt_header;
	...
	for (i = 0; i < nt_header->FileHeader.NumberOfSections; i++) {
...
#endif
		{
			...
			if (section->SizeOfRawData < section->Misc.VirtualSize) {
				printf
				    ("The emacs data section is smaller than expected"
				     "...bailing.\n");
				exit(1);
			}
#endif
...
void read_in_bss(char *filename)
{
#ifndef DUMP_SEPARATE_SECTION
	HANDLE file;
...
void map_in_heap(char *filename)
{
	HANDLE file;
	HANDLE file_mapping;
	void *file_base;
