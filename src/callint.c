      ...
      (function, record_flag, keys))
{
	...
	/* Read interactive arguments */
	{
		...
		for (argnum = 0;; argnum++) {
			...
			const char *prompt_limit =
			    (const char *)strchr(prompt_start, '\n');
			int prompt_length;
			...
			case 'b':	/* Name of existing buffer */
				{
					...
					if (EQ
					    (Fselected_window(Qnil),
					     minibuf_window))
						def =
						    Fother_buffer(def, Qnil,
								  Qnil);
					/* read-buffer returns a buffer name, not a buffer! */
					args[argnum] =
					    call3(Qread_buffer, PROMPT(), def,
						  Qt);
					arg_from_tty = 1;
					break;
				}
