...
inline static size_t aligned_sizeof_opaque(size_t opaque_size)
{
...
print_opaque_ptr(Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
{
	...
	sprintf(buf,
		"#<INTERNAL OBJECT (SXEmacs bug?) (opaque-ptr, adr=0x%lx) 0x%lx>",
...
void reinit_opaque_once_early(void)
{
	Vopaque_ptr_free_list =
	    make_lcrecord_list(sizeof(Lisp_Opaque_Ptr), &lrecord_opaque_ptr);
