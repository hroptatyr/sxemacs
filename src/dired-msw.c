...
static int mswindows_ls_sort_fcn(const void *elem1, const void *elem2)
{
...
static WIN32_FIND_DATA *mswindows_get_files(char *dirfile, int nowild,
					    Lisp_Object pattern, int hide_dot,
					    int hide_system, int *nfiles)
{
	...
	while (1) {
		 ...
		 * sxemacs_stat(), & friends, because xemacs_stat() is VERY expensive in
		      ...
		      int add_newline)
{
	...
	file_size = file->nFileSizeHigh * (double)UINT_MAX + file->nFileSizeLow;
	cptr = buf;
#if INDENT_LISTING
	*cptr++ = ' ';
	*cptr++ = ' ';
	...
	}
	{
		...
		if (
			    ...
			    */
			   FileTimeToLocalFileTime(&file->ftLastWriteTime,
						   &localtime)
      ...
      (file, switches, wildcard, full_directory_p))
{
	...
	sort_by = (mswindows_ls_sort_case_insensitive
		   ? MSWINDOWS_SORT_BY_NAME_NOCASE : MSWINDOWS_SORT_BY_NAME);
	nfiles = 0;
	while (1) {
		handler =
		    Ffind_file_name_handler(file, Qmswindows_insert_directory);
		if (!NILP(handler)) {
			result =
			    call5(handler, Qmswindows_insert_directory, file,
				  switches, wildcard, full_directory_p);
			break;
		}
		CHECK_STRING(file);
		if (!NILP(switches)) {
		...
		if (!NILP(wildcard) || !NILP(full_directory_p)) {
			...
			files =
			    mswindows_get_files(XSTRING_DATA(file), FALSE,
			...
			files =
			    mswindows_get_files(XSTRING_DATA(file), TRUE,
						wildpat, hide_dot, hide_system,
						&nfiles);
		}
		...
		if (nfiles > 1) {
			mswindows_sort_files(sorted_files, nfiles, sort_by,
					     reverse);
		}
		if (!NILP(wildcard) || !NILP(full_directory_p)) {
			/*
		...
		for (i = 0; i < nfiles; ++i) {
			mswindows_format_file(sorted_files[i], fmtbuf,
					      display_size, TRUE);
			buffer_insert1(current_buffer, build_string(fmtbuf));
		}
		break;
