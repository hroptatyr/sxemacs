...
static Lisp_Object check_obarray(Lisp_Object obarray)
{
...
int hash_string(const Bufbyte *ptr, Bytecount length)
//ub4 hash_string( k, length, initval)
...
int hash_string(const Bufbyte *data, Bytecount len)
{
	int hash = len, tmp, rem;
	...
	case SYMVAL_SOME_BUFFER_LOCAL:
		{
			...
			if (!NILP(bfwd->current_buffer) &&
				...
				aelt =
				    buffer_local_alist_element(current_buffer,
							       symbol, bfwd);
				if (NILP(aelt)) {
	...
	case SYMVAL_SOME_BUFFER_LOCAL:
		{
			...
			if (NILP(bfwd->current_alist_element))
				return do_symval_forwarding(bfwd->current_value,
							    current_buffer,
							    XCONSOLE
							    (Vselected_console));
			else
...
static const struct symbol_value_magic guts_of_unbound_marker = {	/* struct symbol_value_magic */
...
#if somebody_can_explain_why_a_symbol_must_not_be_bound
