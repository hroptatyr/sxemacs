...
static void CheckboxClassInit(void)
{
 ...
 /*ARGSUSED*/
    static void
CheckboxInit(Widget request, Widget new, ArgList args, Cardinal * num_args)
{
#if DRAW_CHECK
...
#if DRAW_CHECK
static void
CheckboxRealize(Widget w, Mask * valueMask, XSetWindowAttributes * attributes)
{
	...
	cw->checkbox.checkmark_GC =
	    XtAllocateGC(w, 0, value_mask, &values, dynamic_mask,
			 dontcare_mask);
}
#endif
...
/* ARGSUSED */
static void CheckboxDestroy(Widget w, XtPointer junk, XtPointer garbage)
{
#if DRAW_CHECK
	CheckboxWidget cw = (CheckboxWidget) w;
...
/* ARGSUSED */
static void CheckboxPress(Widget w, XEvent * event, String * params,	/* unused */
			  Cardinal * num_params)
{				/* unused */
	CheckboxWidget cw = (CheckboxWidget) w;
	if (!cw->checkbox.pressed) {
		cw->checkbox.pressed = TRUE;
		((CheckboxWidgetClass) (w->core.widget_class))->radio_class.
		    drawDiamond(w);
	}
}

static void CheckboxUnpress(Widget w, XEvent * event, String * params,	/* unused */
			    Cardinal * num_params)
{				/* unused */
	CheckboxWidget cw = (CheckboxWidget) w;
	int i;
	...
	if (cw->checkbox.pressed) {
		...
		if (*num_params > 0 && **params == 'd')
			((CheckboxWidgetClass) (w->core.widget_class))->
			    radio_class.drawDiamond(w);
	}
}
#endif

...
static void DrawCheck(Widget w)
{
	CheckboxWidget cw = (CheckboxWidget) w;
