...
static void pre_hook(Widget w, XtPointer client_data, XtPointer call_data)
{
...
static void pick_hook(Widget w, XtPointer client_data, XtPointer call_data)
{
	widget_instance *instance = (widget_instance *) client_data;
	...
	if (instance->info->selection_cb &&
	    contents_val && contents_val->enabled && !contents_val->contents)
...

/* creation functions */
static Widget xlw_create_menubar(widget_instance * instance)
{
	Arg al[1];
	Widget widget;
...
static Widget xlw_create_popup_menu(widget_instance * instance)
{
	Arg al[2];
...
static Widget xlw_create_scrollbar(widget_instance * instance, int vertical)
{
	Arg al[20];
	int ac = 0;
	...
	XtSetArg(al[ac], XmNorientation,
		 (vertical ? XmVERTICAL : XmHORIZONTAL));
	ac++;
...
static Widget xlw_create_vertical_scrollbar(widget_instance * instance)
{
	return xlw_create_scrollbar(instance, 1);
}

static Widget xlw_create_horizontal_scrollbar(widget_instance * instance)
{
		     ...
		     widget_value * val)
{
	if (val->scrollbar_data) {
		...
		if (new_sliderSize != widget_sliderSize
			...
			XlwScrollBarSetValues(widget, new_value, new_sliderSize,
					      1, 1, False);
	}
}
...
xlw_tab_control_callback(Widget w, XtPointer client_data, XtPointer call_data)
{
	...
	if (instance->info->selection_cb &&
	    widget_val && widget_val->enabled && !widget_val->contents)
...
static Widget xlw_create_tab_control(widget_instance * instance)
{
	Arg al[20];
	int ac = 0;
	Widget tab = 0;
	...
	ac++;
	XtSetArg(al[ac], XtNresizable, False);
	ac++;
	...
	XtAddCallback(tab, XtNcallback, xlw_tab_control_callback,
		      (XtPointer) instance);
...
static void
xlw_update_tab_control(widget_instance * instance, Widget widget,
		       widget_value * val)
{
	Widget *children;
	...
	XtAddCallback(widget, XtNcallback, xlw_tab_control_callback,
		      (XtPointer) instance);
...
#ifdef HAVE_WIDGETS
static Widget xlw_create_clip_window(widget_instance * instance)
{
	Arg al[20];
	int ac = 0;
	Widget clip = 0;
...
const widget_creation_entry xlw_creation_table[] = {
#ifdef LWLIB_MENUBARS_LUCID
		      ...
		      widget_value * val, Boolean deep_p)
{
	...
	if (0) ;
		...
		Arg al[1];
		if (XtIsShell(widget))
			mw = (XlwMenuWidget) ((CompositeWidget) widget)->
...
void xlw_popup_menu(Widget widget, XEvent * event)
{
	...
	if (event->type == ButtonPress || event->type == ButtonRelease) {
		mw = (XlwMenuWidget) ((CompositeWidget) widget)->composite.
		    children[0];
