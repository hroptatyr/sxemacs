...
dnl 		[sxe_cv_feat_ffmpeg=yes], [sxe_cv_feat_ffmpeg=no])
	...
	_SXE_CHECK_pkgconfig_based([ffmpeg], [libavformat], [49.0.0])
	...
	if test "$ac_cv_header_avformat_h" = "yes" -o \
		...
		sxe_cv_feat_ffmpeg="maybe"
	fi
	if test "$sxe_cv_feat_ffmpeg" = "maybe" -a \
		...
		"$ac_cv_lib_avformat_avcodec_decode_audio2" = "yes" -a \
		"$ac_cv_lib_avformat_avcodec_decode_audio" = "yes" -a \
