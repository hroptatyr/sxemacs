...
AC_DEFUN([SXE_CHECK_CC_EXTERN_INLINE], [dnl
	...
	AC_REQUIRE([SXE_CHECK_CC_GNU89_EXTERN_INLINE])
	## check if gcc dislikes gnu89 inlines in c99
	AC_REQUIRE([SXE_CHECK_CC_GNU89_EXTERN_INLINE_ALLERGY])
...
AC_DEFUN([SXE_WARNFLAGS], [dnl
	...
	if test "$__SUNPRO_C" = "yes"; then
		...
		warnflags="-Wall -Wno-switch"

		dnl Yuck, bad compares have been worth at
		dnl least 3 crashes!
		dnl Warnings about char subscripts are pretty
		dnl pointless, though,
		dnl and we use them in various places.
		warnflags="$warnflags -Wsign-compare -Wno-char-subscripts"
		...
		if test "$__GCC3" = "yes" -o true; then
			warnflags="$warnflags -Wpacked"
		fi
		dnl glibc is intentionally not `-Wpointer-arith'-clean.
		dnl Ulrich Drepper has rejected patches to fix
		dnl the glibc header files.
		if test "$have_glibc" != "yes" -o true; then
		...
		warnflags="$warnflags -Wbad-function-cast -Wcast-qual -Wcast-align"
		...
		warnflags="-Wall -w1 -we147"
		dnl ### Add optimal with_warnflags support
		dnl for other compilers HERE!
	fi
...
AC_DEFUN([SXE_CC_MAXOPT_GNU], [dnl
	...
	optiflags="-O3" ## "-fomit-frame-pointer" ## <- i think that's too harsh
	...
	if test "$sxe_cv_proc_flag_ssse3" = "yes"; then
		if test "$sxe_cv_c_flags__mssse3" = "yes"; then
			...
			optiflags="$optiflags -mssse3"
		elif test "$sxe_cv_c_flags__mssse" = "yes"; then
			sxe_cv_c_flags__msse_added="yes"
			optiflags="$optiflags -mssse"
		elif test "$sxe_cv_c_flags__msse3" = "yes"; then
			sxe_cv_c_flags__msse_added="yes"
			optiflags="$optiflags -msse3"
		fi

	elif test "$sxe_cv_proc_flag_sse3" = "yes" -a \
		"$sxe_cv_c_flags__msse3" = "yes"; then
			sxe_cv_c_flags__msse_added="yes"
			optiflags="$optiflags -msse3"

	elif test "$sxe_cv_proc_flag_sse2" = "yes" -a \
		"$sxe_cv_c_flags__msse2" = "yes"; then
			sxe_cv_c_flags__msse_added="yes"
			optiflags="$optiflags -msse2"

	elif test "$sxe_cv_proc_flag_sse" = "yes" -a \
		"$sxe_cv_c_flags__msse" = "yes"; then
			sxe_cv_c_flags__msse_added="yes"
			optiflags="$optiflags -msse"
	fi
	...
	SXE_CHECK_COMPILER_FLAGS([-fstrict-aliasing],
		[optiflags="$optiflags -fstrict-aliasing"])
	...
	SXE_CHECK_COMPILER_FLAGS([-fstack-protector],
		[optiflags="$optiflags -fstack-protector"])
