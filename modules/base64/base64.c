/* obsolete base64 interface for SXEmacs.
   Copyright (C) 1998, 1999 Free Software Foundation, Inc.

This file is part of SXEmacs, will disappear though.
      ...
      (object, start, end, coding, error_me_not))
{
	...
	if (NILP(coding)) {
		if (BUFFERP(object)) {
			...
			determine_real_coding_system(istr, &coding_system,
						     &eol);
		}
	...
	deststream =
	    make_dynarr_output_stream((unsigned_char_dynarr *) out_dynarr);
	...
	if (char_count != 0) {
		...
		Lstream_write(dstr, &alphabet[bits >> 18],
			      sizeof(unsigned char));
		Lstream_write(dstr, &alphabet[(bits >> 12) & 0x3f],
			      sizeof(unsigned char));
		if (char_count == 1) {
			Lstream_write(dstr, "==", 2 * sizeof(unsigned char));
		} else {
			Lstream_write(dstr, &alphabet[(bits >> 6) & 0x3f],
				      sizeof(unsigned char));
	...
	return (make_string
		(Dynarr_atp(out_dynarr, 0), Dynarr_length(out_dynarr)));
}
      ...
      (object, start, end, coding, error_me_not))
{
	...
	if (hit_eof) {
		if (char_count) {
			error_with_frob(object,
					"base64-decode failed: at least %d bits truncated",
					((4 - char_count) * 6));
		}
	}
	switch (char_count) {
...
void syms_of_base64(void)
{
	DEFSUBR(Fbase64_encode);
	DEFSUBR(Fbase64_decode);
}

void vars_of_base64(void)
{
