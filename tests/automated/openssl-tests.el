...
(unless (featurep 'openssl)
  (error 'unimplemented 'openssl
         "SXEmacs was compiled without OpenSSL support."))
...
(Assert (member 'AES-256-CBC (ossl-available-ciphers)))
(Assert (member 'AES-256-ECB (ossl-available-ciphers)))
(Assert (member 'AES-256-CFB (ossl-available-ciphers)))
(Assert (member 'AES-256-OFB (ossl-available-ciphers)))
...
(let ((encstrs
      ...
      (ciphers
       `(AES-256-CBC AES-256-OFB AES-192-ECB AES-192-CFB
                     ;; choose three random ciphers
                     ,@(let ((oac (ossl-available-ciphers)))
                         (list
                          (nth (random (length oac)) oac)
                          (nth (random (length oac)) oac)
                          (nth (random (length oac)) oac)))))
      (digests
       '(SHA1 MD5 RIPEMD160))
