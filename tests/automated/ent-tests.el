...
(when (featurep 'bigg)
  ...
  (Check-Error wrong-type-argument (format "%d" 1+i))
  (Check-Error wrong-type-argument (format "%f" 1+i))
  (when (featurep 'bigq)
    (Check-Error wrong-type-argument (format "%Q" 1+i)))
  (when (featurep 'bigfr)
    (Check-Error wrong-type-argument (format "%F" 1+i))))
...
(when (featurep 'bigc)
  ...
  (Check-Error wrong-type-argument (format "%d" (sqrt -2)))
  (Check-Error wrong-type-argument (format "%f" (sqrt -2)))
  (when (featurep 'bigq)
    (Check-Error wrong-type-argument (format "%Q" (sqrt -2))))
  (when (featurep 'bigfr)
    (Check-Error wrong-type-argument (format "%F" (sqrt -2)))))
...
(let ((nan-funs '(acos asin atan cos sin tan sec csc cot
  ...
  (mapc #'(lambda (fun)
	    (mapc #'(lambda (val)
		  ...
		  vals))
	'(logb logand logior logxor lognot)))
